...
const Slots: FC = () => {
  ...
  return (
    <SidebarPlayground>
      <h1 className="text-5xl font-semibold mb-4 md:mb-4">插槽 Slots（default / named / scoped）</h1>
            ...
            <div className="card bg-base-100 shadow overflow-auto">
              <div className="card-body p-0">
                <div className="border-b border-base-300 px-4 py-3">
                  ...
                  <p className="text-sm opacity-70">先只看 default、named、scoped 各自最短怎么写。</p>
                </div>
        ...
        {activeTab.value === 'preview' && (
          <div className="grid gap-6">
            <div role="alert" className="alert alert-success">
              <span>
                是，当前这里的 slot 是真实渲染。父组件传进来的不是字符串模板，而是实际的 JSX / renderable；宿主组件内部的 Slot 会在运行时把它取出来并直接渲染。
              </span>
            </div>
            ...
            <div className="card bg-base-100 shadow">
              <div className="card-body gap-5">
                <div>
                  ...
                  <p className="text-sm opacity-70">把 slot 理解成“组件内部预留的洞口”。父组件传什么，洞口里就显示什么；没传就显示 fallback。</p>
                </div>
                ...
                <div className="grid gap-4 xl:grid-cols-3">
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm opacity-70">直接写在组件标签内部的内容，会落到默认插槽。</p>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm opacity-70">给某个直接子节点写 slot="name"，它就会去对应的命名插槽。</p>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm opacity-70">宿主组件把数据通过 props 传给 Slot；父组件用同名函数 prop 接住它。</p>
                    </div>
                ...
                <div role="alert" className="alert alert-soft">
                  <span>如果只想先会用，记住这三条就够了：标签里的普通内容是 default slot，slot="name" 是 named slot，同名函数 prop 是 scoped slot。</span>
                </div>
              </div>
            </div>
              ...
              <div className="card-body gap-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-sm opacity-70">下面把 default、named、scoped、fallback 都放在一个组件里，看完整交互会更直观。</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      {showTitleSlot.value && (
                        <Template slot="title">
                          <h2 className="card-title">支付面板</h2>
                          <p className="text-sm opacity-70">Template slot="title" 可以一次传多个兄弟节点。</p>
                        </Template>
                      )}
            ...
            <div className="grid gap-4 xl:grid-cols-2">
                ...
                <div className="card-body gap-3">
                  ...
                  <ul className="list rounded-box border border-base-300 bg-base-100">
                    <li className="list-row">
                      ...
                      <div className="opacity-70">直接写在组件标签内部，最终会落到 props.children。</div>
                    </li>
                    <li className="list-row">
                      <div className="font-medium">named slot</div>
                      <div className="opacity-70">单个节点可以直接写 slot="name"，多个兄弟节点建议用 Template slot="name" 包起来。</div>
                    </li>
                    <li className="list-row">
                      <div className="font-medium">scoped slot</div>
                      <div className="opacity-70">当前最稳妥的是传同名函数 prop，例如把 row 作为函数属性传给宿主组件。</div>
                    </li>
                    <li className="list-row">
                      <div className="font-medium">fallback</div>
                      <div className="opacity-70">宿主组件内部在 Slot 标签里写的 children，就是 slot 缺失时的回退内容。</div>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="card bg-base-100 shadow">
                <div className="card-body gap-3">
                  ...
                  <div role="alert" className="alert alert-soft">
                    <span>静态命名内容走 slot="name" 和 Template slot="name"，作用域行模板走 row 函数 prop。</span>
                  </div>
                  <div className="mockup-code text-sm">
                    <pre data-prefix="1"><code>{'<Slot source={props} name="title">fallback</Slot>'}</code></pre>
                    <pre data-prefix="2"><code>{'<Template slot="title">...</Template>'}</code></pre>
                    <pre data-prefix="3"><code>{'<button slot="actions">刷新</button>'}</code></pre>
                    <pre data-prefix="4"><code>{'<SlotPanel row={(slotProps) => ...}>...</SlotPanel>'}</code></pre>
                  </div>
                  <p className="text-sm opacity-70 leading-6">
                    这也解释了为什么当前 demo 会同时出现 slot 属性和函数 prop 两种形式：前者覆盖命名静态内容，后者负责 scoped slot。
                  </p>
                </div>
              </div>
            </div>
