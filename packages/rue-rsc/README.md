...
This package provides [Rue Server Components](https://rue.dev/reference/rsc/server-components) (RSC) support for Vite.

## Features

- **Framework-agnostic**: The plugin implements RSC bundler features and provides low level Rue RSC runtime APIs without framework-specific abstractions.
...
- Provides an in-depth overview of API with inline comments to explain how they function within RSC-powered Rue application.
...
- [`./examples/rue-router`](./examples/rue-router) - Rue Router RSC integration
  - Demonstrates how to integrate [experimental Rue Router RSC API](https://remix.run/blog/rsc-preview). Rue Router now provides [official RSC support](https://ruerouter.com/how-to/rue-server-components), so it's recommended to follow Rue Router's official documentation for the latest integration.
...
graph TD
    ...
    subgraph "<strong>rsc environment</strong>"
        A["Rue virtual dom tree"] --> |"[@rue-js/rsc/rsc]<br /><code>renderToReadableStream</code>"| B1["RSC Stream"];
    end
    ...
    subgraph "<strong>ssr environment</strong>"
        B2["RSC Stream"] --> |"[@rue-js/rsc/ssr]<br /><code>createFromReadableStream</code>"| C1["Rue virtual dom tree"];
        C1 --> |"[@rue-js/server-renderer]<br/>SSR"| E["HTML String"];
    end

    subgraph "<strong>client environment</strong>"
        B3["RSC Stream"] --> |"[@rue-js/rsc/browser]<br /><code>createFromReadableStream</code>"| C2["Rue virtual dom tree"];
        C2 --> |"[@rue-js/rue]<br/>CSR: render"| D["DOM Elements"];
    end
...
export default defineConfig({
  ...
  environments: {
    // `rsc` environment loads modules with `rue-server` condition.
    // this environment is responsible for:
    // - RSC stream serialization (Rue VDOM -> RSC stream)
    ...
    // `ssr` environment loads modules without `rue-server` condition.
    // this environment is responsible for:
    // - RSC stream deserialization (RSC stream -> Rue VDOM)
    // - traditional SSR (Rue VDOM -> HTML string/stream)
    ssr: {
      build: {
        rollupOptions: {
          input: {
    ...
    // this environment is responsible for:
    // - RSC stream deserialization (RSC stream -> Rue VDOM)
    // - traditional CSR (Rue VDOM -> Browser DOM tree mount/hydration)
...
```tsx
import { renderToReadableStream } from '@rue-js/rsc/rsc'

// the plugin assumes `rsc` entry having default export of request handler
export default async function handler(request: Request): Promise<Response> {
  // serialize Rue VDOM to RSC stream
  const root = (
    <html>
      <body>
        <h1>Test</h1>
...
```tsx
import { createFromReadableStream } from '@rue-js/rsc/ssr'
import { renderToString } from '@rue-js/server-renderer'

export async function handleSsr(rscStream: ReadableStream) {
  // deserialize RSC stream back to Rue VDOM
  ...
  return { html, bootstrapScriptContent }
}
```
...
```tsx
import { createFromReadableStream } from '@rue-js/rsc/browser'
import { render } from '@rue-js/rue'

async function main() {
  // fetch and deserialize RSC stream back to Rue VDOM
  const rscResponse = await fetch(window.location.href + '.rsc')
  const root = await createFromReadableStream(rscResponse.body)

  // render in the browser (traditional CSR)
...
import.meta.viteRsc.import<typeof import('./entry.ssr.tsx')>('./entry.ssr.tsx', {
  environment: 'ssr',
})
```
...
- Type: `(importer?: string) => Rue.RueNode`
...
This provides a raw js code to execute a browser entry file specified by `environments.client.build.rollupOptions.input.index`. It can be injected into the HTML shell produced by the Rue SSR path.

```js
import { renderToString } from '@rue-js/server-renderer'

const bootstrapScriptContent = await import.meta.viteRsc.loadBootstrapScriptContent('index')
const html = await renderToString(rueNode)
const documentHtml = `${html}<script type="module">${bootstrapScriptContent}</script>`
```
...
```js
import { createFromFetch } from '@rue-js/rsc/browser'
...
This module exports Rue-owned RSC runtime APIs such as:

- `renderToReadableStream`: RSC serialization (Rue VDOM -> RSC stream)
- `createFromReadableStream`: RSC deserialization (RSC stream -> Rue VDOM). This is also available on rsc environment itself. For example, it allows saving serialized RSC and deserializing it for later use.
...
> [!NOTE]
> This is a Vite-specific extension to the RSC runtime API.
...
This module exports the SSR-side Rue RSC runtime API.

- `createFromReadableStream`: RSC deserialization (RSC stream -> Rue VDOM)

### `@rue-js/rsc/browser`

This module exports the browser-side Rue RSC runtime API.
...
### Using `@rue-js/rsc` as a framework package's `dependencies`

When `@rue-js/rsc` is shipped through a framework package, make sure Vite can resolve the package from the project root or through the framework package path. If your framework wraps the package, you can adjust `optimizeDeps.include` to point through your framework package:
    ...
    configEnvironment(_name, config) {
      if (config.optimizeDeps?.include) {
        config.optimizeDeps.include = config.optimizeDeps.include.map(entry => {
          if (entry.startsWith('@rue-js/rsc')) {
...
Types for global API are defined in `@rue-js/rsc/types`. For example, you can add it to `tsconfig.json` to have types for `import.meta.viteRsc` APIs:

```json
{
  "compilerOptions": {
    "types": ["vite/client", "@rue-js/rsc/types"]
  }
}
```
...
<!-- https://overrueed.io/how-imports-work-in-rsc/ -->
...
Additionally, Parcel and Rue Router's work on standardizing the RSC bundler/app responsibility has guided this plugin's API design:

- [Waku](https://github.com/wakujs/waku)
- [@lazarv/rue-server](https://github.com/lazarv/rue-server)
- [Rue Router RSC](https://remix.run/blog/rsc-preview)
