...
import { normalizeRscVirtualId, RUE_RSC_VIRTUAL_PREFIX, rscVirtualPrefixRegex } from './shared'
...
const RESOLVED_ID_PROXY_PATH = 'resolved-id/'
const RESOLVED_ID_PROXY_PREFIX = RUE_RSC_VIRTUAL_PREFIX + RESOLVED_ID_PROXY_PATH
...
export function fromResolvedIdProxy(source: string): string | undefined {
  source = normalizeRscVirtualId(source)
...
export function vitePluginResolvedIdProxy(): Plugin {
  return {
    ...
    resolveId: {
      filter: { id: rscVirtualPrefixRegex(RESOLVED_ID_PROXY_PATH) },
      handler(source) {
