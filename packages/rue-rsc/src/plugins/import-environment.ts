...
import { createVirtualPlugin, normalizeRelativePath, normalizeRollupOpitonsInput } from './utils'
import { evalValue } from './vite-utils'

export const ENV_IMPORTS_MANIFEST_NAME = '__vite_rsc_env_imports_manifest.js'

const ENV_IMPORTS_MANIFEST_PLACEHOLDER = 'virtual:rue-rsc/env-imports-manifest'
const ENV_IMPORTS_ENTRY_FALLBACK = 'virtual:rue-rsc/env-imports-entry-fallback'
...
export function ensureEnvironmentImportsEntryFallback({ environments }: ResolvedConfig): void {
...
export function vitePluginImportEnvironment(manager: RscPluginManager): Plugin[] {
  return [
    {
      ...
      transform: {
        ...
        async handler(code, id) {
          ...
          for (const match of stripLiteral(code).matchAll(
            ...
            const [specifier, options]: [string, { environment: string }] = evalValue(
              `[${argCode}]`,
            )
      ...
      renderChunk(code, chunk) {
        if (code.includes(ENV_IMPORTS_MANIFEST_PLACEHOLDER)) {
          const replacement = normalizeRelativePath(
            path.relative(path.join(chunk.fileName, '..'), ENV_IMPORTS_MANIFEST_NAME),
          )
    ...
    createVirtualPlugin(ENV_IMPORTS_ENTRY_FALLBACK.slice('virtual:'.length), () => {
...
export function writeEnvironmentImportsManifest(manager: RscPluginManager): void {
  ...
  for (const [sourceEnv, byTargetEnv] of Object.entries(manager.environmentImportMetaMap)) {
    ...
    for (const [_targetEnv, imports] of Object.entries(byTargetEnv)) {
      ...
      for (const [resolvedId, meta] of Object.entries(imports)) {
        ...
        if (!bundle) {
          throw new Error(`[vite-rsc] missing bundle for environment import: ${meta.targetEnv}`)
        }
        const chunk = Object.values(bundle).find(
          c => c.type === 'chunk' && c.facadeModuleId === resolvedId,
        )
        if (!chunk) {
          throw new Error(`[vite-rsc] missing output for environment import: ${resolvedId}`)
        }
