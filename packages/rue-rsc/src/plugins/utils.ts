...
export function createVirtualPlugin(name: string, load: Plugin['load']): Plugin {
  const virtualPath = name.startsWith('vite-rsc/')
    ? name.slice('vite-rsc/'.length)
    : name.startsWith('rue-rsc/')
      ? name.slice('rue-rsc/'.length)
      : name
  const virtualId = toRueRscVirtualId(virtualPath)
  const resolvedId = '\0' + virtualId
  return {
    name: `rsc:virtual-${name}`,
    resolveId: {
      filter: { id: rscVirtualExactRegex(virtualPath) },
      handler(source) {
        if (normalizeRscVirtualId(source) === virtualId) {
          return resolvedId
        }
      },
    },
    load: {
      filter: { id: rscResolvedVirtualExactRegex(virtualPath) },
      handler(id, options) {
        const normalizedId = normalizeRscVirtualId(id)
        if (normalizedId === resolvedId) {
          return (load as Function).apply(this, [normalizedId, options])
        }
      },
    },
  }
...
export function getEntrySource(config: Pick<ResolvedConfig, 'build'>, name?: string): string {
...
export function getFallbackRollupEntry(input: Rollup.InputOptions['input'] = {}): {
  name: string
  source: string
} {
...
export function normalizeRollupOpitonsInput(
  ...
  if (Array.isArray(input)) {
    return Object.fromEntries(
      input.map(file => [path.basename(file).slice(0, -path.extname(file).length), file]),
    )
  }
  return input
}
