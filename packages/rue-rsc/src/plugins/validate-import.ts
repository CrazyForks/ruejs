...
import { normalizeRscVirtualId, rscResolvedVirtualPrefixRegex, toRueRscVirtualId } from './shared'
...
export function validateImportPlugin(): Plugin {
  return {
      ...
      async handler(source, _importer, options) {
        ...
        if (source === 'client-only' || source === 'server-only') {
          if (
            ...
            return {
              id: `\0${toRueRscVirtualId(`validate-imports/invalid/${source}`)}`,
              moduleSideEffects: true,
            }
          }
          return {
            id: `\0${toRueRscVirtualId(`validate-imports/valid/${source}`)}`,
    ...
    load: {
      filter: { id: rscResolvedVirtualPrefixRegex('validate-imports/') },
      handler(id) {
        const normalizedId = normalizeRscVirtualId(id)
        if (normalizedId.startsWith(`\0${toRueRscVirtualId('validate-imports/invalid/')}`)) {
          // it should surface as build error but we make a runtime error just in case.
          const source = normalizedId.slice(normalizedId.lastIndexOf('/') + 1)
          return `throw new Error("invalid import of '${source}'")`
        }
        if (normalizedId.startsWith(`\0${toRueRscVirtualId('validate-imports/')}`)) {
          return `export {}`
        }
      },
    },
    ...
    transform: {
      order: 'post',
      filter: {
        id: rscResolvedVirtualPrefixRegex('validate-imports/invalid/'),
      },
      async handler(_code, id) {
        if (this.environment.mode === 'dev') {
          if (
            normalizeRscVirtualId(id).startsWith(
              `\0${toRueRscVirtualId('validate-imports/invalid/')}`,
            )
          ) {
