...
async function scanModuleExports(
  ...
  const bareStars = ast.body.filter(
    (n): n is ExportAllDeclaration => n.type === 'ExportAllDeclaration' && !n.exported,
  )
  ...
  for (const node of bareStars) {
    ...
    if (!resolved) {
      throw Object.assign(
        new Error(`failed to resolve export-all source ${JSON.stringify(source)}`),
        { pos: node.start },
      )
    }
    starSources.push({
...
async function collectExportScan(
  ...
  const names = [...scan.explicitNames, ...resolved.plans.flatMap(item => item.names)]
  return {
    names,
      ...
      if (node.declaration) {
        ...
        for (const spec of node.specifiers) {
          if (spec.exported.type === 'Identifier') {
            ...
            throw Object.assign(new Error('unsupported string literal export name'), {
...
function resolveStarExports(scan: ModuleExportScan): StarExportResolution {
  ...
  for (const source of scan.starSources) {
    const names = source.scan.names.filter(
      name => name !== 'default' && !scan.explicitNames.has(name) && !ambiguousNames.has(name),
    )
    plans.push({
      node: source.node,
      names,
