...
`
    await expect(testTransform(input)).rejects.toThrow('unsupported string literal export name')
  })
  ...
  test('re-export all (ignoreExportAllDeclaration)', async () => {
    ...
    expect(await testTransform(input, { ignoreExportAllDeclaration: true })).toMatchInlineSnapshot(`
      {
        "exportNames": [],
        "output": "",
      }
  ...
  test('re-export all (unresolved throws)', async () => {
    const input = `export * from "./dep"`
    await expect(testTransform(input)).rejects.toThrow('unsupported ExportAllDeclaration')
  })
