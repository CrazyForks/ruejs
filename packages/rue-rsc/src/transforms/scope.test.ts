...
describe('fixtures', () => {
  ...
  for (const [file, mod] of Object.entries(fixtures)) {
    it(path.basename(file), async () => {
      ...
      await expect(JSON.stringify(serialized, null, 2) + '\n').toMatchFileSnapshot(
        file + '.snap.json',
      )
    })
  }
...
function serializeScopeTree(
  ...
  const { nodeScope, referenceToDeclaredScope, referenceToNode, scopeToReferences, moduleScope } =
    scopeTree
  ...
  function getDirectReferences(scope: Scope) {
    ...
    const childRefSet = new Set(
      scopeChildrenMap.get(scope).flatMap(c => scopeToReferences.get(c) ?? []),
    )
    ...
    return {
      ...
      references: getDirectReferences(scope).map(id => {
        ...
        if (options?.showReferenceNode) {
          reference.referenceNode = serializeReferenceNode(referenceToNode.get(id)!)
        }
        return reference
      }),
      children: scopeChildrenMap.get(scope)!.map(child => serializeScope(child)),
    }
  }
