...
describe('fixtures', () => {
  const fixtures = import.meta.glob(['./fixtures/hoist/**/*.js', '!**/*.snap.*'], {
    query: 'raw',
  })

  async function transformFixture(input: string, options?: { encode?: boolean }) {
  ...
  for (const [file, mod] of Object.entries(fixtures)) {
    it(path.basename(file), async () => {
      ...
      await expect.soft(await transformFixture(input)).toMatchFileSnapshot(file + '.snap.js')
      await expect
...
`
  ...
  it('higher order', async () => {
    // packages/rue-server/examples/next/app/actions/header/page.tsx
    // packages/rue-server/examples/next/app/actions/header/validator.ts
    const input = `
...
`
  ...
  // edge case found in https://github.com/remix-run/rue-router/blob/98367e49900701c460cb08eb16c2441da5007efc/playground/rsc-vite/src/routes/home/home.tsx
