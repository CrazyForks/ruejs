...
import encryptionKeySource from 'virtual:rue-rsc/encryption-key'
import { decodeRuePayloadReadableStream, renderRuePayloadToReadableStream } from '../core/payload'
import {
  arrayToStream,
  concatArrayStream,
  decryptBuffer,
...
export async function encryptActionBoundArgs(originalValue: unknown): Promise<string> {
  const serialized = renderRuePayloadToReadableStream(originalValue)
...
export async function decryptActionBoundArgs(
  ...
  return decodeRuePayloadReadableStream(serialized)
}
