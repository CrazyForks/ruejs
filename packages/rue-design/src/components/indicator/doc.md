...
- `placement`：支持 `top-start`、`middle-center`、`bottom-end` 这类组合式定位别名。
- `horizontal` / `vertical`：继续保留；如果同时和 `placement` 一起传入，则以显式值为准。
- `offset`：接受 `[x, y]`，在现有锚点基础上做细调，适合避让边框、圆角或阴影。
- `item` + `itemProps`：单角标快捷写法，不必每次都手写一个 `Indicator.Item`。
- `items`：多角标数据驱动写法，适合同时挂状态点和操作按钮的场景。
