...
- `title`、`overlay`、`content`：在保留 `tip` 和 `Tooltip.Content` 的基础上，补充更语义化的提示内容写法。
- `placement`：支持 `topLeft`、`rightBottom` 等角落别名，内部映射到 daisyUI 的四个方向类。
- `trigger`：支持 `hover`、`focus`、`click`、`contextMenu`，可单独或组合使用。
- `open`、`defaultOpen`、`onOpenChange`：支持受控与非受控的显示状态。
- `color`：既可以使用 daisyUI 的语义色，也可以直接传自定义颜色字符串。
- `classNames`、`styles`、`overlayClassName`、`overlayStyle`：分别用于定制根节点和提示内容。