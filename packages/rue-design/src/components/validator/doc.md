...
- `appearance`：自动补齐 `input`、`select`、`textarea`、`checkbox`、`toggle` 这些宿主类名。
- `size`、`status`：可直接拼接宿主尺寸和状态类，适合服务端校验或异步反馈场景。
- `Validator.Hint.hideUntilInvalid`：自动追加 `hidden`，让提示在无效前不占布局空间。
- `Validator.Hint.lines`：按多行规则渲染提示内容，不必手写多个 `br`。
- `Validator.Field`：统一 `label`、`hint`、`extra`、`requiredMark` 与 `aria-describedby` 的常见表单结构。