---
title: Select
desc: Select keeps the native dropdown behavior and adds Rue-specific data-source, shell, and semantic callback APIs.
...
### ~Rue enhanced data source and grouped options

Rue 的增强封装仍然基于原生 `<select>`，只是把 `options`、`fieldNames`、`placeholder` 这些业务里常见的声明式能力补了进来。
...
### ~Rue shell with prefix, suffix and clear

当前后缀、前缀、附加文案存在时，Rue 会进入 shell 模式，但内部真正提交值的仍然是原生 `<select>` 节点。

```jsx
const owner = ref('platform')
...
### ~Multiple selection with maxCount

`mode="multiple"` 现在默认是紧凑下拉多选，已选项会以内联标签展示；`maxCount` 仍然会把最终选择裁剪回允许范围。

```jsx
const selected = ref(['release', 'design'])
...
如果你就是需要浏览器原生 listbox，可以显式传入 `nativeSize`。这时 Select 会保留原生多选列表框，并用 `nativeSize` 控制可见行数。

```jsx
const selected = ref(['release', 'labs'])
