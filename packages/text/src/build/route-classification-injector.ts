 ...
 *
 * Codegen describes route shape by emitting stable `__TEXT_CLASS` stubs and
...
// `__TEXT_CLASS` retaining its name, which holds because RSC entry chunk
// bindings are not subject to scope-hoisting renames.
const CLASS_STUB_RE = /function __TEXT_CLASS\(routeIdx\)\s*\{\s*return null;?\s*\}/
const REASONS_STUB_RE = /function __TEXT_CLASS_REASONS\(routeIdx\)\s*\{\s*return null;?\s*\}/
...
function findClassificationChunk(
  ...
  // will reference `__TEXT_CLASS` via per-route calls.
  const chunksMentioningStub = chunks.filter(chunk => chunk.code.includes('__TEXT_CLASS'))
  ...
  if (chunkWithStubBody === undefined) {
    throw new Error(
      `text: build-time classification — __TEXT_CLASS is referenced in ${chunksMentioningStub
  ...
  if (chunksWithStubBody.length > 1) {
    throw new Error(
      `text: build-time classification — expected __TEXT_CLASS stub in exactly one RSC chunk, found ${chunksWithStubBody.length}`,
    )
  }
...
export function planRouteClassificationInjection(
  ...
  if (options.enableDebugReasons && !REASONS_STUB_RE.test(target.code)) {
    throw new Error(
      'text: build-time classification — __TEXT_CLASS_REASONS stub is missing alongside __TEXT_CLASS. The generator and generateBundle have drifted.',
    )
  }
  ...
  const patchedBody = `function __TEXT_CLASS(routeIdx) { return (${replacement})(routeIdx); }`
  ...
  if (options.enableDebugReasons) {
    ...
    const patchedReasonsBody = `function __TEXT_CLASS_REASONS(routeIdx) { return (${reasonsReplacement})(routeIdx); }`
