/**
 * Shared utilities for reading/writing text-server.json.
 *
...
/**
 * Read the prerender secret from `text-server.json` in `serverDir`.
 *
...
export function readPrerenderSecret(serverDir: string): string | undefined {
  const manifestPath = path.join(serverDir, 'text-server.json')
