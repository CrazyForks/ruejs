...
export function augmentSsrManifestFromBundle(
  ...
  const textManifest = {} as Record<string, Set<string>>

  for (const [key, files] of Object.entries(ssrManifest)) {
    const normalizedKey = normalizeManifestModuleId(key, root)
    if (!textManifest[normalizedKey]) textManifest[normalizedKey] = new Set<string>()
    for (const file of files) {
      textManifest[normalizedKey].add(normalizeManifestFile(file))
    }
  }
  ...
  for (const item of Object.values(bundle)) {
    ...
    for (const moduleId of Object.keys(chunk.modules ?? {})) {
      ...
      if (!textManifest[key]) textManifest[key] = new Set<string>()
      for (const file of files) {
        textManifest[key].add(file)
      }
    }
  }

  return Object.fromEntries(
    Object.entries(textManifest).map(([key, files]) => [key, [...files]]),
