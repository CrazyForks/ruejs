 ...
 * alongside compressible assets in dist/client/_text/static/. Served
 ...
 *
 * `assetsDir` defaults to `"_text/static"` (Text.js's canonical convention,
 ...
 * `"cdn/_text/static"`); callers should resolve that with
 * `resolveAssetsDir(assetPrefix)` and thread it through. Without this,
 * `assetPrefix` builds would walk an empty `_text/static/` directory and
 * emit zero compressed variants.
 */
export async function precompressAssets(
  clientDir: string,
  options: {
    /** Subdirectory under `clientDir` containing hashed assets. Defaults to `"_text/static"`. */
    assetsDir?: string
