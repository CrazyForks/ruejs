    ...
    for (let layoutIndex = 0; layoutIndex < route.layouts.length; layoutIndex++) {
      ...
      if (source === undefined) {
        try {
          ...
          throw new Error(
            `text: failed to read layout for route ${route.pattern} at ${layoutPath}`,
            { cause },
          )
        }
      }
...
function mergeLayersForRoute(
  ...
  for (const [layoutIdx, kind] of route.layer1) {
    ...
    if (reason === undefined) {
      throw new Error(
        `text: layout ${layoutIdx} in route ${route.pattern} has a Layer 1 decision without a reason`,
      )
    }
 ...
 *
 * The runtime consults this map only when `TEXT_DEBUG_CLASSIFICATION` is
