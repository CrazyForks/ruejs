...
type StandaloneBuildOptions = {
  ...
  outDir: string
  /**
   * Test hook: override text package root used for embedding runtime files.
   */
  textPackageRoot?: string
}
...
/**
 * Read the externals manifest written by the `text:server-externals-manifest`
 * Vite plugin during the production build.
 *
 * The manifest (`dist/server/text-externals.json`) contains the exact set of
...
function readServerExternalsManifest(serverDir: string): string[] {
  const manifestPath = path.join(serverDir, 'text-externals.json')
  ...
  return (
    readJsonFile<string[]>(manifestPath, {
      onError: err => {
        console.warn(
          `[text] Warning: failed to parse ${manifestPath}, proceeding without externals manifest: ${String(err)}`,
        )
      },
    }) ?? []
  )
...
function writeStandaloneServerEntry(filePath: string): void {
  // Uses import.meta.dirname (Node >= 21.2, text requires >= 22) so the
  // entry point is pure ESM — no need for CJS require() or __dirname.
  //
  // The static import of "text/server/prod-server" is intentional: that
  // subpath is a documented export in text's package.json exports map and
  // is always present in the standalone node_modules/text/dist tree
  // (emitStandaloneOutput copies text's dist/ directory in full). A static
...
startProdServer({
  ...
  console.error("[text] Failed to start standalone server");
  console.error(error);
  process.exit(1);
});
`
 ...
 * `dist/server/text-externals.json`, which is written by the
 * `text:server-externals-manifest` Vite plugin during the production build.
...
export function emitStandaloneOutput(options: StandaloneBuildOptions): StandaloneBuildResult {
  ...
  if (!fs.existsSync(clientDir) || !fs.existsSync(serverDir)) {
    throw new Error(`No build output found in ${outDir}. Run ruetext build first.`)
  }
  ...
  // Seed from the manifest written by text:server-externals-manifest during
  ...
  //
  // The manifest is always written to dist/server/text-externals.json regardless
  ...
  const initialPackages = readServerExternalsManifest(serverDir).filter(name => name !== 'text')
  const copiedSet = new Set<string>()
  copyPackageAndRuntimeDeps(root, standaloneNodeModulesDir, initialPackages, copiedSet)

  // Always embed the exact text runtime that produced this build.
  const textPackageRoot = resolveTextPackageRoot(options.textPackageRoot)
  const textDistDir = path.join(textPackageRoot, 'dist')
  if (!fs.existsSync(textDistDir)) {
    throw new Error(`text runtime dist/ not found at ${textPackageRoot}`)
  }
  const textTargetDir = path.join(standaloneNodeModulesDir, 'text')
  fs.mkdirSync(textTargetDir, { recursive: true })
  fs.copyFileSync(
    path.join(textPackageRoot, 'package.json'),
    path.join(textTargetDir, 'package.json'),
  )
  fs.cpSync(textDistDir, path.join(textTargetDir, 'dist'), {
    recursive: true,
    dereference: true,
    // Defensive: skip any node_modules/ that may exist inside text's dist/.
    filter: src => {
      const rel = path.relative(textDistDir, src)
      return !rel.split(path.sep).includes('node_modules')
    },
  })
  copiedSet.add('text')

  // Copy text's own runtime dependencies. The prod-server imports packages
  ...
  // We resolve them from text's package root so nested requires work correctly.
  const textPkg = readPackageJson(path.join(textPackageRoot, 'package.json'))
  const textRuntimeDeps = runtimeDeps(textPkg).filter(name => !copiedSet.has(name))
  copyPackageAndRuntimeDeps(textPackageRoot, standaloneNodeModulesDir, textRuntimeDeps, copiedSet)
