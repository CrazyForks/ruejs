 ...
 * - "framework" — Rue runtime packages loaded on every page
 * - "text"    — text shims (router, head, link, etc.)
 *
 ...
 *   uses per-package splitting. Text.js only isolates packages >160KB.
...
export function createClientManualChunks(shimsDir: string) {
  return function clientManualChunks(id: string): string | undefined {
    if (id.includes('node_modules')) {
      ...
      if ((CLIENT_FRAMEWORK_PACKAGES as readonly string[]).includes(pkg)) {
        return 'framework'
      }
    ...
    // text shims — small runtime, shared across all pages.
 ...
 * libraries like mermaid, @mui/material, lucide-rue, etc. These libraries
...
type TextBuildConfig = NonNullable<UserConfig['build']>
type TextBuildBundlerOptions = NonNullable<TextBuildConfig['rolldownOptions']>
type TextBuildConfigWithLegacy = TextBuildConfig & {
  rollupOptions?: TextBuildBundlerOptions
}

export function getBuildBundlerOptions(
  build: UserConfig['build'] | undefined,
): TextBuildBundlerOptions | undefined {
  const buildConfig = build as TextBuildConfigWithLegacy | undefined
...
export function withBuildBundlerOptions(
  ...
  bundlerOptions: TextBuildBundlerOptions,
): Partial<TextBuildConfigWithLegacy> {
