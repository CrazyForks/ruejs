/**
 * Build report — prints a Text.js-style route table after `text build`.
 *
...
function parseRouteModuleWithLang(code: string, lang: 'ts' | 'tsx'): Program | null {
  try {
    const result = parseSync(`text-route.${lang}`, code, {
      astType: 'ts',
      lang,
      sourceType: 'module',
    })
 ...
 * For re-export specifiers, this intentionally follows Text.js' static analyzer
 ...
 * means "cache indefinitely" in Text.js segment config.
 ...
 *   Infinity — fully static (treated same as false by Text.js)
...
function extractWrappedGetStaticPropsRevalidate(code: string): number | false | null {
  ...
  const program = parseRouteModule(`function __textGetStaticProps() {\n${code}\n}`)
  if (!program) return null

  for (const node of program.body) {
    if (node.type === 'FunctionDeclaration' && node.id?.name === '__textGetStaticProps') {
 ...
 * means no segment config is present and the caller should defer to the text
...
export function buildReportRows(options: {
  ...
  for (const route of options.appRoutes ?? []) {
    ...
    const { type, revalidate } = classifyAppRoute(renderEntryPath, route.routePath, route.isDynamic)
  ...
  // Sort purely by path — mirrors filesystem order, matching Text.js output style
...
/**
 * Formats a list of RouteRows into a Text.js-style build report string.
 *
 * Example output:
 *   Route (pages)
 *   ┌ ○ /
...
export function formatBuildReport(rows: RouteRow[], routerLabel = 'app'): string {
  ...
  if (usedTypes.includes('unknown')) {
    lines.push('')
    lines.push('  ? Some routes could not be classified. text currently uses static analysis')
    lines.push(
...
export function findDir(root: string, ...candidates: string[]): string | null {
  for (const candidate of candidates) {
    ...
    try {
      ...
      // not found or not a directory — try text candidate
    }
  }
  return null
}
 ...
 * Text.js-style build report to stdout.
 *
 * Called at the end of `text build` in cli.ts.
 */
