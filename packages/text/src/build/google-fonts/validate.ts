...
//
// Text drops Text's SWC-coupled `(functionName, fontFunctionArgument)`
...
export function validateGoogleFontOptions(
  ...
  if (availableSubsets.length === 0) {
    ...
    // Deliberate parity gap: Text.js uses `if (!subsets)` so an explicit
    // `subsets: []` passes silently. text rejects it as well, since an
