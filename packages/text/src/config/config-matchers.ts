 ...
 * (prod-server.ts) so both apply text.config.js rules identically.
 */
 ...
 *
 * Redirect/rewrite patterns are static — they come from text.config.js and
 ...
 *
 * Each TextHeader rule has a `source` that is run through escapeHeaderSource()
 ...
 * condition objects are static (from text.config.js) so the compiled RegExp
 ...
 *
 * Many Text.js apps generate 50-100 redirect rules of the form:
 ...
 *   - `/:textInternalLocale(en|fr)/foo` (locale segment mandatory — emitted
...
type RedirectIndex = {
   ...
   */
  linear: Array<[number, TextRedirect]>
}

const _redirectIndexCache = new WeakMap<TextRedirect[], RedirectIndex>()
 ...
 */
function _getRedirectIndex(redirects: TextRedirect[]): RedirectIndex {
  let index = _redirectIndexCache.get(redirects)
  if (index !== undefined) return index

  const localeStatic = new Map<string, LocaleStaticEntry[]>()
  const linear: Array<[number, TextRedirect]> = []
...
export function safeRegExp(pattern: string, flags?: string): RegExp | null {
  if (!isSafeRegex(pattern)) {
    console.warn(
      `[text] Ignoring potentially unsafe regex pattern (ReDoS risk): ${pattern}\n` +
...
/**
 * Convert a Text.js header/rewrite/redirect source pattern into a regex string.
 *
 ...
 * Text.js: the rule only matches when the inbound request was under the
 ...
 *
 * @see .textjs-ref/packages/text/src/lib/load-custom-routes.ts:198-220
 */
...
export function applyMiddlewareRequestHeaders(
  ...
  const textHeaders = buildRequestHeadersFromMiddlewareResponse(
    request.headers,
    middlewareHeaders,
    options,
  )
...
function _cachedConditionRegex(value: string): RegExp | null {
  return getCachedRegex(_compiledConditionCache, value, () =>
    ...
    // Matches Text.js: new RegExp(`^${hasItem.value}$`)
...
/**
 * Match a text config pattern (from redirects/rewrites sources) against a pathname.
 ...
 *
 * Text.js conditionally appends `(/)?` to rewrite/redirect/header source
 * regexes when `trailingSlash: true` (see Text.js
...
export function matchConfigPattern(
  ...
  if (
    ...
    try {
      ...
      // from text.config.js and are static, so we only need to compile each
 ...
 * to evaluate has/missing conditions. Text.js always has request context
 ...
 * Text.js apps — e.g. `/:locale(en|es|fr|...)?/some-path`) are handled via
...
export function matchRedirect(
  ...
  if (index.localeStatic.size > 0) {
    ...
    // by `applyLocaleToRoutes` as `/:textInternalLocale(en|fr)/foo` — must
 ...
 * Text.js handles these as server-side reverse proxies, forwarding the request
...
export async function proxyExternalRequest(
  ...
  // used only by text's own prerender pipeline.
  headers.delete(TEXT_PRERENDER_SECRET_HEADER)
  headers.delete(TEXT_PRERENDER_ROUTE_PARAMS_HEADER)
  // Internal App Router dev middleware context must never leave the dev server.
  headers.delete(TEXT_MW_CTX_HEADER)
  ...
  try {
    ...
    if (e instanceof Error && e.name === 'AbortError') {
      console.error('[text] External rewrite proxy timeout:', targetUrl.href)
 ...
 * Mirrors `escape-string-regexp` semantics used by Text.js's processRoutes.
 */
...
/**
 * Apply Text.js i18n locale-prefix transformation to a set of redirect or
 * rewrite rules. Mirrors the relevant slice of Text.js's `processRoutes`
 ...
 *     starts with `/:textInternalLocale(en|sv|nl)` so that locale-prefixed
 *     URLs match. For redirects only, a second variant prefixed with
 *     `/${defaultLocale}` is also emitted, matching Text.js exactly.
 *   - **Text divergence**: we ALSO retain the original (unprefixed) source
 *     so that requests for the default locale that arrive without a prefix
 *     still match. Text.js solves this upstream by path-normalising every
 *     incoming default-locale request to include the prefix
 *     (`resolve-routes.ts` lines ~251-263); text currently does that
 ...
 * `/:textInternalLocale` for the locale-capture variant so the locale
 * survives the rewrite/redirect target.
 *
 * Mirrors the Text.js reference in
 * packages/text/src/lib/load-custom-routes.ts — see `processRoutes`.
 */
export function applyLocaleToRoutes<T extends TextRedirect | TextRewrite>(
  routes: T[],
  ...
  const internalLocale = `/:textInternalLocale(${localesAlternation})`

  // Mirrors Text.js: the root source `"/"` is collapsed to `""` only when
  // `trailingSlash` is unset. With `trailingSlash: true` the source is
  // preserved so the emitted variant is `/:textInternalLocale(en|fr)/`
  // rather than `/:textInternalLocale(en|fr)`.
  const suffixFor = (source: string): string => (source === '/' && !trailingSlash ? '' : source)

  // For redirects, Text.js emits a per-default-locale literal variant
  // (so that `/${defaultLocale}/old` redirects to the unprefixed destination
  // and the default locale is implicitly stripped). For rewrites Text.js
  // emits only the `:textInternalLocale` form. We mirror that distinction.
  //
  // The list is a single-element array today; domain-locale support (which
  // Text.js wires up alongside `i18n.domains`) will append each domain's
  // `defaultLocale` here once text mirrors that branch — tracked as part
  ...
  for (const r of routes) {
    ...
    // Destinations may be absolute URLs (external) — Text.js skips the
    ...
    // whose destination does NOT carry a locale prefix (Text.js parity).
    if (!isExternal) {
    ...
    // Emit the `:textInternalLocale` variant that matches all locales.
    ...
    if (internalDestination && internalDestination.startsWith('/') && !isExternal) {
      internalDestination = `/:textInternalLocale${
    ...
    // See the docblock above for why this differs from upstream Text.js.
    out.push(r)
  }
  return out
}
