...
type TextEnvMode = 'development' | 'production' | 'test'
 ...
 * by Text.js, when it's anywhere in the workspace) to make `NODE_ENV` a
...
/**
 * Text.js-compatible dotenv lookup order (highest priority first).
 */
export function getDotenvFiles(mode: TextEnvMode): string[] {
  return [`.env.${mode}.local`, ...(mode === 'test' ? [] : ['.env.local']), `.env.${mode}`, '.env']
}

/**
 * Load .env files into processEnv with Text.js-like precedence:
 ...
 *
 * - **text** populates `process.env` so that server-side code (SSR, API
 *   routes, Server Components) can read env vars at runtime, and so the Vite
 *   plugin's `config()` hook can scan `process.env` for `TEXT_PUBLIC_*` vars
 *   to inline via `define`.
 *
 * - **Vite** loads .env files to populate `import.meta.env.VITE_*` for its
 *   own client exposure mechanism (which Text.js apps don't use).
 *
