...
/** Resolve empty-module text to this file (.js in dist, .ts in source). */
...
/** Absolute path to the text empty-module fallback for composed client instrumentation. */
export const INSTRUMENTATION_CLIENT_EMPTY_MODULE = resolveInstrumentationClientEmptyModule(
  import.meta.dirname,
)

/**
 * Generate a virtual ESM module that implements the Text.js
 ...
 * `private-text-instrumentation-client` resolves directly to the user's
 ...
 *
 * **Specifier resolution:** Text.js webpack loader resolves every inject against
 * the project root at build time (`this.resolve(rootContext, spec)`) and emits
 * `require(resolvedPath)`. Text pre-resolves `./` and `../` in the plugin
 * `config()` hook; bare specifiers rely on Vite resolution at bundle time.
 *
 * @param injects - Module specifiers from `textConfig.instrumentationClientInject`
