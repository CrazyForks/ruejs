...
/**
 * Consume the text positional argument as a value for a flag.
 *
 * Throws if:
 * - No argument follows (end of argv)
 * - The value is an empty string
 * - The text argument is another flag (--long or -x short form)
 */
function takeValue(flag: string, args: string[], i: number): string {
  ...
  if (FLAG_PATTERN.test(text)) {
    throw new Error(`${flag} requires a value, but got "${text}" which looks like another flag.`)
  }
  return text
}
