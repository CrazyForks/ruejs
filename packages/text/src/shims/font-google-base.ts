...
/**
 * text/font/google shim
 *
 * Provides a compatible shim for Text.js Google Fonts.
 *
 * Two modes:
 * 1. **Dev / CDN mode** (default): Loads fonts from Google Fonts CDN via <link> tags.
 * 2. **Self-hosted mode** (production build): The text:google-fonts Vite plugin
...
type FontLoaderOptions = FontOptions & {
  /**
   * Internal payload injected by the text:google-fonts transform after
   * metadata validation. Runtime must prefer these values over user options
   * because they represent the resolved Text-compatible face, including
...
function createFontIdentity(
  ...
  return hashString(
    [
      ...
      options._text?.font?.selfHostedCSS ?? '',
      options._text?.font?.fontWeight?.toString() ?? '',
      options._text?.font?.fontStyle ?? '',
    ].join('\0'),
  )
}
 ...
 * (`text:google-fonts` in `src/plugins/fonts.ts`) statically resolves
 * each font call's axis values against the bundled metadata, fetches the
 * Google Fonts CSS, and injects the resulting CSS as
 * `_text.font.selfHostedCSS` so the runtime never queries Google. The shim
 ...
 * Text.js generates equivalent rules at build time.
 *
 * In Text.js, the .className class sets font-family and any single
...
function injectClassNameRule(className: string, fontStyle: FontStyle): void {
  ...
  styleElement.setAttribute('data-text-font-class', className)
 ...
 *
 * In Text.js, the .variable class ONLY sets the CSS variable — it does NOT
...
function injectVariableClassRule(
  ...
  // This matches Text.js behavior where .variable classes only define CSS variables.
  ...
  style.setAttribute('data-text-font-variable', variableClassName)
...
function injectSelfHostedCSS(css: string): void {
  ...
  const style = document.createElement('style')
  style.textContent = css
  style.setAttribute('data-text-font-selfhosted', 'true')
  document.head.appendChild(style)
}
...
export function createFontLoader(family: string): FontLoader {
  return function fontLoader(options: FontLoaderOptions = {}): FontResult {
    const internal = options._text?.font
    ...
    // In Text.js, `variable` returns a CLASS NAME that sets the CSS variable.
...
// Usage: import { Inter } from 'text/font/google'
