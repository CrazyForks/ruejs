...
// Mirrors Text.js's absolute URL classification:
// packages/text/src/shared/lib/utils.ts
...
export function toSameOriginAppPath(url: string, basePath: string): string | null {
  ...
  try {
    const parsed = new URL(localPath, 'http://text.local')
 ...
 *
 * Ported from Text.js: packages/text/src/shared/lib/router/utils/parse-path.ts
 */
 ...
 *
 * Ported from Text.js: packages/text/src/shared/lib/router/utils/remove-trailing-slash.ts
 */
 ...
 * `trailingSlash` config option in `text.config.js`. Used by the `<Link>`
 ...
 *
 * Behaviour matches Text.js's client-side `normalizePathTrailingSlash`:
...
export function isHashOnlyBrowserUrlChange(
  ...
  try {
    ...
    const text = new URL(href, currentHref)
    const currentPathname = stripBasePath(current.pathname, basePath)
    const textPathname = stripBasePath(text.pathname, basePath)
    return currentPathname === textPathname && current.search === text.search && text.hash !== ''
  } catch {
    return false
  }
}
