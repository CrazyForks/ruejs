 ...
 *
 * The real `server-only` npm package uses the `rue-server` export
 * condition: it's a no-op when resolved with rue-server conditions
 ...
 * rue-server conditions, so if the real package is installed it works
