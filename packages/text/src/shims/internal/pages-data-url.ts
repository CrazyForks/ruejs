/**
 * Client-side helpers for the Pages Router `/_text/data/<buildId>/<page>.json`
 * endpoint.
 *
 * Ported from Text.js:
 *   - `packages/text/src/client/page-loader.ts` (`getDataHref`)
 *   - `packages/text/src/shared/lib/router/utils/get-asset-path-from-route.ts`
 *
 ...
 * wire-format contract between text's client navigation and its data
 * endpoint.
 */
import { matchRoutePattern, routePatternParts } from '../../routing/route-pattern.js'

/**
 * Append `.json` and the `/_text/data/<buildId>` prefix to a page pathname.
 *
 * Mirrors Text.js' `getAssetPathFromRoute` + `getDataHref` behaviour:
 *   `/`            → `/_text/data/<id>/index.json`
 *   `/about`       → `/_text/data/<id>/about.json`
 *   `/index`       → `/_text/data/<id>/index/index.json`  (explicit `/index` page)
 *   `/blog/foo`    → `/_text/data/<id>/blog/foo.json`
 *
...
export function buildPagesDataPath(buildId: string, pagePath: string): string {
  ...
  // Text.js' `getAssetPathFromRoute` denormalisation:
  //   "/"            → "/index"
  //   "/index"       → "/index/index"
  ...
  return `/_text/data/${buildId}${asset}.json`
}

/**
 * Build the full data URL including the basePath, the search string, and the
 * `/_text/data/<buildId>/<page>.json` segment.
 *
...
type PagesPatternMatch = {
  /** The matched route pattern in Text.js bracket format (e.g. `/blog/[slug]`). */
  pattern: string
...
/**
 * Find the route pattern (Text.js bracket format) that matches `pathname`.
 *
 ...
 * text handles routes that exist on the server but are not in the
