/**
 * Shared decision helper for the Pages Router `/_text/data/<id>/<page>.json`
 ...
 *   - A buildId on the current `__TEXT_DATA__`, since the data URL embeds it.
 *   - Same-origin (cross-origin URLs do not hit our data endpoint).
 *
 * Locale handling: route patterns in `__TEXT_PAGE_PATTERNS__` are
 ...
 *
 * Ported from Text.js: `packages/text/src/client/page-loader.ts`
 * (`getDataHref`). text's equivalent uses an in-memory loader map instead
 * of Text.js' `_buildManifest.js`.
 */
export function resolvePagesDataNavigationTarget(
  ...
  const loaders = window.__TEXT_PAGE_LOADERS__
  const patterns = window.__TEXT_PAGE_PATTERNS__
  if (!loaders || !patterns || patterns.length === 0) return null

  const buildId = (window.__TEXT_DATA__ as TextTextData | undefined)?.buildId ?? undefined
  ...
  const locale = getLocalePathPrefix(pagePath, window.__TEXT_LOCALES__)
