 ...
 * the renderer from executing past the IO boundary. The promise never resolves; it only
 * rejects if the render signal is aborted (e.g., due to a dynamic error or
 * cache-fill completion).
 *
 * Ported from Text.js: packages/text/src/server/dynamic-rendering-utils.ts
 * https://github.com/vercel/next.js/blob/canary/packages/text/src/server/dynamic-rendering-utils.ts
 */
...
export function makeHangingPromise<T>(
  ...
  // the active use() implementation which handles rejections. If never awaited, the abort
