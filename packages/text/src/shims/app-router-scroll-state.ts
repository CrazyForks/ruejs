...
// A scroll intent is staged by `navigateClientSide` (text/navigation) before an
// RSC navigation and consumed by the committed `AppRouterScrollTarget`. Both run
// in the browser, but text/navigation and this module can be loaded through
...
const _SCROLL_INTENT_KEY = Symbol.for('text.appRouterScrollIntent')
...
function getScrollIntentStore(): ScrollIntentStore {
  ...
  globalState[_SCROLL_INTENT_KEY] ??= { textId: 0, pending: null }
