 ...
 * text/server shim
 *
 * Provides TextRequest, TextResponse, and related types that work with
 ...
 * rather than bug-for-bug parity with Text.js internals.
 */
...
const _USE_CACHE_ALS_KEY = Symbol.for('text.cacheRuntime.contextAls')
const _UNSTABLE_CACHE_ALS_KEY = Symbol.for('text.unstableCache.als')
...
function _recordInvalidDynamicUsageError(error: Error): void {
  try {
    const _unifiedAls = _g[Symbol.for('text.unifiedRequestContext.als')] as
      | { getStore(): unknown }
      | undefined
...
export class TextRequest extends Request {
  ...
  constructor(
    ...
    // Match Text.js: reject relative URLs with the canonical error before any
    // fallback URL parsing kicks in. Text.js calls `validateURL(url)` at the
    // top of its TextRequest constructor; we mirror that here so middleware
    ...
    // with TextResponse, and so `javascript:` / `data:` URIs are blocked too.
    const rawUrl = typeof input !== 'string' && 'url' in input ? input.url : String(input)
    validateURL(rawUrl)
    // Strip textConfig before passing to super() — it's text-internal,
    // not a valid RequestInit property.
    const { textConfig: _textConfig, ...requestInit } = init ?? {}
    ...
    const urlConfig: TextURLConfig | undefined = _textConfig
      ? {
          basePath: _textConfig.basePath,
          textConfig: { i18n: _textConfig.i18n, trailingSlash: _textConfig.trailingSlash },
        }
      : undefined
    this._textUrl = new TextURL(url, undefined, urlConfig)
    this._url = process.env.__TEXT_NO_MIDDLEWARE_URL_NORMALIZE
      ? url.toString()
      : this._textUrl.toString()
  ...
  /**
   * The build ID of the Text.js application.
   * Delegates to `textUrl.buildId` to match Text.js API surface.
   * Can be used in middleware to detect deployment skew between client and server.
   */
  get buildId(): string | undefined {
    return this._textUrl.buildId
  }
}

export function isTextRequest(value: unknown): value is TextRequest {
  if (!(value instanceof Request)) return false
  const maybeRequest = value as Partial<TextRequest>
  return maybeRequest.textUrl instanceof TextURL && maybeRequest.cookies instanceof RequestCookies
}
...
/** Valid HTTP redirect status codes, matching Text.js's REDIRECTS set. */
const REDIRECT_STATUSES = new Set([301, 302, 303, 307, 308])

function validateURL(url: string | URL | TextURL): string {
  ...
  try {
    ...
    throw new Error(
      `URL is malformed "${String(
        url,
      )}". Please use only absolute URLs - https://textjs.org/docs/messages/middleware-relative-urls`,
      { cause: error },
    )
  }
}

export class TextResponse<_Body = unknown> extends Response {
   ...
   */
  static json<JsonBody>(body: JsonBody, init?: ResponseInit): TextResponse<JsonBody> {
    const headers = new Headers(init?.headers)
    if (!headers.has('content-type')) {
      headers.set('content-type', 'application/json')
    }
    return new TextResponse(JSON.stringify(body), {
      ...init,
      headers,
    }) as TextResponse<JsonBody>
  }

  /**
   * Create a redirect response.
   */
  static redirect(url: string | URL | TextURL, init?: number | ResponseInit): TextResponse {
    ...
    return new TextResponse(null, { status, headers })
  }
   ...
   */
  static rewrite(destination: string | URL | TextURL, init?: MiddlewareResponseInit): TextResponse {
    const headers = new Headers(init?.headers)
    ...
    return new TextResponse(null, { ...init, headers })
  }

  /**
   * Continue to the text handler (middleware pattern).
   * Sets the x-middleware-text header.
   */
  static text(init?: MiddlewareResponseInit): TextResponse {
    const headers = new Headers(init?.headers)
    headers.set(MIDDLEWARE_TEXT_HEADER, '1')
    if (init?.request?.headers) {
      encodeMiddlewareRequestHeaders(headers, init.request.headers)
    }
    return new TextResponse(null, { ...init, headers })
...
// ---------------------------------------------------------------------------
// TextURL — lightweight URL wrapper with pathname helpers
// ---------------------------------------------------------------------------

export type TextURLConfig = {
  basePath?: string
  textConfig?: {
    i18n?: {
      locales: string[]
     ...
     * with a trailing slash. Matches Text.js's `formatTextPathnameInfo` so that
     * `TextResponse.redirect(request.textUrl)` and `TextResponse.rewrite(url)`
...
export class TextURL {
  ...
  constructor(input: string | URL, base?: string | URL, config?: TextURLConfig) {
    this._url = new URL(input.toString(), base)
    this._basePath = config?.basePath ?? ''
    this._trailingSlash = config?.textConfig?.trailingSlash ?? false
    this._stripBasePath()
    const i18n = config?.textConfig?.i18n
    if (i18n) {
   ...
   * Mirrors Text.js's internal formatTextPathnameInfo().
   */
  private _formatPathname(): string {
    // Build prefix: basePath + locale (skip defaultLocale — Text.js omits it)
   ...
   * Text.js's `formatTextPathnameInfo`: when `trailingSlash` is true, add a
  ...
  clone(): TextURL {
    const textConfig: NonNullable<TextURLConfig['textConfig']> = {}
    if (this._locales) {
      textConfig.i18n = { locales: [...this._locales], defaultLocale: this._defaultLocale! }
    }
    if (this._trailingSlash) {
      textConfig.trailingSlash = true
    }
    const config: TextURLConfig = {
      basePath: this._basePath,
      textConfig: Object.keys(textConfig).length > 0 ? textConfig : undefined,
    }
    // Pass the full href (with locale/basePath re-added) so the constructor
    // can re-analyze and extract locale correctly.
    return new TextURL(this.href, undefined, config)
  }
  ...
  /**
   * The build ID of the Text.js application.
   * Set from `generateBuildId` in text.config.js, or a random UUID if not configured.
   * Can be used in middleware to detect deployment skew between client and server.
   * Matches the Text.js API: `request.textUrl.buildId`.
   */
  get buildId(): string | undefined {
    return process.env.__TEXT_BUILD_ID ?? undefined
  }
}
...
// TextRequest cookies, while headers.ts owns the text/headers cookies object.
class ReadonlyRequestCookiesError extends Error {
  constructor() {
    super(
      'Cookies can only be modified in a Server Action or Route Handler. Read more: https://textjs.org/docs/app/api-reference/functions/cookies#options',
    )
  }
...
// Keep this error message in sync with headers.ts. This adapter backs
// TextRequest headers in force-static route handlers, while headers.ts owns the
// text/headers object.
class ReadonlyRequestHeadersError extends Error {
  constructor() {
    super(
      'Headers cannot be modified. Read more: https://textjs.org/docs/app/api-reference/functions/headers',
    )
  }
...
export type TextMiddlewareResult = TextResponse | Response | null | undefined | void

export type TextMiddleware = (
  request: TextRequest,
  event: TextFetchEvent,
) => TextMiddlewareResult | Promise<TextMiddlewareResult>

/**
 * Minimal TextFetchEvent — extends FetchEvent where available,
 * otherwise provides the waitUntil pattern standalone.
 */
export class TextFetchEvent {
  sourcePage: string
...
export function after<T>(task: Promise<T> | (() => T | Promise<T>)): void {
  ...
  // NOTE: text runs function tasks concurrently with response streaming (text microtask),
  // whereas Text.js queues them to run strictly after the response is sent via onClose.
  ...
  const guarded = promise.catch(err => {
    console.error('[text] after() task failed:', err)
  })

  // TODO: Text.js throws when after() is called outside a request context or when
  // waitUntil is unavailable, preventing silent task loss. text falls back to
