...
text uses `AsyncLocalStorage` (als) to keep each request's state separate
...
including rue rendering - sees the same context through async continuations.

**cross-environment sharing:** the als instance lives on `globalThis` via
`Symbol.for("text.unifiedRequestContext.als")` so all vite module environments
...
calls `runner.import("text/head-state")` (via the `ModuleImporter` interface)
...
5. if the state is accessed by rue components during ssr in dev, load the
