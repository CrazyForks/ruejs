/**
 * text/font/local shim
 *
 * Provides a runtime-compatible shim for Text.js local fonts.
...
function injectFontFaceCSS(css: string, id: string): void {
  ...
  style.setAttribute('data-text-font', id)
...
function injectClassNameRule(className: string, fontStyle: FontStyle): void {
  ...
  const style = document.createElement('style')
  style.textContent = css
  style.setAttribute('data-text-font-class', className)
  document.head.appendChild(style)
}
...
function collectFontPreloads(sources: LocalFontSrc[]): void {
  ...
  for (const src of sources) {
    ...
    // would resolve incorrectly from different page URLs. The text:local-fonts
...
export default function localFont(options: LocalFontOptions): FontResult {
  ...
  const family = sanitizeInternalFontFamily(options._text?.font?.family) ?? `__local_font_${id}`
