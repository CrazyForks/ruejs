...
import { patternToTextFormat } from './routing/route-validation.js'
...
const TEXT_ENV_FILE_CONTENT = `/// <reference types="text" />
/// <reference types="text/image-types/global" />
import "./.text/types/routes.d.ts";

// NOTE: This file should not be edited
// see https://ruejs.huododo.com for more information.
`
...
export async function generateRouteTypes(options: GenerateRouteTypesOptions): Promise<string> {
  ...
  const outPath = path.join(root, '.text', 'types', 'routes.d.ts')
  ...
  await ensureTextEnvFile(root)
  return outPath
}

async function ensureTextEnvFile(root: string): Promise<void> {
  const envPath = path.join(root, 'text-env.d.ts')
  try {
    await fs.writeFile(envPath, TEXT_ENV_FILE_CONTENT, { encoding: 'utf-8', flag: 'wx' })
  } catch (error) {
...
async function collectRouteTypeModel(
  ...
  for (const route of segmentGraph.pages.values()) {
    ...
    addRoute(
      ...
      pageRouteSet,
      model.params,
      patternToTextFormat(route.pattern),
...
async function findAppDir(root: string): Promise<string | null> {
  for (const rel of ['app', path.join('src', 'app')]) {
    ...
    try {
      ...
      // Try the text conventional app directory.
    }
  }
  return null
}
...
function renderRouteTypes(model: RouteTypeModel): string {
  ...
  return `// This file is generated by text. Do not edit.
...
function layoutRouteKeyForSlot(
  ...
  if (!layout) {
    throw new Error(
      `[text] App route graph invariant violated: slot ${slot.id} references missing owner layout ${slot.ownerLayoutId}`,
    )
  }
...
function addRoute(
  ...
  if (existingParamShape) {
    if (!paramShapesEqual(existingParamShape, paramShape)) {
      throw new Error(`[text] Conflicting route param shapes generated for ${route}`)
    }
    return
  }
