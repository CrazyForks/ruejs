...
import type { CachedPagesValue, CacheControlMetadata } from '../shims/cache.js'
import { buildCachedRevalidateCacheControl } from './cache-control.js'
import { buildCacheStateHeaders } from './cache-headers.js'
import { buildPagesCacheValue, type ISRCacheEntry } from './isr-cache.js'
import {
  buildPagesTextDataScript,
...
// Text.js allows `paths` entries to be either an object with a `params` key
...
export type PagesPageModule = {
   ...
   * match Text.js. User code typically falls back via `params || null`, so
   * passing `null` (rather than `{}`) is required for the value to be
   * observable as `null` once the data flows through to the page props.
   *
   * See: test/e2e/edge-pages-support/index.test.ts in Text.js for the
  ...
  getStaticProps?: (context: {
     ...
     *
     * Mirrors Text.js `render.tsx`'s `revalidateReason` on the
     * `GetStaticPropsContext` type — see
     * `.textjs-ref/packages/text/src/types.ts`.
     */
...
type RenderPagesIsrHtmlOptions = {
  ...
  renderIsrPassToStringAsync: (element: TextRenderable) => Promise<string>
  routePattern: string
...
export type ResolvePagesPageDataOptions = {
  ...
  buildId: string | null
  /**
   * When true, this is a `/_text/data/<buildId>/<page>.json` request. Callers
   ...
   * subsequent HTML hits. Text.js handles this the same way — see
   * `isTextDataRequest` checks in `packages/text/src/server/base-server.ts`.
   */
  isDataReq?: boolean
   ...
   * `text` build phase fetches each statically generated page through the
   * production server). Maps to `revalidateReason: "build"` when
   * `getStaticProps` is invoked. Mirrors Text.js's
   ...
   * is not yet implemented in text. The `"on-demand"` branch in the
...
type ResolvePagesPageDataRenderResult = {
   ...
   * empty props and `useRouter().isFallback === true` (matching Text.js's
...
function buildPagesDataNotFoundResponse(): Response {
  // Matches Text.js: `/_text/data/<buildId>/<page>.json` 404 responses use
 ...
 *
 * Handles both shapes Text.js allows:
 *   - { params: { ... } }
 *   - "string-path"
 *
 * For a string entry, compare the entry against the current request URL using
 * the shared `normalizeStaticPathname` helper from
 * `../routing/route-pattern.ts` (which mirrors the Text.js
 * `removeTrailingSlash` behaviour in
 * `.textjs-ref/packages/text/src/build/static-paths/pages.ts`). For an object
 * entry with a missing `params` key, return false rather than throwing — the
 * caller will respond with a 404 just like Text.js does for unlisted paths.
 */
...
function rewritePagesCachedHtml(
  ...
  const bodyMarker = '<div id="__text">'
  const bodyStart = cachedHtml.indexOf(bodyMarker)
  const contentStart = bodyStart >= 0 ? bodyStart + bodyMarker.length : -1
  // This intentionally looks for the bare inline __TEXT_DATA__ marker.
  // Pages responses with scriptNonce are excluded from ISR writes, so cached
  // HTML should never contain nonce-prefixed __TEXT_DATA__ scripts here.
  const textDataMarker = '<script>window.__TEXT_DATA__'
  const textDataStart = cachedHtml.indexOf(textDataMarker)

  if (contentStart >= 0 && textDataStart >= 0) {
    const region = cachedHtml.slice(contentStart, textDataStart)
    const lastCloseDiv = region.lastIndexOf('</div>')
    const gap = lastCloseDiv >= 0 ? region.slice(lastCloseDiv + 6) : ''
    const textDataEnd = cachedHtml.indexOf('</script>', textDataStart) + 9
    const tail = cachedHtml.slice(textDataEnd)

    return cachedHtml.slice(0, contentStart) + freshBody + '</div>' + gap + textDataScript + tail
  }
...
export async function renderPagesIsrHtml(options: RenderPagesIsrHtmlOptions): Promise<string> {
  ...
  return rewritePagesCachedHtml(options.cachedHtml, freshBody, textDataScript)
}

export async function resolvePagesPageData(
  options: ResolvePagesPageDataOptions,
): Promise<ResolvePagesPageDataResult> {
  // Text.js passes `params: null` (effectively) to gSSP/gSP context for
  ...
  // skipping `getStaticProps`. Matches Text.js `render.tsx`'s
  // `if (isSSG && !isFallback)` gate around `getStaticProps`. Data requests
  // (`/_text/data/...json`) still call `getStaticProps` so the client can
  ...
  if (typeof options.pageModule.getStaticPaths === 'function' && options.route.isDynamic) {
    ...
    if (fallback === false && !isValidPath) {
      // For data requests (`/_text/data/...json`), return a JSON-shaped 404
      // so the client router can `res.json()` without blowing up — matches
      // Text.js' behavior. HTML navigations still get the HTML 404 page.
      return {
        kind: 'response',
  ...
  if (typeof options.pageModule.getServerSideProps === 'function') {
    ...
    if (result?.props) {
      // Text.js explicitly supports a Promise value for `props`. Await it
  ...
  if (typeof options.pageModule.getStaticProps === 'function') {
        ...
        async function () {
          return options.runInFreshUnifiedContext(async () => {
            const freshResult = await options.pageModule.getStaticProps?.({
              ...
              // Text.js `render.tsx` (`isBuildTimeSSG ? "build" : "stale"`,
    ...
    const result = await options.pageModule.getStaticProps({
      ...
      locales: options.i18n.locales,
      defaultLocale: options.i18n.defaultLocale,
      // Maps Text.js's resolution in `render.tsx`:
