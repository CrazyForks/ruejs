...
function findGeneratedImageId(
  ...
  for (const item of imageMetadata) {
    ...
    if (!isValidMetadataImageId(itemId)) {
      console.warn(
        `[text] Skipping metadata route ${servedUrl} image id "${itemId}" because metadata image ids must match /^[a-zA-Z0-9-_.]+$/.`,
      )
      continue
    }
    if (itemId === imageId) {
...
async function callDynamicMetadataRoute(
  ...
  if (!functions.defaultExport) {
    console.warn(`[text] Dynamic metadata route ${route.servedUrl} has no default export.`)
    return notFoundResponse()
  }
...
function serveStaticMetadataRoute(route: MetadataRuntimeRoute): Response {
  if (typeof route.fileDataBase64 !== 'string') {
    throw new Error(
      `[text] Static metadata route ${route.servedUrl} is missing embedded file data.`,
    )
  }
  ...
  try {
    ...
    throw new Error(
      `[text] Failed to decode embedded metadata route file data for ${route.servedUrl}${reason}`,
      { cause: error },
    )
  }
}
  ...
  for (const route of options.metadataRoutes) {
    ...
    if (route.type === 'sitemap' && route.isDynamic) {
      if (functions.generateSitemaps) {
        ...
        // Text.js serves only generated sitemap children when generateSitemaps()
