import type { FetchCacheMode } from '../shims/fetch-cache.js'
...
function resolveRevalidateSeconds(current: number | null, value: unknown): number | null {
  // revalidate = false means "cache indefinitely" in Text.js segment config.
 ...
 *
 * Text.js collects config from every segment in the loader tree and reduces it
 * into the effective route config. The generated text entry already knows
