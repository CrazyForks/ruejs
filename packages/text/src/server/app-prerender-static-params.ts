import { pickRootParams, runWithRootParamsScope, type RootParams } from '../shims/root-params.js'
...
export function createAppPrerenderStaticParamsResolver(
  ...
  return async ({ params }) => {
    ...
    for (const generateStaticParams of generateStaticParamsFns) {
      const textParamSets: RootParams[] = []
      ...
      for (const parentParams of paramSets) {
        ...
        for (const item of result) {
          if (!isRootParams(item)) return []
          textParamSets.push({ ...parentParams, ...item })
        }
      }

      paramSets = textParamSets
    }
