import type { TextHeader, TextI18nConfig } from '../config/text-config.js'
...
type FinalizeAppRscResponseOptions = {
   ...
   * Text.js evaluates config header has/missing conditions against the
 ...
 * headers that throw on mutation, and Text.js does not apply config headers
...
export function finalizeAppRscResponse(
  ...
  // and Text.js deliberately excludes config headers from redirect responses.
  if (response.status >= 300 && response.status < 400) {
    return response
  }

  if (!response.headers.has(TEXT_STATIC_FILE_HEADER)) {
    mergeVaryHeader(response.headers, TEXT_RSC_VARY_HEADER)
  }
