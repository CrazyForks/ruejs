import type { CachedAppPageValue, CacheControlMetadata } from '../shims/cache.js'
import {
...
import { TEXT_MOUNTED_SLOTS_HEADER } from './headers.js'
...
export function buildAppPageCachedResponse(
  ...
  if (options.isRscRequest) {
    ...
    const rscHeaders = buildAppPageCachedHeaders({
      ...
      contentType: TEXT_RSC_CONTENT_TYPE,
  ...
  return new Response(ensureHtmlDoctype(cachedValue.html), {
    status,
    headers: htmlHeaders,
  })
}
...
export async function readAppPageCacheResponse(
  ...
  try {
    ...
    if (cached?.isStale && cachedValue) {
      ...
      options.scheduleBackgroundRegeneration(regenerationKey, async () => {
        ...
        if (!options.isRscRequest) {
          ...
          // be regenerated independently by the text full-page HTML request.
          writes.push(
            options.isrSet(
              options.isrHtmlKey(options.cleanPathname),
              buildAppPageCacheValue(
    ...
    console.error('[text] ISR cache read error:', isrReadError)
  }
...
export function finalizeAppPageHtmlCacheResponse(
  ...
  const cachePromise = (async () => {
    try {
      const cachedHtml = ensureHtmlDoctype(await readStreamAsText(streamForCache))
      ...
      console.error('[text] ISR cache write error:', cacheError)
    }
  })()
...
export function scheduleAppPageRscCacheWrite(
  ...
  const cachePromise = (async () => {
    try {
      ...
      console.error('[text] ISR RSC cache write error:', cacheError)
    }
  })()
