...
type FileBasedMetadataOptions = {
  ...
  /**
   * The configured text.config `basePath`, prefixed to all file-based
   * metadata URLs emitted in the page <head> (icons, opengraph-image,
   * twitter-image, manifest, apple-icon, favicon).
   *
   * Mirrors Text.js, which bakes basePath into static metadata route URLs
   ...
   *
   * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/build/webpack/loaders/text-metadata-image-loader.ts#L63
   * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/build/webpack/loaders/metadata/discover.ts#L88
   */
  basePath?: string
}
...
function normalizeMetadataImageId(route: MetadataFileRoute, id: string | number): string | null {
  ...
  if (!isValidMetadataImageId(normalizedId)) {
    console.warn(
      `[text] Skipping metadata route ${route.servedUrl} image id "${normalizedId}" because metadata image ids must match /^[a-zA-Z0-9-_.]+$/.`,
    )
    return null
  }
  return normalizedId
...
function iconRouteHasExplicitIconsAtSource(
  ...
  // Text suppresses file icon routes when any resolved icons metadata exists.
  ...
  for (const entry of result) {
    if (typeof entry === 'object' && entry !== null) {
      ...
      if (source.id === undefined) {
        console.warn(
          `[text] Skipping metadata route ${route.servedUrl} image metadata entry because generateImageMetadata entries must include an id.`,
        )
        continue
      }
      sources.push(source)
...
async function resolveRouteHeadData(
  ...
  if (!resolvedUrl) {
    console.warn(
      `[text] Skipping metadata route ${route.servedUrl} because params did not fill all dynamic segments.`,
    )
    return []
  }
...
async function resolveHeadDataList(
  ...
  const headDataList = await Promise.all(routes.map(route => resolveRouteHeadData(route, params)))
...
export async function applyFileBasedMetadata(
  ...
  // Matches Text.js, which bakes basePath into both static and dynamic
  ...
  const textMetadata: Metadata = metadata ? { ...metadata } : {}
  ...
  if (faviconEntries.length > 0) {
    const textIcons = cloneIconMap(textMetadata.icons)
    const normalizedIcons = normalizeIconEntries(textIcons)
    textIcons.icon = [...faviconEntries, ...normalizedIcons]
    textMetadata.icons = textIcons
  }

  {
    ...
    if (iconEntries.length > 0) {
      const normalizedIcons = normalizeIconEntries(textIcons)
      textIcons.icon = [...iconEntries, ...normalizedIcons]
    }
    ...
    if (appleEntries.length > 0) {
      const existingApple = textIcons.apple
      ...
      textIcons.apple = [...appleEntries, ...normalizedAppleEntries]
    }
  ...
  if (openGraphHeadData.length > 0) {
    ...
    if (socialEntries.length > 0) {
      const textOpenGraph: NonNullable<Metadata['openGraph']> = textMetadata.openGraph
        ? { ...textMetadata.openGraph }
        : {}
      textOpenGraph.images = socialEntries
      textMetadata.openGraph = textOpenGraph
    }
  }
  ...
  if (twitterHeadData.length > 0) {
    ...
    if (socialEntries.length > 0) {
      const textTwitter: NonNullable<Metadata['twitter']> = textMetadata.twitter
        ? { ...textMetadata.twitter }
        : {}
      textTwitter.images = socialEntries
      textMetadata.twitter = textTwitter
    }
  }

  if (manifestHeadData.length > 0 && manifestHeadData[0].kind === 'manifest') {
    textMetadata.manifest = manifestHeadData[0].href
  }
