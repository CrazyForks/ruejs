/**
 * Default Cloudflare Worker entry point for text App Router.
 *
 * Use this directly in wrangler.jsonc:
 *   "main": "text/server/app-router-entry"
 *
 * Or import and delegate to it from a custom worker:
 *   import handler from "./app-router-entry.js";
...
// Virtual module generated by text at build time. The default export is
// the request handler; `__assetPrefix` and `__basePath` are named exports
// inlined from text.config (both as string literals) so this worker can
// recognise asset-shaped requests without invoking the handler. Types
// live in `virtual-text-rsc-entry.d.ts`; the generator is in
// `entries/app-rsc-entry.ts`.
import rscHandler, {
  __assetPrefix as __rscAssetPrefix,
  __basePath as __rscBasePath,
} from 'virtual:text-rsc-entry'
...
import { assetPrefixPathname, isTextStaticPath } from '../utils/asset-prefix.js'

// Precompute the path components used for framework static asset 404 short-circuit
...
async function handleRequest(
  ...
  // Invalid framework static asset paths short-circuit with a plain-text 404
  ...
  // Matches Text.js: packages/text/src/server/lib/router-server.ts.
  if (isTextStaticPath(url.pathname, __workerBasePath, __workerAssetPathPrefix)) {
  ...
  // (virtual:text-rsc-entry) is the single point of decoding — it calls
