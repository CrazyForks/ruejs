 ...
 *
 * Reads `text-prerender.json` and the corresponding HTML/RSC files from
 ...
 *
 * @param serverDir - Path to `dist/server/` (where text-prerender.json lives)
...
export async function seedMemoryCacheFromPrerender(
  ...
  const manifestPath = path.join(serverDir, 'text-prerender.json')
  ...
  try {
    ...
    console.warn('[text] Failed to parse text-prerender.json, skipping cache seeding:', err)
    return 0
  }
  ...
  for (const route of routes) {
    ...
    // so seeded keys match process.env.__TEXT_BUILD_ID exactly.
