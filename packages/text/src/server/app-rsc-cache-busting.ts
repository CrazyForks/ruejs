...
import {
  TEXT_ROUTER_PREFETCH_HEADER,
  TEXT_ROUTER_SEGMENT_PREFETCH_HEADER,
  TEXT_ROUTER_STATE_TREE_HEADER,
  TEXT_URL_HEADER,
  RSC_HEADER,
  TEXT_INTERCEPTION_CONTEXT_HEADER,
  TEXT_MOUNTED_SLOTS_HEADER,
  TEXT_RSC_RENDER_MODE_HEADER,
} from './headers.js'
 ...
 */
export const TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM = '_rsc'
export const TEXT_RSC_COMPATIBILITY_ID_HEADER = 'X-Text-RSC-Compatibility-Id'
export const TEXT_RSC_CONTENT_TYPE = 'text/x-component'

// Re-export so existing consumers that import from this module keep working.
export { TEXT_RSC_RENDER_MODE_HEADER } from './headers.js'

export const TEXT_RSC_VARY_HEADER = [
  RSC_HEADER,
  'Accept',
  TEXT_ROUTER_STATE_TREE_HEADER,
  TEXT_ROUTER_PREFETCH_HEADER,
...
export function getTextRscCompatibilityId(): string | null {
  return normalizeCompatibilityId(process.env.__TEXT_RSC_COMPATIBILITY_ID)
}

export function applyRscCompatibilityIdHeader(
  headers: Headers,
  compatibilityId: string | null | undefined = getTextRscCompatibilityId(),
): void {
  const normalized = normalizeCompatibilityId(compatibilityId)
  if (normalized) {
    headers.set(TEXT_RSC_COMPATIBILITY_ID_HEADER, normalized)
  } else {
    headers.delete(TEXT_RSC_COMPATIBILITY_ID_HEADER)
  }
}

export function isRscCompatibilityIdCompatible(
  responseCompatibilityId: string | null | undefined,
  clientCompatibilityId: string | null | undefined = getTextRscCompatibilityId(),
): boolean {
...
function createCacheBustingInput(
  ...
  const values = [
    headers.get(TEXT_ROUTER_PREFETCH_HEADER),
    headers.get(TEXT_ROUTER_SEGMENT_PREFETCH_HEADER),
    headers.get(TEXT_ROUTER_STATE_TREE_HEADER),
    headers.get(TEXT_URL_HEADER),
    headers.get(TEXT_INTERCEPTION_CONTEXT_HEADER),
    headers.get(TEXT_MOUNTED_SLOTS_HEADER),
    ...(options.includeRenderModeHeader === false
      ? []
      : [normalizeRenderModeHeaderValue(headers.get(TEXT_RSC_RENDER_MODE_HEADER))]),
  ]
...
function getSearchPairsWithoutRscCacheBusting(url: URL): string[] {
  ...
  return rawQuery.split('&').filter(pair => pair.length > 0 && !isRscCacheBustingSearchPair(pair))
}
...
function isRscCacheBustingSearchPair(pair: string): boolean {
  ...
  try {
    return decodeURIComponent(rawKey.replaceAll('+', ' ')) === TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM
  } catch {
    return rawKey === TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM
  }
}
...
export function setRscCacheBustingSearchParam(url: URL, hash: string): void {
  ...
  pairs.push(
    hash.length > 0
      ? `${TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM}=${hash}`
      : TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM,
  )
...
export function createRscRequestHeaders(options: CreateRscRequestHeadersOptions = {}): Headers {
  const headers = new Headers({
    Accept: TEXT_RSC_CONTENT_TYPE,
    [RSC_HEADER]: '1',
  })

  if (options.interceptionContext !== undefined && options.interceptionContext !== null) {
    headers.set(TEXT_INTERCEPTION_CONTEXT_HEADER, options.interceptionContext)
  }
  ...
  if (renderMode !== APP_RSC_RENDER_MODE_NAVIGATION) {
    headers.set(TEXT_RSC_RENDER_MODE_HEADER, renderMode)
  }
...
export async function createRscRequestUrl(href: string, headers: Headers): Promise<string> {
  const url = new URL(toRscRequestPath(href), 'http://text.local')
...
export async function resolveInvalidRscCacheBustingRequest(
  ...
  const actualHash = url.searchParams.get(TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM)
  ...
  if (actualHash !== null && actualHash !== expectedHash) {
    ...
    if (
      normalizeRenderModeHeaderValue(options.request.headers.get(TEXT_RSC_RENDER_MODE_HEADER)) ===
      null
    ) {
