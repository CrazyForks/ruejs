 ...
 * suspended/streamed in Text.js: its redirects ride inside the App payload
 ...
 * and Text.js test/e2e/app-dir/metadata-navigation.
 */
...
type ResolveAppPageHeadOptions<TModule extends AppPageHeadModule = AppPageHeadModule> = {
  /**
   * Configured text.config `basePath`. Threaded into `applyFileBasedMetadata`
...
async function resolveAppPageHeadInner<TModule extends AppPageHeadModule>(
  ...
  // <title> when the matched page already provides one. This preserves Text.js
  ...
  // Text.js's loader-tree walk which appends slot metadata items in tree order
  // with no title suppression.
  // Reference: https://github.com/vercel/next.js/blob/canary/packages/text/src/lib/metadata/resolve-metadata.ts
  ...
  try {
    ...
    console.error(
      `[text] File-based metadata resolution failed while rendering error boundary for ${options.routePath}:`,
      error,
    )
  }
