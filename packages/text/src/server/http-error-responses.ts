 ...
 *
 * Used for malformed percent-encoding, invalid HTTP methods (where Text.js
 ...
 *
 * The body matches Text.js's plain-text 404 response exactly. Text.js writes
 * `res.end('This page could not be found')` (no trailing period) for the
 * fallback 404 path; see in `.textjs-ref`:
 *   - packages/text/src/server/route-modules/pages/pages-handler.ts L121, L535
 *   - packages/text/src/build/templates/app-route.ts L170, L349
 *   - packages/text/src/build/templates/app-page.ts L701, L1043
 * (The Rue-rendered not-found component in `packages/text/src/client/components/builtin/not-found.tsx`
...
/**
 * Build a 404 plain-text response for invalid framework static asset paths.
 *
 * text short-circuits invalid `_text/static/*` requests with a plain-text
 ...
 *
 * Body and content-type match Text.js exactly:
 *   res.statusCode = 404
 *   res.setHeader('Content-Type', 'text/plain; charset=utf-8')
 *   res.end('Not Found')
 *
 * @see packages/text/src/server/lib/router-server.ts in `.textjs-ref`
 */
