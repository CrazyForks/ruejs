import { TEXT_PRERENDER_ROUTE_PARAMS_HEADER, TEXT_PRERENDER_SECRET_HEADER } from './headers.js'
...
export function readTrustedPrerenderRouteParamsFromHeaders(
  ...
  if (process.env.TEXT_PRERENDER !== '1') return null
  const secret = headers.get(TEXT_PRERENDER_SECRET_HEADER)
  if (secret === null) return null
  if (expectedSecret !== undefined && secret !== expectedSecret) return null
  const header = headers.get(TEXT_PRERENDER_ROUTE_PARAMS_HEADER)
  if (header === null) return null
  const params = parsePrerenderRouteParamsHeader(header)
  if (params === null) {
    throw new Error('[text] Invalid internal prerender route params header.')
  }
  return params
}
...
// request and deliberately omits `expectedSecret`. The `TEXT_PRERENDER=1`
...
function decodedPrerenderRouteParamEquals(
  ...
  if (Array.isArray(prerenderValue) || Array.isArray(matchedValue)) {
    ...
    return prerenderValue.every((item, index) => {
      ...
      const decodedMatched = decodePrerenderRouteParam(matchedValue[index])
      return decoded !== null && decodedMatched !== null && decoded === decodedMatched
    })
  }

  const decoded = decodePrerenderRouteParam(prerenderValue)
  const decodedMatched = decodePrerenderRouteParam(matchedValue)
  return decoded !== null && decodedMatched !== null && decoded === decodedMatched
}
