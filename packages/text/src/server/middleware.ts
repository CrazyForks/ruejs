 ...
 *
 * Loads and executes the user's proxy.ts (Text.js 16) or middleware.ts file
 * before routing. Runs in Node (not Edge Runtime), per the text design.
 *
 * In Text.js 16, proxy.ts replaces middleware.ts:
 ...
 *
 * The proxy/middleware receives a TextRequest and can:
 * - Return TextResponse.text() to continue to the route
 * - Return TextResponse.redirect() to redirect
 * - Return TextResponse.rewrite() to rewrite the URL
 ...
 *
 * Matches Text.js behavior where each file type only accepts its own
 * named export or a default export:
 * https://github.com/vercel/next.js/blob/canary/packages/text/src/build/templates/middleware.ts
 */
 ...
 * Matches Text.js behavior: for proxy files, check `proxy` then `default`;
 * for middleware files, check `middleware` then `default`.
 *
 * Throws if the file exists but doesn't export a valid function, matching
 * Text.js's ProxyMissingExportError behavior.
 *
 * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/build/templates/middleware.ts
 ...
 * Checks for proxy.ts (Text.js 16) first, then falls back to middleware.ts.
 ...
 * build. The warning is benign — it does not abort the build. Text.js itself
 ...
 *
 * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/build/index.ts
...
export function findMiddlewareFile(root: string, fileMatcher: ValidFileMatcher): string | null {
  // Check proxy.ts first (Text.js 16 replacement for middleware.ts)
  ...
  // Fall back to middleware.ts (deprecated in Text.js 16).
  // This is a warning, not an error: middleware.ts is still fully supported
  // by both Text.js 16 and text. Do not change to `throw` or `process.exit`.
  //
  // Warning text matches Text.js canonical wording from
  // packages/text/src/build/index.ts (search for "file convention is
  // deprecated") so Text.js's own deprecation-warnings and app-middleware
  // e2e suites pass when run against text.
  for (const dir of MIDDLEWARE_LOCATIONS) {
    for (const ext of fileMatcher.dottedExtensions) {
      ...
      if (fs.existsSync(fullPath)) {
        console.warn(
          `The "middleware" file convention is deprecated. Please use "proxy" instead.\n\n` +
            ...
            `  npx @text/codemod@canary middleware-to-proxy .\n\n` +
            `  Learn more: https://textjs.org/docs/messages/middleware-to-proxy`,
        )
        return fullPath
      }
    }
