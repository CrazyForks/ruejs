...
const COMPAT_ELEMENT_TYPE_NAMES = new Set([
  'Symbol(rue.element)',
  'Symbol(rue.transitional.element)',
])
const COMPAT_SUSPENSE_TYPE_NAME = 'Symbol(rue.suspense)'
    ...
    for (const [index, part] of parts.entries()) {
      ...
      if (part.startsWith(':')) {
        ...
        if (node.dynamicChild === null) {
          ...
          console.warn(
            `[text] Optimistic route trie found conflicting dynamic segments at the same level: :${node.dynamicChild.paramName} vs ${part}`,
          )
        }
...
function hrefToRouteParts(href: string, basePath: string): string[] | null {
  let url: URL
  try {
    url = new URL(href, 'https://text.local')
  } catch {
    return null
  }
...
function isInspectableElement(value: unknown): value is Record<PropertyKey, unknown> {
  return (typeof value === 'object' || typeof value === 'function') && value !== null
}

function isCompatSuspenseElement(value: Record<PropertyKey, unknown>): boolean {
  const elementType = Reflect.get(value, '$$typeof')
  if (typeof elementType !== 'symbol' || !COMPAT_ELEMENT_TYPE_NAMES.has(String(elementType))) {
    return false
  }

  return String(Reflect.get(value, 'type')) === COMPAT_SUSPENSE_TYPE_NAME
}

function isRueSuspenseElement(value: Record<PropertyKey, unknown>): boolean {
  return Reflect.get(value, RUE_SUSPENSE_ELEMENT_MARKER) === true
}

function readInspectableElementProps(value: Record<PropertyKey, unknown>): unknown {
  return Reflect.get(value, 'props')
}
...
function elementHasSuspenseFallback(value: unknown, depth = 0): boolean {
  ...
  if (!isInspectableElement(value)) return false

  const props = readInspectableElementProps(value)
  if ((isCompatSuspenseElement(value) || isRueSuspenseElement(value)) && isUnknownRecord(props)) {
...
export function createOptimisticRouteElements(template: OptimisticRouteTemplate): AppElements {
  ...
  for (const pageElementId of template.pageElementIds) {
    elements[pageElementId] = createAppServerElement(OptimisticRouteSegment)
  }
  return elements
}
