...
type AppRscInterceptForMatching = {
   ...
   *
   * Text.js implements interception as a rewrite that fires only when the
   * `Text-URL` header matches `^<sourceMatchPattern>(?:/.*)?$`. text's
   ...
   * provided source pathname (X-Text-Interception-Context / Text-URL)
...
export function createAppRscRouteMatcher<Route extends AppRscRouteForMatching>(
  ...
  return {
    ...
    findIntercept(pathname, sourcePathname = null) {
      // Mirror Text.js' rewrite semantics: interception only fires when the
      // Text-URL header is present AND matches the intercepting route's regex
      ...
      for (const entry of interceptLookup) {
        ...
        // request's source pathname to satisfy it. This mirrors Text.js'
...
/**
 * Check whether the request's source pathname (Text-URL / interception
 * context) satisfies the intercept entry's intercepting-route pattern, with
 * descendants allowed. Mirrors the header regex shape Text.js emits for the
