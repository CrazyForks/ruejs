...
export type AppNavigationPayloadOrigin = Readonly<{ origin: 'fresh' } | { origin: 'visited-cache' }>
...
function requiresCacheEntryReuseProof(origin: AppNavigationPayloadOrigin): boolean {
  switch (origin.origin) {
    ...
    default: {
      ...
      throw new Error('[text] Unknown App Router payload origin: ' + String(_exhaustive))
    }
  }
}
...
export function resolveInterceptionContextFromPreviousTextUrl(
  previousTextUrl: string | null,
  basePath: string = '',
): string | null {
  if (previousTextUrl === null) {
    return null
  }

  const parsedUrl = new URL(previousTextUrl, 'http://localhost')
 ...
 *
 * Text.js sends `Text-URL: state.previousTextUrl || state.textUrl` on action
 * POSTs when `hasInterceptionRouteInCurrentTree(state.tree)`. Text's
 * X-Text-Interception-Context is the equivalent signal for the server-side
...
export function resolveServerActionRequestState(
  ...
  const interceptionContext = resolveInterceptionContextFromPreviousTextUrl(
    options.previousTextUrl,
    options.basePath,
  )
  if (interceptionContext !== null) {
    headers.set(TEXT_INTERCEPTION_CONTEXT_HEADER, interceptionContext)
  }
...
export function resolvePendingNavigationCommitDispositionDecision(options: {
  ...
  return mapNavigationDecisionToPendingDisposition(
    planPendingRootBoundaryPayloadResponse({
...
function createPendingNavigationTraceFields(options: {
  ...
  return {
    ...createNavigationLifecycleTraceFields({
      ...
      textRootLayoutTreePath: options.pending.rootLayoutTreePath,
...
function planPendingRootBoundaryPayloadResponse(options: {
  currentState: AppRouterState
  pending: PendingNavigationCommit
  routeManifest: RouteManifest | null
  targetHref?: string
...
function mapNavigationDecisionToPendingDisposition(
  ...
  switch (decision.kind) {
    ...
    case 'requestWork':
      throw new Error(
        `[text] Root-boundary commit planning returned requestWork (${decision.work.kind}); payloadResponseArrived should never request work`,
      )
    default: {
      const _exhaustive: never = decision
      throw new Error('[text] Unknown navigation decision: ' + String(_exhaustive))
    }
  }
}
...
export async function createPendingNavigationCommit(options: {
  ...
  // current visible previousTextUrl.
  previousTextUrl?: string | null
  renderId: number
  type: 'navigate' | 'replace' | 'traverse'
}): Promise<PendingNavigationCommit> {
  const elements = await options.textElements
  ...
  const cacheEntryReuseProof =
    ...
    (requiresCacheEntryReuseProof(options.payloadOrigin)
      ...
      : undefined)
  const requestedPreviousTextUrl =
    options.previousTextUrl !== undefined
      ? options.previousTextUrl
      : options.currentState.previousTextUrl
  const previousTextUrl = metadata.interception === null ? null : requestedPreviousTextUrl
