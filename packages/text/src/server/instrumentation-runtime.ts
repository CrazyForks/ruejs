 ...
 * ## Text.js semantics
 *
 * Text.js calls `register()` once when the server process starts, before any
...
export async function ensureInstrumentationRegistered(
  ...
  if (process.env.TEXT_PRERENDER === '1') return
  ...
  initPromise = (async () => {
    ...
    if (isOnRequestErrorHandler(instrumentationModule.onRequestError)) {
      globalThis.__TEXT_onRequestErrorHandler__ = instrumentationModule.onRequestError
    }
