...
const TEXT_CACHE_IMPLICIT_TAG_ID = '_N_T_'
...
function appendDerivedTags(tags: string[], routePath: string): void {
  appendUnique(tags, `${TEXT_CACHE_IMPLICIT_TAG_ID}/layout`)
  ...
  for (let i = 1; i <= routeParts.length; i++) {
    ...
    appendUnique(tags, `${TEXT_CACHE_IMPLICIT_TAG_ID}${currentPathname}`)
  }
}
...
export function buildPageCacheTags(
  ...
  const tags = [pathname, `${TEXT_CACHE_IMPLICIT_TAG_ID}${pathname}`]
  if (pathname === '/') appendUnique(tags, `${TEXT_CACHE_IMPLICIT_TAG_ID}/index`)
  if (pathname === '/index') appendUnique(tags, `${TEXT_CACHE_IMPLICIT_TAG_ID}/`)
