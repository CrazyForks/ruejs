...
import { mergeElements } from '../shims/slot-core.js'
import {
...
export type ApprovedVisibleCommit = {
  ...
  readonly previousTextUrl: string | null
...
function assertApprovedVisibleCommit(commit: ApprovedVisibleCommit): void {
  if (commit[approvedVisibleCommitBrand] !== true) {
    throw new Error('[text] Visible router state mutation requires ApprovedVisibleCommit')
  }
}
...
function commitVisibleRouterState(
  state: AppRouterState,
  textState: Omit<AppRouterState, 'activeOperation' | 'visibleCommitVersion'>,
  operation: PendingOperationRecord,
): AppRouterState {
...
function mergeSlotBindings(
  ...
  textBindings: readonly AppElementsSlotBinding[],
  layoutIds: readonly string[],
  preservePreviousSlotIds: readonly string[],
): readonly AppElementsSlotBinding[] {
  if (preservePreviousSlotIds.length === 0) return textBindings
...
function reduceApprovedVisibleCommitState(
  ...
  switch (action.type) {
    ...
    case 'navigate':
      return commitVisibleRouterState(
        state,
        {
          ...
          previousTextUrl: action.previousTextUrl,
    ...
    default: {
      ...
      throw new Error('[text] Unknown router action: ' + String(_exhaustive))
    }
  }
}
...
function resolvePendingNavigationCommitDecision(options: {
  ...
  switch (decision.disposition) {
    ...
    default: {
      ...
      throw new Error('[text] Unknown navigation commit disposition: ' + String(_exhaustive))
    }
  }
}
...
function mergeLayoutFlags(
  ...
  textFlags: AppRouterState['layoutFlags'],
  preserveElementIds: readonly string[],
): AppRouterState['layoutFlags'] {
  const merged: Record<string, 's' | 'd'> = { ...textFlags }
...
function createApprovedVisibleCommit(options: {
  ...
  return {
    ...
    previousTextUrl: options.pending.previousTextUrl,
...
function addCommitTransactionTrace(
  ...
  switch (decision.disposition) {
    ...
    default: {
      const _exhaustive: never = decision
      throw new Error('[text] Unknown commit decision: ' + String(_exhaustive))
    }
  }
}
...
export function approveHmrVisibleCommit(pending: PendingNavigationCommit): ApprovedVisibleCommit {
  if (pending.action.operation.lane !== 'hmr') {
    throw new Error('[text] HMR visible commit approval requires an HMR pending operation')
  }
  ...
  if (decision.disposition !== 'commit') {
    throw new Error('[text] HMR visible commit approval did not produce a commit decision')
  }
...
export async function resolveAndClassifyNavigationCommit(options: {
  activeNavigationId: number
  currentState: AppRouterState
  // When provided, these getters are called after awaiting textElements so
