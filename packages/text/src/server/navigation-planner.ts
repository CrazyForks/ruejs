...
export type NavigationPlannerStateV0 = {
  ...
  // through one planner surface. payloadResponseArrived uses event.token; later
  // #726 slices can split this by event kind once more result paths are routed
  // through the planner.
  textOperationToken: OperationToken
...
export type NavigationEvent =
  ...
  | { kind: 'payloadResponseArrived'; token: OperationToken; result: PayloadResultV0 }

export type RequestedWork =
  | { kind: 'payload'; href: string; mode: 'push' | 'replace' | 'refresh' }
  | { direction: TraverseDirection; historyState: unknown; kind: 'traversePayload' }
  | { kind: 'prefetch'; href: string }
...
function createRequestWorkDecision(options: {
  ...
  const traverseFields =
    options.work.kind === 'traversePayload' ? { traverseDirection: options.work.direction } : {}
  return {
    kind: 'requestWork',
    token: options.state.textOperationToken,
    work: options.work,
...
function getRequestedWorkTargetHref(work: RequestedWork): string | null {
  switch (work.kind) {
    ...
    case 'prefetch':
      return work.href
    case 'traversePayload':
      return null
    default: {
      const _exhaustive: never = work
      throw new Error('[text] Unknown requested navigation work: ' + String(_exhaustive))
    }
  }
}
...
function getMatchedUrlPathname(matchedUrl: string): string {
  try {
    return new URL(matchedUrl, 'https://text.local').pathname
  } catch {
...
function createRootBoundaryTraceFields(options: {
  ...
  event: Extract<NavigationEvent, { kind: 'payloadResponseArrived' }>
  ...
  if (options.state.traceFields) {
    return {
      ...
      textRootLayoutTreePath: options.textRootLayoutTreePath,
    }
  }
...
function classifyRootBoundaryTransition(
  ...
  if (currentRootBoundaryId === null || textRootBoundaryId === null) {
    return 'rootBoundaryUnknown'
  }

  return currentRootBoundaryId === textRootBoundaryId
    ? 'currentRootBoundary'
    : 'rootBoundaryChanged'
}
...
function planPayloadResponseArrived(options: {
  event: Extract<NavigationEvent, { kind: 'payloadResponseArrived' }>
  routeManifest: RouteManifest | null
  state: NavigationPlannerStateV0
}): NavigationDecisionV0 {
  ...
  if (currentTopology.kind !== 'known' || targetTopology.kind !== 'known') {
    throw new Error('[text] Current-root navigation planning requires manifest topology')
  }
...
function planNavigation(input: NavigationPlannerInput): NavigationDecisionV0 {
  switch (input.event.kind) {
    ...
    case 'payloadResponseArrived':
      return planPayloadResponseArrived({
        event: input.event,
    ...
    default: {
      ...
      throw new Error('[text] Unknown navigation event: ' + String(_exhaustive))
    }
  }
}
