...
import { TEXT_ACTION_HEADER, RSC_ACTION_HEADER } from './headers.js'
...
export function isPossibleAppRouteActionRequest(
  ...
  return (
    ...
    request.headers.has(TEXT_ACTION_HEADER) ||
    // Text.js uses strict equality here, so charset variants intentionally do
...
export function getAppRouteHandlerRevalidateSeconds(
  ...
  // revalidate = false means "cache indefinitely" (Text.js segment config
