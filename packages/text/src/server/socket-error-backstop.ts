 ...
 *
 * Three real call sites in text that hit this:
 ...
 * inside a `textTick` callback. The throw escapes to
 ...
 * is more conservative than Text.js's equivalent
 * (`router-server.ts`'s log-only handler), which silently swallows
 * every uncaught — text keeps real bugs surfacing.
 *
 ...
 * the build. Prerender already sets `TEXT_PRERENDER=1` for its
 ...
 * `TEXT_PRERENDER` is set the listener has already been installed
 ...
 * `TEXT_DEBUG_SOCKET_ERRORS=1` to log peer-disconnect codes if
 ...
 * top of every user's `vite.config.ts`, so text's listener registers
 ...
 * registered after text don't observe the event. Users who need
 * crash-reporter visibility for non-peer-disconnect errors must
 * register their handler before importing text.
 *
 * **Symbol.for caveat.** `Symbol.for("text.socketErrorBackstop")`
 * is process-global, so if two different text versions are loaded
 * in the same process the first to evaluate wins and the second's
 * filter rules silently don't apply.
 *
 * Set `TEXT_DEBUG_SOCKET_ERRORS=1` to log a one-line marker each
 * time the listener absorbs an error.
 */
const SOCKET_BACKSTOP_FLAG = Symbol.for('text.socketErrorBackstop')
...
export function installSocketErrorBackstop(): void {
  ...
  const debug = process.env.TEXT_DEBUG_SOCKET_ERRORS === '1'
  if (debug) console.warn('[text] socket-error backstop installed')
  process.on('uncaughtException', (err: Error) => {
    if (process.env.TEXT_PRERENDER === '1') throw err
    const code = peerDisconnectCode(err)
    if (code) {
      if (debug) console.warn(`[text] absorbed uncaughtException ${code}`)
      return
    }
    throw err
  })
  process.on('unhandledRejection', (reason: unknown) => {
    if (process.env.TEXT_PRERENDER === '1') throw reason
    const code = peerDisconnectCode(reason)
    if (code) {
      if (debug) console.warn(`[text] absorbed unhandledRejection ${code}`)
      return
    }
    throw reason
  })
