...
import type { TextHeader } from '../config/text-config.js'
 ...
 *
 * Text.js returns 404 for these paths. We check the RAW pathname before
 ...
 * @param basePath - The basePath from text.config.js (empty string if not set)
...
/**
 * Apply matched text.config.js headers to a Web Headers object.
 *
 * Text.js evaluates config header match conditions against the original
...
/**
 * Apply matched text.config.js headers to the early response header record used
...
export function createStaticFileSignal(
  ...
  const headers = new Headers({
    [TEXT_STATIC_FILE_HEADER]: encodeURIComponent(pathname),
  })
  if (context.headers) {
...
/**
 * Resolve the public/ filesystem-route slot in the Text.js routing order.
 *
...
export function normalizeTrailingSlashPathname(
  ...
  if (trailingSlash) {
    // Text.js emits two internal redirect rules for trailingSlash:true:
 ...
 *
 * Matches Text.js behavior: compares the Origin header against the Host
...
export function validateCsrfOrigin(
  ...
  if (originHeader === 'null') {
    ...
    console.warn(
      `[text] CSRF origin "null" blocked for server action. To allow requests from sandboxed contexts, add "null" to experimental.serverActions.allowedOrigins.`,
    )
  ...
  // Check allowedOrigins from text.config.js
  if (allowedOrigins.length > 0 && isOriginAllowed(originHost, allowedOrigins)) return null

  console.warn(
    `[text] CSRF origin mismatch: origin "${originHost}" does not match host "${hostHeader}". Blocking server action request.`,
  )
  return forbiddenResponse()
}

/**
 * Reject malformed RSC container reference graphs in server action payloads.
 *
 * `@vitejs/plugin-rsc` vendors its own Rue Flight decoder. Malicious action
 ...
 *
 * Legitimate compat-encoded container payloads use separate numeric backing
 ...
 *
 * Ported from Text.js: packages/text/src/server/app-render/csrf-protection.ts
 * https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/csrf-protection.ts
 */
 ...
 * `x-middleware-text`, `x-middleware-rewrite`, `x-middleware-request-*`).
...
/**
 * Headers that are only used internally by Text.js and must not be honored
 ...
 */
export { INTERNAL_HEADERS, TEXT_INTERNAL_HEADERS } from './headers.js'
...
function getRequestProperty<K extends keyof Request>(
  request: Request,
  key: K,
  fallback: Request[K],
): Request[K] {
...
export function cloneRequestWithHeaders(request: Request, headers: Headers): Request {
  ...
  try {
    ...
    const body = getRequestProperty(request, 'body', null)
    const init: RequestInitWithCf = {
      method: getRequestProperty(request, 'method', 'GET'),
      headers,
      body: body ?? undefined,
      redirect: getRequestProperty(request, 'redirect', 'follow'),
      signal: getRequestProperty(request, 'signal', undefined as unknown as AbortSignal),
      integrity: getRequestProperty(request, 'integrity', ''),
      cache: getRequestProperty(request, 'cache', 'default'),
      mode: getRequestProperty(request, 'mode', 'cors'),
      credentials: getRequestProperty(request, 'credentials', 'same-origin'),
      referrer: getRequestProperty(request, 'referrer', 'about:client'),
      referrerPolicy: getRequestProperty(request, 'referrerPolicy', ''),
    }
    if (body) {
      // @ts-expect-error — duplex needed for streaming request bodies
      init.duplex = 'half'
    }
    cloned = new Request(getRequestProperty(request, 'url', 'http://localhost/'), init)
  }
