...
import { FLIGHT_HEADERS, TEXT_MW_CTX_HEADER } from './headers.js'
...
export type ApplyAppMiddlewareOptions = {
  ...
  /**
   * Whether the inbound request was a `_text/data` fetch. Captured from the
   * raw incoming headers by the caller, because `x-textjs-data` is in
  ...
  request: Request
  /**
   * Forwarded to `executeMiddleware` so the TextRequest exposes a TextURL with
   * the configured trailingSlash policy. This is what makes
   * `TextResponse.redirect(request.textUrl)` emit a Location that honours
   * `trailingSlash`.
   */
...
const RSC_MIDDLEWARE_HEADER_SET = new Set(FLIGHT_HEADERS)

function isForwardedMiddlewareContext(value: unknown): value is ForwardedMiddlewareContext {
  return !!value && typeof value === 'object'
}

function requestWithoutRscMiddlewareHeaders(request: Request): Request {
  let hasRscMiddlewareHeader = false
  const headers = new Headers()

  for (const [key, value] of request.headers) {
    if (RSC_MIDDLEWARE_HEADER_SET.has(key.toLowerCase())) {
      hasRscMiddlewareHeader = true
    } else {
      headers.append(key, value)
    }
  }

  if (!hasRscMiddlewareHeader) return request
...
function applyForwardedMiddlewareContext(
  ...
  const header = request.headers.get(TEXT_MW_CTX_HEADER)
  ...
  try {
    ...
    console.error('[text] Failed to parse forwarded middleware context:', e)
    return { applied: false }
  }
}
...
export async function applyAppMiddleware(
  ...
  const middlewareRequest = requestWithoutRscMiddlewareHeaders(options.request)
  ...
  if (forwarded.rewriteUrl) {
    try {
      ...
      console.error('[text] Failed to apply forwarded middleware rewrite:', e)
