...
import { TEXT_RSC_REDIRECT_HEADER } from './headers.js'
import { applyEdgeRuntimeHeader } from './app-page-response.js'
import { mergeMiddlewareResponseHeaders } from './middleware-response-headers.js'
import { parseTextHttpErrorDigest, parseTextRedirectDigest } from './text-error-digest.js'
import { addBasePathToPathname } from '../utils/base-path.js'

/**
 * Builds the canonical `TEXT_REDIRECT;<type>;<url>;<status>;` digest that
 * Text.js encodes on `redirect()` / `permanentRedirect()` throws. Used when
 * we synthesize an App payload for an RSC navigation: the digest must
 ...
 *
 * The URL is included verbatim, not encoded — Text.js's `getRedirectError`
 ...
 * server actions, matching Text.js's `getRedirectError` default.
 *
 * Reference:
 *   `.textjs-ref/packages/text/src/client/components/redirect.ts:20-23`
 *   `.textjs-ref/packages/text/src/client/components/redirect-error.ts`
 */
function formatTextRedirectDigest(options: { url: string; statusCode: number }): string {
  return `TEXT_REDIRECT;replace;${options.url};${options.statusCode};`
}
 ...
 * suspended/streamed in Text.js, so a redirect from metadata never becomes an
 * HTTP-level 307; it rides inside the App payload with a 200 status,
 ...
 */
const APP_PAGE_METADATA_ERROR_MARKER = Symbol.for('text.appPage.metadataError')
...
type AppPageRscStreamCapture = {
  /** Stream for the SSR payload reader. Always set. */
...
/**
 * Builds an App/RSC payload that encodes a `redirect()` with the canonical
 * `TEXT_REDIRECT;<type>;<url>;<status>;` digest. Mirrors Text.js's dynamic
 * redirect render path where the thrown redirect is serialized into the
 * streamed payload, so the HTTP response stays 200 instead of carrying the
 * redirect on the status line.
 *
 ...
 */
type BuildRscRedirectPayloadStream = (options: { digest: string }) => ReadableStream<Uint8Array>
...
type BuildAppPageSpecialErrorResponseOptions = {
   ...
   * `Location: /blog/about` for `redirect("/about")`. Mirrors Text.js's
  ...
  basePath?: string
  /**
   * Builds the App/RSC payload used when a redirect must be encoded inside
   ...
   */
  buildRscRedirectPayloadStream?: BuildRscRedirectPayloadStream
   ...
   * redirect("/")` preserves the cookie on the 307. Mirrors Text.js's
   * `appendMutableCookies(headers, requestStore.mutableCookies)` in
   * app-render.tsx. Only applied to redirect responses to match Text.js;
...
export type LayoutClassificationOptions = {
   ...
   * `TEXT_DEBUG_CLASSIFICATION` at BOTH lifecycle points: at build time so
   * the plugin patches the `__TEXT_CLASS_REASONS` dispatch stub, and at
   ...
   * layout ID. Set by the generator when `TEXT_DEBUG_CLASSIFICATION` is
 ...
 *
 * Mirrors Text.js's `addPathPrefix(getURLFromRedirectError(err), basePath)`
 ...
 * we embed in the App payload matches Text.js's convention; the digest
...
export async function buildAppPageSpecialErrorResponse(
  ...
  if (options.specialError.kind === 'redirect') {
    ...
    // Two cases need a 200 + payload encoding instead of an HTTP 307:
    //   1. RSC navigation requests (`Rsc: 1` header) — the client router
    //      decodes the redirect digest from the payload stream. A raw 307
    //      bypasses that path and breaks cache-busting validation.
    //   2. `generateMetadata()` redirects — metadata is suspended in Text.js,
    //      so the redirect rides inside the streamed payload even for
    //      full document SSR. The status line stays 200.
    // Mirrors Text.js's dynamic redirect render path, where the redirect error
    // is serialized with its digest instead of becoming the response status.
    const shouldEmbedRedirectInPayload =
      Boolean(options.buildRscRedirectPayloadStream) &&
      (options.isRscRequest || options.specialError.fromMetadata === true)

    if (shouldEmbedRedirectInPayload && options.buildRscRedirectPayloadStream) {
      // Reduce the resolved (absolute) URL back to a path-only form for
      // same-origin redirects. Text.js's digest stores the raw URL passed to
      ...
      // their absolute form, matching Text.js's external-redirect handling.
      const digestUrl = sameOriginPathOrAbsolute(prefixedLocation, options.request.url)
      const digest = formatTextRedirectDigest({
        url: digestUrl,
        statusCode: options.specialError.statusCode,
      })
      const stream = options.buildRscRedirectPayloadStream({ digest })

      const headers = new Headers({
        'Content-Type': TEXT_RSC_CONTENT_TYPE,
        // Side-channel signal so text's client loop can detect the redirect
        // without having to decode the payload body first. See
        // `TEXT_RSC_REDIRECT_HEADER` in server/headers.ts for the rationale.
        [TEXT_RSC_REDIRECT_HEADER]: digestUrl,
      })
    ...
    for (let layoutIndex = options.layoutCount - 1; layoutIndex >= 0; layoutIndex--) {
      ...
      if (cls && buildTimeResult) {
        ...
        if (cls.debugClassification) {
          ...
          // because the build was run without `TEXT_DEBUG_CLASSIFICATION` or
...
export async function probeAppPageComponent(
  ...
  return options.runWithSuppressedHookWarning(async () => {
    ...
    try {
      const pageResult = options.probePage()
      if (isPromiseLike(pageResult) && options.awaitAsyncResult) {
        await pageResult
      }
    } catch (error) {
      return options.onError(error)
