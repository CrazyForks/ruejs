...
>(options: BuildAppPageElementsOptions<TModule, TErrorModule>): AppElements {
  ...
  for (const templateEntry of templateEntries) {
    ...
    const templateElement = createAppServerElement(
      TemplateComponent,
      { params: options.matchedParams },
      createAppChildrenPlaceholder(),
    )
    const templateElementWithBarrier = templateDependency
      ? renderWithAppDependencyBarrier(templateElement, templateDependency)
      : templateElement
    elements[templateEntry.id] = renderAfterAppDependencies(
      templateElementWithBarrier,
  ...
  for (let index = 0; index < layoutEntries.length; index++) {
    ...
    const layoutElement = createAppServerElement(
      LayoutComponent,
      layoutProps as AppPageComponentProps,
      createAppChildrenPlaceholder(),
    )
    const layoutElementWithBarrier = layoutDependency
      ? renderWithAppDependencyBarrier(layoutElement, layoutDependency)
      : layoutElement
    elements[layoutEntry.id] = renderAfterAppDependencies(
      layoutElementWithBarrier,
