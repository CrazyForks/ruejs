...
export type NormalizedRscRequest = {
  ...
  /** Sanitized X-Text-Interception-Context header (null bytes stripped). null when absent. */
  interceptionContextHeader: string | null
  /** Normalized x-text-mounted-slots header (deduplicated, sorted). null when absent or blank. */
 ...
 *      `/__text/` bypasses this for internal prerender endpoints.
 *   6. RSC detection: `.rsc` suffix only. In dev, accept legacy Vite/RSC HMR
 *      `?_rsc` payload requests when the RSC headers are also present. RSC
 *      headers do not select payload rendering at the canonical HTML URL in
 *      production, so caches that ignore Vary cannot store RSC payload
 *      responses under HTML URLs.
 *   7. cleanPathname — pathname with `.rsc` suffix stripped
 *   8. Sanitize X-Text-Interception-Context — strip null bytes (header injection)
 *   9. Normalize x-text-mounted-slots — dedup and sort for canonical cache keys
...
export function normalizeRscRequest(
  ...
  // /__text/ prefix bypasses the check for internal prerender endpoints
  // that must be reachable regardless of basePath configuration.
  if (basePath) {
    if (!hasBasePath(pathname, basePath) && !pathname.startsWith('/__text/')) {
  ...
  const isRscRequest = pathname.endsWith('.rsc') || isDevLegacyQueryRscRequest(request, url)
  const cleanPathname = stripRscSuffix(pathname)

  // Step 8: Sanitize X-Text-Interception-Context.
  // Null bytes in header values can be used for injection in some HTTP stacks.
  const interceptionContextHeader =
    request.headers.get(TEXT_INTERCEPTION_CONTEXT_HEADER)?.replaceAll('\0', '') || null

  // Step 9: Normalize mounted-slots header for canonical cache keying.
  const mountedSlotsHeader = normalizeMountedSlotsHeader(
    request.headers.get(TEXT_MOUNTED_SLOTS_HEADER),
  )
  const renderMode = isRscRequest
    ? parseAppRscRenderMode(request.headers.get(TEXT_RSC_RENDER_MODE_HEADER))
    : APP_RSC_RENDER_MODE_NAVIGATION
  const clientReuseManifest = isRscRequest
    ? parseClientReuseManifestHeader(request.headers.get(TEXT_CLIENT_REUSE_MANIFEST_HEADER))
...
function isDevLegacyQueryRscRequest(request: Request, url: URL): boolean {
  if (process.env.NODE_ENV === 'production') return false
  if (!url.searchParams.has(TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM)) return false
  if (request.headers.get(RSC_HEADER) !== '1') return false
  return (
    request.headers
      .get('accept')
      ?.toLowerCase()
      .split(',')
      .some(value => value.trim().startsWith(TEXT_RSC_CONTENT_TYPE)) === true
  )
}
