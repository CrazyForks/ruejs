...
// overlay UI (dev-error-overlay.tsx) can subscribe via
...
export function reportToOverlay(error: ReportedError): void {
  ...
  const text = [...snapshot.errors, error]
  const dropped = text.length > MAX_DEV_OVERLAY_ERRORS ? text.length - MAX_DEV_OVERLAY_ERRORS : 0
  const errors = dropped > 0 ? text.slice(dropped) : text
  snapshot = {
    errors,
