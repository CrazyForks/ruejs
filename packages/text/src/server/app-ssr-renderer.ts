...
async function renderRueAppSsrToReadableStream(
  ...
  const shellHtml = normalizeAppDocumentHtml(
...
async function renderAppSsrNodeToHtmlOnce(
  ...
  if (isRueRenderable(value)) {
    return renderRueRenderableToString(value as TextRenderable)
  }
  ...
  if (typeof type === 'function') {
    try {
      return await renderProtocolComponentToHtml(type, props, options)
    } catch (error) {
      options.onError?.(error)
      throw error
    }
...
function normalizeAppDocumentHtml(html: string): string {
  ...
  if (/^\s*<head\b/i.test(afterHtmlOpen)) {
    return html
  }
