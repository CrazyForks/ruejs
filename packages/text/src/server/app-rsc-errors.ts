...
/**
 * djb2 hash matching Text.js's string-hash package for RSC error digests.
 */
...
export function createRscOnErrorHandler(
  ...
  return error => {
    ...
    if (
      ...
      console.error(
        '[text] RSC serialization error: a non-plain object was passed from a Server Component to a Client Component.\n' +
          '\n' +
          'Common causes:\n' +
          '  * Passing a module namespace (import * as X) directly as a prop.\n' +
          '    Unlike Text.js (webpack), Vite produces real ESM module namespace objects\n' +
