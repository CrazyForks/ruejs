...
function areStaticParamsAllowed(
  ...
  return staticParams.some(staticParamSet =>
    paramKeys.every(key => {
      ...
      // Text.js passes them top-down through nested generateStaticParams calls.
      ...
      if (Array.isArray(value)) {
        if (Array.isArray(staticValue)) {
          return routeParamValuesEqual(value, staticValue)
        }
      ...
      if (
        ...
        return routeParamValueEquals(value, String(staticValue))
      }
...
function decodeRouteParamValue(value: unknown): string {
...
function routeParamValueEquals(value: unknown, staticValue: string): boolean {
  return String(value) === staticValue || decodeRouteParamValue(value) === staticValue
}

function routeParamValuesEqual(values: readonly unknown[], staticValues: readonly unknown[]) {
  return (
    values.length === staticValues.length &&
    values.every((value, index) => routeParamValueEquals(value, String(staticValues[index])))
  )
}
