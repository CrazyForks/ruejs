 ...
 *
 * Text.js supports special files in the app/ directory that auto-generate
 * metadata routes:
...
// -------------------------------------------------------------------
// Types matching Text.js MetadataRoute
// -------------------------------------------------------------------
...
//
// Ported from Text.js: packages/text/src/lib/metadata/get-metadata-route.ts
// https://github.com/vercel/next.js/blob/7873aea/packages/text/src/lib/metadata/get-metadata-route.ts
//
...
function getStaticMetadataRoute(appDirPath: string): string {
  ...
  for (const seg of segments) {
    ...
    // and must also be invisible in the URL, matching Text.js behavior.
...
function metadataRouteSuffix(parentSegments: string[], metaType: string): string {
  if (metaType === 'sitemap') {
    // Sitemap is exempt per Text.js (robots/manifest are root-only, so
...
export function scanMetadataFiles(appDir: string): MetadataFileRoute[] {
  ...
  function scan(dir: string, urlPrefix: string, parentSegments: string[]): void {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        ...
        scan(path.join(dir, dirName), textUrlPrefix, [...parentSegments, dirName])
        continue
      }
  ...
  // keep only the dynamic one (matches Text.js behavior).
