...
import type { AppServerComponent, AppServerRenderable } from './app-server-tree.js'
...
type AppFallbackRendererOptions<TModule extends AppPageModule = AppPageModule> = {
   ...
   * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/server/app-render/app-render.tsx
   */
...
type AppFallbackRendererCallContext = {
  /**
   * Whether the matched (or invoking) route opts into Text.js' edge runtime via
...
export function createAppFallbackRenderer<TModule extends AppPageModule>(
  ...
  // When the app does not define `app/global-error.tsx`, fall back to text's
  // built-in default global error component so that uncaught render errors
  // produce the same UI Text.js ships out of the box (matching markup, inline
  // styles, theme CSS, and the "ERROR <digest>" footer for server errors).
  // See packages/text/src/shims/default-global-error.tsx and
  // packages/text/src/server/default-global-error-module.ts.
  ...
  // `app/global-not-found.tsx`), fall back to text's built-in default
  // not-found component so route-miss 404s render the canonical Text.js
  // markup (status + "This page could not be found." message). Matches the
  // default not-found UI shipped with Text.js's app loader.
  // See packages/text/src/shims/default-not-found.tsx and
  // packages/text/src/server/default-not-found-module.ts.
