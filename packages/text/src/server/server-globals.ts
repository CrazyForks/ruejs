/**
 * Server runtime global setup shared by text's generated server entries.
 *
...
function clearBrowserGlobal(name: BrowserGlobalName): void {
  ...
  if (typeof Reflect.get(globalThis, name) !== 'undefined') {
    throw new Error(
      `[text] Server runtime exposes a non-removable \`${name}\` global. ` +
        'This breaks Text.js SSR semantics where browser globals must be absent.',
    )
  }
}
...
export function installServerGlobals(): void {
  ...
  // Text.js's edge sandbox exposes AsyncLocalStorage as a global. Cloudflare
  // Workers exposes it via node:async_hooks under nodejs_compat, so mirror the
  // global binding for user code written against Text.js's runtime.
