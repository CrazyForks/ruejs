...
import { ASSET_PREFIX_URL_DIR, LEGACY_ASSET_PREFIX_URL_DIR } from '../utils/asset-prefix.js'
...
export class StaticFileCache {
  ...
  static async create(clientDir: string): Promise<StaticFileCache> {
    ...
    for (const [relativePath, fileInfo] of allFiles) {
      ...
      // layout writes to `<ASSET_PREFIX_URL_DIR>/` (Text.js's canonical
      ...
      const isHashed =
        ...
        relativePath.startsWith(`${LEGACY_ASSET_PREFIX_URL_DIR}/`) ||
        relativePath.includes(`/${LEGACY_ASSET_PREFIX_URL_DIR}/`)
