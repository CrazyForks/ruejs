...
function decodeAppElementsPromise(
  ...
  return Promise.resolve(payload).then(elements => {
    const decoded = AppElementsWire.decode(elements)
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (frame.type === 'root' && frame.value && typeof frame.value === 'object') {
        return AppElementsWire.decode(frame.value as AppWireElements)
      }
    }
  } catch {
    return null
...
function BrowserRoot({
  ...
  const initialRenderEntryId = resolvePrimaryRenderEntryId(initialRouterState)
  beginCurrentSsrAppElements()
  setCurrentSsrAppElements(treeState.elements)
  ...
  const innerTree = renderSlotElement({ id: initialRenderEntryId, elements: resolvedElements })
