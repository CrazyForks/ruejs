...
import { isTextRequest, TextFetchEvent, TextRequest } from '../shims/server.js'
...
type ExecuteMiddlewareOptions = {
  ...
  /**
   * Whether the incoming request was a Text.js `_text/data` fetch (carried
   * `x-textjs-data: 1`). The header itself is stripped by `filterInternalHeaders`
  ...
  request: Request
  /**
   * The user's `trailingSlash` config. Plumbed into the TextRequest's TextURL
   * so `request.textUrl.toString()` formats with the configured slash policy,
   * which feeds into `TextResponse.redirect(request.textUrl)` Location headers.
 ...
 * returned unchanged. Mirrors Text.js's `getRelativeURL` behaviour:
 * https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/relativize-url.ts
 */
 ...
 * used by Text.js for `_text/data` requests: a 200 OK with the redirect target
 * carried in the `x-textjs-redirect` header. The client router consumes this
 ...
 *
 * Reference: packages/text/src/server/web/adapter.ts
...
function dataRedirectResponse(target: string, originalResponse: Response): Response {
  ...
  headers.set('x-textjs-redirect', target)
...
function drainFetchEvent(fetchEvent: TextFetchEvent): Promise<unknown>[] {
...
function createTextRequest(
  ...
  const hasTextConfig = basePath || i18nConfig || trailingSlash
  const textConfig = hasTextConfig
    ? {
        basePath: basePath ?? '',
  ...
  return isTextRequest(mwRequest)
    ? mwRequest
    : new TextRequest(mwRequest, textConfig ? { textConfig } : undefined)
}
...
export async function executeMiddleware(
  ...
  const textRequest = createTextRequest(
    options.request,
    normalizedPathname,
    options.i18nConfig,
    options.basePath,
  ...
  const fetchEvent = new TextFetchEvent({ page: normalizedPathname })

  let response: Response | undefined | void
  try {
    response = await middlewareFn(textRequest, fetchEvent)
  } catch (e) {
    console.error('[text] Middleware error:', e)
  ...
  if (response.headers.get(MIDDLEWARE_TEXT_HEADER) === '1') {
    return {
      continue: true,
  ...
  if (response.status >= 300 && response.status < 400) {
    ...
    if (location) {
      // Make same-host Location relative for parity with Text.js, which only
      // emits absolute URLs for cross-origin redirects:
      // https://github.com/vercel/next.js/blob/canary/packages/text/src/server/web/adapter.ts
      const relativeLocation = relativizeLocation(location, options.request.url)

      // For `_text/data` requests, translate the HTTP redirect into the
      // `x-textjs-redirect` soft-redirect protocol so the client router can
      // perform the navigation without tripping CORS on cross-origin targets.
      // `x-textjs-data` lives in INTERNAL_HEADERS and is stripped before the
  ...
  if (rewriteUrl) {
    ...
    try {
      ...
      if (rewriteParsed.origin === requestOrigin) {
        ...
        // Text.js merges via `Object.assign(parsedUrl.query, rewrittenParsedUrl.query)`
