 ...
 *
 * Differences vs Text.js:
 * - No native `flock()`. Text.js uses Rust SWC bindings for cross-platform
 *   advisory locking; text uses a JSON file plus a PID liveness check
 ...
 * - Lock file lives in `<root>/.text/dev/lock.json` (mirroring Text.js'
 *   `.text/dev/lock` layout). `.text/` is already used by the fonts plugin
...
const LOCK_DIR_RELATIVE = path.join('.text', 'dev')
 ...
 *
 * Matches Text.js' error layout so AI agents and CLIs can parse the same
 * `- PID: ` / `- Local: ` lines.
 *
...
type AcquireOptions = {
  /** Project root. Lock file goes in `<root>/.text/dev/lock.json`. */
  root: string
...
export function tryAcquireLockfile(opts: AcquireOptions): AcquireResult {
  ...
  // error. A native flock() (the approach Text.js takes via Rust bindings)
  ...
  const lockfile: DevLockfile = {
    path: lockfilePath,
    update(text: DevServerInfo): void {
      try {
        writeLockfile(lockfilePath, text)
      } catch {
