...
function readMetadataRouteFile(route: MetadataFileRoute): Buffer {
  try {
    ...
    throw new Error(
      `[text] Failed to read metadata route file ${route.filePath} for ${route.servedUrl}${reason}`,
      { cause: error },
    )
  }
}
...
function readMetadataRouteTextFile(filePath: string, route: MetadataFileRoute): string {
  try {
    ...
    const reason = error instanceof Error && error.message ? `: ${error.message}` : ''
    throw new Error(
      `[text] Failed to read metadata route file ${filePath} for ${route.servedUrl}${reason}`,
      { cause: error },
    )
  }
}
...
function readMetadataImageDimensions(buffer: Buffer, route: MetadataFileRoute): ImageDimensions {
  try {
    ...
    const reason = error instanceof Error && error.message ? `: ${error.message}` : ''
    throw new Error(
      `[text] Failed to read metadata image dimensions for ${route.filePath} (${route.servedUrl})${reason}`,
      { cause: error },
    )
  }
}
...
function getDynamicMetadataRouteModuleName(
  ...
  if (!moduleName) {
    throw new Error(
      `[text] Missing generated module import for dynamic metadata route ${route.filePath}`,
    )
  }
  return moduleName
}
