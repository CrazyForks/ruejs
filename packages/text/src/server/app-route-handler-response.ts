import type { CachedRouteValue, CacheControlMetadata } from '../shims/cache.js'
import {
...
const APP_ROUTE_REWRITE_ERROR =
  'TextResponse.rewrite() was used in a app route handler, this is not currently supported. Please remove the invocation to continue.'
const APP_ROUTE_TEXT_ERROR =
  'TextResponse.text() was used in a app route handler, this is not supported. See here for more info: https://textjs.org/docs/messages/text-response-text-in-app-route-handler'
...
export function assertSupportedAppRouteHandlerResponse(response: Response): void {
  // TextResponse.text() and rewrite() are middleware control-flow signals.
  // Once an App Route handler has returned, Text.js rejects those responses.
  ...
  if (response.headers.get(MIDDLEWARE_TEXT_HEADER) === '1') {
    throw new Error(APP_ROUTE_TEXT_ERROR)
  }
}
...
export async function buildAppRouteCacheValue(response: Response): Promise<CachedRouteValue> {
  ...
  response.headers.forEach((value, key) => {
    if (
      key === 'set-cookie' ||
      key === TEXT_CACHE_HEADER.toLowerCase() ||
      key === TEXT_LEGACY_CACHE_HEADER.toLowerCase() ||
      key === TEXTJS_CACHE_HEADER.toLowerCase() ||
      key === 'cache-control' ||
