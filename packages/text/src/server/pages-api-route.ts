...
type PagesApiRouteModule = {
   ...
   *
   * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/build/analysis/get-page-static-info.ts
   */
  runtime?: string
...
async function _handlePagesApiRoute(options: HandlePagesApiRouteOptions): Promise<Response> {
  ...
  try {
    if (isEdgeApiRouteModule(route.module)) {
      ...
      const textRequest = new TextRequest(options.request)
      const response = await route.module.default(textRequest)
