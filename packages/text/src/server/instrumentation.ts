 ...
 *
 * Text.js supports an `instrumentation.ts` file at the project root that
 ...
 *
 * References:
 * - https://textjs.org/docs/app/building-your-application/optimizing/instrumentation
 *
 * ## App Router
 *
...
/**
 * The onRequestError handler type from Text.js instrumentation.
 *
...
export function getOnRequestErrorHandler(): OnRequestErrorHandler | null {
  return globalThis.__TEXT_onRequestErrorHandler__ ?? null
}
...
export async function runInstrumentation(
  ...
  try {
    ...
    if (typeof mod.onRequestError === 'function') {
      globalThis.__TEXT_onRequestErrorHandler__ = mod.onRequestError as OnRequestErrorHandler
    }
  } catch (err) {
    console.error(
...
export function reportRequestError(
  ...
  const promise = (async () => {
    try {
      ...
      console.error(
        '[text] onRequestError handler threw:',
