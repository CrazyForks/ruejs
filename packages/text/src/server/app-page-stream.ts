...
function wrapAppClientReferenceSsrStream(
  ...
  return new ReadableStream<Uint8Array>({
    ...
    cancel(reason) {
      const restore = enterAppClientReferenceSsr()
      return reader.cancel(reason).finally(restore)
    },
  })
}
...
export async function renderAppPageHtmlStream(
  ...
  const htmlStream = await runWithAppClientReferenceSsr(() =>
    options.ssrHandler.handleSsr(
      options.rscStream,
  ...
  return wrapAppClientReferenceSsrStream(htmlStream)
}
