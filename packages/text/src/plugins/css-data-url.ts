 ...
 * (see issue #1363).
 *
 * Text.js itself only honours these imports under Turbopack; the official
 * test (`test/e2e/app-dir/css-modules-data-urls/`) skips outside
 * `IS_TURBOPACK_TEST`. text aims to match Turbopack behaviour here so apps
 * authored against the Text.js 16+ App Router build cleanly.
 *
 * ## Strategy
 *
 ...
 * `\0text-data-css/<sha1>.module.css` (or `.css`) specifier. The synthetic
 ...
 * `cssModuleRE` filters.
 */
const VIRTUAL_PREFIX = '\0text-data-css/'
...
function decode(payload: string, isBase64: boolean): string {
  ...
  // Percent-decoding matches RFC 3986. Turbopack/Text.js author CSS data URLs
      ...
      const rewritten = code.replace(
        ...
        (_match, quote: string, moduleFlag, base64Flag, payload) => {
          ...
          try {
            ...
            throw new Error(
              `[text] Failed to decode CSS data URL import in ${id}: ${(err as Error).message}`,
            )
          }
