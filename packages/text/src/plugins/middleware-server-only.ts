 ...
 * semantically correct. However, text bundles middleware into the SSR
 * environment (via `virtual:text-server-entry`), and @vitejs/plugin-rsc's
 ...
 *
 * Text.js solves this with webpack `issuerLayer` rules: middleware (and
 * `instrumentation`) sit in the `neutralTarget` layer where `server-only`
 * is aliased to a no-op while `client-only` still errors. See
 *   packages/text/src/build/webpack-config.ts ("Alias server-only and
 ...
 *      handler sees the text `server-only` request.
 ...
 * through the HTML SSR environment) still hit the rsc:validate-imports
 * rejection, preserving the original safety net for accidental client-side
 * `server-only` leakage.
 *
 * Ported from Text.js handling of `WEBPACK_LAYERS.middleware` /
 * `WEBPACK_LAYERS.GROUP.neutralTarget`:
 * https://github.com/vercel/next.js/blob/canary/packages/text/src/build/webpack-config.ts
 */
export function createMiddlewareServerOnlyPlugin(options: {
  ...
  return {
    name: 'text:middleware-server-only',
    ...
    resolveId: {
      ...
      async handler(id, importer, opts) {
        ...
        // rue-server export condition already makes `server-only` a no-op,
