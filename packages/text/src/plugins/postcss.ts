 ...
 *
 * Text.js (via postcss-load-config) resolves string plugin names in the
 * object form `{ plugins: { "pkg-name": opts } }` but NOT in the array form
 * `{ plugins: ["pkg-name"] }`. Since many Text.js projects use the array
...
async function resolvePostcssStringPluginsUncached(
  ...
  try {
    ...
    const jiti = createJiti(path.join(projectRoot, 'package.json'), { interopDefault: true })
  ...
  const resolved = await Promise.all(
    config.plugins.filter(Boolean).map(async (plugin: unknown) => {
      if (typeof plugin === 'string') {
        ...
        const fn = await jiti.import(resolved, { default: true })
