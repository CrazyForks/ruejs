/**
 * text OG image asset plugins
 *
 * Exports two Vite plugins:
 *
 * `createOgInlineFetchAssetsPlugin` — text:og-inline-fetch-assets
...
/**
 * Create the `text:og-inline-fetch-assets` Vite plugin.
 *
...
export function createOgInlineFetchAssetsPlugin(): Plugin {
  ...
  return {
    name: 'text:og-inline-fetch-assets',
    enforce: 'pre',
    ...
    async transform(code, id) {
      ...
      if (code.includes('readFileSync(')) {
        ...
        for (const match of newCode.matchAll(readFilePattern)) {
          ...
          // Buffer is always available in Node.js and in the text SSR/RSC environments.
...
/**
 * The `text:og-assets` Vite plugin.
 *
 * Copies @vercel/og binary assets (e.g. resvg.wasm) to the RSC output
 * directory for production builds. The edge build inlines fonts as base64 via
 * `text:og-inline-fetch-assets`; this plugin is a safety net to ensure
  ...
  return {
    ...
    writeBundle: {
      ...
      async handler(options) {
        ...
        // The font is inlined as base64 by text:og-inline-fetch-assets, so only
...
export const ogAssetsPlugin: Plugin = createOgAssetsPlugin()
