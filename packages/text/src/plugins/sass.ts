/**
 * Map a Text.js `sassOptions` object onto Vite's
 * `css.preprocessorOptions.scss` / `.sass` shape.
 *
 * Text.js (webpack + sass-loader) accepts:
 ...
 *
 * Reference (Text.js source — destructures the same keys before forwarding
 * the rest to sass-loader):
 *   .textjs-ref/packages/text/src/build/webpack/config/blocks/css/index.ts#L150-L180
 ...
 *   flattened text to `additionalData`
 *
...
export function buildSassPreprocessorOptions(
  ...
  // Text.js forwards `sassPrependData || sassAdditionalData` to sass-loader's
  // `additionalData` (truthy-OR, see
  // .textjs-ref/packages/text/src/build/webpack/config/blocks/css/index.ts:178),
  // so falsy values like `prependData: ""` fall through to `additionalData`.
  // Mirror that precedence exactly so users migrating from Text.js 12
