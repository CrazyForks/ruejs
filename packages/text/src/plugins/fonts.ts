 ...
 *
 * `createGoogleFontsPlugin` — text:google-fonts
 *   1. Rewrites named `text/font/google` imports/exports to tiny virtual modules
 ...
 *      them locally under `.text/fonts/`, and injects `_text.font` into
 ...
 *      receive adjusted fallback CSS when Text.js-compatible fallback metrics
 *      exist for the selected Google Font.
 *
 * `createLocalFontsPlugin` — text:local-fonts
...
export const VIRTUAL_GOOGLE_FONTS = 'virtual:text-google-fonts'
export const RESOLVED_VIRTUAL_GOOGLE_FONTS = '\0' + VIRTUAL_GOOGLE_FONTS

// ── Constants ─────────────────────────────────────────────────────────────────

// IMPORTANT: keep this set in sync with the non-font utility exports from
// packages/text/src/shims/font-google.ts (and its re-export barrel).
 ...
 *
 * `fetchAndCacheFont()` downloads .woff2 files into `<root>/.text/fonts/`
 ...
 * is embedded in the bundle as `_text.font.selfHostedCSS`, the filesystem
 ...
 * the font files into `<clientOutDir>/<assetsDir>/_text_fonts/` so the
 ...
 */
const TEXT_FONT_URL_NAMESPACE = '_text_fonts'
 ...
 * the bundle as `_text.font.selfHostedCSS`. Every downstream consumer reads
 * from the same rewritten CSS: the injected `<style data-text-fonts>` block, the
 ...
 * (e.g. `/home/user/project/.text/fonts/geist-<hash>/geist-<hash>.woff2`)
...
export function _rewriteCachedFontCssToServedUrls(
  ...
  return css.split(cacheDir).join(`/${prefix}/${TEXT_FONT_URL_NAMESPACE}`)
}

/**
 * Default `build.assetsDir` — matches text's resolved default in
 * `resolveAssetsDir("")` (Text.js's canonical convention). Used as the
 ...
 *
 * Cache dir structure: .text/fonts/<family-hash>/
...
async function fetchAndCacheFont(
  ...
  for (const [fontUrl, filename] of urls) {
    ...
    // `/<assetsDir>/_text_fonts/`. The filesystem path is only the
    // on-disk intermediate form — it must never reach the bundle, the
    // injected `<style data-text-fonts>` block, the HTML `<link
...
/**
 * Create the `text:google-fonts` Vite plugin.
 *
 ...
 *   files from transform (they contain `text/font/google` references that must
 *   not be rewritten).
 */
 ...
 * Returns `null` if the text non-whitespace character is not `)`.
 */
...
export function createGoogleFontsPlugin(fontGoogleShimPath: string, shimsDir: string): Plugin {
  ...
  return {
    ...
    configResolved(config) {
      cacheDir = path.join(config.root, '.text', 'fonts')
    },
    ...
    // .text/fonts/ tree is served directly under the same URL prefix that
    // `_rewriteCachedFontCssToServedUrls()` embeds into the @font-face CSS
    // (`/<assetsDir>/_text_fonts/...`). Without this hook the rewritten
    // URLs 404 — and once `_text.font.selfHostedCSS` is injected, the shim no longer
    ...
    configureServer(server) {
      ...
      const assetsDir =
        ...
        DEFAULT_ASSETS_DIR
      const urlPrefix = `/${assetsDir}/${TEXT_FONT_URL_NAMESPACE}/`
      server.middlewares.use((req, res, text) => {
        const url = req.url
        if (!url || !url.startsWith(urlPrefix)) return text()
        ...
        fs.stat(filePath, (err, stat) => {
          if (err || !stat.isFile()) return text()
    ...
    transform: {
      // Hook filter: only invoke JS when code contains 'text/font/google'.
      // This still eliminates nearly all Rust-to-JS calls since very few files
      // import from text/font/google.
      filter: {
        id: {
          include: /\.(tsx?|jsx?|mjs)$/,
        },
        code: 'text/font/google',
      },
      async handler(code, id) {
        // Defensive guard — duplicates filter logic
        if (id.startsWith('\0')) return null
        if (!id.match(/\.(tsx?|jsx?|mjs)$/)) return null
        if (!code.includes('text/font/google')) return null
        ...
        // (e.g. `import type { Metadata } from 'text'`) can't be swallowed
        ...
        const importRe =
          /^[ \t]*import\s+((?:\{[^}]*?\}|[^;\n])+?)\s+from\s*(["'])text\/font\/google\2\s*;?/gm
        let importMatch
        while ((importMatch = importRe.exec(code)) !== null) {
          ...
          if (parsed.namespaceLocal) {
            const proxyImportName = `__text_google_fonts_proxy_${proxyImportCounter++}`
        ...
        const exportRe = /^[ \t]*export\s*\{([^}]+)\}\s*from\s*(["'])text\/font\/google\2\s*;?/gm
        let exportMatch
        ...
        async function injectSelfHostedCss(
          ...
          try {
            ...
            throw new Error(`[text:google-fonts] ${id}: ${message}`)
          }
          ...
          if (!localCSS) {
            try {
              ...
              if (err instanceof GoogleFontsHttpError) {
                ...
                // to the browser.
                throw new Error(
                  `[text:google-fonts] ${id}: Google Fonts returned HTTP ${err.status} for ${err.url}.\n${formatGoogleFontsErrorBody(err.responseBody)}`,
                )
              }
          ...
          // Rewrite absolute `.text/fonts/` filesystem paths in the cached
          // CSS to served URLs under `/<assetsDir>/_text_fonts/` so the
          // embedded `_text.font.selfHostedCSS` string has origin-relative URLs that
          ...
          const validatedFontWeight =
            validated.weights.length === 1 && validated.weights[0] !== 'variable'
              ...
              : undefined
          const validatedFontStyle = validated.styles.length === 1 ? validated.styles[0] : undefined
          ...
          const injectedProperties = [`_text: { font: { ${internalFontProperties.join(', ')} } }`]
        ...
        // directly from `.text/fonts/` so the rewritten URLs resolve
    ...
    //
    // `fetchAndCacheFont` downloads files into `<root>/.text/fonts/` and
    // leaves them there — nothing else copies them. Without this hook, the
    // rewritten `/assets/_text_fonts/...` URLs would 404 in production.
    writeBundle: {
      sequential: true,
      order: 'post' as const,
      handler(outputOptions: { dir?: string }) {
        ...
        // `text:precompress` and `text:cloudflare-build` plugins both
        ...
        const targetRoot = path.join(outDir, assetsDir, TEXT_FONT_URL_NAMESPACE)
...
/**
 * Create the `text:local-fonts` Vite plugin.
 *
 * Rewrites relative font file paths in `text/font/local` calls into Vite
  ...
  return {
    ...
    transform: {
      ...
      handler(code, id) {
        ...
        if (id.startsWith('\0')) return null
        if (!id.match(/\.(tsx?|jsx?|mjs)$/)) return null
        if (!code.includes('text/font/local')) return null
        // Skip text's own font-local shim — it contains example paths
        // in comments that would be incorrectly rewritten.
        if (id.includes('font-local')) return null

        // Verify there's actually a default import from text/font/local and
        // remember its local binding so family payloads only attach to real
        // localFont calls.
        const importMatch =
          /\bimport\s+([A-Za-z_$][A-Za-z0-9_$]*)\s+from\s*(['"])text\/font\/local\2/.exec(code)
        ...
        while ((match = fontPathRe.exec(code)) !== null) {
          ...
          const varName = `__text_local_font_${fontImportCounter++}`
          ...
          // Replace: path: "./font.woff2" -> path: __text_local_font_0
        ...
        while ((localFontCallMatch = localFontCallRe.exec(code)) !== null) {
          ...
          const optionsStr = code.slice(objRange[0], objRange[1])
          if (/(?:^|[,{])\s*_text\s*:/.test(optionsStr)) continue
          ...
          s.appendLeft(
            insertAt,
            `${separator}_text: { font: { family: ${JSON.stringify(bindingName)} } }`,
          )
