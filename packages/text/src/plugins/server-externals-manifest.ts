...
/**
 * text:server-externals-manifest
 *
 * A `writeBundle` plugin that collects the packages left external by the
 * SSR/RSC bundler and writes them to `<outDir>/text-externals.json`.
 *
 ...
 *   ["@rue-js/rue", "@rue-js/server-renderer"]
 *
...
export function createServerExternalsManifestPlugin(): Plugin {
  ...
  return {
    name: 'text:server-externals-manifest',
    apply: 'build',
    enforce: 'post',
    ...
    writeBundle: {
      ...
      handler(options, bundle) {
        ...
        for (const item of Object.values(bundle)) {
          ...
          // specifiers (e.g. "rue", "@mdx-js/rue") since they were never
        ...
        if (outDir && fs.existsSync(outDir)) {
          const manifestPath = path.join(outDir, 'text-externals.json')
