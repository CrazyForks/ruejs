/**
 * Dynamic route validation adapted from Text.js' sorted-routes implementation.
 * Source:
 * https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/sorted-routes.ts
 */
...
class UrlNode {
  ...
  private insertSegments(urlPaths: string[], slugNames: string[], isCatchAll: boolean): void {
    ...
    let textSegment = urlPaths[0]

    if (textSegment.startsWith('[') && textSegment.endsWith(']')) {
      let segmentName = textSegment.slice(1, -1)
      ...
      const handleSlug = (previousSlug: string | null, textSlug: string): void => {
        if (previousSlug !== null && previousSlug !== textSlug) {
          throw new Error(
            `You cannot use different slug names for the same dynamic path ('${previousSlug}' !== '${textSlug}').`,
          )
        }
        ...
        for (const slug of slugNames) {
          if (slug === textSlug) {
            throw new Error(
              `You cannot have the same slug name "${textSlug}" repeat within a single dynamic path`,
            )
          }

          if (slug.replace(/\W/g, '') === textSegment.replace(/\W/g, '')) {
            throw new Error(
              `You cannot have the slug names "${slug}" and "${textSlug}" differ only by non-word symbols within a single dynamic path`,
            )
          }
        }

        slugNames.push(textSlug)
      }
    ...
    let child = this.children.get(textSegment)
    if (!child) {
      child = new UrlNode()
      this.children.set(textSegment, child)
    }
        ...
        segment === '[]'
          ...
          : segment === '[...]'
            ...
            : segment === '[[...]]'
              ...
              : segment
      child.assertOptionalCatchAllSpecificity(`${prefix}${textPrefixSegment}/`)
    }
  }
}

export function patternToTextFormat(pattern: string): string {
  if (pattern === '/') return '/'
...
export function validateRoutePatterns(patterns: readonly string[]): void {
  ...
  for (const pattern of patterns) {
    ...
    root.insert(patternToTextFormat(normalizedPattern))
  }
