...
export type InterceptingRoute = {
   ...
   * segments stripped. Mirrors Text.js' `interceptingRoute` from
   * `extractInterceptionRouteInformation`.
   *
   * Used at request time to gate `findIntercept` against the Text-URL /
   ...
   *
   * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/lib/generate-interception-routes-rewrites.ts
   * @see https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/interception-routes.ts
   */
...
export type AppRoute = {
   ...
   * a layout is caught by the parent layout's boundary (matching Text.js behavior).
   */
...
function assertRouteManifestTreePosition(
  ...
  throw new Error(
    `[text] App route graph invariant violated: missing ${kind} tree position for ${id} on ${route.pattern}`,
  )
}
...
function assertRouteManifestBoundaryTreePosition(
  ...
  throw new Error(
    `[text] App route graph invariant violated: missing boundary tree position for ${boundaryPath} on ${route.pattern}`,
  )
}
...
function assertRouteManifestRootBoundary(
  ...
  throw new Error(
    `[text] App route graph invariant violated: ${kind} ${id} is shared across root boundaries (${existingRootBoundaryId ?? 'none'} and ${route.ids.rootBoundary ?? 'none'}) on ${route.pattern}`,
  )
}
...
export async function buildAppRouteGraph(
  ...
  // and _private folders (Text.js convention for colocated non-route files).
  //
  // Interception marker directories (e.g. `(.)photo`, `(..)showcase`,
  // `(..)(..)hoge`, `(...)photos`) are also excluded from the global page
  // scan because the marker is not a real URL segment — Text.js treats these
  ...
  // segment has no children page. Text.js uses this for modal/feed patterns
  ...
  // In Text.js, pages nested inside @slot directories create additional URL routes.
...
function directoryToAppRoute(
  ...
  // Discover per-segment error boundaries. Text.js loader trees carry an
  // error convention for a segment even when that segment has no layout.
  // In Text.js, each segment independently wraps its children with an ErrorBoundary.
  ...
  // These are used for per-layout NotFoundBoundary to match Text.js behavior where
 ...
 *
 * Text.js stores conventions on every loader-tree segment; a route-group
 ...
 *
 * This is used for per-layout error boundaries. In Text.js, each layout level
 ...
 *
 * In Text.js, parallel slots belong to the layout that defines them. When a
...
/**
 * Map a pattern segment to the tree-node type used by Text.js' route
 ...
 * and parallel slots (`@slot`) are stripped because Text.js'
 * `normalizeAppPath` treats them as invisible in the URL.
 *
 * Mirrors Text.js' computation in `extractInterceptionRouteInformation`:
...
export function convertSegmentsToRouteParts(
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    // Matches Text.js PARAMETER_PATTERN: any non-] chars inside brackets.
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/get-dynamic-param.ts
