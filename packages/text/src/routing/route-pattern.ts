 ...
 *
 * Text.js allows both shapes:
 *   - a raw string path, e.g. `"/blog/hello"`
 *   - an object `{ params, locale? }`
 *
 * See:
 *   https://textjs.org/docs/pages/api-reference/functions/get-static-paths
 *   .textjs-ref/packages/text/src/build/static-paths/pages.ts (the
 ...
 *
 * Mirrors the Text.js `removeTrailingSlash` helper used in
 * `.textjs-ref/packages/text/src/build/static-paths/pages.ts`. Kept here so
 ...
 *
 * Handles both Text.js-supported shapes:
 *   - For a string entry, match it against `routePattern` to extract params,
 *     mirroring `_routeMatcher(cleanedEntry)` in
 *     `.textjs-ref/packages/text/src/build/static-paths/pages.ts`. If the
 *     string does not match the pattern, Text.js throws; we return an
 *     `{ error }` result so the caller can record a per-route error instead
 *     of crashing the build.
 *   - For an object entry, require a `params` key (Text.js raises
