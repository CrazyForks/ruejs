...
import { TEXT_REVALIDATE_HEADER } from '../server/headers.js'
 ...
 * account_id, TEXT_CACHE KV namespace ID, and custom domain.
 */
...
function extractFromJSON(config: Record<string, unknown>): WranglerConfig {
  ...
  // KV namespace ID for TEXT_CACHE
  if (Array.isArray(config.kv_namespaces)) {
    const textKV = config.kv_namespaces.find(
      (ns: Record<string, unknown>) => ns && typeof ns === 'object' && ns.binding === 'TEXT_CACHE',
    )
    if (textKV && typeof textKV.id === 'string' && textKV.id !== '<your-kv-namespace-id>') {
      result.kvNamespaceId = textKV.id
    }
  }
...
function filterTrafficPaths(entries: TrafficEntry[]): TrafficEntry[] {
  return entries.filter(e => {
    ...
    // Internal routes
    if (e.path.startsWith('/_text/') || e.path.startsWith('/__text/')) return false
    // RSC requests
...
/**
 * Spawn a subprocess running the text production server.
 * Uses the same Node.js binary and resolves prod-server.js relative
 * to the current module (works whether text is installed or linked).
 */
function startLocalServer(root: string, port: number): ChildProcess {
  ...
  const script = [
    ...
    `.catch(e => { console.error("[text-tpr] Server failed to start:", e); process.exit(1); });`,
  ].join('')
...
export function buildTprKVPairs(
  ...
  for (const [routePath, result] of entries) {
    const revalidateHeader = result.headers[TEXT_REVALIDATE_HEADER]
 ...
 * the `ruetext deploy` pipeline. Gracefully skips (never errors) when
...
export async function runTPR(options: TPROptions): Promise<TPRResult> {
  ...
  if (!wranglerConfig.kvNamespaceId) {
    return skip('no TEXT_CACHE KV namespace configured')
  }
  ...
  // ── 10. Upload to KV ──────────────────────────────────────────
  // Read buildId from the BUILD_ID file written by text:build-id plugin.
  let buildId: string
  try {
    buildId = fs.readFileSync(path.join(root, 'dist', 'server', 'BUILD_ID'), 'utf-8').trim()
  } catch {
    // BUILD_ID is written by text:build-id during every production build.
