...
import { finalizeAppRscResponse } from '../src/server/app-rsc-response-finalizer.js'
import type { RequestContext } from '../src/config/config-matchers.js'
...
describe('finalizeAppRscResponse — config header application', () => {
  it('applies a matching config header to a 200 response', () => {
    // Behavior: /about page response gets x-added header from text.config.js headers[].
  ...
  it('adds the App Router RSC vary header when no config headers are configured', () => {
    ...
    // no text.config.js headers match. This covers app route handlers that
    ...
    expect(result.headers.get('vary')).toBe(TEXT_RSC_VARY_HEADER)
  })
...
describe('finalizeAppRscResponse — App Router RSC vary header', () => {
  ...
  it('does not duplicate RSC vary tokens already set by app page helpers', () => {
    const response = new Response('body', {
      status: 200,
      headers: { Vary: TEXT_RSC_VARY_HEADER },
    })
    const request = new Request('http://example.com/about')
...
describe('finalizeAppRscResponse — default-locale path normalisation', () => {
  it('matches a config header rule with a :locale placeholder against an unprefixed request', () => {
    ...
    // "/about" when the i18n default locale is "en", because Text.js splices the
