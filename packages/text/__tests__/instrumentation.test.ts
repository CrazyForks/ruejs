...
import { generateInstrumentationClientInjectModule } from '../src/client/instrumentation-client-inject.js'
import { createValidFileMatcher } from '../src/routing/file-matcher.js'

const RESOLVED_INSTRUMENTATION_CLIENT = '\0private-text-instrumentation-client.mjs'
...
function setupInjectProject(options: {
  ...
  const tmpDir = fs.mkdtempSync(path.join(getTextTestTmpRoot(), 'text-instr-client-inject-'))
  fs.writeFileSync(
...
describe('findInstrumentationFile', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(getTextTestTmpRoot(), 'text-instr-'))
  })
...
describe('findInstrumentationClientFile', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(getTextTestTmpRoot(), 'text-instr-client-'))
  })
...
describe('runInstrumentation', () => {
  let runInstrumentation: typeof import('../src/server/instrumentation.js').runInstrumentation
  let getOnRequestErrorHandler: typeof import('../src/server/instrumentation.js').getOnRequestErrorHandler
  ...
  afterEach(() => {
    delete globalThis.__TEXT_onRequestErrorHandler__
  })
...
describe('ensureInstrumentationRegistered', () => {
  let ensureInstrumentationRegistered: typeof import('../src/server/instrumentation-runtime.js').ensureInstrumentationRegistered
  let reportRequestError: typeof import('../src/server/instrumentation.js').reportRequestError
  ...
  beforeEach(async () => {
    ...
    const mod = await import('../src/server/instrumentation-runtime.js')
    ensureInstrumentationRegistered = mod.ensureInstrumentationRegistered
    const instMod = await import('../src/server/instrumentation.js')
  ...
  it('no-ops when TEXT_PRERENDER is set', async () => {
...
describe('reportRequestError', () => {
  let runInstrumentation: typeof import('../src/server/instrumentation.js').runInstrumentation
  let reportRequestError: typeof import('../src/server/instrumentation.js').reportRequestError
  let runWithExecutionContext: typeof import('../src/shims/request-context.js').runWithExecutionContext
  ...
  beforeEach(async () => {
    ...
    const ctxMod = await import('../src/shims/request-context.js')
  ...
  it('catches and logs errors thrown by the handler', async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith('[text] onRequestError handler threw:', 'handler broke')
    consoleSpy.mockRestore()
  })
...
// Ported from Text.js: packages/text/src/build/webpack/loaders/text-instrumentation-client-loader.ts
// https://github.com/vercel/next.js/blob/canary/packages/text/src/build/webpack/loaders/text-instrumentation-client-loader.ts
...
`
  ...
  it('does not intercept private-text-instrumentation-client when injects is empty', async () => {
    await withInjectClientServer(
      { instrumentationClientInject: [], userClientSource: USER_CLIENT },
      async ({ container }) => {
        const resolved = await container.resolveId('private-text-instrumentation-client')
  ...
  it('serves and composes the virtual module in inject order', async () => {
    await withInjectClientServer(
      ...
      async ({ tmpDir, container }) => {
        ...
        const entryPath = path.join(tmpDir, '.text-composed-instrumentation-client.mjs')
