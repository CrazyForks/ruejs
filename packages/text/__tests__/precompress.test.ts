 ...
 *
 * precompressAssets() runs after `ruetext build` and generates .br (brotli) and
...
describe('precompressAssets', () => {
  ...
  beforeEach(async () => {
    clientDir = path.join(
      os.tmpdir(),
      `text-precompress-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    )
  ...
  it('generates .br and .gz files for compressible hashed assets', async () => {
    ...
    await writeAsset(clientDir, '_text/static/index-abc123.js', jsContent)

    const result = await precompressAssets(clientDir)

    // Both compressed variants should exist on disk
    expect(fs.existsSync(path.join(clientDir, '_text/static/index-abc123.js.br'))).toBe(true)
    expect(fs.existsSync(path.join(clientDir, '_text/static/index-abc123.js.gz'))).toBe(true)
  ...
  it('brotli output decompresses to original content', async () => {
    ...
    await writeAsset(clientDir, '_text/static/hello-def456.js', jsContent)

    await precompressAssets(clientDir)

    const brBuffer = await fsp.readFile(path.join(clientDir, '_text/static/hello-def456.js.br'))
  ...
  it('gzip output decompresses to original content', async () => {
    ...
    await writeAsset(clientDir, '_text/static/styles-789abc.css', cssContent)

    await precompressAssets(clientDir)

    const gzBuffer = await fsp.readFile(path.join(clientDir, '_text/static/styles-789abc.css.gz'))
  ...
  it('skips files below the compression threshold', async () => {
    ...
    await writeAsset(clientDir, '_text/static/tiny-aaa111.js', 'const x = 1;')

    const result = await precompressAssets(clientDir)

    expect(fs.existsSync(path.join(clientDir, '_text/static/tiny-aaa111.js.br'))).toBe(false)
    expect(fs.existsSync(path.join(clientDir, '_text/static/tiny-aaa111.js.gz'))).toBe(false)
  ...
  it('skips non-compressible file types (images, fonts)', async () => {
    ...
    await writeAsset(clientDir, '_text/static/logo-bbb222.png', pngHeader.toString('binary'))

    // WOFF2 font (already compressed)
    const fontData = Buffer.alloc(2048, 0x77)
    await writeAsset(clientDir, '_text/static/font-ccc333.woff2', fontData.toString('binary'))

    const result = await precompressAssets(clientDir)

    expect(fs.existsSync(path.join(clientDir, '_text/static/logo-bbb222.png.br'))).toBe(false)
    expect(fs.existsSync(path.join(clientDir, '_text/static/font-ccc333.woff2.br'))).toBe(false)
    expect(result.filesCompressed).toBe(0)
  })
  ...
  it('compresses CSS files alongside JS', async () => {
    ...
    await writeAsset(clientDir, '_text/static/app-aaa111.js', jsContent)
    await writeAsset(clientDir, '_text/static/app-bbb222.css', cssContent)

    const result = await precompressAssets(clientDir)

    expect(result.filesCompressed).toBe(2)
    expect(fs.existsSync(path.join(clientDir, '_text/static/app-aaa111.js.br'))).toBe(true)
    expect(fs.existsSync(path.join(clientDir, '_text/static/app-bbb222.css.br'))).toBe(true)
  })
  ...
  it('does not re-compress existing .br or .gz files', async () => {
    ...
    expect(fs.existsSync(path.join(clientDir, '_text/static/index-abc123.js.br.br'))).toBe(false)
    expect(fs.existsSync(path.join(clientDir, '_text/static/index-abc123.js.gz.gz'))).toBe(false)
  })

  it('handles nested directories under assets/', async () => {
    const jsContent = 'export default {}\n'.repeat(200)
    await writeAsset(clientDir, '_text/static/chunks/vendor-ddd444.js', jsContent)

    const result = await precompressAssets(clientDir)

    expect(result.filesCompressed).toBe(1)
    expect(fs.existsSync(path.join(clientDir, '_text/static/chunks/vendor-ddd444.js.br'))).toBe(
      true,
    )
    expect(fs.existsSync(path.join(clientDir, '_text/static/chunks/vendor-ddd444.js.gz'))).toBe(
      true,
    )
  })
  ...
  it('only compresses files inside assets/ not other client files', async () => {
    ...
    await writeAsset(clientDir, '_text/static/main-eee555.js', jsContent)
  ...
  itWithZstd('generates .zst files alongside .br and .gz', async () => {
    ...
    expect(fs.existsSync(path.join(clientDir, '_text/static/zstd-aaa111.js.zst'))).toBe(true)
    expect(fs.existsSync(path.join(clientDir, '_text/static/zstd-aaa111.js.br'))).toBe(true)
    expect(fs.existsSync(path.join(clientDir, '_text/static/zstd-aaa111.js.gz'))).toBe(true)
    expect(result.filesCompressed).toBe(1)
  })
  ...
  itWithZstd('zstd output decompresses to original content', async () => {
    ...
    const zstdBuffer = await fsp.readFile(path.join(clientDir, '_text/static/hello-zstd.js.zst'))
  ...
  it('compresses assets under a custom assetsDir (assetPrefix layout)', async () => {
    ...
    // `<clientDir>/cdn/_text/static/...` instead of the default `assets/`.
    // Caller threads the resolved subdir in via options.assetsDir.
    const jsContent = 'const x = 1;\n'.repeat(200)
    await writeAsset(clientDir, 'cdn/_text/static/main-abc123.js', jsContent)
    ...
    await writeAsset(clientDir, '_text/static/legacy-def456.js', jsContent)

    const result = await precompressAssets(clientDir, { assetsDir: 'cdn/_text/static' })

    expect(result.filesCompressed).toBe(1)
    expect(fs.existsSync(path.join(clientDir, 'cdn/_text/static/main-abc123.js.br'))).toBe(true)
    expect(fs.existsSync(path.join(clientDir, 'cdn/_text/static/main-abc123.js.gz'))).toBe(true)
    // Legacy location is untouched
    expect(fs.existsSync(path.join(clientDir, '_text/static/legacy-def456.js.br'))).toBe(false)
  })

  it('compresses assets under _text/static for absolute-URL assetPrefix', async () => {
    // With an absolute-URL `assetPrefix` (e.g. `https://cdn.example.com`)
    // assets are written to `_text/static/` on disk; the URL prefix is
    // applied at render time, not on the filesystem.
    const jsContent = 'function f() { return 1; }\n'.repeat(200)
    await writeAsset(clientDir, '_text/static/chunk-xyz999.js', jsContent)

    const result = await precompressAssets(clientDir, { assetsDir: '_text/static' })

    expect(result.filesCompressed).toBe(1)
    expect(fs.existsSync(path.join(clientDir, '_text/static/chunk-xyz999.js.br'))).toBe(true)
  })

  it('reports total original and compressed byte sizes', async () => {
    const jsContent = 'const x = 1;\n'.repeat(500)
    await writeAsset(clientDir, '_text/static/big-fff666.js', jsContent)
