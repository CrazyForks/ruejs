 ...
 *
 * Text.js allows JSX syntax in .js files (Babel/SWC handle it transparently).
 * Vite 8's OXC transform defaults exclude .js files (include: /\.(m?ts|[jt]sx)$/,
 * exclude: /\.js$/). text overrides these defaults to match Text.js behavior.
 *
...
describe('JSX in plain .js files', () => {
  ...
  it('should render a page component defined in a .js file with JSX', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/jsx-in-js')
