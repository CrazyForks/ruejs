...
import { manifestFileWithBase } from '../src/utils/manifest-paths.js'
import { scanPublicFileRoutes } from '../src/utils/public-routes.js'
import { computeLazyChunks } from '../src/utils/lazy-chunks.js'
import { mergeHeaders, resolveStaticAssetSignal } from '../src/server/worker-utils.js'
import { domainCandidates, parseWranglerConfig } from '../src/cloudflare/tpr.js'
...
function createTmpDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-deploy-test-'))
  return dir
}
...
describe('generateWranglerConfig', () => {
  ...
  it('includes KV namespace when ISR detected', () => {
    ...
    expect(parsed.kv_namespaces[0].binding).toBe('TEXT_CACHE')
  })
...
describe('generateAppRouterWorkerEntry', () => {
  ...
  it('does not include KV wiring when hasISR is false', () => {
    ...
    expect(content).not.toContain('TEXT_CACHE')
  })

  it('includes KV wiring when hasISR is true', () => {
    const content = generateAppRouterWorkerEntry(true)
    expect(content).toContain('import { KVCacheHandler } from "text/cloudflare"')
    expect(content).toContain('import { setCacheHandler } from "text/shims/cache"')
    expect(content).toContain('TEXT_CACHE: KVNamespace')
    expect(content).toContain('new KVCacheHandler(env.TEXT_CACHE)')
...
describe('generatePagesRouterWorkerEntry', () => {
  ...
  it('applies text.config.js redirects before middleware', () => {
    const content = generatePagesRouterWorkerEntry()
  ...
  it('preserves responseHeaders on middleware redirect', () => {
    ...
    // text "if (result.response)". This block must reference responseHeaders
  ...
  it('applies text.config.js redirects', () => {
    const content = generatePagesRouterWorkerEntry()
  ...
  it('applies text.config.js rewrites (beforeFiles, afterFiles, fallback)', () => {
    const content = generatePagesRouterWorkerEntry()
  ...
  it('applies text.config.js custom headers', () => {
    const content = generatePagesRouterWorkerEntry()
    expect(content).toContain('configHeaders')
    expect(content).toContain('applyConfigHeadersToHeaderRecord')
    expect(content).toContain('from "text/server/request-pipeline"')
  })
  ...
  it('generated worker entry includes the no-body and streamed content-length merge guards', () => {
    ...
    expect(content).toContain('__textStreamedHtmlResponse')
  ...
  it('resolveStaticAssetSignal fetches and merges static asset responses with middleware status', async () => {
    const signalResponse = new Response(null, {
      status: 403,
      headers: [
        ['x-text-static-file', encodeURIComponent('/logo/logo.svg')],
  ...
  it('preserves x-middleware-request-* headers for prod request override handling', () => {
    ...
    expect(content).toContain('text/config/config-matchers')
  })
...
describe('generateAppRouterViteConfig', () => {
  it('includes text and cloudflare plugins', () => {
    const content = generateAppRouterViteConfig()
    expect(content).toContain('import text from "@rue-js/text"')
...
describe('generatePagesRouterViteConfig', () => {
  it('includes text and cloudflare plugins only', () => {
...
describe('getMissingDeps', () => {
  it('does not require @rue-js/vite-plugin-rue', () => {
    mkdir(tmpDir, 'pages')
    const info = detectProject(tmpDir)
    ...
    expect(missing).not.toContainEqual(expect.objectContaining({ name: '@rue-js/vite-plugin-rue' }))
  })
  ...
  it('does not require @vitejs/plugin-rsc for App Router', () => {
    mkdir(tmpDir, 'app')
    const info = detectProject(tmpDir)
    info.hasCloudflarePlugin = true
    info.hasWrangler = true
  ...
  it('does not require rue-rsc-payload for App Router', () => {
    mkdir(tmpDir, 'app')
    const info = detectProject(tmpDir)
    info.hasCloudflarePlugin = true
    info.hasWrangler = true
    ...
    const notResolvable = () => false
    const missing = getMissingDeps(info, notResolvable)
    expect(missing).not.toContainEqual(expect.objectContaining({ name: 'rue-rsc-payload' }))
  })

  it('does not require rue-rsc-payload for Pages Router', () => {
    mkdir(tmpDir, 'pages')
    const info = detectProject(tmpDir)
    info.hasCloudflarePlugin = true
    info.hasWrangler = true
...
describe('getFilesToGenerate', () => {
  ...
  it('generates App Router worker entry for App Router project', () => {
    ...
    expect(workerFile!.content).toContain('text/server/app-router-entry')
    expect(workerFile!.content).not.toContain('virtual:text-server-entry')
  })
  ...
  it('generates Pages Router worker entry for Pages Router project', () => {
    ...
    const workerFile = files.find(f => f.description === 'worker/index.ts')
    expect(workerFile).toBeDefined()
    expect(workerFile!.content).toContain('virtual:text-server-entry')
  ...
  it('generates App Router vite config for App Router project', () => {
    ...
    expect(viteFile!.content).toContain('text()')
...
export default defineConfig({
  plugins: [text(), cloudflare({ viteEnvironment: { name: "rsc", childEnvironments: ["ssr"] } })],
});
`,
    )
    expect(viteConfigHasCloudflarePlugin(tmpDir)).toBe(true)
...
import text from "@rue-js/text";
import { defineConfig } from "vite";
export default defineConfig({ plugins: [text()] });
`,
    )
    expect(viteConfigHasCloudflarePlugin(tmpDir)).toBe(false)
  })

  it('returns false for the minimal config generated by ruetext init', () => {
...
describe('formatMissingCloudflarePluginError', () => {
  ...
  it('always starts with the [text] prefix', () => {
    const msg = formatMissingCloudflarePluginError({ isAppRouter: false })
    expect(msg).toMatch(/^\[text\] Missing @cloudflare\/vite-plugin/)
  })
})
...
describe('detectProject — new detection features', () => {
  ...
  it('detects MDX via @text/mdx in text.config', () => {
    mkdir(tmpDir, 'app')
    writeFile(tmpDir, 'text.config.mjs', `import mdx from "@text/mdx";\nexport default mdx()({});`)
    const info = detectProject(tmpDir)
    expect(info.hasMDX).toBe(true)
  })
  ...
  it('hasCodeHike is false when not a dependency', () => {
    mkdir(tmpDir, 'app')
    writeFile(tmpDir, 'package.json', JSON.stringify({ dependencies: { rue: '^19.0.0' } }))
    const info = detectProject(tmpDir)
...
describe('generateAppRouterViteConfig — with project info', () => {
  it('delegates MDX to text plugin auto-injection (no separate mdx() call)', () => {
    mkdir(tmpDir, 'app')
    writeFile(tmpDir, 'app/about/page.mdx', '# About')
    const info = detectProject(tmpDir)
    const config = generateAppRouterViteConfig(info)
    // MDX is now handled by the text plugin's auto-injection at runtime,
  ...
  it('does not include CodeHike plugins in generated config (handled by text plugin)', () => {
    mkdir(tmpDir, 'app')
    writeFile(tmpDir, 'app/about/page.mdx', '# About')
    writeFile(tmpDir, 'package.json', JSON.stringify({ dependencies: { codehike: '^1.0.0' } }))
    const info = detectProject(tmpDir)
    const config = generateAppRouterViteConfig(info)
    // CodeHike plugins are extracted from text.config at runtime by the text plugin
...
// These tests exercise the same logic used by the text:cloudflare-build
...
describe('Cloudflare _headers file generation', () => {
  ...
  function generateHeaders(clientDir: string, assetsDir = '_text/static'): void {
  ...
  it('generates _headers with correct Cloudflare format', () => {
    ...
    expect(content).toContain('/_text/static/*')
    ...
    const pathLine = lines.findIndex(l => l === '/_text/static/*')
  ...
  it('skips generation when _headers already exists', () => {
    ...
    expect(content).not.toContain('/_text/static/*')
  })
...
// These tests verify that the text:cloudflare-build closeBundle hook correctly
// injects __TEXT_LAZY_CHUNKS__ and other globals into the worker entry for
...
describe('Cloudflare closeBundle lazy chunk injection', () => {
   ...
   * is at dist/server/index.js. The RSC plugin handles __TEXT_CLIENT_ENTRY__,
   * but we still need to inject __TEXT_LAZY_CHUNKS__ and __TEXT_SSR_MANIFEST__.
   */
  function simulateCloseBundleAppRouter(buildRoot: string, base = '/'): void {
    ...
    // App Router: inject into dist/server/index.js (NOT __TEXT_CLIENT_ENTRY__)
    ...
    if (fs.existsSync(workerEntry) && (lazyChunksData || ssrManifestData)) {
      ...
      if (ssrManifestData) {
        globals.push(`globalThis.__TEXT_SSR_MANIFEST__ = ${JSON.stringify(ssrManifestData)};`)
      }
      if (lazyChunksData) {
  ...
  function simulateCloseBundlePagesRouter(buildRoot: string, base = '/'): void {
    ...
    if (clientEntryFile || ssrManifestData || lazyChunksData) {
      ...
      const globals: string[] = []
      if (clientEntryFile) {
        globals.push(`globalThis.__TEXT_CLIENT_ENTRY__ = ${JSON.stringify(clientEntryFile)};`)
      }
      if (ssrManifestData) {
        globals.push(`globalThis.__TEXT_SSR_MANIFEST__ = ${JSON.stringify(ssrManifestData)};`)
      }
  ...
  const manifestWithLazyChunks = {
    'virtual:text-app-browser-entry': {
      file: 'assets/app-entry.js',
      isEntry: true,
      imports: ['node_modules/rue/index.js'],
      dynamicImports: ['src/components/MermaidChart.tsx'],
    },
    'node_modules/rue/index.js': {
  ...
  it('App Router: injects __TEXT_LAZY_CHUNKS__ into dist/server/index.js', () => {
    ...
    expect(code).toContain('globalThis.__TEXT_LAZY_CHUNKS__')

    // Verify the lazy chunks are correct (mermaid-chart and mermaid-vendor are lazy)
    const match = code.match(/globalThis\.__TEXT_LAZY_CHUNKS__\s*=\s*(\[.*?\]);/)
  ...
  it('App Router: does NOT inject __TEXT_CLIENT_ENTRY__', () => {
    setupAppRouterBuildOutput(tmpDir, manifestWithLazyChunks)
    ...
    expect(code).not.toContain('__TEXT_CLIENT_ENTRY__')
  })

  it('App Router: injects __TEXT_SSR_MANIFEST__ when present', () => {
    const ssrManifest = {
    ...
    const code = fs.readFileSync(path.join(tmpDir, 'dist', 'server', 'index.js'), 'utf-8')
    expect(code).toContain('globalThis.__TEXT_SSR_MANIFEST__')
  ...
  it('App Router: skips injection when no lazy chunks and no SSR manifest', () => {
    ...
    expect(code).not.toContain('globalThis.__TEXT_LAZY_CHUNKS__')
    expect(code).not.toContain('globalThis.__TEXT_SSR_MANIFEST__')
  ...
  it('Pages Router: injects all three globals into worker entry', () => {
    ...
    expect(code).toContain('globalThis.__TEXT_CLIENT_ENTRY__')
    expect(code).toContain('globalThis.__TEXT_SSR_MANIFEST__')
    expect(code).toContain('globalThis.__TEXT_LAZY_CHUNKS__')
  })
  ...
  it('Pages Router: prefixes client entry and lazy chunks with basePath', () => {
    ...
    const code = fs.readFileSync(path.join(tmpDir, 'dist', 'worker', 'index.js'), 'utf-8')
    expect(code).toContain('globalThis.__TEXT_CLIENT_ENTRY__ = "docs/assets/app-entry.js";')
...
describe('findInNodeModules', () => {
  ...
  it('walks up to find package in monorepo root node_modules', () => {
    writeFile(tmpDir, 'node_modules/.bin/wrangler', '#!/usr/bin/env node')
    const appDir = path.join(tmpDir, 'apps', 'web-text')
    fs.mkdirSync(appDir, { recursive: true })
...
describe('parseWranglerConfig — custom domain extraction', () => {
  ...
  it('extracts KV namespace ID for TEXT_CACHE', () => {
    writeFile(
      tmpDir,
      'wrangler.json',
      JSON.stringify({
        kv_namespaces: [{ binding: 'TEXT_CACHE', id: 'abc123' }],
      }),
    )
    const config = parseWranglerConfig(tmpDir)
