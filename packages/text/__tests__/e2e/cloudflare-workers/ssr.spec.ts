...
test.describe('Cloudflare Workers SSR', () => {
  test('home page renders server-side HTML', async ({ page }) => {
    ...
    await expect(page.locator('p').first()).toContainText('server-rendered by text')
  })
  ...
  test('about page renders correctly', async ({ page }) => {
    ...
    await expect(page.locator('p').first()).toContainText('text app deployed on Cloudflare Workers')
  })
  ...
  test('root layout wraps pages with html/head/body', async ({ page }) => {
    ...
    expect(title).toBe('text on Cloudflare Workers')
