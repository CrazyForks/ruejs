...
test.describe('Middleware (Pages Router)', () => {
  ...
  test('rewrite preserves getServerSideProps data', async ({ page }) => {
    ...
    // __TEXT_DATA__ should be present for hydration
  ...
  test('middleware does not affect static file requests', async ({ request }) => {
    ...
    // and paths starting with /_text. This is implicit from the matcher
    // config: /((?!api|_text|favicon\.ico).*)
    // We verify that /_text and /favicon.ico paths aren't intercepted
