...
test.describe('text/script', () => {
  // Ported from Text.js: packages/text/src/client/script.tsx
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/client/script.tsx
  // Text.js keeps a ScriptCache for in-flight remote scripts so same-src
  ...
  test('deduplicates simultaneous same-src scripts before load completes', async ({ page }) => {
    ...
    await expect
      .poll(() => page.evaluate(() => Reflect.get(window, '__textScriptDedupeExecutions')))
      .toBe(1)
  })
})
