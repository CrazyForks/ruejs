...
/**
 * Wait for the Pages Router to hydrate (the __TEXT_ROOT__ is set).
 */
async function waitForHydration(page: import('@playwright/test').Page) {
  await expect(async () => {
    const ready = await page.evaluate(() => !!(window as any).__TEXT_ROOT__)
...
test.describe('Shallow routing (Pages Router)', () => {
  ...
  test('dynamic route params update on shallow push across the same route template', async ({
    ...
    await page.evaluate(() =>
      (window as any).text.router.push('/posts/43', undefined, {
        shallow: true,
      }),
    )
  ...
  test('<Link shallow> click skips GSSP refetch (issue #1332 sub-problem 3)', async ({ page }) => {
    // Mirrors Text.js test/e2e/middleware-trailing-slash 'allows shallow linking
    // with middleware' — clicking <Link href="..." shallow> should update the
    // URL and router.query without triggering a /_text/data fetch.
    await page.goto(`${BASE}/shallow-test`)
    await expect(page.locator('h1')).toHaveText('Shallow Routing Test')
    await waitForHydration(page)
    ...
    page.on('request', req => {
      const url = req.url()
      if (url.includes('/_text/data/')) dataRequests.push(url)
    })
    ...
    // No _text/data requests fired by the shallow click.
  ...
  test('router.query preserves catch-all route params as arrays', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ;(window as any).__TEXT_DATA__.page = '/docs/[...slug]'
      ;(window as any).__TEXT_DATA__.query = { slug: ['a', 'b'] }
