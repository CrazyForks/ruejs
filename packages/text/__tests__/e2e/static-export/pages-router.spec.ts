...
test.describe('Static Export — Pages Router', () => {
  ...
  test('__TEXT_DATA__ is present in Pages Router output', async ({ page }) => {
    await page.goto(`${BASE}/old-school`)
    const textData = await page.evaluate(() => (window as any).__TEXT_DATA__)
    expect(textData).toBeDefined()
    expect(textData.props).toBeDefined()
    expect(textData.props.pageProps).toBeDefined()
  })

  test('product page __TEXT_DATA__ contains props from getStaticProps', async ({ page }) => {
    await page.goto(`${BASE}/products/widget`)
    const textData = await page.evaluate(() => (window as any).__TEXT_DATA__)
    expect(textData).toBeDefined()
    expect(textData.props).toBeDefined()
    expect(textData.props.pageProps).toBeDefined()
    expect(textData.props.pageProps.id).toBe('widget')
    expect(textData.props.pageProps.name).toBe('The Widget')
  })
