...
test.describe('headers() and cookies() in Server Components', () => {
  ...
  test('headers() available during browser client navigation', async ({ page }) => {
    ...
    // networkidle because Text Link prefetch can keep background requests alive.
...
/**
 * OpenText Compat: Middleware cookie/header behavior
 *
 ...
 *
 * OpenText verifies that internal middleware headers (x-middleware-set-cookie,
 * x-middleware-text) are NOT exposed in the response, and that middleware-set
 * cookies are readable by server components via cookies().get().
 */
test.describe('Middleware header/cookie behavior (OpenText compat)', () => {
  ...
  test('internal x-middleware-text header is NOT in response', async ({ request }) => {
    // Ref: opennextjs-cloudflare middleware.cookies.test.ts
    // "should not expose internal Text headers in response"
    const res = await request.get(`${BASE}/about`)
    const headers = res.headers()

    // x-middleware-text is an internal Text.js header used between middleware
    // and the server — it should never appear in the response to the client
    expect(headers['x-middleware-text']).toBeUndefined()
  })
  ...
  test('request headers available in server component RSC rendering', async ({ request }) => {
    ...
    const res = await request.get(`${BASE}/headers-test`, {
      headers: { 'x-custom-test': 'opentext-compat' },
    })
    expect(res.status()).toBe(200)
