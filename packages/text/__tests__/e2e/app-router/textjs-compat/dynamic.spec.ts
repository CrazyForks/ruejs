...
test.describe('Text.js compat: text/dynamic (browser)', () => {
  // Text.js: 'should handle text/dynamic in hydration correctly'
  ...
  test('ssr:false component appears after hydration', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/dynamic`)
    ...
    await expect(async () => {
      ...
      expect(text).toContain('text-dynamic dynamic no ssr on client')
  ...
  test('dynamic() components remain visible after hydration', async ({ page }) => {
    ...
    await expect(page.locator('#css-text-lazy')).toContainText('text-dynamic lazy')
    await expect(page.locator('#css-text-dynamic-server')).toContainText(
      'text-dynamic dynamic on server',
    )
    await expect(page.locator('#css-text-dynamic-client')).toContainText(
      'text-dynamic dynamic on client',
    )
    await expect(page.locator('#text-dynamic-server-import-client')).toContainText(
      'text-dynamic server import client',
    )
  })

  // Text.js: 'should support dynamic import with accessing named exports'
  ...
  test('named export via dynamic() renders button after hydration', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/dynamic/named-export`)
    await waitForAppRouterHydration(page)
  ...
  test('ssr:false page shows dynamic content after hydration', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/dynamic/ssr-false-only`)
  ...
  test('ssr:false from server component loads after hydration', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/dynamic/ssr-false-server`)
