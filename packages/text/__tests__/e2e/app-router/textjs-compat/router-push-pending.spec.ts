/**
 * Text.js Compat E2E: router pending state
 *
 * Text.js references:
 ...
 * - https://github.com/vercel/next.js/blob/canary/packages/text/src/client/components/app-router-instance.ts
 * - https://github.com/vercel/next.js/blob/canary/packages/text/src/client/components/use-action-queue.ts
 *
...
test.describe('Text.js compat: router pending state (browser)', () => {
  test('same-route search param push keeps useTransition pending until commit', async ({
    page,
  }) => {
    await page.goto(`${BASE}/textjs-compat/router-push-pending`)
  ...
  test('push to server-redirecting page keeps useTransition pending through redirect', async ({
    ...
    expect(page.url()).toContain('/textjs-compat/router-push-pending-destination')
  })

  test('router.refresh keeps useTransition pending until the refreshed route commits', async ({
    page,
  }) => {
    await page.goto(`${BASE}/textjs-compat/router-refresh-pending`)
    await waitForAppRouterHydration(page)
   ...
   * lifecycle. The final destination is the committed history entry; text
   * must not replace the source entry before the redirected payload is approved.
   *
   * Text.js source reference:
   * .textjs-ref/packages/text/src/client/components/router-reducer/fetch-server-response.ts
  ...
  test('back after router.push to a server-redirecting page returns to the source entry', async ({
    ...
    expect(new URL(page.url()).pathname).toBe('/textjs-compat/router-push-pending-destination')

    await page.goBack()
    await expect(page.locator('#router-push-pending-title')).toBeVisible({
      timeout: 10_000,
    })
    expect(new URL(page.url()).pathname).toBe('/textjs-compat/router-push-pending')
  })
})
