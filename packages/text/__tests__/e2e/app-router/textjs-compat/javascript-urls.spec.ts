  ...
  await expect
    .poll(async () => {
      const logs = await page.evaluate(() => {
        const value = Reflect.get(window, '__TEXT_TEST_CONSOLE_ERRORS__')
...
test.describe('javascript-urls', () => {
  test.beforeEach(async ({ page }) => {
    await page.addInitScript(() => {
      Reflect.set(window, '__TEXT_TEST_CONSOLE_ERRORS__', [])
  ...
  test('should prevent javascript URLs in server action redirect through onClick', async ({
    ...
    await page.goto(`${BASE}/textjs-compat/javascript-urls/action-redirect-onclick`)
    ...
    await page.locator('a[href="/textjs-compat/javascript-urls/safe"]').click()
    await expect(page).toHaveURL(`${BASE}/textjs-compat/javascript-urls/safe`)
  })
  ...
  test('should prevent javascript URLs in server action redirect through form action', async ({
    ...
    await page.goto(`${BASE}/textjs-compat/javascript-urls/action-redirect-form`)
    await waitForAppRouterHydration(page)
    const initialUrl = page.url()
  ...
  // Text.js keeps user-provided Link onClick handlers on anchors whose href is
  // later blocked by Rue/Text.js javascript: URL protections.
  // Reference implementation: https://github.com/vercel/next.js/blob/canary/packages/text/src/client/link.tsx
  test('preserves custom Link onClick handlers when blocking unsafe hrefs', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/javascript-urls/link-onclick`)
    await waitForAppRouterHydration(page)
    const initialUrl = page.url()
