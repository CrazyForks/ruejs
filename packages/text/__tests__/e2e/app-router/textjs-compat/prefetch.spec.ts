/**
 * Text.js Compat E2E: app-prefetch (browser tests)
 *
...
type PrefetchTestWindow = Window & Partial<Record<'__TEXT_PREFETCH_TEST__', PrefetchTestState>>

test.describe('Text.js compat: prefetch (browser)', () => {
  // Text.js: 'should navigate when prefetch is false'
  test('should navigate when prefetch is false', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/prefetch-test`)
  ...
  test('Link with prefetch={false} does not prefetch RSC payload in dev', async ({ page }) => {
    await page.addInitScript(() => {
      ...
      testWindow.__TEXT_PREFETCH_TEST__ = state
    ...
    await page.evaluate(async () => {
      await window.fetch('/textjs-compat/prefetch-test/target.rsc')
    })
    await expect
      .poll(async () =>
        page.evaluate(() => {
          const testWindow: PrefetchTestWindow = window
          const state = testWindow.__TEXT_PREFETCH_TEST__
