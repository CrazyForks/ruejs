...
const ROUTE_BASE = `${BASE}/textjs-compat/router-autoscroll`
...
declare global {
  // oxlint-disable-text-line typescript/consistent-type-definitions -- Window augmentation requires interface merging.
  interface Window {
...
async function waitForControls(page: Page) {
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const controls = window.__textRouterAutoscroll
        return {
...
test.describe('Text.js compat: App Router autoscroll', () => {
  // Ported from Text.js:
  ...
  test('scrolls to top of document when navigating between pages without layout offset', async ({
    ...
    await push(page, '/textjs-compat/router-autoscroll/0/0/100/10000/page2')
  ...
  test('scrolls down to the navigated page when it is below the viewport', async ({ page }) => {
    ...
    await push(page, '/textjs-compat/router-autoscroll/0/1000/100/1000/page2')
    await expect(page.locator('#page')).toHaveText('page2')
  ...
  test('does not scroll when the navigated page top is already in the viewport', async ({
    ...
    await push(page, '/textjs-compat/router-autoscroll/10/1000/100/1000/page2')
    await expect(page.locator('#page')).toHaveText('page2')
  ...
  test('preserves horizontal scroll while vertically autoscrolling', async ({ page }) => {
    ...
    await push(page, '/textjs-compat/router-autoscroll/0/0/10000/10000/page2')
    await expect(page.locator('#page')).toHaveText('page2')
  ...
  test('focuses the interactive navigated segment', async ({ page }) => {
    ...
    await push(page, '/textjs-compat/router-autoscroll/focus-target')
  ...
  test('preserves horizontal scroll when focusing the navigated segment', async ({ page }) => {
    // Text's horizontal autoscroll coverage uses a non-focusable route root, so it
    // misses the second browser scroll caused by focusing an offscreen target.
    // Text intentionally prevents that extra focus scroll.
    await page.goto(`${ROUTE_BASE}/0/0/10000/10000/page1`)
    await waitForControls(page)
