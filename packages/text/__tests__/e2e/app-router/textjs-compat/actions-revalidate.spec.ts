/**
 * Text.js Compat E2E: actions-revalidate-remount + revalidatetag-rsc
 *
 * Sources:
...
test.describe('Text.js compat: actions-revalidate (browser)', () => {
  function waitForActionDiscardingResponse(page: Page) {
    return page.waitForResponse(
      response =>
        response.request().method() === 'POST' &&
        response.url().includes('/textjs-compat/action-discarding.rsc'),
    )
  }
  ...
  async function expectActionRefreshPreservesLoading(page: Page, buttonSelector: string) {
    ...
    await page.goto(`${BASE}/textjs-compat/action-refresh-no-rerender`)
    ...
    await expect(async () => {
      const textValue = await page.locator('#flag-value').textContent()
      expect(textValue).toBeTruthy()
      expect(textValue).not.toBe(initialValue)
  ...
  // "action discarding" coverage. Text uses a local counter instead of a
  ...
  test('discarded server action without revalidation does not refresh current route', async ({
    ...
    await page.goto(`${BASE}/textjs-compat/action-discarding`)
    await waitForAppRouterHydration(page)
  ...
  // Text.js: 'should not remount the page + loading component when revalidating'
  ...
  test('revalidatePath via server action updates page data', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/action-revalidate`)
    await waitForAppRouterHydration(page)
  ...
  test('router.refresh() updates page data', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/refresh-test`)
    await waitForAppRouterHydration(page)
