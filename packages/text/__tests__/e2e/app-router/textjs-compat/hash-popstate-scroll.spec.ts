...
test.describe('Text.js compat: hash popstate scroll', () => {
  async function readTextHistoryIndex(page: Page) {
    return page.evaluate(() => {
      const state = window.history.state
      if (!state || typeof state !== 'object') return null

      const value = (state as Record<string, unknown>).__text_historyIndex
  ...
  async function expectHashForwardTraversal(
    ...
    await page.goto(`${BASE}/textjs-compat/hash-popstate-scroll`)
    await waitForAppRouterHydration(page)
    await expect(page.locator('h1')).toHaveText('Hash Popstate Scroll')

    await page.click(linkSelector)
    await expect(page).toHaveURL(`${BASE}/textjs-compat/hash-popstate-scroll${hash}`)

    await page.goBack()
    await expect(page).toHaveURL(`${BASE}/textjs-compat/hash-popstate-scroll`)
  ...
  // Text.js stores hash scroll intent in focusAndScrollRef and layout-router
  ...
  test('forward traversal to a hash-only Link entry scrolls the anchor into view', async ({
    ...
    await expect(page).toHaveURL(`${BASE}/textjs-compat/hash-popstate-scroll#content`)
  ...
  test('hash-only Link entries carry text traversal metadata', async ({ page }) => {
    await page.goto(`${BASE}/textjs-compat/hash-popstate-scroll`)
    await waitForAppRouterHydration(page)
    await expect(page.locator('h1')).toHaveText('Hash Popstate Scroll')
    expect(await readTextHistoryIndex(page)).toBe(0)

    await page.click('#hash-link')
    await expect(page).toHaveURL(`${BASE}/textjs-compat/hash-popstate-scroll#content`)
    expect(await readTextHistoryIndex(page)).toBe(1)
  ...
  test('hash-only replace after back keeps the current traversal index', async ({ page }) => {
    ...
    await expect(page).toHaveURL(`${BASE}/textjs-compat/hash-popstate-scroll#top`)
    expect(await readTextHistoryIndex(page)).toBe(0)
  })
  ...
  // Text.js App Router handles popstate with ACTION_RESTORE and classifies
  ...
  // Source:
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/client/components/app-router.tsx
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/client/components/segment-cache/navigation.ts
  test('back/forward traversal between hash entries skips RSC navigation', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const testWindow = window as Window & { __textHashPopstateRscCalls?: number }
      const runtime = Reflect.get(window, Symbol.for('text.navigationRuntime')) as
      ...
      runtime.functions.navigate = async (...args: Parameters<TestNavigate>) => {
        testWindow.__textHashPopstateRscCalls = (testWindow.__textHashPopstateRscCalls ?? 0) + 1
        return originalNavigate(...args)
      }
      testWindow.__textHashPopstateRscCalls = 0
    })
    ...
    await expect
      .poll(() =>
        page.evaluate(
          () =>
            (window as Window & { __textHashPopstateRscCalls?: number })
              .__textHashPopstateRscCalls ?? 0,
        ),
      )
      .toBe(0)
  })
  ...
  test('forward traversal falls back to named anchors', async ({ page }) => {
    await expectHashForwardTraversal(page, '#name-link', '#legacy-anchor', '[name="legacy-anchor"]')
  })
