...
// client reference runtime map. It requires packages/text/dist to exist.

type ProductionApp = {
  baseUrl: string
}

const requireFromTextPackage = createRequire(
  path.resolve(process.cwd(), 'packages/text/package.json'),
)

async function linkFixtureNodeModules(fixtureRoot: string): Promise<void> {
  const sourceNodeModules = path.resolve(process.cwd(), '__tests__/fixtures/app-basic/node_modules')
...
async function writePackageClientReferenceFixture(fixtureRoot: string): Promise<void> {
  const packageDir = path.join(fixtureRoot, 'node_modules', '@text-test', 'client-package')
...
`,
  ...
  page += `import { PackageProbe } from "@text-test/client-package";\n`
  await fs.writeFile(
...
async function assertPackageClientReferenceCssDeps(fixtureRoot: string): Promise<void> {
  const cssAsset = await findCssAssetContaining(
    path.join(fixtureRoot, 'dist', 'client', '_text', 'static'),
    'package-probe',
  )
  const manifestUrl =
...
async function buildAndServeProductionFixture(): Promise<{
  ...
  const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'text-client-reference-'))
  ...
  const textSource = path.resolve(process.cwd(), 'packages/text/src/index.ts')
  const ruePluginSource = requireFromTextPackage.resolve('@rue-js/vite-plugin-rue')
  await fs.writeFile(
    configFile,
    `import { defineConfig } from "vite";
import rue from ${JSON.stringify(pathToFileURL(ruePluginSource).href)};
import text from ${JSON.stringify(pathToFileURL(textSource).href)};

export default defineConfig({
  optimizeDeps: { exclude: ["@text-test/client-package"] },
  plugins: [rue(), text({ appDir: import.meta.dirname, rue: false })],
  ssr: { noExternal: ["@text-test/client-package"] },
});
`,
  )
  ...
  const { startProdServer } = await import(
    pathToFileURL(path.resolve(process.cwd(), 'packages/text/dist/server/prod-server.js')).href
  )
...
/* oxlint-disable eslint-plugin-rue-hooks/rules-of-hooks -- Playwright fixture `use`, not a Rue hook */
