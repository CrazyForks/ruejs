/**
 * OpenText Compat: Middleware redirect, rewrite, and block behavior.
 *
 * Ported from:
...
test.describe('Middleware Redirect (OpenText compat)', () => {
  ...
  test('direct load of /middleware-redirect redirects', async ({ request }) => {
    ...
    // Should be a 307 redirect (Text.js default for temporary redirect)
    expect([301, 302, 307, 308]).toContain(res.status())
    expect(res.headers()['location']).toMatch(/\/about$/)
  })
})

test.describe('Middleware Rewrite (OpenText compat)', () => {
...
test.describe('Middleware Block (OpenText compat)', () => {
...
test.describe('Middleware execution count', () => {
  ...
  // forwards middleware results to the RSC entry via x-text-mw-ctx so that
