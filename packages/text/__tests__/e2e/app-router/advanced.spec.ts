...
test.describe('Intercepting Routes', () => {
  ...
  test('refresh after chained intercepted navigation keeps the proven source context', async ({
    ...
    // Same user-visible contract as Text.js intercepted refresh coverage:
  ...
  test('refresh on direct target clears stale intercepted history context', async ({ page }) => {
    ...
    page.on('request', request => {
      ...
      if (url.pathname === '/photos/42.rsc') {
        refreshInterceptionHeaders.push(request.headers()['x-text-interception-context'] ?? null)
      }
    })
    ...
    await page.evaluate(() => {
      ...
      Reflect.set(textState, '__text_previousTextUrl', '/feed')

      const clientNavigationState = Reflect.get(window, Symbol.for('text.clientNavigationState'))
      ...
      if (typeof originalReplaceState !== 'function') {
        throw new Error('Expected Text original history.replaceState to be installed')
      }

      originalReplaceState.call(window.history, textState, '', window.location.href)
    })
    await expect
      .poll(() =>
        page.evaluate(() => {
          ...
          const value = Reflect.get(currentState, '__text_previousTextUrl')
    ...
    await page.evaluate(async () => {
      ...
      if (typeof navigate !== 'function') {
        throw new Error('Expected Text RSC navigation executor to be installed')
      }
  ...
  test('server action from intercepted modal preserves modal tree', async ({ page }) => {
    ...
    // final assertion in the Text.js reference test.
    await page.reload()
    await waitForAppRouterHydration(page)
    await expect(page.locator('[data-testid="photo-page"]')).toBeVisible()
    await expect(page.locator('[data-testid="photo-modal"]')).not.toBeVisible()
  ...
  test('sibling (..) intercepted navigation mounts the modal slot', async ({ page }) => {
    // Ported from the sibling-interception behavior covered by Text.js:
  ...
  test('sibling (..) modal preserves source content in a parallel feed slot', async ({ page }) => {
    // Ported from Text.js: test/e2e/app-dir/parallel-routes-and-interception/app/(group)/intercepting-parallel-modal
...
test.describe('Shallow Routing (history.pushState/replaceState)', () => {
  test('pushState updates useSearchParams', async ({ page }) => {
    ...
    await page.waitForFunction(
      () => typeof (window as any).__TEXT_RSC_ROOT__ !== 'undefined',
      null,
      { timeout: 10000 },
    )
    ...
    // useSearchParams should rue to the URL change
