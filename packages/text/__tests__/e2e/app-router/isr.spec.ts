...
test.describe('App Router ISR', () => {
  ...
  // Text.js behavior (every request re-renders fresh). These tests should run
  ...
  test.skip('second request within TTL is a cache HIT with same timestamp', async ({ request }) => {
    ...
    const cacheHeader = res2.headers()['x-text-cache']
  ...
  test.skip('request after TTL expires returns STALE with same cached content', async ({
    ...
    const html2 = await res2.text()
    const ts2 = html2.match(/data-testid="timestamp">(\d+)</)?.[1]
    const cacheHeader2 = res2.headers()['x-text-cache']
  ...
  test.skip('after STALE triggers regen, subsequent request is HIT', async ({ request }) => {
    ...
    expect(staleRes.headers()['x-text-cache']).toBe('STALE')

    await new Promise(r => setTimeout(r, 500))

    const hitRes = await request.get(`${BASE}/isr-test`)
    expect(hitRes.headers()['x-text-cache']).toBe('HIT')
  })
  ...
  test('non-ISR page does not have ISR cache headers', async ({ request }) => {
    ...
    const cacheHeader = res.headers()['x-text-cache']
...
/**
 * OpenText Compat: ISR dynamicParams cache header tests
 *
 * Ported from: https://github.com/opennextjs/opennextjs-cloudflare/blob/main/examples/e2e/app-router/e2e/isr.test.ts
 *
 * OpenText verifies that `dynamicParams=true` pages return HIT for prebuilt paths,
 * MISS for non-prebuilt, and 404 for notFound(). `dynamicParams=false` returns 404
 * for unknown params. These tests verify the same cache header semantics in text.
 */
test.describe('ISR dynamicParams cache headers', () => {
  test.describe('dynamicParams=false (products)', () => {
    ...
    test('should return 200 on a prebuilt path', async ({ request }) => {
      ...
      // and x-text-cache may not be set. We verify the page renders correctly.
...
/**
 * OpenText Compat: revalidateTag / revalidatePath E2E lifecycle tests.
 *
 ...
 *
 * OpenText verifies the full tag-based cache invalidation lifecycle:
 ...
 */
test.describe('revalidateTag / revalidatePath lifecycle (OpenText compat)', () => {
  ...
  test.skip('revalidateTag invalidates cached page and regenerates', async ({ request }) => {
    ...
    const html1 = await res1.text()
    // Rue SSR may insert <!-- --> comment nodes between text and expressions,
    ...
    const cacheHeader3 = res3.headers()['x-text-cache']
    if (cacheHeader3) {
  ...
  test.skip('after invalidation + regen, subsequent request is HIT', async ({ request }) => {
    // Ref: opennextjs-cloudflare revalidateTag.test.ts — after MISS, text request should be HIT
    test.setTimeout(30_000)
    ...
    const cacheHeader = hitRes.headers()['x-text-cache']
    if (cacheHeader) {
      expect(cacheHeader).toBe('HIT')
    }
  })
...
/**
 * OpenText Compat: ISR data cache (unstable_cache) separation from page cache.
 *
 * Ported from: https://github.com/opennextjs/opennextjs-cloudflare/blob/main/examples/e2e/app-router/e2e/isr.test.ts
 ...
 */
test.describe('unstable_cache data cache (OpenText compat)', () => {
