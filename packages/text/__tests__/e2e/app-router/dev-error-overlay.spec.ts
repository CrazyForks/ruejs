...
// Rue doesn't attach onClick until hydration completes. The click can land
...
async function clickUntilOverlay(page: Page, triggerTestId: string): Promise<void> {
  ...
  const dialog = page.getByTestId('text-dev-error-overlay')
  const indicator = page.getByTestId('text-dev-error-indicator')
...
test.describe('Dev error overlay', () => {
  test('surfaces Rue render errors with no error.tsx boundary', async ({ page }) => {
    ...
    // depending on which Rue error callback fires last. Either is fine — the
    // point is that the developer sees the error.
    await expect(page.getByTestId('text-dev-error-title')).toContainText('Runtime Error')
    await expect(page.getByTestId('text-dev-error-message')).toContainText(
  ...
  test('surfaces global script errors via window.onerror', async ({ page }) => {
    ...
    await expect(page.getByTestId('text-dev-error-title')).toHaveText('Unhandled Script Error')
    await expect(page.getByTestId('text-dev-error-message')).toContainText('uncaught timer error')
  })
  ...
  test('surfaces unhandled promise rejections', async ({ page }) => {
    ...
    await expect(page.getByTestId('text-dev-error-title')).toHaveText('Unhandled Promise Rejection')
    await expect(page.getByTestId('text-dev-error-message')).toContainText(
  ...
  test('dismiss button hides the overlay', async ({ page }) => {
    ...
    await page.getByTestId('text-dev-error-close').click()
    await expect(page.getByTestId('text-dev-error-overlay')).toBeHidden()
  })
  ...
  test('subsequent errors update the overlay and expose pagination', async ({ page }) => {
    ...
    await expect(page.getByTestId('text-dev-error-pagination')).toBeHidden()

    // The dialog covers the page; minimize so the text trigger is reachable.
    await page.getByTestId('text-dev-error-minimize').click()
    await page.getByTestId('trigger-unhandled-rejection').click()
    // A non-caught error re-expands the dialog automatically.
    await expect(page.getByTestId('text-dev-error-message')).toContainText(
      'unhandled rejection from button',
    )
    await expect(page.getByTestId('text-dev-error-counter')).toHaveText('2 of 2')
  })

  test('prev/text pagination switches between reported errors', async ({ page }) => {
    await page.goto(`${BASE}/dev-overlay-test`)
    await clickUntilOverlay(page, 'trigger-window-error')
    await page.getByTestId('text-dev-error-minimize').click()
    await page.getByTestId('trigger-unhandled-rejection').click()
    ...
    await page.getByTestId('text-dev-error-prev').click()
    await expect(page.getByTestId('text-dev-error-counter')).toHaveText('1 of 2')
    await expect(page.getByTestId('text-dev-error-message')).toContainText('uncaught timer error')

    await page.getByTestId('text-dev-error-text').click()
    await expect(page.getByTestId('text-dev-error-counter')).toHaveText('2 of 2')
    await expect(page.getByTestId('text-dev-error-message')).toContainText(
      'unhandled rejection from button',
    )
  ...
  test('renders a parsed stack when one is available', async ({ page }) => {
    ...
    const stack = page.getByTestId('text-dev-error-stack')
  ...
  test('soft-nav to a broken route still updates the URL', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ;(window as unknown as { __textReloadCanary?: boolean }).__textReloadCanary = true
    })
    ...
    const canary = await page.evaluate(
      () => (window as unknown as { __textReloadCanary?: boolean }).__textReloadCanary,
    )
  ...
  test('clicking the backdrop minimizes the dialog to a corner indicator', async ({ page }) => {
    ...
    // underneath.
    await page.getByTestId('text-dev-error-backdrop').click({ position: { x: 5, y: 5 } })

    await expect(page.getByTestId('text-dev-error-overlay')).toBeHidden()
    const indicator = page.getByTestId('text-dev-error-indicator')
    await expect(indicator).toBeVisible()
    await expect(page.getByTestId('text-dev-error-indicator-count')).toHaveText('1')

    // Clicking the indicator restores the full dialog.
    await indicator.click()
    await expect(page.getByTestId('text-dev-error-overlay')).toBeVisible()
