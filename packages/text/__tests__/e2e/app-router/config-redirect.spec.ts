/**
 * OpenText Compat: text.config.js redirects, rewrites, and custom headers.
 *
 * Ported from:
...
test.describe('Config Redirects (OpenText compat)', () => {
...
test.describe('Config Rewrites (OpenText compat)', () => {
...
test.describe('Config Custom Headers (OpenText compat)', () => {
  // Ref: opennextjs-cloudflare headers.test.ts — "Headers"
  test('custom header from text.config headers() is present on pages', async ({ request }) => {
    const res = await request.get(`${BASE}/about`)
    expect(res.status()).toBe(200)
    // The /(.*) catch-all header applies to all routes
    expect(res.headers()['x-e2e-header']).toBe('text-e2e')
  ...
  test('custom header applied to API routes', async ({ request }) => {
    ...
    expect(res.status()).toBe(200)
    // The /api/(.*) header
    expect(res.headers()['x-custom-header']).toBe('text-app')
    // The /(.*) catch-all header
    expect(res.headers()['x-e2e-header']).toBe('text-e2e')
  })

  // Ref: opennextjs-cloudflare headers.test.ts — "x-powered-by should be absent"
  // text never sends X-Powered-By (matching Text.js poweredByHeader: false behavior).
  ...
  // Middleware response headers always take precedence over text.config.js `headers()` rules for
  // the same key, matching Text.js behavior.
  // Fixture: middleware sets e2e-headers=middleware for /headers/override-from-middleware;
  //          text.config.ts sets e2e-headers=text.config.js on /(.*)
