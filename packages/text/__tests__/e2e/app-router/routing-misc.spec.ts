/**
 * OpenText Compat: Trailing slash, catch-all routes, host URL, search params.
 *
 * Ported from:
...
test.describe('Trailing Slash (OpenText compat)', () => {
  ...
  // Text.js returns 404 for //example.com/ to prevent protocol-relative redirects.
...
test.describe('Catch-all API Route with Hyphen (OpenText compat)', () => {
  ...
  test('catch-all API route captures multiple segments including hyphens', async ({ request }) => {
    const res = await request.get(`${BASE}/api/catch-all/open-text/is/really/cool`)
    expect(res.status()).toBe(200)
    expect(res.headers()['content-type']).toContain('application/json')
    const json = await res.json()
    expect(json).toStrictEqual({
      slugs: ['open-text', 'is', 'really', 'cool'],
    })
  })
...
test.describe('Host URL (OpenText compat)', () => {
...
test.describe('Search Params in RSC (OpenText compat)', () => {
