...
const VISITED_CACHE_MARKER = '__TEXT_VISITED_CACHE_MARKER__'
const RSC_NAVIGATION_PROMISE_MARKER = '__TEXT_TEST_RSC_NAVIGATION_PROMISE__'

async function pushAppRoute(page: Page, pathname: string): Promise<void> {
  await page.evaluate(target => {
    const router = window.text?.router
    if (!router) {
      throw new Error('window.text.router is not installed')
    }
...
async function waitForLastRscNavigation(page: Page): Promise<void> {
  await page.waitForFunction(marker => Reflect.get(window, marker), RSC_NAVIGATION_PROMISE_MARKER)
