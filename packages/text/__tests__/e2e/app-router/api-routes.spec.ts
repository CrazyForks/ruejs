  ...
  test.describe('POST /api/hello', () => {
    test('echoes the request body', async ({ request }) => {
      const response = await request.post(`${BASE}/api/hello`, {
        data: { name: 'text', version: 1 },
      })
      expect(response.status()).toBe(200)
      const body = await response.json()
      expect(body).toEqual({ echo: { name: 'text', version: 1 } })
    })
...
/**
 * OpenText Compat: Exhaustive HTTP method tests for route handlers.
 *
 ...
 *
 * OpenText tests every HTTP method (GET, POST, PUT, PATCH, DELETE, HEAD, OPTIONS)
 * plus formData, cookies, redirects, dynamic segments, and query params. These
 * tests verify the same behavior in text's route handler implementation.
 */
test.describe('Route Handler HTTP Methods (OpenText compat)', () => {
  ...
  test('GET returns 200 with JSON', async ({ request }) => {
    ...
    expect(data.message).toBe('text route handler')
  })
  ...
  test('PUT returns 201 with JSON body merged', async ({ request }) => {
    ...
    const res = await request.put(`${BASE}/api/methods`, {
      data: { message: 'text PUT' },
    })
    expect(res.status()).toBe(201)
    const data = await res.json()
    // Route handler spreads body into response: { message: "ok", ...body }
    // Since body has message: "text PUT", it overrides to "text PUT"
    expect(data.message).toBe('text PUT')
  })
  ...
  test('HEAD returns 200 with custom headers and empty body', async ({ request }) => {
    ...
    expect(headers['special-header']).toBe('text is great')
  })
  ...
  test('OPTIONS returns 204 with Allow header', async ({ request }) => {
    ...
    expect(res.status()).toBe(204)
    // Text's auto-OPTIONS sets Allow based on detected exports
    const headers = res.headers()
  ...
  test('formData should work in POST route handler', async ({ request }) => {
    ...
    expect(data.email).toBe('text@text.dev')
  })
  ...
  test('query parameters should work in route handlers', async ({ request }) => {
    ...
    const res = await request.get(`${BASE}/api/methods/query?query=text+is+awesome`)
    expect(res.status()).toBe(200)
    const data = await res.json()
    expect(data.query).toBe('text is awesome')
  })
})

/**
 * OpenText Compat: Route handler Cache-Control headers.
 *
 * Ported from: https://github.com/opennextjs/opennextjs-cloudflare/blob/main/examples/e2e/app-router/e2e/methods.test.ts
 * Tests: ON-3 #13-14 in TRACKING.md
 *
 * In Text.js, a GET-only route handler with `export const revalidate = N`
 * receives Cache-Control: s-maxage=N, stale-while-revalidate.
 */
test.describe('Route Handler Cache Headers (OpenText compat)', () => {
