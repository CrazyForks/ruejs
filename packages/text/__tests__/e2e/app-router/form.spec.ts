...
test.describe('text/form GET interception', () => {
  ...
  test('Form GET submission updates URL query params', async ({ page }) => {
    ...
    await page.fill('#search-input', 'rue')
    await expect(page.locator('#search-input')).toHaveValue('rue')
    await expect(page.locator('#search-button')).toBeEnabled()
    await page.locator('#search-button').click({ noWaitAfter: true })

    await expect(page.locator('#search-result')).toHaveText('Results for: rue', {
      timeout: 10_000,
    })
    const url = new URL(page.url())
    expect(url.pathname).toBe('/search')
    expect(url.searchParams.get('q')).toBe('rue')
  })
  ...
  test('Form GET submission includes hidden inputs in the destination URL', async ({ page }) => {
    ...
    await page.fill('#search-input-with-query', 'rue')
    await expect(page.locator('#search-input-with-query')).toHaveValue('rue')
    ...
    await expect.poll(() => new URL(page.url()).search, { timeout: 10_000 }).toBe('?lang=en&q=rue')

    const url = new URL(page.url())
    expect(url.pathname).toBe('/search')
    expect(url.searchParams.get('lang')).toBe('en')
    expect(url.searchParams.get('q')).toBe('rue')
    expect(url.search).toBe('?lang=en&q=rue')
  })
