...
const instrumentationClientPath = `${process.cwd()}/__tests__/fixtures/app-basic/instrumentation-client.ts`
  ...
  test('executes instrumentation-client before hydration', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      const win = window as Window & {
        ...
        __TEXT_HYDRATED_AT?: number
      }
      return (
        win.__INSTRUMENTATION_CLIENT_EXECUTED_AT !== undefined &&
        win.__TEXT_HYDRATED_AT !== undefined
      )
    })
    ...
    const timing = await page.evaluate(() => {
      ...
      return {
        ...
        hydration: win.__TEXT_HYDRATED_AT,
      }
    })
