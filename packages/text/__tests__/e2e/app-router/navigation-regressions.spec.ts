...
test.describe('Navigation regression tests (#652 Firefox hang fix)', () => {
  ...
  test('importing text/router in an App Router client bundle does not hijack back/forward', async ({
    page,
  }) => {
    ...
    await expect
      .poll(() => page.evaluate(() => (window as RouterSideEffectWindow).__TEXT_ROUTER_IMPORTED__))
      .toBe(true)
  ...
  test('cross-route then same-route navigation works', async ({ page }) => {
    ...
    await page.click('#link-rue')
    await expect(page.locator('#query-title')).toHaveText('Search: rue', { timeout: 10_000 })
    await expect(page.locator('#hook-query')).toHaveText('q: rue')
    expect(page.url()).toContain('q=rue')
  })
  ...
  test('cancelled slow navigation does not leak params to committed route', async ({ page }) => {
    ...
    // that could be committed by the text navigation.
