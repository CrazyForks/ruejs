 ...
 * text() immediately after running middleware, delegating all rendering to the
 ...
 * "text on Cloudflare Workers" while the pages/ stub renders "pages index".
...
test.describe('middleware.ts with @cloudflare/vite-plugin', () => {
  ...
  test('/api/hello runs inside the Cloudflare Worker', async ({ request }) => {
    ...
    // middleware then calls text(), handing off to the Cloudflare plugin.
  ...
  test('root route runs inside the Cloudflare Worker', async ({ request }) => {
    ...
    // The connect handler must call text() so the Cloudflare plugin dispatches
    // the request to app/page.tsx inside the Worker.
    //
    // "text on Cloudflare Workers" — app/page.tsx rendered in the Worker
    // "pages index"                  — pages stub rendered by the connect handler
    const res = await request.get(`${BASE}/`)
    expect(res.status()).toBe(200)

    const body = await res.text()
    expect(body).toContain('text on Cloudflare Workers')
