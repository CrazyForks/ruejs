...
test.describe('Pages Router client hydration on Cloudflare Workers', () => {
  // Pages Router uses __TEXT_DATA__ for hydration. Timestamps work correctly
  ...
  test('client JS bundle is loaded', async ({ page }) => {
    ...
    expect(html).toMatch(/script\s+type="module"\s+src="\/_text\/static\/[^"]+\.js"/)
  })
  ...
  test('counter becomes interactive after hydration', async ({ page }) => {
    ...
    // Wait for hydration — the client JS loads and Rue hydrates
  ...
  test('index page with GSSP timestamp hydrates without errors', async ({ page }) => {
    ...
    // are serialized to __TEXT_DATA__ and reused during hydration.
    const errors: string[] = []
    page.on('console', msg => {
    ...
    // Verify timestamp is in __TEXT_DATA__
    const textData = await page.evaluate(() => (window as any).__TEXT_DATA__)
    expect(textData.props.pageProps.timestamp).toBeDefined()
  ...
  test('GSSP page hydrates with server data', async ({ page }) => {
    ...
    // __TEXT_DATA__ should have the GSSP props
    const textData = await page.evaluate(() => (window as any).__TEXT_DATA__)
    expect(textData).toBeDefined()
    expect(textData.props.pageProps.message).toBe('Server-Side Rendered on Workers')
  })
  ...
  test('GSSP page with timestamp hydrates without errors (correct pattern)', async ({ page }) => {
    ...
    // The timestamp is serialized in __TEXT_DATA__ and reused during hydration,
    ...
    // The timestamp from __TEXT_DATA__ should match what's rendered
    const textData = await page.evaluate(() => (window as any).__TEXT_DATA__)
    expect(textData.props.pageProps.timestamp).toBeDefined()
