...
test.describe('Pages Router navigation on Cloudflare Workers', () => {
  ...
  test('each page has correct __TEXT_DATA__.page value', async ({ page }) => {
    // Home
  ...
  // must fetch /_text/data/<buildId>/<page>.json (the JSON data endpoint) and
  // re-render in place — no document reload. If the build pipeline regresses
  // and stops exposing __TEXT_PAGE_LOADERS__, navigateClient() silently falls
  ...
  test('Link click fetches /_text/data JSON, not full HTML', async ({ page }) => {
    await page.goto(BASE + '/')
    // Wait for hydration to expose the loader manifest.
    await page.waitForFunction(() => (window as any).__TEXT_HYDRATED_AT !== undefined)

    const buildId = await page.evaluate(() => (window as any).__TEXT_DATA__.buildId)
    ...
    page.on('request', req => {
      if (req.url().includes(`/_text/data/${buildId}/ssr.json`)) {
