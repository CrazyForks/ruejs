...
`,
  ...
  children: import('@rue-js/rue').Renderable;
}) {
  return (
    <html lang="en">
      <body>{children}</body>
...
describe('App Router tsconfig path aliases in production builds', () => {
  ...
  it('transforms alias-based import.meta.glob and alias-based dynamic import in Cloudflare builds', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'text-tsconfig-alias-build-'))
    tmpDirs.push(root)
    writeCloudflareAppFixture(root, 'text-tsconfig-alias-build')
    writeFixtureFile(
      root,
      'app/page.tsx',
...
This file exists only to trigger text's MDX auto-detection in the fixture.
`,
    )
    writeFixtureFile(
      root,
...
`,
  ...
  it('import.meta.glob with MDX files containing frontmatter does not cause parse errors (issue #659)', async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), 'text-mdx-frontmatter-build-'))
    tmpDirs.push(root)
    writeCloudflareAppFixture(root, 'text-mdx-frontmatter-test')
    writeFixtureFile(
      root,
      'app/page.tsx',
      `import { getGlobPostCount } from "../lib/mdx-loader";
