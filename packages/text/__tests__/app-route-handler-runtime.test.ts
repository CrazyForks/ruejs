...
} from '../src/server/app-route-handler-runtime.js'
import { TextRequest, TextURL } from '../src/shims/server.js'
...
describe('app route handler runtime helpers', () => {
  ...
  it('stubs request-specific fields for force-static route handlers', () => {
    ...
    expect(tracked.request.textUrl.href).toBe('http://localhost:3000/demo')
    expect(tracked.request.textUrl.search).toBe('')
    expect(tracked.request.textUrl.searchParams.get('secret')).toBeNull()
    expect(tracked.didAccessDynamicRequest()).toBe(false)
    expect(accesses).toEqual([])
  })

  it('removes credentials from force-static route handler URLs', () => {
    const request = new TextRequest('https://tenant.example.com/demo')
    Object.defineProperty(request, 'textUrl', {
      configurable: true,
      value: new TextURL('https://user:pass@tenant.example.com/demo?secret=from-user#fragment'),
    })
  ...
  it('stubs body-reading APIs for force-static route handlers', async () => {
    ...
    await expect(createTrackedPost().request.arrayBuffer()).resolves.toHaveProperty('byteLength', 0)
  ...
  it('throws on dynamic request access for dynamic error route handlers', () => {
    const expectedMessage = (expression?: string): string =>
      `Route /private with \`dynamic = "error"\` couldn't be rendered statically because it used ${expression ?? 'a dynamic request API'}. See more info here: https://textjs.org/docs/app/building-your-application/rendering/static-and-dynamic#dynamic-rendering`
    const tracked = createTrackedAppRouteRequest(
    ...
    expect(() => tracked.request.textUrl.search).toThrow(expectedMessage('textUrl.search'))
    expect(() => tracked.request.textUrl.searchParams).toThrow(
      expectedMessage('textUrl.searchParams'),
    )
    expect(() => tracked.request.textUrl.href).toThrow(expectedMessage('textUrl.href'))
    expect(() => tracked.request.textUrl.origin).toThrow(expectedMessage('textUrl.origin'))
    expect(() => Reflect.get(tracked.request.textUrl, 'toString')).toThrow(
      expectedMessage('textUrl.toString'),
    )

    const clonedRequest = tracked.request.clone()
    expect(() => clonedRequest.headers).toThrow(expectedMessage('request.headers'))

    const clonedTextUrl = tracked.request.textUrl.clone()
    expect(() => clonedTextUrl.search).toThrow(expectedMessage('textUrl.search'))
  })
  ...
  it('normalizes request.url through textUrl for stripped internal app route requests', () => {
    const tracked = createTrackedAppRouteRequest(
    ...
    expect(tracked.request.textUrl.href).toBe('https://example.com/base/fr/demo?ping=from-url')
  ...
  it('tracks dynamic textUrl fields but not pathname', () => {
    const accesses: string[] = []
    const tracked = createTrackedAppRouteRequest(
      new Request('https://example.com/base/fr/demo?ping=from-text-url'),
      {
        basePath: '/base',
        i18n: { locales: ['en', 'fr'], defaultLocale: 'en' },
        onDynamicAccess(access) {
    ...
    expect(tracked.request.textUrl.pathname).toBe('/demo')
    expect(tracked.request.textUrl.locale).toBe('fr')
    expect(tracked.didAccessDynamicRequest()).toBe(false)

    expect(tracked.request.textUrl.searchParams.get('ping')).toBe('from-text-url')
    expect(tracked.request.textUrl.href).toBe('https://example.com/base/fr/demo?ping=from-text-url')
    expect(accesses).toEqual(['textUrl.searchParams', 'textUrl.href'])
    expect(tracked.didAccessDynamicRequest()).toBe(true)
  })
