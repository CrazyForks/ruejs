...
import { createAppRscHandler } from '../src/server/app-rsc-handler.js'
...
describe('createAppRscHandler', () => {
  ...
  it('marks progressive action page renders even when decoded form state is null', async () => {
    ...
    const response = await handler(
      new Request('https://example.test/docs/about', {
        method: 'POST',
        headers: { 'content-type': 'multipart/form-data; boundary=text' },
      }),
      null,
    )
    ...
    try {
      const response = await handler(
        new Request('https://example.test/docs/prerender-encoding/sticks%20%26%20stones', {
          headers: {
            'x-text-prerender-secret': 'test-secret',
            'x-text-prerender-route-params': prerenderRouteParamsHeader({
  ...
  it('redirects invalid RSC cache-busting requests before middleware', async () => {
    ...
    expect(response.status).toBe(307)
    expect(response.headers.get('location')).toBe(`/docs/about.rsc?tab=latest&_rsc=${expectedHash}`)
  ...
  it('accepts the text action header name for server actions', async () => {
    ...
    await handler(
      new Request('https://example.test/docs/about', {
        method: 'POST',
        headers: { 'x-rsc-action': 'text-action' },
      }),
      null,
    )

    expect(handleServerActionRequest).toHaveBeenCalledWith(
      expect.objectContaining({ actionId: 'text-action' }),
    )
  })
  ...
  // Matches Text.js behavior: non-dynamic route handlers receive params=null.
  // See test/e2e/app-dir/app-routes/app-custom-routes.test.ts in text.js.
  ...
  it('appends App Router RSC vary values to route handler responses', async () => {
    ...
    expect(response.status).toBe(200)
    expect(response.headers.get('vary')).toBe(`User-Agent, ${TEXT_RSC_VARY_HEADER}`)
  })
