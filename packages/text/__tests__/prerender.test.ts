...
import { buildPagesFixture, buildCloudflareAppFixture, buildIsolatedAppFixture } from './helpers.js'
import {
...
const RSC_RUNTIME_BOOTSTRAP_EXPRESSION =
  '((self[Symbol.for("text.navigationRuntime")]??={bootstrap:{routeManifest:null},functions:{}}).bootstrap.rsc??={rsc:[]})'
...
function runtimeMirroredRscChunkScript(chunk: string | [3, string]): string {
  const chunkJson = safeJsonStringify(chunk)
  return (
    `<script>${RSC_RUNTIME_BOOTSTRAP_EXPRESSION}.rsc.push(${chunkJson});` +
    `(self.__TEXT_RSC_CHUNKS__??=[]).push(${chunkJson})</script>`
  )
}
...
function runtimeMirroredRscDoneScript(): string {
  return `<script>${RSC_RUNTIME_BOOTSTRAP_EXPRESSION}.done=true;self.__TEXT_RSC_DONE__=true</script>`
}

function legacyRscChunkScript(chunk: string | [3, string]): string {
  return (
    '<script>self.__TEXT_RSC_CHUNKS__=self.__TEXT_RSC_CHUNKS__||[];' +
    `self.__TEXT_RSC_CHUNKS__.push(${safeJsonStringify(chunk)})</script>`
  )
}

function legacyRscDoneScript(): string {
  return '<script>self.__TEXT_RSC_DONE__=true</script>'
}
...
describe('extractRscPayloadFromPrerenderedHtml', () => {
  ...
  it('reconstructs runtime chunks mirrored to the legacy global in one script', () => {
    const chunks = [
      '0:D{"name":"layout"}\n',
      '1:["$","div",null,{"children":"mirrored ) chunk"}]\n',
    ]
    const html =
      '<html><body>' +
      chunks.map(chunk => runtimeMirroredRscChunkScript(chunk)).join('') +
      runtimeMirroredRscDoneScript() +
      '</body></html>'
  ...
  it('does not treat marker-looking RSC payload text as the done control script', () => {
    const html =
      '<html><body>' + runtimeRscChunkScript('0:["__TEXT_RSC_DONE__=true"]\n') + '</body></html>'
  ...
  it('rejects chunk scripts with trailing code after the payload push', () => {
    ...
    expect(() => extractRscPayloadFromPrerenderedHtml(html)).toThrow(
      '[text] Malformed prerender RSC embed: unexpected chunk script shape',
    )
  })
  ...
  it('rejects chunk scripts with invalid JSON', () => {
    ...
    expect(() => extractRscPayloadFromPrerenderedHtml(html)).toThrow(
      '[text] Malformed prerender RSC embed: invalid chunk JSON',
    )
  })
...
describe('prerenderApp — RSC extraction', () => {
  it('writes the .rsc file from rendered HTML without a second RSC request', async () => {
    const root = tmpDir('text-prerender-rsc-dedupe-')
    ...
    const server = createServer((req, res) => {
      ...
      if (req.url === '/__text_nonexistent_for_404__') {
        res.statusCode = 404
    ...
    try {
      const { prerenderApp } = await import('../src/build/prerender.js')
  ...
  it('falls back to a second RSC: 1 invocation when middleware short-circuits with custom HTML', async () => {
    ...
    const root = tmpDir('text-prerender-rsc-fallback-')
    const outDir = path.join(root, 'out')
    const appDir = path.join(root, 'app')
    const pagePath = path.join(appDir, 'page.tsx')
    fs.mkdirSync(appDir, { recursive: true })
  ...
  it('errors without writing .rsc when the middleware short-circuit fallback RSC request fails', async () => {
    const root = tmpDir('text-prerender-rsc-fallback-failure-')
    const outDir = path.join(root, 'out')
    const appDir = path.join(root, 'app')
    const pagePath = path.join(appDir, 'page.tsx')
    fs.mkdirSync(appDir, { recursive: true })
    ...
    try {
      ...
      expect(route.error).toContain('[text] prerenderApp: RSC fallback returned 500 for /')
...
describe('prerenderPages — default mode (pages-basic)', () => {
  ...
  beforeAll(async () => {
    ...
    outDir = tmpDir('text-prerender-pages-')

    const { prerenderPages } = await import('../src/build/prerender.js')
  ...
  // Text.js accepts both `paths: Array<{ params }>` and `paths: Array<string>`
  // from getStaticPaths. The string-path variant is documented at
  // https://textjs.org/docs/pages/api-reference/functions/get-static-paths and
  // implemented in .textjs-ref/packages/text/src/build/static-paths/pages.ts
  ...
  // Text.js rejects entries with a missing `params` key — see
  //   .textjs-ref/packages/text/src/build/static-paths/pages.ts (around line 169)
  ...
  it('surfaces missing-params entries as a per-route error (does not crash)', () => {
    const errored = results.find(r => r.route === '/missing-params/:slug' && r.status === 'error')
  ...
  it('writes text-prerender.json with correct structure', () => {
    const indexPath = path.join(outDir, 'text-prerender.json')
...
describe('prerenderPages — export mode (pages-basic)', () => {
  ...
  beforeAll(async () => {
    const pagesBundlePath = await buildPagesFixture(PAGES_FIXTURE)
    outDir = tmpDir('text-prerender-pages-export-')
  ...
  it('includes stack trace in error when enablePrerenderSourceMaps is true', () => {
    // enablePrerenderSourceMaps defaults to true in resolveTextConfig (line 230)
...
describe('prerenderApp — default mode (app-basic)', () => {
  ...
  beforeAll(async () => {
    const built = await buildIsolatedAppFixture(APP_FIXTURE, 'text-prerender-app-fixture-')
    fixtureDir = built.fixtureDir
    const rscBundlePath = built.rscBundlePath
    outDir = tmpDir('text-prerender-app-')
  ...
  it('renders layout-only routes whose content comes from parallel slots', () => {
    // Ported from Text.js: test/e2e/app-dir/parallel-routes-and-interception/parallel-routes-and-interception.test.ts
...
describe('runPrerender — hybrid app+pages (app-basic)', () => {
  ...
  beforeAll(async () => {
    ...
    manifestDir = tmpDir('text-prerender-hybrid-')
...
describe("runPrerender — output: 'export' wiring", () => {
  ...
  beforeAll(async () => {
    ...
    // Pass the bundle path and a textConfigOverride to runPrerender so it
    // exercises output: 'export' without touching the real text.config.mjs.
    pagesBundlePath = await buildPagesFixture(PAGES_FIXTURE)
  }, 120_000)

  it("throws when text.config output: 'export' and SSR routes exist", async () => {
    const { runPrerender } = await import('../src/build/run-prerender.js')
    await expect(
      runPrerender({
        root: PAGES_FIXTURE,
        textConfigOverride: { output: 'export' },
        pagesBundlePath,
      }),
...
function mockRoute(pattern: string, opts: { pagePath?: string | null } = {}): AppRoute {
  ...
  return {
    ...
    params: parts.filter(p => p.startsWith(':')).map(p => p.replace(/^:/, '').replace(/[+*]$/, '')),
    patternParts: parts,
  }
}
...
describe('resolveParentParams', () => {
  ...
  it('resolves layout-level parent generateStaticParams without requiring a parent page', async () => {
    // Ported from Text.js: test/e2e/app-dir/app-root-params-getters/generate-static-params.test.ts
