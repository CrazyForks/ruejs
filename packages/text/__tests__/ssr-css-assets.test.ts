/**
 * Tests that text's SSR environment emits CSS (and other) assets, not
 ...
 *
 * text sets `environments.ssr.build.emitAssets = true` for both the
 ...
 *
 * Relates to Text.js deploy-suite fixtures that import CSS from server
 * components or layouts:
 *   test/e2e/app-dir/text-dynamic-css/
 ...
 * fixture can resolve Rue, text, and @vitejs/plugin-rsc.
 */
async function makeAppRouterCssFixture(): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-ssr-css-asset-'))
...
describe('SSR build emits CSS assets referenced by SSR chunks', () => {
  ...
  it('Pages Router SSR environment is configured with emitAssets: true', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-ssr-css-pages-'))
    try {
      await fs.symlink(ROOT_NODE_MODULES, path.join(tmpDir, 'node_modules'), 'junction')
  ...
  it('every CSS import in dist/server/ssr/*.js resolves to a real file on disk', async () => {
    const tmpDir = await makeAppRouterCssFixture()
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-ssr-css-out-'))
    try {
