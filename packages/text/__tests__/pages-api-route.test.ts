...
import { handlePagesApiRoute, type PagesApiRouteMatch } from '../src/server/pages-api-route.js'
...
describe('pages api route', () => {
  ...
  it('passes a TextRequest with textUrl.searchParams to edge API handlers', async () => {
    ...
    // Text.js wraps the request in a TextRequest before invoking the user's
    // edge API handler, so handlers can use `req.textUrl.searchParams`.
    const response = await handlePagesApiRoute({
      match: createMatch(
        (request: Request) => {
          const textUrl = (request as Request & { textUrl?: URL }).textUrl
          if (!textUrl) {
            return new Response('missing textUrl', { status: 500 })
          }
