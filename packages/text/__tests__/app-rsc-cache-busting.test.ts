...
describe('App Router RSC cache-busting', () => {
  ...
  it('hashes Text RSC variant headers into the request URL', async () => {
    const headers = createRscRequestHeaders({
    ...
    await expect(createRscRequestUrl('/photos/42', headers)).resolves.toBe(
      `/photos/42.rsc?${TEXT_RSC_CACHE_BUSTING_SEARCH_PARAM}=${hash}`,
    )
  })
  ...
  it('normalizes invalid render modes to normal navigation for cache-busting', async () => {
    const headers = createRscRequestHeaders()
    headers.set(TEXT_RSC_RENDER_MODE_HEADER, 'invalid')
  ...
  it('exports the full Vary value for RSC-bearing App Router responses', () => {
    expect(TEXT_RSC_VARY_HEADER).toBe(
      [
        RSC_HEADER,
        'Accept',
        TEXT_ROUTER_STATE_TREE_HEADER,
  ...
  it('applies the current compatibility ID to RSC response headers when available', () => {
    ...
    expect(headers.get(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe('compat-a')
  })

  it('uses the injected RSC compatibility ID by default', () => {
    const headers = new Headers()

    withEnvVar('__TEXT_RSC_COMPATIBILITY_ID', 'compat-env', () =>
      applyRscCompatibilityIdHeader(headers),
    )

    expect(headers.get(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe('compat-env')
  })

  it('removes a spoofed compatibility ID header when no framework ID is available', () => {
    const headers = new Headers({
      [TEXT_RSC_COMPATIBILITY_ID_HEADER]: 'spoofed-compat',
    })

    applyRscCompatibilityIdHeader(headers, '')

    expect(headers.has(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe(false)
  })
