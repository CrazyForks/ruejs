...
describe('useServerInsertedHTML', () => {
  ...
  it('callbacks can return Rue-like elements', async () => {
    const { useServerInsertedHTML, flushServerInsertedHTML } =
      await import('../src/shims/navigation.js')

    // Simulate a Rue element (like styled-components would return)
    const fakeElement = {
      $$typeof: Symbol.for(`${LEGACY_RUNTIME_PACKAGE}.element`),
      type: 'style',
      props: { children: '.x{}' },
    }
