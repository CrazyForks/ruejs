...
async function withTempApp<T>(run: (appDir: string) => Promise<T>): Promise<T> {
  const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'text-app-route-graph-'))
...
describe('App Router route graph builder', () => {
  ...
  it('materializes synthetic routes from nested parallel slot pages', async () => {
    await withTempApp(async appDir => {
      ...
      expect(graph.routes.map(route => route.pattern)).toEqual(['/dashboard', '/dashboard/members'])
  ...
  it('does not let locale collation affect RouteManifest graphVersion', async () => {
    const graphVersions = await withTempApp(async appDir => {
      ...
      return [normalGraph.routeManifest.graphVersion, reverseLocaleGraph.routeManifest.graphVersion]
    })
  ...
  it('accepts dynamic segment names with dots and at-signs (Text.js parity)', async () => {
    // Text.js PARAMETER_PATTERN accepts any non-] characters inside brackets.
    // See: https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/get-dynamic-param.ts
    // Note: colon (:) is tested via patternToTextFormat in route-sorting.test.ts
  ...
  // Intercepting route source-pattern computation. Mirrors Text.js'
  ...
  // against the Text-URL header.
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/interception-routes.ts
  describe('intercepting routes', () => {
    ...
    it('computes intercepting route across `(..)(..)` two-levels-up marker', async () => {
      ...
      // but adapted to use a parallel slot, which is the structure text
    ...
    it('ignores interception marker directories that live outside a parallel slot', async () => {
      // Ported from Text.js: test/e2e/app-dir/interception-routes-multiple-catchall
      // and test/e2e/app-dir/interception-segments-two-levels-above. Text.js
    ...
    it('ignores `(..)(..)` interception marker outside a parallel slot', async () => {
      // Ported from Text.js: test/e2e/app-dir/interception-segments-two-levels-above
