/**
 * Tests for text's `sassOptions` passthrough from text.config into
 * Vite's `css.preprocessorOptions.scss`.
 *
 * Text.js (sass-loader) destructures `prependData`, `additionalData`, and
 ...
 * Sass renamed `includePaths` → `loadPaths`; text accepts either and
 ...
 * https://github.com/vercel/next.js/blob/canary/packages/text/src/build/webpack/config/blocks/css/index.ts
 */
...
import { buildSassPreprocessorOptions } from '../src/plugins/sass.js'

// The text config hook mutates process.env.NODE_ENV as a side effect.
...
describe('buildSassPreprocessorOptions', () => {
  ...
  it('prefers prependData over additionalData (matches Text.js precedence)', () => {
    // Text.js: `additionalData: sassPrependData || sassAdditionalData`
  ...
  it('falls through to additionalData when prependData is empty (matches Text.js truthy-OR)', () => {
    // Text.js: `sassPrependData || sassAdditionalData` — falsy prependData
...
describe('text config hook threads sassOptions into css.preprocessorOptions', () => {
  async function runConfigHook(
    textConfigSrc: string,
    ...
    const mainPlugin = plugins.find(
      ...
      (p: any) => p.name === 'text:config' && typeof p.config === 'function',
    )
    expect(mainPlugin).toBeDefined()

    const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-sass-config-'))
    ...
    await fsp.writeFile(path.join(tmpDir, 'text.config.mjs'), textConfigSrc)
