...
/** Extract the text:image-imports plugin from the plugin array */
function getImagePlugin(): Plugin & { _dimCache: Map<string, { width: number; height: number }> } {
  const plugins = text() as Plugin[]
  const plugin = plugins.find(p => p.name === 'text:image-imports')
  if (!plugin) throw new Error('text:image-imports plugin not found')
  return plugin as any
}

// ── resolveId ─────────────────────────────────────────────────
describe('text:image-imports — resolveId', () => {
  it('resolves ?text-meta suffix to virtual module ID', () => {
    const plugin = getImagePlugin()
    const resolve = unwrapHook(plugin.resolveId)
    const result = resolve.call(plugin, '/abs/path/hero.jpg?text-meta', '/some/file.tsx')
    expect(result).toBe('\0text-image-meta:/abs/path/hero.jpg')
  })
  ...
  it('returns null for non-meta imports', () => {
    ...
    expect(resolve.call(plugin, 'rue', '/some/file.tsx')).toBeNull()
    expect(resolve.call(plugin, 'text/image', '/some/file.tsx')).toBeNull()
  })
})

// ── load ──────────────────────────────────────────────────────
describe('text:image-imports — load', () => {
  it('returns dimensions for a PNG file', async () => {
    const plugin = getImagePlugin()
    const load = plugin.load as Function
    const result = await load.call(plugin, `\0text-image-meta:${PNG_PATH}`)
  ...
  it('returns dimensions for a JPEG file', async () => {
    const plugin = getImagePlugin()
    const load = plugin.load as Function
    const result = await load.call(plugin, `\0text-image-meta:${JPG_PATH}`)
    const json = result.replace('export default ', '').replace(';', '')
    const dims = JSON.parse(json)
  ...
  it('returns 0x0 for non-existent file', async () => {
    const plugin = getImagePlugin()
    const load = plugin.load as Function
    const result = await load.call(plugin, '\0text-image-meta:/no/such/file.png')
    const json = result.replace('export default ', '').replace(';', '')
    const dims = JSON.parse(json)
  ...
  it('returns null for non-image-meta IDs', async () => {
    ...
    expect(await load.call(plugin, 'rue')).toBeNull()
  })
  ...
  it('caches dimensions on second call', async () => {
    ...
    const load = plugin.load as Function
    await load.call(plugin, `\0text-image-meta:${PNG_PATH}`)
...
// ── transform ─────────────────────────────────────────────────
describe('text:image-imports — transform', () => {
   ...
   * dimensions resolved through a sibling `?text-meta` import. We verify the
  ...
  function expectImageBinding(code: string, name: string, fileBasename: string) {
    ...
    expect(code).toContain(path.join(IMAGES_DIR, fileBasename) + '?text-meta')
  }
  ...
  it('preserves non-image imports alongside image imports', async () => {
    ...
    const result = await transform.call(plugin, code, fakeId)
    expect(result).not.toBeNull()
    // Non-image imports should still be there
