...
describe('App Router intercepting routes in production builds', () => {
  it('builds when an inherited modal slot intercepts the same target route as a standalone page', async () => {
    // Ported from Text.js route interception behavior:
    ...
    await withTempDir('text-intercept-build-', async root => {
      fs.symlinkSync(
      ...
      writeFixtureFile(
        root,
        'package.json',
        JSON.stringify({ name: 'text-intercept-build', private: true, type: 'module' }, null, 2),
      )
      writeFixtureFile(
        root,
        'tsconfig.json',
