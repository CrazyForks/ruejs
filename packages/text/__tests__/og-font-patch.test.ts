...
function createOgFontPatchPlugin(): Plugin {
  ...
  const plugin = plugins.find(p => p.name === 'text:og-font-patch')
  if (!plugin) throw new Error('text:og-font-patch plugin not found')
  return plugin
}
...
describe('text:og-font-patch plugin', () => {
  it('exists in the plugin array', () => {
    const plugin = createOgFontPatchPlugin()
    expect(plugin.name).toBe('text:og-font-patch')
