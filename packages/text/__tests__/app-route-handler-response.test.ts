...
import type { CachedRouteValue } from '../src/shims/cache.js'
import {
...
} from '../src/server/app-route-handler-response.js'
...
describe('app route handler response helpers', () => {
  ...
  it('appends additive middleware headers for Set-Cookie and Vary', async () => {
    ...
    expect(result.headers.get('vary')).toBe('RSC, Accept, Text-Router-State-Tree')
  ...
  it('builds cached HIT and STALE route handler responses', async () => {
    ...
    expect(hit.headers.get('x-text-cache')).toBe('HIT')
    expect(hit.headers.get('x-textjs-cache')).toBe('HIT')
    ...
    expect(staleHead.headers.get('x-text-cache')).toBe('STALE')
    expect(staleHead.headers.get('x-textjs-cache')).toBe('STALE')
  ...
  it('uses mutable cookies as fallbacks and keeps returned response cookies final', async () => {
    // Matches Text.js appendMutableCookies:
    // packages/text/src/server/web/spec-extension/adapters/request-cookies.ts
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/server/web/spec-extension/adapters/request-cookies.ts
    const response = new Response('body', {
      headers: [
  ...
  it('only rejects the active x-middleware-text control signal', () => {
    expect(() =>
      assertSupportedAppRouteHandlerResponse(
        new Response(null, {
          headers: { 'x-middleware-text': '0' },
        }),
      ),
    ).not.toThrow()
    expect(() =>
      assertSupportedAppRouteHandlerResponse(
        new Response(null, {
          headers: { 'x-middleware-text': 'true' },
        }),
      ),
    ).not.toThrow()
    expect(() =>
    ...
    ).toThrow('TextResponse.text() was used in a app route handler')
  })
  ...
  it('emits a no-store Cache-Control for revalidate = 0 route handlers', () => {
    ...
    // The exact string matches Text.js's own cache-control helper:
    // .textjs-ref/packages/text/src/server/lib/cache-control.ts:29.
