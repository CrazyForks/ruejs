...
} from '../src/server/app-rsc-route-matching.js'
...
describe('App RSC route matching', () => {
  ...
  it('omits optional catch-all params when zero segments are matched', () => {
    // Text.js represents a missing optional catch-all param as absent at the
    // route-match boundary; app rendering later treats that as the `null`
    // tree segment for optional catch-all.
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/route-matcher.ts
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/get-dynamic-param.test.ts
  ...
  // Ported from Text.js: route-matcher.ts decodeURIComponent behaviour
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/route-matcher.ts#L25-L27
  it('decodes matched params via decodeURIComponent (mirrors Text.js)', () => {
  ...
  it('canonicalizes encoded URL parts before matching app routes', () => {
    // Text.js canonicalizes URL pathname parts before deriving dynamic segment
    // cache keys; text should apply the same decode-first discipline at the
    // App RSC route-match boundary so encoded static segments and params use
    // one normalized representation.
    // https://github.com/vercel/next.js/blob/47bcfa0956679c2a5fea0b941b76bb2d69878d9c/packages/text/src/client/route-params.ts
    const matcher = createAppRscRouteMatcher([
  ...
  // Ported from Text.js: lib/generate-interception-routes-rewrites.test.ts
  // https://github.com/vercel/next.js/blob/canary/packages/text/src/lib/generate-interception-routes-rewrites.test.ts
  //
  // Interception is implemented in Text.js as a rewrite that only fires when
  // the Text-URL header matches the *intercepting route* regex (the path that
  // owns the slot, with route groups and @slot segments stripped).
  //
  // text must enforce the same constraint at the matcher boundary: an
  ...
  // rendered.
  describe('source-pathname filtering (mirrors Text.js Text-URL header rewrite)', () => {
    ...
    it('returns null when no source pathname is provided (no Text-URL header)', () => {
      // Without a Text-URL header the rewrite cannot fire in Text.js, so the
      // direct page must render. text should mirror that.
      const matcher = createAppRscRouteMatcher([
        route('/templates/:catchAll+', ['templates', ':catchAll+'], {
          modal: {
            intercepts: [
    ...
    it('treats a sourceMatchPattern of `/` as matching any source', () => {
      ...
      // which Text.js implements as `^/.*$` — i.e. any source.
      const matcher = createAppRscRouteMatcher([
        route('/', [], {
          modal: {
            intercepts: [
      ...
      // But still must require a source pathname (Text-URL header) to fire.
...
type TestIntercept = {
   ...
   * with route groups and `@slot` segments stripped). Mirrors Text.js'
   * `interceptingRoute` from `extractInterceptionRouteInformation` and is
   * used to gate `findIntercept` against the Text-URL header.
   */
