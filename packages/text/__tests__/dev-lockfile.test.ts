 ...
 *
 * Behavior modeled on Text.js' dev lock file:
...
import {
  ...
  readLockfile,
  tryAcquireLockfile,
} from '../src/server/dev-lockfile.js'

function makeTempRoot(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'text-lockfile-'))
}
...
describe('getLockfilePath', () => {
  it('places the lock file under .text/dev/', () => {
    const root = '/projects/my-app'
    expect(getLockfilePath(root)).toBe(path.join('/projects/my-app', '.text', 'dev', 'lock.json'))
  })
})
...
describe('formatAlreadyRunningError', () => {
  ...
  it('falls back to a generic message when the lock file is corrupt', () => {
    ...
    expect(msg).toContain('.text/dev/lock.json')
  })
})
