...
} from '../src/server/metadata-route-build-data.js'
import type { MetadataFileRoute } from '../src/server/metadata-routes.js'

const imagePath = path.resolve('__tests__/fixtures/images/test-4x3.png')
...
describe('metadata route build data', () => {
  it('embeds static image route data with content hash, dimensions, alt text, and base64', () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-metadata-route-'))
  ...
  it('fails with a path-specific error when a metadata file cannot be read', () => {
    const missingPath = path.join(os.tmpdir(), 'text-missing-metadata-file.png')
    ...
    expect(() =>
      ...
      `[text] Failed to read metadata route file ${missingPath} for /missing/opengraph-image.png`,
    )
  })
  ...
  it('fails with a path-specific error when image dimensions cannot be read', () => {
    ...
    expect(() =>
      ...
      `[text] Failed to read metadata image dimensions for ${corruptImagePath} (/corrupt/opengraph-image.png)`,
    )
  })
  ...
  it('fails when a dynamic metadata route has not been registered for import', () => {
    ...
    expect(() => createMetadataRouteEntriesSource([route], new Map())).toThrow(
      `[text] Missing generated module import for dynamic metadata route ${imagePath}`,
    )
  })
})
