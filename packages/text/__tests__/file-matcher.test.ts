...
import { createValidFileMatcher, normalizePageExtensions } from '../src/routing/file-matcher.js'
import { shouldInvalidateAppRouteFile } from '../src/server/dev-route-files.js'
...
describe('file matcher', () => {
  ...
  it('matches app router page and route files by configured extensions', () => {
    // Ported from Text.js matcher tests:
  ...
  it('classifies only app route structure files as dev route invalidations', () => {
    // Mirrors Text.js dev route discovery:
    // packages/text/src/server/lib/find-page-file.ts
    // packages/text/src/server/lib/router-utils/setup-dev-bundler.ts
    const matcher = createValidFileMatcher(['tsx', 'ts', 'jsx', 'js', 'mdx'])
    ...
    expect(shouldInvalidateAppRouteFile(appDir, '/project/app/blog/layout.tsx', matcher)).toBe(true)
    ...
    expect(shouldInvalidateAppRouteFile(appDir, '/project/app-utils/page.tsx', matcher)).toBe(false)
    expect(shouldInvalidateAppRouteFile(appDir, '/project/app/blog/robots.ts', matcher)).toBe(false)
  })
})
