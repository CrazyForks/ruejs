...
import { invalidateRouteCache } from '../src/routing/pages-router.js'

const FIXTURES_PAGES = path.resolve('__tests__/fixtures/pages-basic/pages')
const FIXTURES_APP = path.resolve('__tests__/fixtures/app-basic/app')

// ─── hasNamedExport ───────────────────────────────────────────────────────────

describe('hasNamedExport', () => {
  it('detects async function declaration', () => {
    expect(hasNamedExport('export async function getStaticProps() {}', 'getStaticProps')).toBe(true)
  })
  ...
  it('does not match partial names (false positive guard)', () => {
    ...
    expect(hasNamedExport('export function getStaticPropsExtra() {}', 'getStaticProps')).toBe(false)
  })
...
`
  ...
  it('handles inline comment after value (fixture file style)', () => {
    // From __tests__/fixtures/pages-basic/pages/isr-test.tsx:
...
describe('printBuildReport respects pageExtensions', () => {
  ...
  it('app router: only reports routes matching configured pageExtensions', async () => {
    // Ported from Text.js MDX e2e pageExtensions behaviour:
    // test/e2e/app-dir/mdx/text.config.ts
    // https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/mdx/text.config.ts
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'text-report-app-'))
  ...
  it('app router: reports mdx routes when pageExtensions includes mdx', async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'text-report-app-mdx-'))
    const appDir = path.join(tmpRoot, 'app')
    await fs.mkdir(path.join(appDir, 'about'), { recursive: true })
    await fs.writeFile(
      path.join(appDir, 'layout.tsx'),
  ...
  it('pages router: only reports routes matching configured pageExtensions', async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'text-report-pages-'))
  ...
  it('pages router: reports mdx routes when pageExtensions includes mdx', async () => {
    tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'text-report-pages-mdx-'))
    const pagesDir = path.join(tmpRoot, 'pages')
    await fs.mkdir(pagesDir, { recursive: true })
    await fs.writeFile(
      path.join(pagesDir, 'index.tsx'),
