...
} from '../src/server/isr-cache.js'
import {
  APP_RSC_RENDER_MODE_PREFETCH_LOADING_SHELL,
  APP_RSC_RENDER_MODE_REFRESH_PRESERVE_UI,
} from '../src/server/app-rsc-render-mode.js'
import { buildPageCacheTags } from '../src/server/implicit-tags.js'
import { runWithExecutionContext } from '../src/shims/request-context.js'
import {
...
describe('App Router ISR cache key primitives', () => {
  ...
  it('includes the build id when present', () => {
    process.env.__TEXT_BUILD_ID = 'build-42'
...
describe('triggerBackgroundRegeneration', () => {
  ...
  it('reports error via onRequestError handler when errorContext is provided', async () => {
    const handler = vi.fn()
    globalThis.__TEXT_onRequestErrorHandler__ = handler
    const consoleError = vi.spyOn(console, 'error').mockImplementation(() => {})
