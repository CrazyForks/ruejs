...
/**
 * Install a fetch stub that returns the given body for any
...
function mockGoogleFontsCSS(
  ...
  return mockGoogleFontsResponse(
    () =>
      new Response(body, {
        status: opts.status,
        headers: opts.headers ?? { 'content-type': 'text/css' },
      }),
  )
}

function mockGoogleFontsResponse(handler: (url: string) => Response | Promise<Response>): {
  urls: string[]
  count: () => number
} {
  const urls: string[] = []
...
function mockInterGoogleFontFiles() {
  globalThis.fetch = async (input, init) => {
    const url = typeof input === 'string' ? input : input instanceof URL ? input.href : input.url
    if (url.startsWith('https://fonts.googleapis.com/')) {
      return new Response(
        [
          ...
          '  font-style: normal;',
          '  font-weight: 400 700;',
          "  src: url(https://fonts.gstatic.com/s/inter/v19/fake-inter.woff2) format('woff2');",
          '}',
        ].join('\n'),
...
/** Extract the text:google-fonts plugin from the plugin array */
function getGoogleFontsPlugin(): Plugin {
  const plugins = text() as Plugin[]
  const plugin = plugins.find(p => p.name === 'text:google-fonts')
  if (!plugin) throw new Error('text:google-fonts plugin not found')
  return plugin
}
...
function initPlugin(plugin: Plugin, opts: { command?: 'build' | 'serve'; root?: string }) {
  if (opts.root && path.basename(opts.root).startsWith('.test-font-root')) {
    fs.rmSync(opts.root, { recursive: true, force: true })
  }
  const fakeConfig = {
...
describe('text/font/google shim', () => {
  ...
  it('matches Text.js style exports for single weight and style', async () => {
  ...
  it('uses transform-resolved Google style metadata for italic-only fonts', async () => {
    // Ported from Text.js: packages/font/src/google/loader.ts
    ...
    const result = Molle({
      weight: '400',
      subsets: ['latin'],
      _text: { font: { fontWeight: 400, fontStyle: 'italic' } },
    } as any)
  ...
  it('uses adjusted Google font fallback metrics by default', async () => {
    // Ported from Text.js: packages/font/src/google/loader.ts and
    // packages/text/src/build/webpack/loaders/text-font-loader/postcss-text-font.ts
    // https://github.com/vercel/next.js/blob/canary/packages/font/src/google/loader.ts
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/build/webpack/loaders/text-font-loader/postcss-text-font.ts
    const { createFontLoader, getSSRFontStyles } = await import('../src/shims/font-google.js')
  ...
  size-adjust: 90.00%;
    ...
    const result = Inter({
      weight: '400',
      subsets: ['latin'],
      _text: { font: { adjustedFallbackCSS: fallbackCSS } },
    } as any)
  ...
  it('accepts internal self-hosted CSS for self-hosted mode', async () => {
    ...
    const result = Inter({ weight: ['400'], _text: { font: { selfHostedCSS: fakeCSS } } } as any)
    expect(result.className).toBeDefined()
    expect(result.style.fontFamily).toContain('Inter')
  })

  it('exports buildGoogleFontsUrl', async () => {
    const { buildGoogleFontsUrl } = await import('../src/shims/font-google.js')
  ...
  it('does not emit Text-incompatible :root font variable rules', async () => {
    const { createFontLoader, getSSRFontStyles } = await import('../src/shims/font-google.js')
    const beforeStyles = getSSRFontStyles()
  ...
  it('keeps utility and explicit font exports in sync with the shim barrel', async () => {
    const mod = await import('../src/shims/font-google.js')
    ...
    expect(typeof mod.Geist).toBe('function')
    expect(typeof mod.Geist_Mono).toBe('function')
    expect(mod.Geist({ subsets: ['latin'] }).style.fontFamily).toContain('Geist')
    expect(mod.Geist_Mono({ subsets: ['latin'] }).style.fontFamily).toContain('Geist Mono')
  })
...
describe('text:google-fonts plugin', () => {
  it('exists in the plugin array', () => {
    const plugin = getGoogleFontsPlugin()
    expect(plugin.name).toBe('text:google-fonts')
  ...
  it('rewrites named font imports in dev mode', async () => {
    ...
    // Playwright spec under __tests__/e2e/font-google/ — so this unit test
    ...
    const code = `import { Inter } from 'text/font/google';`
    const result = await transform.call(plugin, code, '/app/layout.tsx')
    expect(result).not.toBeNull()
    expect(result.code).toContain('virtual:text-google-fonts?')
    expect(result.code).not.toContain('selfHostedCSS')
  })

  it('returns null for files without text/font/google imports', async () => {
    const plugin = getGoogleFontsPlugin()
  ...
  it('rewrites dependency files that import text/font/google', async () => {
    const plugin = getGoogleFontsPlugin()
    initPlugin(plugin, { command: 'serve' })
    const transform = unwrapHook(plugin.transform)
    const code = `import { Inter } from 'text/font/google';`
  ...
  it('rewrites the named import even when a preceding line lacks a semicolon', async () => {
    ...
    const code = [
      ...
      `import { Inter } from 'text/font/google'`,
      `export { Inter }`,
    ].join('\n')
    const result = await transform.call(plugin, code, '/app/layout.tsx')
    expect(result).not.toBeNull()
    ...
    expect(result.code).toContain(`import type { Metadata } from 'text'`)
  })

  it('rewrites multi-line bracket imports (Prettier wraps past printWidth)', async () => {
    // Repro: `import { A, B, C, D } from 'text/font/google'` exceeds the
  ...
  it('rewrites multi-line bracket re-exports', async () => {
    // Same wrap logic applies to `export { ... } from 'text/font/google'`
    // when the specifier list crosses printWidth. Less common in user code
    // than the import case but kept for parity.
    const plugin = getGoogleFontsPlugin()
    initPlugin(plugin, { command: 'build' })
    const transform = unwrapHook(plugin.transform)
    const code = [`export {`, `  Inter,`, `  Roboto,`, `} from 'text/font/google'`].join('\n')
  ...
  it('rewrites namespace imports to the default proxy', async () => {
    ...
    const code = `import * as fonts from 'text/font/google';\nexport const Inter = fonts.Inter;`
    const result = await transform.call(plugin, code, '/app/layout.tsx')
    expect(result).not.toBeNull()
    expect(result.code).toContain('__text_google_fonts_proxy_0')
    expect(result.code).toContain('var fonts = __text_google_fonts_proxy_0;')
  })
  ...
  it('rewrites named re-exports through a virtual module', async () => {
    ...
    initPlugin(plugin, { command: 'serve' })
    const transform = unwrapHook(plugin.transform)
    const code = `export { Inter, buildGoogleFontsUrl } from 'text/font/google';`
    const result = await transform.call(plugin, code, '/app/fonts.ts')
    expect(result).not.toBeNull()
    expect(result.code).toContain('virtual:text-google-fonts?')
  })
  ...
  it('transforms font call to include the internal font payload during build', async () => {
    ...
    expect(result).not.toBeNull()
    expect(result.code).toContain('virtual:text-google-fonts?')
    expect(result.code).toContain('_text')
    ...
    const cacheDir = path.join(root, '.text', 'fonts')
  ...
  it('injects validated single-face metadata for italic-only Google fonts', async () => {
    ...
    try {
      const transform = unwrapHook(plugin.transform)
      const code = [
        `import { Molle } from 'text/font/google';`,
  ...
  it('uses cached fonts on second call', async () => {
    ...
    try {
      ...
      expect(result1.code).toContain('virtual:text-google-fonts?')
  ...
  it('handles multiple font imports in one file', async () => {
    ...
    try {
      const transform = unwrapHook(plugin.transform)
      const code = [
        `import { Inter, Roboto } from 'text/font/google';`,
        `const inter = Inter({ weight: '400', subsets: ['latin'] });`,
  ...
  it('self-hosts namespace member calls with nested-brace options', async () => {
    ...
    try {
      ...
      const code = [
        `import fonts from 'text/font/google';`,
  ...
  it('self-hosts aliased lowercase font imports during build', async () => {
    ...
    try {
      const transform = unwrapHook(plugin.transform)
      const code = [
        `import { Inter as inter } from 'text/font/google';`,
  ...
  it('resolves narrow-axis variable fonts to their real wght range (regression for #885)', async () => {
    // Sen's wght axis is 400..800. Pre-port text built
    ...
    try {
      const transform = unwrapHook(plugin.transform)
      const code = [
        `import { Sen } from 'text/font/google';`,
  ...
  it('self-hosts variable font axes through the plugin pipeline', async () => {
    ...
    try {
      const transform = unwrapHook(plugin.transform)
      const code = [
        `import { Roboto_Flex } from 'text/font/google';`,
  ...
  it('throws a build error on unknown font families (regression for #885)', async () => {
    // Pre-port text built a URL for any property name on the proxy and
    ...
    const transform = unwrapHook(plugin.transform)
    const code = [
      `import { NotARealFont } from 'text/font/google';`,
  ...
  it('throws a build error when a static font is called without an explicit weight (regression for #885)', async () => {
    ...
    // without weight should error at build time. Pre-port text silently
    // emitted `:wght@100..900` which Google rejected with HTTP 400.
    const plugin = getGoogleFontsPlugin()
    initPlugin(plugin, { command: 'build', root: import.meta.dirname })
    const transform = unwrapHook(plugin.transform)
    const code = [
      `import { Anton } from 'text/font/google';`,
...
describe('_rewriteCachedFontCssToServedUrls', () => {
  // Regression for a bug where self-hosted text/font/google built by text
  // emitted absolute dev-machine filesystem paths into every preload path
  // — the <style data-text-fonts> @font-face src: url(), the HTML body's
  ...
  // 404s (`<origin>/home/user/project/.text/fonts/...`) on every request.
  //
  // The fix replaces the cache-dir prefix with the served URL namespace
  // `/_text/static/_text_fonts` before the CSS string is handed off to
  // the bundle. The plugin's writeBundle hook then copies the cached font
  // files into the matching `dist/client/_text/static/_text_fonts/`
  ...
  it('rewrites absolute cache-dir paths in url() references to served URLs', () => {
    const cacheDir = '/home/user/project/.text/fonts'
    const css = [
      '@font-face {',
      "  font-family: 'Geist';",
      "  src: url(/home/user/project/.text/fonts/geist-4db05770f54f/geist-8e42e564.woff2) format('woff2');",
      '}',
      '@font-face {',
      "  font-family: 'Geist';",
      "  src: url(/home/user/project/.text/fonts/geist-4db05770f54f/geist-bd9fc9d8.woff2) format('woff2');",
      '}',
    ].join('\n')

    const out = rewriteCachedFontCssToServedUrls(css, cacheDir)

    expect(out).toContain('url(/_text/static/_text_fonts/geist-4db05770f54f/geist-8e42e564.woff2)')
    expect(out).toContain('url(/_text/static/_text_fonts/geist-4db05770f54f/geist-bd9fc9d8.woff2)')
    // The dev-machine filesystem prefix must not leak into the rewritten CSS.
    expect(out).not.toContain('/home/user/project/.text/fonts')
  ...
  it('rewrites every occurrence when the same path appears multiple times', () => {
    ...
    const cacheDir = '/root/.text/fonts'
    const css = [
      'src: url(/root/.text/fonts/geist/a.woff2);',
      'src: url(/root/.text/fonts/geist/b.woff2);',
      'src: url(/root/.text/fonts/geist/a.woff2);',
    ].join('\n')

    const out = rewriteCachedFontCssToServedUrls(css, cacheDir)

    expect(out).not.toContain('/root/.text/fonts')
    const aCount = (out.match(/\/_text\/static\/_text_fonts\/geist\/a\.woff2/g) ?? []).length
    const bCount = (out.match(/\/_text\/static\/_text_fonts\/geist\/b\.woff2/g) ?? []).length
  ...
  it('handles cache directories containing regex metacharacters', () => {
    ...
    const cacheDir = '/tmp/build (1)/.text/fonts'
    const css = "src: url(/tmp/build (1)/.text/fonts/inter-xyz/inter-abc.woff2) format('woff2');"

    const out = rewriteCachedFontCssToServedUrls(css, cacheDir)

    expect(out).toBe(
      "src: url(/_text/static/_text_fonts/inter-xyz/inter-abc.woff2) format('woff2');",
    )
  })
  ...
  it('is a no-op when cacheDir is empty', () => {
    ...
    const css = 'src: url(/home/user/project/.text/fonts/geist/a.woff2);'
  ...
  it('uses a custom assetsDir when passed through from plugin state', () => {
    ...
    // the embedded CSS point at `/_text/static/_text_fonts/...` while
    // the physical files landed in `<outDir>/static/_text_fonts/...`,
    ...
    const cacheDir = '/home/user/project/.text/fonts'
    const css =
      "src: url(/home/user/project/.text/fonts/geist-abc/geist-def.woff2) format('woff2');"

    const out = rewriteCachedFontCssToServedUrls(css, cacheDir, 'static')

    expect(out).toBe("src: url(/static/_text_fonts/geist-abc/geist-def.woff2) format('woff2');")
    expect(out).not.toContain('/_text/static/_text_fonts/')
  })
  ...
  it('falls back to the default assetsDir when an empty string is passed', () => {
    ...
    // default `_text/static` prefix so the URL always has a real
    // directory segment between the root and the `_text_fonts`
    // namespace.
    const cacheDir = '/root/.text/fonts'
    const css = 'src: url(/root/.text/fonts/geist/a.woff2);'

    const out = rewriteCachedFontCssToServedUrls(css, cacheDir, '')

    expect(out).toBe('src: url(/_text/static/_text_fonts/geist/a.woff2);')
    expect(out).not.toContain('//_text_fonts')
  })
})
...
describe('_findCallEnd', () => {
  ...
  it("returns null when text non-whitespace is not ')'", () => {
    const code = `foo({}, extra)`
    // objEnd is 6; text non-whitespace is ',' not ')'
