 ...
 * then extract the generated __TEXT_CLASS dispatch function from the emitted
...
const FIXTURE_PREFIX = 'text-class-integration-'
...
/**
 * Extracts the __TEXT_CLASS function body from the RSC chunk source and
...
function extractDispatch(chunkSource: string): Dispatch {
  const stubRe = /function\s+__TEXT_CLASS\s*\(routeIdx\)\s*\{\s*return null;?\s*\}/
  if (stubRe.test(chunkSource)) {
    throw new Error('__TEXT_CLASS was not patched — still returns null unconditionally')
  }
  ...
  const re =
    /function\s+__TEXT_CLASS\s*\(routeIdx\)\s*\{\s*return\s+(\([\s\S]*?\))\(routeIdx\);\s*\}/
  const match = re.exec(chunkSource)
  if (!match) {
    throw new Error('Could not locate patched __TEXT_CLASS in chunk source')
  }
  ...
  if (typeof raw !== 'function') {
    throw new Error('Patched __TEXT_CLASS body did not evaluate to a function')
  }
 ...
 * by matching `__TEXT_CLASS(N)` call expressions alongside each pattern.
...
function extractRouteIndexByPattern(chunkSource: string): Map<string, number> {
  ...
  const re = /__buildTimeClassifications:\s*__TEXT_CLASS\((\d+)\)[\s\S]*?pattern:\s*"([^"]+)"/g
  let match: RegExpExecArray | null
  while ((match = re.exec(chunkSource)) !== null) {
    result.set(match[2]!, Number(match[1]!))
  }
  if (result.size === 0) {
    throw new Error('No route entries with __TEXT_CLASS + pattern found in chunk source')
  }
  return result
}
...
async function buildMinimalFixture({
  ...
  // "/dyn" — nested layout that uses text/headers, should remain unclassified
...
export default function ForceStaticLayout({ children }) {
  ...
  // Symlink workspace node_modules so text and Vite dependencies resolve.
  ...
  // The plugin reads `TEXT_DEBUG_CLASSIFICATION` directly from `process.env`
  ...
  const envKey = 'TEXT_DEBUG_CLASSIFICATION'
...
describe('build-time classification integration', () => {
  ...
  it('gates the reasons sidecar behind __classDebug in the route table', () => {
    expect(built.chunkSource).toMatch(
      /__buildTimeReasons:\s*__classDebug\s*\?\s*__TEXT_CLASS_REASONS\(\d+\)\s*:\s*null/,
    )
  })

  it('leaves __TEXT_CLASS_REASONS as a null stub when build-time debug is off', () => {
    expect(built.chunkSource).toMatch(
      /function\s+__TEXT_CLASS_REASONS\s*\(routeIdx\)\s*\{\s*return null;?\s*\}/,
    )
    expect(built.chunkSource).not.toMatch(
      /function\s+__TEXT_CLASS_REASONS\s*\(routeIdx\)\s*\{[^}]*switch/,
    )
  })
  ...
  it('omits layouts that import text/headers from the build-time map', () => {
    const routeIdx = built.routeIndexByPattern.get('/dyn')
    expect(routeIdx).toBeDefined()
    const map = built.dispatch(routeIdx!)
    // The nested layout at index 1 pulls in text/headers, so Layer 2 returns
...
/**
 * Extracts and evaluates `__TEXT_CLASS_REASONS` from a build output that was
 * produced with `TEXT_DEBUG_CLASSIFICATION=1`. Mirrors `extractDispatch` but
...
function extractReasonsDispatch(
  ...
  const stubRe = /function\s+__TEXT_CLASS_REASONS\s*\(routeIdx\)\s*\{\s*return null;?\s*\}/
  if (stubRe.test(chunkSource)) {
    throw new Error('__TEXT_CLASS_REASONS was not patched despite TEXT_DEBUG_CLASSIFICATION=1')
  }
  const re =
    /function\s+__TEXT_CLASS_REASONS\s*\(routeIdx\)\s*\{\s*return\s+(\([\s\S]*?\))\(routeIdx\);\s*\}/
  const match = re.exec(chunkSource)
  if (!match) {
    throw new Error('Could not locate patched __TEXT_CLASS_REASONS in chunk source')
  }
  const raw: unknown = vm.runInThisContext(match[1]!)
  if (typeof raw !== 'function') {
    throw new Error('Patched __TEXT_CLASS_REASONS body did not evaluate to a function')
  }
  return (routeIdx: number) => {
    const result: unknown = Reflect.apply(raw, null, [routeIdx])
    if (result === null) return null
...
describe('build-time classification integration (debug on)', () => {
  ...
  it('patches __TEXT_CLASS_REASONS with a populated dispatcher', () => {
    expect(built.chunkSource).toMatch(
      /function\s+__TEXT_CLASS_REASONS\s*\(routeIdx\)\s*\{[^}]*switch/,
    )
    expect(built.chunkSource).not.toMatch(
