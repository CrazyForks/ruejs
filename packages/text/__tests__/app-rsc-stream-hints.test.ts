...
import { normalizeRscPreloadHints } from '../src/server/rsc-stream-hints.js'
...
describe('RSC stream hint helpers', () => {
  it('rewrites RSC stylesheet preload hints', async () => {
    const stream = normalizeRscPreloadHints(
      streamFromChunks([
  ...
  it('only rewrites stylesheet preload hints in mixed RSC content', async () => {
    const stream = normalizeRscPreloadHints(
      streamFromChunks([
        '0:D{"name":"page"}\n' +
  ...
  it('buffers partial RSC lines across chunks before rewriting hints', async () => {
    const stream = normalizeRscPreloadHints(
  ...
  it('rewrites a final unterminated RSC line during flush', async () => {
    const stream = normalizeRscPreloadHints(
