...
async function setupClientDir(): Promise<string> {
  const dir = path.join(
    os.tmpdir(),
    `text-cache-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  )
...
describe('StaticFileCache', () => {
  ...
  it('creates a cache by scanning the client directory', async () => {
    await writeFile(clientDir, '_text/static/app-abc123.js', 'const x = 1;')
  ...
  it('handles empty client directory', async () => {
    ...
    expect(cache.lookup('/_text/static/nope.js')).toBeUndefined()
  })
  ...
  it('returns cached metadata for an existing file', async () => {
    await writeFile(clientDir, '_text/static/index-abc123.js', 'const x = 1;')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/index-abc123.js')
    ...
    expect(entry!.original.path).toBe(path.join(clientDir, '_text/static/index-abc123.js'))
  })

  it('returns undefined for non-existent files', async () => {
    await writeFile(clientDir, '_text/static/real-abc123.js', 'x')

    const cache = await StaticFileCache.create(clientDir)

    expect(cache.lookup('/_text/static/missing-xyz789.js')).toBeUndefined()
  })

  it('sets immutable cache-control for hashed assets under /assets/', async () => {
    await writeFile(clientDir, '_text/static/bundle-abc123.js', 'x'.repeat(100))

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/bundle-abc123.js')
  ...
  it('generates weak etag from filename hash for hashed assets', async () => {
    await writeFile(clientDir, '_text/static/app-abc123.css', '.body { margin: 0; }')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/app-abc123.css')
  ...
  it('falls back to mtime etag for assets without hash suffix', async () => {
    await writeFile(clientDir, '_text/static/logo.svg', '<svg></svg>')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/logo.svg')
  ...
  it('does not treat non-hash suffixes as hashed asset etags', async () => {
    await writeFile(clientDir, '_text/static/my-library-v2.0.0.js', 'export {};')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/my-library-v2.0.0.js')
  ...
  it('detects brotli precompressed variant', async () => {
    ...
    await writeFile(clientDir, '_text/static/app-abc123.js', content)
    ...
    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/app-abc123.js')

    expect(entry!.br?.path).toBe(path.join(clientDir, '_text/static/app-abc123.js.br'))
  ...
  it('detects gzip precompressed variant', async () => {
    ...
    await writeFile(clientDir, '_text/static/styles-def456.css', content)
    const gzContent = zlib.gzipSync(Buffer.from(content))
    await writeFile(clientDir, '_text/static/styles-def456.css.gz', gzContent)

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/styles-def456.css')

    expect(entry!.gz?.path).toBe(path.join(clientDir, '_text/static/styles-def456.css.gz'))
  ...
  it('detects zstandard precompressed variant', async () => {
    ...
    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/app-zstd.js')

    expect(entry!.zst?.path).toBe(path.join(clientDir, '_text/static/app-zstd.js.zst'))
  ...
  it('does not expose .br/.gz/.zst files as standalone entries', async () => {
    ...
    expect(cache.lookup('/_text/static/app-abc123.js.br')).toBeUndefined()
    expect(cache.lookup('/_text/static/app-abc123.js.gz')).toBeUndefined()
    expect(cache.lookup('/_text/static/app-abc123.js.zst')).toBeUndefined()
  })
  ...
  it('detects content types from file extensions', async () => {
    await writeFile(clientDir, '_text/static/style-aaa.css', 'body{}')
    await writeFile(clientDir, '_text/static/data-bbb.json', '{}')
    await writeFile(clientDir, 'logo.svg', '<svg/>')
    await writeFile(clientDir, 'photo.webp', 'webp-data')

    const cache = await StaticFileCache.create(clientDir)

    expect(cache.lookup('/_text/static/style-aaa.css')!.original.headers['Content-Type']).toBe(
      'text/css',
    )
    expect(cache.lookup('/_text/static/data-bbb.json')!.original.headers['Content-Type']).toBe(
      'application/json',
    )
  ...
  it('falls back to application/octet-stream for unknown extensions', async () => {
    await writeFile(clientDir, '_text/static/data-ccc.xyz', 'unknown-data')

    const cache = await StaticFileCache.create(clientDir)

    expect(cache.lookup('/_text/static/data-ccc.xyz')!.original.headers['Content-Type']).toBe(
  ...
  it('scans nested directories recursively', async () => {
    await writeFile(clientDir, '_text/static/chunks/vendor-aaa.js', 'vendor code')
    await writeFile(clientDir, '_text/static/chunks/lazy/page-bbb.js', 'page code')

    const cache = await StaticFileCache.create(clientDir)

    expect(cache.lookup('/_text/static/chunks/vendor-aaa.js')).toBeDefined()
    expect(cache.lookup('/_text/static/chunks/lazy/page-bbb.js')).toBeDefined()
  })
})
