...
// fixtures mirror the behaviour Text.js's edge adapter applies after a
// middleware returns a redirect Response:
//   - Same-host Location headers are made relative.
//   - When the original request carries `x-textjs-data: 1`, the redirect is
//     translated into a 200 response with `x-textjs-redirect`.
// Reference: packages/text/src/server/web/adapter.ts (canary)
...
describe('middleware redirect protocol', () => {
  ...
  it('translates same-host redirects to x-textjs-redirect for data requests', async () => {
    const module = {
    ...
    // The protocol: 200 response, no Location, x-textjs-redirect header set.
    expect(result.continue).toBe(false)
    expect(result.response).toBeDefined()
    expect(result.response?.status).toBe(200)
    expect(result.response?.headers.get('x-textjs-redirect')).toBe('/new-home')
  ...
  it('translates external redirects to x-textjs-redirect for data requests', async () => {
    const module = {
      default: () => Response.redirect('https://example.vercel.sh/', 307),
    }
    ...
    expect(result.continue).toBe(false)
    expect(result.response?.status).toBe(200)
    expect(result.response?.headers.get('x-textjs-redirect')).toBe('https://example.vercel.sh/')
    expect(result.response?.headers.get('Location')).toBeNull()
    expect(result.redirectUrl).toBeUndefined()
  })

  it('ignores a forged x-textjs-data header when the caller did not opt in', async () => {
    // `x-textjs-data` is in INTERNAL_HEADERS and gets stripped by the caller
    ...
    expect(result.response?.headers.get('x-textjs-redirect')).toBeNull()
  })

  it('does not translate redirects to x-textjs-redirect when x-textjs-data is absent', async () => {
    const module = {
      default: (req: Request) => Response.redirect(new URL('/new-home', req.url).toString(), 307),
    }
