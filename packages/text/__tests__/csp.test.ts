...
describe('CSP nonce helpers', () => {
  ...
  it('prefers request CSP over fallback header sources', () => {
    ...
    expect(getScriptNonceFromHeaderSources(requestHeaders, middlewareHeaders)).toBe('request-nonce')
  })
  ...
  it('throws on HTML-escape characters using Text.js-compatible messaging', () => {
    expect(() => getScriptNonceFromHeader("script-src 'nonce-bad&nonce'")).toThrow(
      'Nonce value from Content-Security-Policy contained HTML escape characters.\nLearn more: https://textjs.org/docs/messages/nonce-contained-invalid-characters',
    )
  })
})
