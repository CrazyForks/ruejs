 ...
 *
 * The fix: text flattens the cause chain into `err.message` and `err.stack`
 * before the error escapes its handler boundary, so single-pass formatters
 * surface the root cause.
 */
import { describe, it, expect } from 'vite-plus/test'
import { inspect } from 'node:util'
import { flattenErrorCauses } from '../src/utils/error-cause.js'
...
describe('flattenErrorCauses', () => {
  ...
  it('walks a multi-level cause chain', () => {
    const root = new Error('role "text" does not exist')
    ...
    expect(top.message).toContain('role "text" does not exist')
  })
  ...
  it('the flatten marker is invisible to util.inspect (no symbol leak in error output)', () => {
    ...
    // "[Symbol(text.errorCausesFlattened)]: true" line.
    const rendered = inspect(err, { showHidden: false })
    expect(rendered).not.toContain('text.errorCausesFlattened')
