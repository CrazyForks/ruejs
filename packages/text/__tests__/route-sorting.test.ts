 ...
 *
 * Mirrors test cases from Text.js test/unit/page-route-sorter.test.ts,
 * adapted for text's route format (colon params instead of bracket params).
 *
 * Tests that dynamic sorts before catch-all, deterministic ordering,
 * static-over-dynamic preference, patternToTextFormat conversion,
...
import { appRouter, invalidateAppRouteCache } from '../src/routing/app-router.js'
import { validateRoutePatterns } from '../src/routing/route-validation.js'
...
// ─── Pages Router sorting (mirrors Text.js page-route-sorter.test.ts) ───
...
describe('Pages Router route sorting', () => {
  ...
  it('rejects sibling dynamic routes that differ only by param name', async () => {
    // Ported from Text.js: test/unit/page-route-sorter.test.ts
    // https://github.com/vercel/next.js/blob/canary/test/unit/page-route-sorter.test.ts
    const tmpRoot = await makeTempDir('text-pages-dynamic-conflict-')
  ...
  it('rejects sibling dynamic API routes that differ only by param name', async () => {
    const tmpRoot = await makeTempDir('text-api-dynamic-conflict-')
    const pagesDir = path.join(tmpRoot, 'pages')
...
describe('patternToTextFormat', () => {
  it('converts dynamic :id to [id]', () => {
    expect(patternToTextFormat('/posts/:id')).toBe('/posts/[id]')
  })

  it('converts catch-all :slug+ to [...slug]', () => {
    expect(patternToTextFormat('/docs/:slug+')).toBe('/docs/[...slug]')
  })

  it('converts optional catch-all :slug* to [[...slug]]', () => {
    expect(patternToTextFormat('/:slug*')).toBe('/[[...slug]]')
  })

  it('handles multiple dynamic segments', () => {
    expect(patternToTextFormat('/:category/:id')).toBe('/[category]/[id]')
  })

  it('preserves static segments', () => {
    expect(patternToTextFormat('/about')).toBe('/about')
    expect(patternToTextFormat('/')).toBe('/')
  })

  it('handles mixed static and dynamic', () => {
    expect(patternToTextFormat('/blog/:year/:month/:slug')).toBe('/blog/[year]/[month]/[slug]')
  })

  it('converts hyphenated dynamic :auth-method to [auth-method]', () => {
    expect(patternToTextFormat('/auth/:auth-method')).toBe('/auth/[auth-method]')
  })

  it('converts hyphenated catch-all :sign-in+ to [...sign-in]', () => {
    expect(patternToTextFormat('/login/:sign-in+')).toBe('/login/[...sign-in]')
  })

  it('converts hyphenated optional catch-all :sign-up* to [[...sign-up]]', () => {
    expect(patternToTextFormat('/register/:sign-up*')).toBe('/register/[[...sign-up]]')
  })

  it('converts dotted, at-sign, and colon param names (Text.js PARAMETER_PATTERN parity)', () => {
    expect(patternToTextFormat('/:variant.id')).toBe('/[variant.id]')
    expect(patternToTextFormat('/:user@domain')).toBe('/[user@domain]')
    // Colon-only: tested here because : is not a valid NTFS filename character on Windows.
    expect(patternToTextFormat('/:repo:name')).toBe('/[repo:name]')
  })
  ...
  it('handles param names with internal + or * correctly (no backtracking corruption)', () => {
    ...
    expect(patternToTextFormat('/:c++lang')).toBe('/[c++lang]')
    expect(patternToTextFormat('/:a*b')).toBe('/[a*b]')
    expect(patternToTextFormat('/utils/:a+b/c')).toBe('/utils/[a+b]/c')
  })
})
...
describe('App Router route sorting (additional)', () => {
  ...
  it('rejects sibling dynamic app routes that differ only by param name', async () => {
    const tmpRoot = await makeTempDir('text-app-dynamic-conflict-')
    ...
    try {
      ...
      await fs.writeFile(
        ...
        "export default function Layout({ children }: { children: import('@rue-js/rue').Renderable }) { return <html><body>{children}</body></html>; }",
      )
      await fs.writeFile(
  ...
  it('rejects route groups that resolve to the same URL path', async () => {
    // Text.js validates normalized app paths after route groups are stripped:
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/build/validate-app-paths.test.ts
    ...
    const tmpRoot = await makeTempDir('text-app-route-group-conflict-')
    const appDir = path.join(tmpRoot, 'app')
  ...
  it('rejects grouped slot sub-pages that resolve to the same URL path within one slot', async () => {
    const tmpRoot = await makeTempDir('text-app-slot-route-group-conflict-')
    const appDir = path.join(tmpRoot, 'app')
  ...
  it('merges grouped slot sub-pages from different slots onto the same synthesized route', async () => {
    const tmpRoot = await makeTempDir('text-app-slot-route-group-merge-')
    const appDir = path.join(tmpRoot, 'app')
  ...
  it('rejects slot sub-pages that collide with route handlers at the same URL', async () => {
    const tmpRoot = await makeTempDir('text-app-slot-route-handler-conflict-')
    const appDir = path.join(tmpRoot, 'app')
  ...
  it('does not overwrite a child slot with a parent slot sub-page that shares the same name', async () => {
    const tmpRoot = await makeTempDir('text-app-slot-shadowing-')
    const appDir = path.join(tmpRoot, 'app')
  ...
  it('does not assign child slot sub-pages to inherited same-named slots in generated sub-routes', async () => {
    const tmpRoot = await makeTempDir('text-app-slot-subroute-ownership-')
    const appDir = path.join(tmpRoot, 'app')
