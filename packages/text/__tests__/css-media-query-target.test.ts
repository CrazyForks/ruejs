 ...
 *
 * Verifies that text's `build.cssTarget` is pinned old enough to stop
 * esbuild's CSS minifier from rewriting `@media (max-width: ...)` queries
 * to the Media Queries Level 4 range syntax `@media (width <= ...)`.
 *
 * Ported from Text.js: test/e2e/app-dir/css-media-query/css-media-query.test.ts
 ...
 * that pattern-match the raw query string. Text.js does not perform this
...
async function makeFixture(): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-css-mq-'))
...
describe('CSS media-query syntax preservation in production build', () => {
  it('preserves `max-width: ...` instead of rewriting to range syntax', async () => {
    ...
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-css-mq-build-'))
    try {
      await build({
        root: tmpDir,
        configFile: false,
