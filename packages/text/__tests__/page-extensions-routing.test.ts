...
import { apiRouter, invalidateRouteCache, pagesRouter } from '../src/routing/pages-router.js'
...
describe('pageExtensions route discovery', () => {
  ...
  it('does not discover app/page.mdx routes when mdx is not configured', async () => {
    const tmpRoot = await makeTempDir('text-app-ext-no-mdx-')
    const appDir = path.join(tmpRoot, 'app')
    try {
      await fs.mkdir(path.join(appDir, 'about'), { recursive: true })
      await fs.writeFile(
  ...
  it('excludes _ prefixed files from api routes', async () => {
    // Text.js ignores _-prefixed files in pages/api/ the same way it does in pages/.
    const tmpRoot = await makeTempDir('text-api-underscore-')
  ...
  it('discovers pages and api files using configured pageExtensions', async () => {
    const tmpRoot = await makeTempDir('text-pages-ext-mdx-')
    const pagesDir = path.join(tmpRoot, 'pages')
    try {
      await fs.mkdir(path.join(pagesDir, 'api'), { recursive: true })
      await fs.writeFile(
