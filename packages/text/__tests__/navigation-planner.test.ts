...
} from '../src/server/navigation-planner.js'
import type {
  GraphVersion,
  RootBoundaryId,
  RouteManifest,
...
import { createCacheEntryReuseProof, type CacheEntryReuseProof } from '../src/server/cache-proof.js'
...
function planPayloadResponse(rootBoundaryId: string | null): NavigationDecisionV0 {
  ...
  const state: NavigationPlannerStateV0 = {
    textOperationToken: token,
    traceFields: {
      currentRootLayoutTreePath: '/',
      currentVisibleCommitVersion: 2,
      textRootLayoutTreePath: rootBoundaryId,
...
function planPayloadResponseFromRootBoundaries(options: {
  currentRootBoundaryId: string | null
  textRootBoundaryId: string | null
}): NavigationDecisionV0 {
  const token = createOperationToken({
    targetSnapshotFingerprint: `route:/dashboard|root:${options.textRootBoundaryId ?? 'unknown'}`,
  })
  ...
  const routeManifest = createTestRouteManifest([
    ...
    {
      layoutIds:
        options.textRootBoundaryId === null ? [] : [`layout:${options.textRootBoundaryId}`],
      pattern: '/dashboard',
      rootBoundaryId:
        options.textRootBoundaryId === null ? null : `root-boundary:${options.textRootBoundaryId}`,
    },
  ])
  ...
  return navigationPlanner.plan({
    event: {
      ...
      result: {
        href: 'https://example.com/dashboard',
        targetSnapshot: {
          ...createRouteSnapshot(options.textRootBoundaryId),
          matchedUrl: '/dashboard',
          routeId: 'route:/dashboard',
        },
      },
    ...
    state: {
      ...
      traceFields: {
        ...
        currentVisibleCommitVersion: 2,
        textRootLayoutTreePath: options.textRootBoundaryId,
        startedVisibleCommitVersion: 2,
      },
      visibleCommitVersion: 2,
      visibleSnapshot: {
...
function planPayloadResponseFromSnapshots(options: {
...
describe('navigationPlanner root-boundary decisions', () => {
  // Root-layout MPA semantics match Text.js coverage:
  // .textjs-ref/test/e2e/app-dir/root-layout/root-layout.test.ts
  // .textjs-ref/test/e2e/app-dir/segment-cache/mpa-navigations/mpa-navigations.test.ts
  it('proposes a visible commit for same-root payload responses', () => {
    const decision = planPayloadResponse('/')
  ...
  it('rejects runtime cache entries when the cache proof is missing', () => {
    ...
    const decision = planPayloadResponseFromSnapshots({
  ...
  it('hard-navigates cross-root payload responses', () => {
    const transition: RootBoundaryTransition = navigationPlanner.classifyRootBoundaryTransition(
      '/',
      '/(dashboard)',
    )
    const decision = planPayloadResponse('/(dashboard)')
  ...
  it('uses an unproven payload fallback when the target root identity is unknown', () => {
    const decision = planPayloadResponse(null)
  ...
  it('uses an unproven payload fallback when the visible root identity is unknown', () => {
    ...
    const decision = planPayloadResponseFromRootBoundaries({
  ...
  it('preserves mounted parallel slots owned by approved same-layout ancestors', () => {
    // Mirrors Text.js mounted parallel route preservation covered by:
    // .textjs-ref/test/e2e/app-dir/parallel-routes-and-interception-catchall/parallel-routes-and-interception-catchall.test.ts
    const currentSnapshot = createRouteSnapshot(
      '/',
      ['layout:/', 'layout:/feed'],
      [
  ...
  it('preserves previous slot ids for default and unmatched target bindings', () => {
    // Mirrors Text.js default-slot reuse semantics:
    // .textjs-ref/packages/text/src/client/components/router-reducer/ppr-navigations.ts
    const currentSnapshot = createRouteSnapshot(
      '/',
  ...
  it('does not preserve default target slots when their owner layout is not retained', () => {
    ...
    const decision = planPayloadResponseFromSnapshots({ currentSnapshot, targetSnapshot })
  ...
  it('uses RouteManifest root boundaries instead of stale snapshot roots', () => {
    ...
    expect(decision.trace.entries[0]?.fields.textRootLayoutTreePath).toBe('/marketing')
  })
  ...
  it('approves intercepted preservation only from explicit source and slot proof', () => {
    // Core-15 oracle, porting the visible behavior from Text.js:
  ...
  it('never hard-navigates prefetch payload responses', () => {
    const token = createOperationToken({
      lane: 'prefetch',
