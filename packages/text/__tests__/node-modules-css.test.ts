...
const drainDevServerModuleLoads = () => new Promise(resolve => setTimeout(resolve, 50))
...
describe('node_modules CSS import (App Router)', () => {
  ...
  afterAll(async () => {
    await drainDevServerModuleLoads()
    await server?.close()
  })

  it('renders page that imports CSS from node_modules without crashing', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/node-modules-css')
...
describe('node_modules CSS import (Pages Router)', () => {
  ...
  beforeAll(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-nm-css-pages-'))
    ...
    const requiredPackages = ['vite', '@vitejs', 'text']
...
export default function Page() {
  return (
    ...
    // `app/` directory) this just gives text a root to resolve from; the
    // plugin ignores the absence of `app/`.
    const plugins: any[] = [text({ appDir: tmpDir })]
