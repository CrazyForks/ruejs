 ...
 * to Text.js's metadata-dynamic-routes tests, verifying that text
...
import {
  sitemapToXml,
  sitemapToXml as textResolveSitemap,
  robotsToText,
  robotsToText as textResolveRobots,
  manifestToJson,
  ...
  type RobotsConfig,
  type ManifestConfig,
} from '../src/server/metadata-routes.js'

// Parity guard against Text.js's current sitemap serializer.
// We intentionally compare against the installed implementation because
// several edge cases are surprising but observable in Text itself.
function expectSitemapToMatchText(entries: SitemapEntry[]): void {
  expect(sitemapToXml(entries)).toBe(textResolveSitemap(entries))
}

type TextRobotsConfig = Parameters<typeof textResolveRobots>[0]

function expectRobotsToMatchText(config: RobotsConfig & TextRobotsConfig): void {
  expect(robotsToText(config)).toBe(textResolveRobots(config))
}
...
describe('sitemapToXml', () => {
  it('generates valid XML for basic entries', () => {
    ...
    expectSitemapToMatchText(entries)
  ...
  it('emits all supported optional video fields', () => {
    ...
    expect(xml).toContain('<video:expiration_date>2024-08-01T12:00:00.000Z</video:expiration_date>')
  ...
  it('escapes XML-sensitive values instead of raw-interpolating them', () => {
    // Text.js raw-interpolates these values, producing invalid XML.
    // text intentionally diverges to produce well-formed XML that
  ...
  it('omits zero-valued optional video numerics like Text', () => {
    const entries: SitemapEntry[] = [
      {
        url: 'https://example.com',
        videos: [
  ...
  it('matches Text when uploader content is missing', () => {
    const entries: SitemapEntry[] = [
      {
        url: 'https://example.com',
        videos: [
  ...
  it('matches Text when video date fields are Date objects', () => {
    const entries: SitemapEntry[] = [
      {
        url: 'https://example.com',
        videos: [
...
describe('robotsToText', () => {
  ...
  // Ported from Text.js: test/unit/webpack/loaders/metadata/resolve-route-data.test.ts
  ...
  it('emits Host before Sitemap like Text.js dynamic robots routes', () => {
    ...
    expectRobotsToMatchText(config)
...
// ─── fillStaticMetadataSegment ────────────────────────────────────────────
// Ported from Text.js: packages/text/src/lib/metadata/get-metadata-route.test.ts
// https://github.com/vercel/next.js/blob/7873aea/packages/text/src/lib/metadata/get-metadata-route.test.ts
...
describe('scanMetadataFiles', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-test-metadata-'))
  })
