...
} from '../src/build/route-classification-manifest.js'
import type { ClassificationReason } from '../src/build/layout-classification-types.js'
...
beforeEach(async () => {
  tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-classification-manifest-'))
})
...
describe('buildGenerateBundleReplacement', () => {
  function evalDispatch(source: string): (routeIdx: number) => unknown {
    ...
    //   function __TEXT_CLASS(routeIdx) { return (<replacement>)(routeIdx); }
