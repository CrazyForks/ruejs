...
import { planRouteClassificationInjection } from '../src/build/route-classification-injector.js'
import type { RouteClassificationManifest } from '../src/build/route-classification-manifest.js'
...
const STUBS = [
  'function __TEXT_CLASS(routeIdx) { return null; }',
  'function __TEXT_CLASS_REASONS(routeIdx) { return null; }',
  'const route = { __buildTimeClassifications: __TEXT_CLASS(0) };',
].join('\n')
...
describe('planRouteClassificationInjection', () => {
  ...
  it('patches the classification stub and invalidates the target source map', () => {
    ...
    expect(plan.code).not.toContain('function __TEXT_CLASS(routeIdx) { return null; }')
    expect(plan.code).toContain('function __TEXT_CLASS(routeIdx) { return ((routeIdx) => {')
    expect(plan.code).toContain('case 0: return new Map([[0, "dynamic"], [1, "static"]]);')
    expect(plan.code).toContain('function __TEXT_CLASS_REASONS(routeIdx) { return null; }')
  })
  ...
  it('patches the debug reasons sidecar when debug reasons are enabled', () => {
    ...
    if (plan.kind !== 'patch') throw new Error('Expected patch plan')
    expect(plan.code).not.toContain('function __TEXT_CLASS_REASONS(routeIdx) { return null; }')
    expect(plan.code).toContain('function __TEXT_CLASS_REASONS(routeIdx) { return ((routeIdx) => {')
  ...
  it('fails loudly when the dispatch is referenced but the stub body is absent', () => {
    expect(() =>
      planRouteClassificationInjection({
        chunks: [makeChunk('const route = { __buildTimeClassifications: __TEXT_CLASS(0) };')],
        dynamicShimPaths: new Set(),
        enableDebugReasons: false,
        manifest: makeManifest(),
        moduleInfo: {
  ...
  it('fails loudly when multiple chunks contain the dispatch stub body', () => {
    ...
    ).toThrow(/expected __TEXT_CLASS stub in exactly one RSC chunk, found 2/)
  })
  ...
  it('fails loudly when debug reasons are enabled but the reasons stub is missing', () => {
    ...
    ).toThrow(/__TEXT_CLASS_REASONS stub is missing/)
  })
})
