...
import { googleFontsMetadata } from '../../src/build/google-fonts/font-metadata.js'
import { validateGoogleFontOptions } from '../../src/build/google-fonts/validate.js'

// Ported from Text.js: packages/font/src/google/validate-google-font-function-call.ts.
// text drops the SWC-style `(functionName, fontFunctionArgument)` calling
// convention since the plugin already knows the family name; everything else
// matches Text's contract one-for-one.
...
describe('validateGoogleFontOptions', () => {
  ...
  it('throws on missing weight for fonts with no variable face (Anton)', () => {
    ...
    // text silently builds a `:wght@100..900` URL and Google returns 400;
  ...
  it('disables preload automatically for fonts with no preloadable subsets', () => {
    ...
    // flip preload to false instead of throwing, matching Text.js behavior.
