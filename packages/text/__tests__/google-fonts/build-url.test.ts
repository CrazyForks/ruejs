...
import { buildGoogleFontsUrl } from '../../src/build/google-fonts/build-url.js'

// Ported from Text.js: packages/font/src/google/get-google-fonts-url.ts.
// These tests pin the URL shape that Google Fonts CDN actually accepts. The
// pre-port text implementation hardcoded `:wght@100..900`, which is invalid
...
describe('buildGoogleFontsUrl', () => {
  ...
  it('joins multiple weights with semicolons in numeric order', () => {
    ...
    expect(url).toBe('https://fonts.googleapis.com/css2?family=Inter:wght@100;400;700&display=swap')
  })
  ...
  it('emits only variableAxes when neither wght nor ital is given', () => {
    // Edge case from Text.js source: a variable font where the caller
