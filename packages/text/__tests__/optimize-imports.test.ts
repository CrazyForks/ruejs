/**
 * Tests for the text:optimize-imports plugin and barrel export map helpers.
 *
...
} from '../src/plugins/optimize-imports.js'
...
describe('text:optimize-imports plugin', () => {
  it('exists and has the correct name', () => {
    const plugin = createOptimizeImportsPlugin(
      () => undefined,
      () => '/fake/root',
    ) as Plugin
    expect(plugin.name).toBe('text:optimize-imports')
  ...
  it('picks up optimizePackageImports from text({ textConfig })', async () => {
    const tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'text-inline-optimize-')))
    ...
    try {
      ...
      const configPlugin = plugins.find(p => p.name === 'text:config') as Plugin & {
        config?: (...args: any[]) => Promise<any>
      }
      const optimizePlugin = plugins.find(p => p.name === 'text:optimize-imports') as Plugin
...
describe('buildBarrelExportMap', () => {
  it("handles export * as Name from 'sub-pkg'", async () => {
    ...
    const barrelCode = `export * as Slot from "@scope/ui-slot";
export * as Tooltip from "@scope/ui-tooltip";`
    ...
    expect(map!.get('Slot')).toEqual({
      source: '@scope/ui-slot',
      isNamespace: true,
    })
    expect(map!.get('Tooltip')).toEqual({
      source: '@scope/ui-tooltip',
      isNamespace: true,
    })
  })
...
export { Input } from "./input";`
  ...
  it('handles import * as X; export { X }', async () => {
    ...
    const barrelCode = `import * as AlertDialog from "@scope/ui-alert-dialog";
export { AlertDialog };`
...
export { Button } from "./button";`
  ...
  it('DEFAULT_OPTIMIZE_PACKAGES includes expected packages', () => {
    expect(DEFAULT_OPTIMIZE_PACKAGES).toContain('lucide-rue')
  ...
  it("resolves wildcard export * from './mod' where mod.jsx exists (jsx extension)", async () => {
    ...
    // may use .jsx for component modules without a separate compile step.
...
describe('text:optimize-imports transform', () => {
  let tmpDir: string
  ...
  async function setupTransform(
    ...
    // text.config.js) for the plugin's buildStart to include it.
    tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'text-optimize-test-')))
    fs.writeFileSync(
  ...
  it("rewrites namespace re-export: import { X } from 'barrel' → import * as X from 'sub-pkg'", async () => {
    // lucide-rue is in DEFAULT_OPTIMIZE_PACKAGES. The barrel contents below use
    // scoped UI sub-packages intentionally — this tests the namespace rewrite path
    // with an arbitrary barrel; the package name just needs to be in the optimized list.
    const call = await setupTransform(
      'lucide-rue',
      `export * as Slot from "@scope/ui-slot";\nexport * as Dialog from "@scope/ui-dialog";`,
    )
    const code = `import { Slot, Dialog } from "lucide-rue";\nconst x = Slot;`
    const result = await call(code, '/app/component.tsx')
    expect(result).not.toBeNull()
    expect(result!.code).toContain(`import * as Slot from "@scope/ui-slot"`)
    expect(result!.code).toContain(`import * as Dialog from "@scope/ui-dialog"`)
    expect(result!.code).not.toContain(`from "lucide-rue"`)
  })
  ...
  it('leaves import unchanged when a specifier is not in the barrel map', async () => {
    ...
    const call = await setupTransform('rxjs', `export * as Slot from "@scope/ui-slot";`)
  ...
  it("leaves namespace import unchanged: import * as Pkg from 'barrel' cannot be optimized", async () => {
    ...
    const code = `import * as LucideRue from "lucide-rue";`
    const result = await call(code, '/app/page.tsx')
  ...
  it('skips transform on client environment', async () => {
    ...
    if (buildStartHook) await buildStartHook.call(plugin)
    const transform = unwrapHook(plugin.transform)!
    // lucide-rue is in DEFAULT_OPTIMIZE_PACKAGES — use it to hit the env guard
    const code = `import { Sun } from "lucide-rue";`
  ...
  it('groups multiple specifiers from same source into one import statement', async () => {
    ...
    const call = await setupTransform('ramda', `export { Button, buttonVariants } from "./button";`)
  ...
  it('populates subpkgOrigin independently for RSC and SSR when they share the same barrel entry', async () => {
    ...
    // When RSC and SSR share the same barrel entry path (common — most packages have no rue-server
    ...
    // Use a package with no rue-server condition so RSC and SSR resolve the same entry.
    const pkgDir = path.join(tmpDir, 'node_modules', 'lucide-rue')
    fs.mkdirSync(pkgDir, { recursive: true })
    fs.writeFileSync(
      path.join(pkgDir, 'package.json'),
      JSON.stringify({
    ...
    fs.writeFileSync(path.join(pkgDir, 'index.js'), `export { Slot } from "@scope/ui-slot";`)
    ...
    const rscResult = await rscCall(`import { Slot } from "lucide-rue";`, '/app/page.tsx')
    expect(rscResult).not.toBeNull()
    expect(rscResult!.code).toContain(`from "@scope/ui-slot"`)

    // SSR processes the same barrel text — with the bug it would skip the inner loop
    ...
    const ssrResult = await ssrCall(`import { Slot } from "lucide-rue";`, '/app/layout.tsx')
    expect(ssrResult).not.toBeNull()
    expect(ssrResult!.code).toContain(`from "@scope/ui-slot"`)
  })

  it('prefers rue-server export condition in RSC but not in SSR', async () => {
    // Simulates a package that exposes different barrel entries under
    // "rue-server" vs "import" export conditions.
    // In the RSC environment the plugin should pick the rue-server entry;
    // in SSR it must use the standard import entry.
    tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'text-optimize-test-')))
    fs.writeFileSync(
      path.join(tmpDir, 'package.json'),
      JSON.stringify({ name: 'test-app', type: 'module' }),
    ...
    // Package with diverging rue-server vs import entries
    fs.writeFileSync(
      path.join(pkgDir, 'package.json'),
      JSON.stringify({
        name: 'antd',
    ...
    // RSC environment: should use rue-server entry → knows about RscButton
    const rscCall = async (code: string, id: string) =>
      await (transform as any).call({ ...plugin, environment: { name: 'rsc' } }, code, id)
    ...
    // RSC: RscButton is exported from the rue-server barrel → rewrite succeeds
