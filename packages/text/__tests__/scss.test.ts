/**
 * Tests for SCSS / Sass preprocessing in text (Pages Router).
 *
 * Mirrors Text.js's SCSS support: when a page imports a `.scss` file,
 ...
 * `sass-embedded`). text relies on that built-in handling; this test
 * verifies text does not interfere with the pipeline, and that a
 ...
 * `__tests__/fixtures/*` tree. A shared SCSS fixture would break every test
 ...
 *
 * Ported from Text.js: test/e2e/app-dir/scss/single-global/single-global.test.ts
...
// peer dependency contract: text relies on Vite's built-in handling
...
try {
  const sassSpecifier = 'sass'
  // @ts-ignore Optional peer dependency, not declared in this repo
  await import(/* @vite-ignore */ sassSpecifier)
  sassAvailable = true
} catch {
  sassAvailable = false
}
 ...
 *
 * Imports the SCSS file via `pages/_app.tsx` to match Text.js's
 ...
 * `text`, Vite, and (if installed) `sass` without
 * an extra install step.
 */
async function makePagesRouterScssFixture(): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-scss-pages-'))
...
describeIfSass('SCSS preprocessing (Pages Router)', () => {
  ...
  it('links and serves resolved SCSS through the production Pages Router server', async () => {
    // End-to-end production parity check. Mirrors what a Text.js
    ...
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-scss-pages-build-'))
    try {
      await build({
        root: tmpDir,
        configFile: false,
