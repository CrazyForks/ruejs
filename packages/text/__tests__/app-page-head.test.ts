...
import { collectAppPageSearchParams, resolveAppPageHead } from '../src/server/app-page-head.js'
...
describe('app page head resolution', () => {
  ...
  it('resolves layout and page metadata with parent chaining and page-only search params', async () => {
    ...
    const result = await resolveAppPageHead<Record<string, unknown>>({
      ...
      searchParams: new URLSearchParams('tag=text&tag=text'),
    })

    expect(result.metadata).toEqual({
      description: 'tag text,text',
      openGraph: {
        description: 'tag text,text',
    ...
    expect(result.pageSearchParams).toEqual({ tag: ['text', 'text'] })
  ...
  it('passes scoped params to layout metadata and full params/searchParams to page metadata', async () => {
    // Ported from Text.js: test/e2e/app-dir/layout-params/layout-params.test.ts
  ...
  it('uses parallel route slot page title when no primary page module is present', async () => {
    // Ported from Text.js: test/e2e/app-dir/metadata-streaming-parallel-routes/metadata-streaming-parallel-routes.test.ts
