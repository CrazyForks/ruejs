...
function makeTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'text-cfg-ext-'))
}

describe('loadTextConfig — Text.js text-config-ts-native extension/import parity', () => {
  // These tests mirror the fixtures under .textjs-ref/test/e2e/app-dir/
  // text-config-ts-native-{mts,ts}/ that exercise extension probing,
  ...
  // Ported from Text.js: test/e2e/app-dir/text-config-ts-native-mts/nested-imports/
  // https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/text-config-ts-native-mts/nested-imports/
  it('resolves nested .ts imports from text.config.ts (explicit .ts extension)', async () => {
    tmpDir = makeTempDir()
    fs.writeFileSync(
  ...
  // Ported from Text.js: test/e2e/app-dir/text-config-ts-native-mts/import-js-extensions-{cjs,esm}/
  // https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/text-config-ts-native-mts/import-js-extensions-cjs/
  it('resolves .mts/.cts/.mjs/.cjs/.ts/.js imports from text.config.mts', async () => {
    tmpDir = makeTempDir()
    ...
    fs.writeFileSync(
      path.join(tmpDir, 'text.config.mts'),
  ...
  // Ported from Text.js: test/e2e/app-dir/text-config-ts-native-mts/import-from-node-modules/
  // https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/text-config-ts-native-mts/import-from-node-modules/
  it('resolves bare-import packages from text.config.ts (.cjs/.mjs/.js)', async () => {
    tmpDir = makeTempDir()
  ...
  it('resolves nested .ts imports when text.config.mts is in a CJS package', async () => {
    tmpDir = makeTempDir()
    fs.writeFileSync(
