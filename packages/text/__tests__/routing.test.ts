...
import {
  ...
  type Route,
} from '../src/routing/pages-router.js'
import {
  appRouteGraph,
  appRouter,
  ...
  type AppRoute,
} from '../src/routing/app-router.js'
...
describe('pagesRouter - route discovery', () => {
  ...
  it('rejects non-terminal catch-all routes during discovery', async () => {
    await withTempDir('text-pages-nonterminal-catchall-', async tmpDir => {
  ...
  it('rejects non-terminal optional catch-all routes during discovery', async () => {
    await withTempDir('text-pages-nonterminal-optional-catchall-', async tmpDir => {
      const pagesDir = path.join(tmpDir, 'pages')
      await mkdir(path.join(pagesDir, '[[...slug]]', 'edit'), { recursive: true })
      await writeFile(path.join(pagesDir, '[[...slug]]', 'edit', 'index.tsx'), EMPTY_PAGE)
...
describe('apiRouter - route discovery', () => {
  it('rejects non-terminal catch-all API routes during discovery', async () => {
    await withTempDir('text-api-nonterminal-catchall-', async tmpDir => {
      const pagesDir = path.join(tmpDir, 'pages')
  ...
  it('rejects non-terminal optional catch-all API routes during discovery', async () => {
    await withTempDir('text-api-nonterminal-optional-catchall-', async tmpDir => {
      const pagesDir = path.join(tmpDir, 'pages')
      await mkdir(path.join(pagesDir, 'api', '[[...slug]]'), { recursive: true })
      await writeFile(path.join(pagesDir, 'api', '[[...slug]]', 'edit.ts'), EMPTY_ROUTE)
...
describe('appRouter - route discovery', () => {
  ...
  it('rejects page and route handler files at the same app route', async () => {
    // Text.js docs forbid page.js and route.js at the same normalized route:
    ...
    await withTempDir('text-app-page-route-conflict-', async tmpDir => {
  ...
  it('discovers dynamic params captured by a nested root layout', async () => {
    await withTempDir('text-app-root-params-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('preserves root layout params on synthetic parallel slot routes', async () => {
    await withTempDir('text-app-root-params-parallel-slot-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects non-terminal catch-all pages during discovery', async () => {
    await withTempDir('text-app-nonterminal-catchall-page-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects non-terminal optional catch-all route handlers during discovery', async () => {
    await withTempDir('text-app-nonterminal-optional-catchall-route-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      await mkdir(path.join(appDir, '[[...slug]]', 'edit'), { recursive: true })
  ...
  it('rejects non-terminal catch-all routes even when the suffix is behind a route group', async () => {
    await withTempDir('text-app-nonterminal-catchall-group-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('allows terminal catch-all when only transparent route groups follow on disk', async () => {
    await withTempDir('text-app-terminal-catchall-group-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('allows terminal optional catch-all when only transparent route groups follow on disk', async () => {
    await withTempDir('text-app-terminal-optional-catchall-group-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      await mkdir(path.join(appDir, '[[...slug]]', '(admin)'), { recursive: true })
  ...
  it('rejects non-terminal catch-all in synthetic @slot subroutes', async () => {
    await withTempDir('text-app-slot-nonterminal-catchall-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects non-terminal optional catch-all in synthetic @slot subroutes', async () => {
    await withTempDir('text-app-slot-nonterminal-optional-catchall-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      await mkdir(path.join(appDir, 'dashboard', '@modal', '[[...slug]]', 'edit'), {
        recursive: true,
      })
  ...
  it('allows terminal synthetic @slot catch-all when only route groups follow', async () => {
    await withTempDir('text-app-slot-terminal-catchall-group-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('does not create nested @slot sub-routes without a children default fallback', async () => {
    await withTempDir('text-app-slot-missing-children-default-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('does not discover nested @slot sub-routes when the slot root has no page or default', async () => {
    await withTempDir('text-app-slot-nested-only-rootless-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      await mkdir(path.join(appDir, 'inbox', '@modal', 'profile'), { recursive: true })
      await writeFile(path.join(appDir, 'inbox', 'page.tsx'), EMPTY_PAGE)
  ...
  it('rejects non-terminal catch-all intercept targets', async () => {
    await withTempDir('text-app-intercept-nonterminal-catchall-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects non-terminal optional catch-all intercept targets', async () => {
    await withTempDir('text-app-intercept-nonterminal-optional-catchall-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      await mkdir(path.join(appDir, 'feed', '@modal', '(...)photos', '[[...slug]]', 'edit'), {
        recursive: true,
      })
  ...
  it('allows terminal catch-all intercept targets when only route groups follow', async () => {
    await withTempDir('text-app-intercept-terminal-catchall-group-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects (..) intercepting routes at the normalized root', async () => {
    // Ported from Text.js:
    // packages/text/src/shared/lib/router/utils/interception-routes.test.ts
    // https://github.com/vercel/next.js/blob/ae61573e062e900050b8e6b24626e450accc4570/packages/text/src/shared/lib/router/utils/interception-routes.test.ts#L66-L75
    await withTempDir('text-app-intercept-root-parent-marker-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects (..) intercepting routes at the root after route group normalization', async () => {
    // Text.js validates the normalized intercepting route, so transparent
    // route groups do not make an upward marker valid at the URL root.
    // https://github.com/vercel/next.js/blob/ae61573e062e900050b8e6b24626e450accc4570/packages/text/src/shared/lib/router/utils/interception-routes.ts#L60-L95
    await withTempDir('text-app-intercept-group-root-parent-marker-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects (..)(..) intercepting routes at root or one visible level up', async () => {
    ...
    // packages/text/src/shared/lib/router/utils/interception-routes.test.ts
    // https://github.com/vercel/next.js/blob/ae61573e062e900050b8e6b24626e450accc4570/packages/text/src/shared/lib/router/utils/interception-routes.test.ts#L66-L75
    await withTempDir('text-app-intercept-two-up-marker-root-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      await mkdir(path.join(appDir, '@modal', '(..)(..)foo'), { recursive: true })
      await writeFile(path.join(appDir, 'page.tsx'), EMPTY_PAGE)
      await writeFile(path.join(appDir, '@modal', '(..)(..)foo', 'page.tsx'), EMPTY_PAGE)
    ...
    await withTempDir('text-app-intercept-two-up-marker-one-level-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('allows (..) intercepting routes in route groups with one visible parent segment', async () => {
    // Text.js validates the normalized intercepting route: /(group) is root,
    // but /shop/(group) has one visible segment and can climb to /.
    // https://github.com/vercel/next.js/blob/ae61573e062e900050b8e6b24626e450accc4570/packages/text/src/shared/lib/router/utils/interception-routes.ts#L60-L95
    await withTempDir('text-app-intercept-group-with-visible-parent-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('(..) climbs visible route segments, not filesystem dirs (route group between segments)', async () => {
    ...
    await withTempDir('text-app-intercept-route-group-climb-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('(..) with single route group between segments resolves correctly', async () => {
    ...
    // Target: /shop/cart
    await withTempDir('text-app-intercept-single-group-climb-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('(..) skips multiple consecutive route groups when climbing', async () => {
    ...
    await withTempDir('text-app-intercept-multi-group-climb-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('rejects sibling intercept targets that differ only by param name', async () => {
    await withTempDir('text-app-intercept-dynamic-conflict-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('excludes private folders (underscore prefix) from route discovery', async () => {
    await withTempDir('text-app-private-folder-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
...
describe('matchAppRoute - URL matching', () => {
  ...
  it('keeps encoded slashes distinct from real nested routes', async () => {
    await withTempDir('text-app-encoded-slash-route-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('discovers intercept layout chains inside parallel slots', async () => {
    ...
    await withTempDir('text-app-intercept-layout-chain-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('discovers nested intercept layout chains in outermost-to-innermost order', async () => {
    await withTempDir('text-app-intercept-layout-depth-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('allows inherited intercepting slots to reuse the same target pattern', async () => {
    await withTempDir('text-app-intercept-inherited-slot-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      ...
      await writeFile(path.join(appDir, 'intercepting-routes', '@modal', 'default.tsx'), EMPTY_PAGE)
      await writeFile(
      ...
      const detailRoute = routes.find(route => route.pattern === '/intercepting-routes/photo/:id')
  ...
  it('discovers layout routes whose own content is parallel slot pages', async () => {
    await withTempDir('text-app-layout-parallel-slot-route-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      ...
      await writeFile(path.join(appDir, 'users', '[username]', '@modal', 'default.tsx'), EMPTY_PAGE)
  ...
  it('discovers nested parallel slot sub-routes from layout-only parent', async () => {
    ...
    // Text.js fixture: app/parallel-nested/home/layout.tsx + @parallelB/default.tsx + @parallelB/nested/page.tsx (no home/page.tsx)
    await withTempDir('text-app-nested-parallel-slot-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('does not attach pre-root parallel slots when the root layout is below app', async () => {
    // Text.js allows multiple root layouts when app/layout is omitted:
    ...
    // https://github.com/vercel/next.js/blob/ae61573e062e900050b8e6b24626e450accc4570/packages/text/src/server/app-render/create-component-tree.tsx#L1273-L1290
    await withTempDir('text-app-pre-root-parallel-slot-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('attaches parallel slots inside the selected root layout subtree', async () => {
    await withTempDir('text-app-root-subtree-parallel-slot-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
  ...
  it('preserves same-named parallel slots from multiple layout levels', async () => {
    await withTempDir('text-app-parallel-slot-priority-', async tmpDir => {
      const appDir = path.join(tmpDir, 'app')
      ...
      const sidebarByOwner = new Map(
        sidebarSlots.map(slot => [path.relative(appDir, slot.ownerDir).replace(/\\/g, '/'), slot]),
      )
...
describe('pagesRouter - dotted/colon param names (Text.js parity)', () => {
  it('discovers dynamic segments with dots and colons', async () => {
    await withTempDir('text-pages-dotted-param-', async tmpDir => {
      const pagesDir = path.join(tmpDir, 'pages')
  ...
  it('extracts params correctly for dotted dynamic segments', async () => {
    await withTempDir('text-pages-dotted-match-', async tmpDir => {
      const pagesDir = path.join(tmpDir, 'pages')
  ...
  it('skips routes whose param names end in + or * (would collide with internal modifiers)', async () => {
    await withTempDir('text-pages-skip-plus-', async tmpDir => {
      const pagesDir = path.join(tmpDir, 'pages')
      await mkdir(pagesDir, { recursive: true })
      await writeFile(
