...
/**
 * Create a fresh text:og-inline-fetch-assets plugin instance.
...
function createOgInlinePlugin(command: 'serve' | 'build' = 'serve'): Plugin {
  ...
  const plugin = plugins.find(p => p.name === 'text:og-inline-fetch-assets')
  if (!plugin) throw new Error('text:og-inline-fetch-assets plugin not found')
...
describe('text:og-inline-fetch-assets plugin', () => {
  afterEach(() => vi.restoreAllMocks())

  it('exists in the plugin array', () => {
    const plugin = createOgInlinePlugin()
    expect(plugin.name).toBe('text:og-inline-fetch-assets')
