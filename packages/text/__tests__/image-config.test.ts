 ...
 * attacks via text/image. Covers hostname globs, pathname globs, protocol,
 * port, and search matching — mirroring Text.js's matchRemotePattern semantics.
 */
...
import {
  ...
  isPrivateIp,
  type RemotePattern,
} from '../src/shims/image-config.js'
...
// ─── isPrivateIp ─────────────────────────────────────────────────────────
// Ported from Text.js: packages/text/src/server/is-private-ip.test.ts
// https://github.com/vercel/next.js/blob/canary/packages/text/src/server/is-private-ip.test.ts
...
describe('isPrivateIp', () => {
  ...
  it('returns false for domain names', () => {
    ...
    expect(isPrivateIp('textjs.org')).toBe(false)
    expect(isPrivateIp('docs.textjs.org')).toBe(false)
  })
})
