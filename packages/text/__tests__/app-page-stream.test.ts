...
} from '../src/server/app-page-stream.js'
...
describe('app page stream helpers', () => {
  ...
  it('forwards in-process SSR payloads to the SSR handler', async () => {
    ...
    const ssrHandler = vi.fn(async () => createStream(['<html>inline-payload</html>']))
    ...
    await expect(new Response(htmlStream).text()).resolves.toBe('<html>inline-payload</html>')
    expect(ssrHandler).toHaveBeenCalledWith(
      expect.anything(),
      null,
      expect.anything(),
      expect.objectContaining({ ssrPayload }),
    )
  })

  it('forwards explicit SSR payload decoders to the SSR handler', async () => {
    const ssrPayloadDecoder = vi.fn(async () => ({
      [AppElementsWire.keys.interceptionContext]: null,
      [AppElementsWire.keys.rootLayout]: '/',
      [AppElementsWire.keys.route]: AppElementsWire.encodeRouteId('/', null),
      [AppElementsWire.encodePageId('/', null)]: 'decoded home',
    }))
    const ssrHandler = vi.fn(async () => createStream(['<html>decoded-payload</html>']))
    ...
    const htmlStream = await renderAppPageHtmlStream({
      ...
      navigationContext: null,
      rscStream: createStream(['wire-payload']),
      ssrHandler: { handleSsr: ssrHandler },
      ssrPayloadDecoder,
    })

    await expect(new Response(htmlStream).text()).resolves.toBe('<html>decoded-payload</html>')
    expect(ssrHandler).toHaveBeenCalledWith(
      expect.anything(),
      null,
      expect.anything(),
      expect.objectContaining({ ssrPayloadDecoder }),
    )
  })
  ...
  it('calls onFlush when the upstream stream errors mid-consumption', async () => {
    ...
    // Emit one chunk, then error on the text pull — simulates a component
  ...
  it('falls back to the error boundary response for non-special SSR failures', async () => {
    ...
    const renderErrorBoundaryResponse = vi.fn(async () => new Response('boundary', { status: 200 }))
  ...
  it('tracks non-navigation RSC errors while preserving the base onError callback', () => {
    ...
    tracker.onRenderError({ digest: 'TEXT_NOT_FOUND' }, { path: '/test' }, { chunk: 2 })
    expect((tracker.getCapturedError() as Error).message).toBe('boom')
    expect(baseOnError).toHaveBeenCalledTimes(2)
  })

  it('replaces placeholder client reference RSC errors with concrete SSR errors', () => {
    const baseOnError = vi.fn(() => 'base-result')
    const tracker = createAppPageRscErrorTracker(baseOnError)

    tracker.onRenderError(
      new Error("Unexpectedly client reference export 'default' is called on server"),
      { path: '/test' },
      { chunk: 1 },
    )
    tracker.onRenderError(new Error('client page error'), { path: '/test' }, { chunk: 2 })

    expect((tracker.getCapturedError() as Error).message).toBe('client page error')
    expect(baseOnError).toHaveBeenCalledTimes(2)
  })

  it('rerenders with the route error boundary when an RSC error was captured', () => {
    expect(
      shouldRerenderAppPageWithGlobalError({
