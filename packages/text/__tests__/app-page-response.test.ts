...
} from '../src/server/app-page-response.js'
import {
...
describe('app page response helpers', () => {
  ...
  it('builds RSC responses with params, middleware headers, and timing', async () => {
    ...
    middlewareHeaders.append('vary', 'Text-Router-State-Tree')
    ...
    expect(response.headers.get('x-text-params')).toBe(encodeURIComponent('{"slug":"test"}'))
    ...
    expect(response.headers.get('x-text-timing')).toBe('10,5,-1')
    await expect(response.text()).resolves.toBe('flight')
  })

  it('emits the `x-edge-runtime: 1` marker on RSC responses when the route opts into the edge runtime (issue #1531)', () => {
    // Text.js sets `x-edge-runtime: 1` on edge-runtime app responses (see
    // edge-ssr-app.ts in the Text.js source). Mirror that only for routes
  ...
  it('builds RSC responses with the current compatibility ID header', () => {
    const response = withEnvVar('__TEXT_RSC_COMPATIBILITY_ID', 'compat-a', () =>
    ...
    expect(response.headers.get(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe('compat-a')
  })

  it('keeps the framework compatibility ID when middleware sets the internal header', () => {
    const middlewareHeaders = new Headers({
      [TEXT_RSC_COMPATIBILITY_ID_HEADER]: 'middleware-compat',
    })

    const response = withEnvVar('__TEXT_RSC_COMPATIBILITY_ID', 'framework-compat', () =>
      buildAppPageRscResponse(createBody('flight'), {
    ...
    expect(response.headers.get(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe('framework-compat')
  })

  it('percent-encodes X-Text-Params so non-ASCII characters survive the ByteString header constraint (issue #676)', () => {
    ...
    const rawHeader = response.headers.get('x-text-params')!
  ...
  it('builds HTML responses with middleware override/append header semantics', async () => {
    ...
    middlewareHeaders.set('vary', 'Text-Router-State-Tree')
    ...
    expect(response.headers.get('cache-control')).toBe('private, max-age=5')
    expect(response.headers.get('x-text-cache')).toBe('STATIC')
    expect(response.headers.get('x-textjs-cache')).toBe('HIT')
    expect(response.headers.get('vary')).toBe(TEXT_RSC_VARY_HEADER)
    expect(response.headers.get('link')).toBe(
      '</font.woff2>; rel=preload; as=font; type=font/woff2; crossorigin',
    )
    expect(response.headers.get('x-extra')).toBe('present')
    expect(response.headers.get('x-text-timing')).toBe('10,2,8')

    const setCookies = response.headers.getSetCookie()
    expect(setCookies).toContain('__prerender_bypass=token; Path=/')
    expect(setCookies).toContain('mw=1; Path=/')
    await expect(response.text()).resolves.toBe('<!DOCTYPE html><h1>page</h1>')
  })
...
describe('mergeMiddlewareResponseHeaders', () => {
  ...
  it('appends Vary headers instead of overriding', () => {
    ...
    const mwHeaders = new Headers()
    mwHeaders.set('Vary', 'Text-Router-State-Tree')

    mergeMiddlewareResponseHeaders(target, mwHeaders)

    expect(target.get('Vary')).toBe('RSC, Accept, Text-Router-State-Tree')
  })

  it('deduplicates Vary values when appending middleware headers', () => {
    const target = new Headers({ Vary: TEXT_RSC_VARY_HEADER })
    const mwHeaders = new Headers()
    mwHeaders.set('Vary', 'text-router-state-tree, X-Auth-State')

    mergeMiddlewareResponseHeaders(target, mwHeaders)

    expect(target.get('Vary')).toBe(`${TEXT_RSC_VARY_HEADER}, X-Auth-State`)
  })
