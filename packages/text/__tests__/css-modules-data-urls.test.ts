 ...
 *
 * Turbopack supports this Text.js-only import syntax. Webpack does not, and
 * the Text.js fixture skips outside `IS_TURBOPACK_TEST`:
 *
 ...
 * breaking the entire text build.
 *
 * text's `text:css-data-url` plugin rewrites these imports into
 ...
 *
 * Mirrors Text.js: test/e2e/app-dir/css-modules-data-urls/
...
async function makeFixture(): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-css-data-url-'))
  ...
  await fs.writeFile(
    ...
    [
      ...
      '// @ts-expect-error - data: import is rewritten by text:css-data-url',
...
describe('CSS data URL imports', () => {
  it('rewrites `data:text/css+module,...` imports into virtual CSS modules', async () => {
    ...
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-css-data-url-out-'))
    try {
      await build({
        root: tmpDir,
        configFile: false,
  ...
  it('decodes base64-encoded `data:text/css+module;base64,...` imports', async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-css-data-b64-'))
    await fs.symlink(ROOT_NODE_MODULES, path.join(tmpDir, 'node_modules'), 'junction')
    const outDir = await fs.mkdtemp(path.join(os.tmpdir(), 'text-css-data-b64-out-'))
    try {
