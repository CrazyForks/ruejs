 ...
 * cookie parsing, req/res Text.js extensions (status, json, send, redirect),
...
import { handleApiRoute } from '../src/server/api-handler.js'
import { reportRequestError, type ModuleImporter } from '../src/server/instrumentation.js'
import type { Route } from '../src/routing/pages-router.js'

vi.mock('../src/server/instrumentation.js', () => ({
...
describe('handleApiRoute', () => {
  ...
  describe('edge runtime', () => {
    it('calls edge API route handlers with a Fetch Request and writes their Response', async () => {
      // Ported from Text.js: test/e2e/edge-async-local-storage/index.test.ts
    ...
    it('passes a TextRequest with textUrl.searchParams to edge handlers', async () => {
      // Ported from Text.js: test/e2e/edge-pages-support/app/pages/api/hello.js
      ...
      const handler = vi.fn((request: Request) => {
        const textUrl = (request as { textUrl?: URL }).textUrl
        if (!textUrl) return new Response('missing textUrl', { status: 500 })
        return Response.json({
          hello: 'world',
          query: Object.fromEntries(textUrl.searchParams),
        })
      })
      const server = mockServer({
        config: { runtime: 'edge' },
    ...
    it('recognises bare "export const runtime = \'edge\'" as an edge API route', async () => {
      // Ported from Text.js: packages/text/src/build/analysis/get-page-static-info.ts
      // Both `export const runtime = "edge"` and `export const config = { runtime: "edge" }`
      // are valid ways to mark a Pages Router API route as edge. Text.js resolves
