...
import { cleanBuildOutput } from '../src/build/clean-output.js'
...
describe('cleanBuildOutput', () => {
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-clean-output-'))
  })
  ...
  it('removes stale server modules from the build output by default', () => {
    const staleModule = writeFile(
      tmpDir,
      'dist/server/ssr/_text/static/stale-dead-module.js',
      'export default 1;\n',
    )
  ...
  it('does not remove an output directory outside the project root by default', () => {
    const externalOutDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-external-output-'))
    try {
