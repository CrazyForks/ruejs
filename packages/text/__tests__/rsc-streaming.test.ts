 ...
 * <script> tags into the HTML stream between compat SSR flush cycles. These
...
import { createSsrErrorMetaRenderer } from '../src/server/app-ssr-error-meta.js'

/**
 * Create a ReadableStream from an array of string chunks, with optional
 * delay between groups to simulate compat SSR flush cycles.
 *
 * Each entry in `chunkGroups` is an array of strings that are written
 * synchronously (same macrotask), simulating how the compat SSR renderer
 * flushes multiple chunks within one output pass.
 *
  ...
  return new ReadableStream({
    async start(controller) {
      for (let i = 0; i < chunkGroups.length; i++) {
        ...
        // Wait for text macrotask between groups (simulates SSR flush boundary)
...
const RSC_RUNTIME_KEY = 'Symbol.for("text.navigationRuntime")'
...
describe('Tick-buffered RSC streaming (behavioral)', () => {
  ...
  it('adds nonce attributes to progressive RSC scripts when provided', async () => {
    ...
    const rscEmbed = createRscEmbedTransform(rsc.stream, 'text-test-nonce')
    await new Promise(resolve => setTimeout(resolve, 10))
    ...
    expect(output).toContain(`<script nonce="text-test-nonce">((self[${RSC_RUNTIME_KEY}]`)
    expect(output).toContain(`<script nonce="text-test-nonce">((self[${RSC_RUNTIME_KEY}]`)
    expect(output).toContain(RSC_RUNTIME_DONE)
  })

  it('does not inject scripts mid-HTML-chunk (DOM corruption prevention)', async () => {
    // This tests the core safety invariant: when the SSR renderer flushes multiple
    // chunks synchronously (same macrotask), scripts must NOT appear between them.
    // For example, the renderer might split an SVG element across chunks:
    ...
    // Simulate the renderer splitting an element across chunks in the SAME macrotask
    const htmlStream = createMockHtmlStream([
  ...
  it('injects head content before </head>', async () => {
    ...
    const injectHTML = '<script>self.__TEXT_RSC_PARAMS__={"slug":"test"}</script>'
    ...
    const injectPos = output.indexOf('__TEXT_RSC_PARAMS__')
  ...
  it('emits new server-inserted HTML before streamed Suspense chunks after head injection', async () => {
    // Ported from Text.js: test/e2e/app-dir/use-server-inserted-html/use-server-inserted-html.test.ts
  ...
  it('emits SSR-captured redirect meta tags after the shell has already flushed', async () => {
    // Ported from Text.js: test/e2e/app-dir/navigation/navigation.test.ts
    ...
    const htmlStream = new ReadableStream<Uint8Array>({
      async start(controller) {
        ...
        errorMetaRenderer.capture(
          Object.assign(new Error('TEXT_REDIRECT'), {
            digest: 'TEXT_REDIRECT;replace;/redirect/result;307',
          }),
        )
    ...
    const redirectMetaPos = output.indexOf(
      '<meta id="__text-page-redirect" http-equiv="refresh" content="1;url=/redirect/result" />',
    )
