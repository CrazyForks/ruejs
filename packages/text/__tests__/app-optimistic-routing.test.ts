...
} from '../src/server/app-optimistic-routing.js'
import type {
  GraphVersion,
  RouteManifest,
  RouteManifestRoute,
...
function createBlogElements(): AppElements {
  ...
  const routeElement = createElement(
    'Suspense',
    { fallback: createElement('p', { id: 'loading-message' }, 'Loading...') },
    createElement('main', null, 'Page slot'),
  )
  Object.defineProperty(routeElement, RUE_SUSPENSE_ELEMENT_MARKER, {
    configurable: true,
    value: true,
  })
...
function createUnmarkedElement(
  type: unknown,
...
describe('App Router optimistic routing', () => {
  ...
  it('does not learn routes without a loading boundary', () => {
    ...
    const elements: AppElements = {
      ...
      [routeId]: createUnmarkedElement('main', null, 'No loading boundary'),
    }
