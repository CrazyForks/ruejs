...
import type { Metadata } from '../src/shims/metadata.js'
...
describe('applyFileBasedMetadata', () => {
  ...
  it('drops generateImageMetadata ids that are not path-segment safe', async () => {
    ...
    try {
      ...
      expect(warn).toHaveBeenCalledWith(
        '[text] Skipping metadata route /opengraph-image image id "a/b" because metadata image ids must match /^[a-zA-Z0-9-_.]+$/.',
      )
    } finally {
      warn.mockRestore()
    }
  ...
  it('drops generateImageMetadata entries without ids', async () => {
    ...
    try {
      ...
      expect(result).toBeNull()
      expect(warn).toHaveBeenCalledWith(
        '[text] Skipping metadata route /opengraph-image image metadata entry because generateImageMetadata entries must include an id.',
      )
    } finally {
      warn.mockRestore()
    }
  ...
  it('drops dynamic metadata head URLs when params cannot fill servedUrl segments', async () => {
    ...
    try {
      ...
      expect(result).toBeNull()
      expect(warn).toHaveBeenCalledWith(
        '[text] Skipping metadata route /blog/[slug]/icon because params did not fill all dynamic segments.',
      )
    } finally {
      warn.mockRestore()
    }
  ...
  it('drops required catch-all metadata head URLs when params contain no segments', async () => {
    ...
    try {
      ...
      expect(result).toBeNull()
      expect(warn).toHaveBeenCalledWith(
        '[text] Skipping metadata route /docs/[...slug]/icon because params did not fill all dynamic segments.',
      )
    } finally {
      warn.mockRestore()
    }
