...
} from '../src/shims/unified-request-context.js'
import { consumeRenderRequestApiUsage, markRenderRequestApiUsage } from '../src/shims/headers.js'
import {
  getRequestExecutionContext,
  runWithExecutionContext,
} from '../src/shims/request-context.js'
...
describe('unified-request-context', () => {
  ...
  describe('legacy wrapper semantics inside unified scope', () => {
    it('runWithHeadersContext restores the outer headers sub-state', async () => {
      const { runWithHeadersContext } = await import('../src/shims/headers.js')
    ...
    it('runWithI18nState restores the outer i18n sub-state', async () => {
      const { runWithI18nState } = await import('../src/shims/i18n-state.js')
      const { getI18nContext, setI18nContext } = await import('../src/shims/i18n-context.js')
    ...
    it('cache/private/fetch/router/head sub-scopes reset and restore correctly', async () => {
      const { _runWithCacheState } = await import('../src/shims/cache.js')
      const { runWithPrivateCache } = await import('../src/shims/cache-runtime.js')
      const { runWithFetchCache, getCollectedFetchTags } =
        await import('../src/shims/fetch-cache.js')
      const { runWithRouterState } = await import('../src/shims/router-state.js')
      const { setSSRContext } = await import('../src/shims/router.js')
      const { runWithHeadState } = await import('../src/shims/head-state.js')
