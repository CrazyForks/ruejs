 ...
 *
 * Text.js's edge sandbox installs `AsyncLocalStorage` on the global context
 * (see `packages/text/src/server/web/sandbox/context.ts`). text executes
 ...
 * Without this shim, fixtures like `.textjs-ref/test/e2e/edge-async-local-storage/`
...
import { installServerGlobals } from '../src/server/server-globals.js'
import { handlePagesApiRoute } from '../src/server/pages-api-route.js'
...
describe('edge runtime globals', () => {
  ...
  it('lets a Pages API handler use AsyncLocalStorage as a global, like Text.js edge runtime', async () => {
    // This mirrors `.textjs-ref/test/e2e/edge-async-local-storage/` —
