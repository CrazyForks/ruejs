 ...
 *
 * Verifies that seedMemoryCacheFromPrerender() reads text-prerender.json
...
import { isrCacheKey, getRevalidateDuration } from '../src/server/isr-cache.js'
import { seedMemoryCacheFromPrerender } from '../src/server/seed-cache.js'

// ─── Helpers ──────────────────────────────────────────────────────────────────

function createTempServerDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'text-seed-cache-'))
}

/**
 * Write a text-prerender.json manifest and corresponding pre-rendered files
...
function setupPrerenderFixture(
  ...
  fs.writeFileSync(
    path.join(serverDir, 'text-prerender.json'),
...
/**
 * Write raw content to text-prerender.json (for corrupt manifest tests).
 */
function writeRawManifest(serverDir: string, content: string): void {
  fs.writeFileSync(path.join(serverDir, 'text-prerender.json'), content, 'utf-8')
}
...
describe('seedMemoryCacheFromPrerender', () => {
  ...
  it('is a no-op when text-prerender.json does not exist', async () => {
    const count = await seedMemoryCacheFromPrerender(serverDir)
    expect(count).toBe(0)
  })
