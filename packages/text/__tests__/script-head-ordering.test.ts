 ...
 * running before stylesheets parse. Rue Float hoists stylesheet/preload/
 ...
 *
 * text captures inline `<Script strategy="beforeInteractive">` content via
 ...
 * before any Rue-emitted resource hints. These tests pin that ordering:
 *
 ...
 * rendered inline by Rue in source order, where Rue Float's hoisted
...
describe('inline beforeInteractive head ordering', () => {
  ...
  it('emits the inline beforeInteractive <script> before the first stylesheet link', async () => {
    ...
    const scriptIndex = indexOfMatch(head, /<script\b[^>]*id="text-test-theme-init"/)
  ...
  it('emits the inline beforeInteractive <script> before every preload link', async () => {
    ...
    // Any preload link (stylesheet, script, font) emitted by Rue Float or
    // text must follow the inline script.
  ...
  it('does not duplicate the inline script inside <head>', async () => {
    const { html } = await fetchHtml(baseUrl, ROUTE)
    const head = extractHeadHtml(html)
    const matches = [...head.matchAll(/<script\b[^>]*id="text-test-theme-init"/g)]
  ...
  it("preserves the inline script's body content", async () => {
    const { html } = await fetchHtml(baseUrl, ROUTE)
    expect(html).toMatch(
      /<script\b[^>]*id="text-test-theme-init"[^>]*>self\.__textThemeInitRan = true;<\/script>/,
    )
  })
  ...
  it('preserves passthrough attributes like data-*', async () => {
    const { html } = await fetchHtml(baseUrl, ROUTE)
    const head = extractHeadHtml(html)
    const tagMatch = /<script\b[^>]*id="text-test-theme-init"[^>]*>/.exec(head)
    expect(tagMatch?.[0]).toContain('data-text-test="theme-init"')
  })
  ...
  it('applies the CSP nonce header to the hoisted inline script', async () => {
    ...
    // with that nonce. text extracts the nonce and threads it through
