...
import { StaticFileCache } from '../src/server/static-file-cache.js'
import { tryServeStatic } from '../src/server/prod-server.js'
...
describe('tryServeStatic (with StaticFileCache)', () => {
  ...
  beforeEach(async () => {
    clientDir = path.join(
      os.tmpdir(),
      `text-serve-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    )
  ...
  it('serves precompressed brotli for hashed assets when client accepts br', async () => {
    ...
    await writeFile(clientDir, '_text/static/app-abc123.js', jsContent)
    const brContent = zlib.brotliCompressSync(Buffer.from(jsContent))
    await writeFile(clientDir, '_text/static/app-abc123.js.br', brContent)
  ...
  it('serves precompressed gzip when client accepts gzip but not br', async () => {
    ...
    await writeFile(clientDir, '_text/static/styles-def456.css', cssContent)
    const gzContent = zlib.gzipSync(Buffer.from(cssContent))
    await writeFile(clientDir, '_text/static/styles-def456.css.gz', gzContent)
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/styles-def456.css',
      true,
      cache,
    )
  ...
  it('serves original file with Content-Length when no encoding accepted', async () => {
    ...
    await writeFile(clientDir, '_text/static/plain-ghi789.js', jsContent)
  ...
  it('preserves a non-200 status override for cached static files', async () => {
    ...
    await writeFile(clientDir, '_text/static/status-override-abc123.js', jsContent)
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/status-override-abc123.js',
      true,
      cache,
  ...
  it('sets immutable cache-control for /assets/ files', async () => {
    await writeFile(clientDir, '_text/static/chunk-aaa111.js', 'code'.repeat(100))

    const cache = await StaticFileCache.create(clientDir)
    const req = mockReq()
    const { res, captured } = mockRes()

    await tryServeStatic(req, res, clientDir, '/_text/static/chunk-aaa111.js', true, cache)
  ...
  it('sets Vary: Accept-Encoding when serving precompressed content', async () => {
    ...
    await writeFile(clientDir, '_text/static/vary-bbb222.js', jsContent)
    await writeFile(
      clientDir,
      '_text/static/vary-bbb222.js.br',
    ...
    await tryServeStatic(req, res, clientDir, '/_text/static/vary-bbb222.js', true, cache)
  ...
  it('blocks directory traversal attempts', async () => {
    await writeFile(clientDir, '_text/static/safe-ccc333.js', 'safe')
  ...
  it('returns 304 when If-None-Match matches the ETag', async () => {
    await writeFile(clientDir, '_text/static/cached-aaa111.js', 'cached content')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/cached-aaa111.js')
  ...
  it('returns 304 when If-None-Match contains the ETag in a comma-separated list', async () => {
    await writeFile(clientDir, '_text/static/cached-list-aaa111.js', 'cached content')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/cached-list-aaa111.js')
    const etag = entry!.etag
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/cached-list-aaa111.js',
      true,
      cache,
    )
  ...
  it('returns 200 when If-None-Match does not match', async () => {
    await writeFile(clientDir, '_text/static/stale-bbb222.js', 'new content')
  ...
  it('returns 200 when no If-None-Match header is present', async () => {
    await writeFile(clientDir, '_text/static/fresh-ccc333.js', 'fresh content')

    const cache = await StaticFileCache.create(clientDir)
    const req = mockReq()
    const { res, captured } = mockRes()

    await tryServeStatic(req, res, clientDir, '/_text/static/fresh-ccc333.js', true, cache)
  ...
  it('304 response excludes Content-Type per RFC 9110', async () => {
    await writeFile(clientDir, '_text/static/rfc-aaa111.js', 'rfc content')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/rfc-aaa111.js')

    const req = mockReq(undefined, { 'if-none-match': entry!.etag })
    const { res, captured } = mockRes()

    await tryServeStatic(req, res, clientDir, '/_text/static/rfc-aaa111.js', true, cache)
  ...
  it('304 response includes ETag and Cache-Control but no Content-Length', async () => {
    await writeFile(clientDir, '_text/static/headers-ddd444.js', 'header test content')

    const cache = await StaticFileCache.create(clientDir)
    const entry = cache.lookup('/_text/static/headers-ddd444.js')

    const req = mockReq(undefined, { 'if-none-match': entry!.etag })
    const { res, captured } = mockRes()

    await tryServeStatic(req, res, clientDir, '/_text/static/headers-ddd444.js', true, cache)
  ...
  it('HEAD request returns headers without streaming body', async () => {
    const jsContent = 'const x = 1;\n'.repeat(200)
    await writeFile(clientDir, '_text/static/head-eee555.js', jsContent)
  ...
  it('HEAD request includes compressed Content-Length when precompressed variant exists', async () => {
    const jsContent = 'const app = () => {};\n'.repeat(200)
    await writeFile(clientDir, '_text/static/head-br-fff666.js', jsContent)
    const brContent = zlib.brotliCompressSync(Buffer.from(jsContent))
    await writeFile(clientDir, '_text/static/head-br-fff666.js.br', brContent)

    const cache = await StaticFileCache.create(clientDir)
    const req = mockReq('br', undefined, 'HEAD')
    const { res, captured } = mockRes()

    await tryServeStatic(req, res, clientDir, '/_text/static/head-br-fff666.js', true, cache)
  ...
  it('prefers zstd over br when both accepted and available', async () => {
    ...
    await writeFile(clientDir, '_text/static/priority-hhh888.js', jsContent)
    await writeFile(
      clientDir,
    ...
    await writeFile(
      clientDir,
      '_text/static/priority-hhh888.js.br',
      zlib.brotliCompressSync(Buffer.from(jsContent)),
    )
    ...
    await tryServeStatic(req, res, clientDir, '/_text/static/priority-hhh888.js', true, cache)
  ...
  it('falls back to br when zstd accepted but no .zst file exists', async () => {
    ...
    await writeFile(clientDir, '_text/static/fallback-iii999.js', jsContent)
    await writeFile(
      clientDir,
      '_text/static/fallback-iii999.js.br',
      zlib.brotliCompressSync(Buffer.from(jsContent)),
    )
    // No .zst file
    ...
    await tryServeStatic(req, res, clientDir, '/_text/static/fallback-iii999.js', true, cache)
  ...
  it('slow path serves static file without cache', async () => {
    await writeFile(clientDir, '_text/static/nocache-aaa111.js', 'slow path content')
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/nocache-aaa111.js',
      false,
    )
  ...
  it('slow path serves HEAD without body', async () => {
    await writeFile(clientDir, '_text/static/head-slow-bbb222.js', 'head content')
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/head-slow-bbb222.js',
      false,
    )
  ...
  it('slow path serves HEAD without body for compressed response', async () => {
    await writeFile(clientDir, '_text/static/head-slow-comp-ccc333.js', 'compress me')
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/head-slow-comp-ccc333.js',
      true,
    )
  ...
  it('returns false for malformed percent-encoded pathname', async () => {
    await writeFile(clientDir, '_text/static/safe-ddd444.js', 'safe')
  ...
  it('slow path returns 304 for hashed asset with matching filename-hash ETag', async () => {
    await writeFile(clientDir, '_text/static/etag-slow-abc123.js', 'etag content')
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/etag-slow-abc123.js',
      true,
    )
  ...
  it('slow path returns 200 when ETag does not match', async () => {
    await writeFile(clientDir, '_text/static/etag-slow-miss-xyz999.js', 'fresh content')
    ...
    const served = await tryServeStatic(
      ...
      res,
      clientDir,
      '/_text/static/etag-slow-miss-xyz999.js',
      true,
    )
  ...
  it('slow path 304 includes Vary: Accept-Encoding for compressible content', async () => {
    await writeFile(clientDir, '_text/static/vary-slow-abc123.js', 'vary content')

    const req = mockReq(undefined, { 'if-none-match': 'W/"abc123"' })
    const { res, captured } = mockRes()

    await tryServeStatic(req, res, clientDir, '/_text/static/vary-slow-abc123.js', true)
