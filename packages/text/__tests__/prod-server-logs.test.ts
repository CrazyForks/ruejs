...
function createPagesBuild(): string {
  const root = fs.mkdtempSync(path.join(getTextTestTmpRoot(), 'text-prod-server-logs-'))
...
function createAppBuild(): string {
  const root = fs.mkdtempSync(path.join(getTextTestTmpRoot(), 'text-prod-server-app-logs-'))
  const distDir = path.join(root, 'dist')
  const clientDir = path.join(distDir, 'client')
  const serverDir = path.join(distDir, 'server')
...
describe('startProdServer logging', () => {
  ...
  it('keeps the generic production server log by default', async () => {
    ...
    expect(messages).toEqual([`[text] Production server running at http://127.0.0.1:${port}`])
  })

  it('prints a clickable local URL for wildcard binds', async () => {
    const root = createPagesBuild()
    roots.push(root)
    const messages: string[] = []
    vi.spyOn(console, 'log').mockImplementation((message: string) => {
  ...
  it('logs a prerender-specific production server URL when requested', async () => {
    ...
    expect(messages).toEqual([
      `[text] Production server for prerendering running at http://127.0.0.1:${port}`,
    ])
  })
