...
} from '../src/build/nitro-route-rules.js'
...
function findNamedPlugin(plugins: ReturnType<typeof text>, name: string) {
...
function createAppProject(): string {
  const root = makeTempProject('text-nitro-app-')
  writeProjectFile(
    root,
    'app/layout.tsx',
...
function createPagesProject(): string {
  const root = makeTempProject('text-nitro-pages-')
  writeProjectFile(
    root,
    'pages/index.tsx',
    'export default function Home() { return <div>home</div>; }\n',
...
async function initializeNitroSetupPlugin(root: string): Promise<NitroSetupPlugin> {
  const plugins = text({ appDir: root, rsc: false }) as ReturnType<typeof text>
  const configPlugin = findNamedPlugin(plugins, 'text:config') as Plugin & {
    config?: (
  ...
  if (!configPlugin?.config) {
    throw new Error('text:config plugin not found')
  }
  ...
  const nitroPlugin = findNamedPlugin(plugins, 'text:nitro-route-rules') as NitroSetupPlugin
  if (!nitroPlugin?.nitro?.setup) {
    throw new Error('text:nitro-route-rules plugin not found')
  }
...
describe('text Nitro setup integration', () => {
