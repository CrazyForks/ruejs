...
import { cookies, headers, setHeadersContext } from '../src/shims/headers.js'
import { isKnownDynamicAppRoute } from '../src/server/app-route-handler-runtime.js'
import {
  executeAppRouteHandler,
  runAppRouteHandler,
} from '../src/server/app-route-handler-execution.js'
...
describe('app route handler execution helpers', () => {
      ...
      const { dynamicUsedInHandler, response } = await runAppRouteHandler({
        ...
        async handlerFn(request) {
          ...
          return Response.json({
            ...
            search: request.textUrl.search,
            searchParam: request.textUrl.searchParams.get('token'),
          })
        },
        markDynamicUsage: dynamicUsage.markDynamicUsage,
        params: {},
  ...
  it('maps special route handler errors and reports generic failures', async () => {
    ...
    const redirectResponse = await executeAppRouteHandler({
      ...
      handler: { dynamic: 'auto' },
      handlerFn() {
        throw { digest: 'TEXT_REDIRECT;replace;%2Ftarget;308' }
      },
      isAutoHead: false,
      isProduction: true,
      isrRouteKey(pathname) {
