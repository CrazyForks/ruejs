...
} from '../src/server/app-route-handler-policy.js'
...
describe('app route handler policy helpers', () => {
  ...
  it('determines when route handler ISR cache reads are allowed', () => {
    ...
    expect(shouldReadAppRouteHandlerCache({ ...base, method: 'HEAD', isAutoHead: true })).toBe(true)
  ...
  it('determines when route handler cache headers and writes are allowed', () => {
    ...
    expect(shouldWriteAppRouteHandlerCache({ ...base, dynamicConfig: 'force-dynamic' })).toBe(false)
  ...
  it('maps special route handler digests to typed redirect and status results', () => {
    expect(
      resolveAppRouteHandlerSpecialError(
        { digest: 'TEXT_REDIRECT;replace;%2Ftarget%3Fok%3D1;308' },
    ...
    expect(
      resolveAppRouteHandlerSpecialError(
        { digest: 'TEXT_HTTP_ERROR_FALLBACK;401' },
        'https://example.com/source',
      ),
    ).toEqual({
      kind: 'status',
  ...
  it('classifies possible app-route action requests like Text.js', () => {
    expect(
      isPossibleAppRouteActionRequest(
