 ...
 *
 * Mirrors Text.js's `assetPrefix` behaviour:
 *  - Path prefix (e.g. `/custom-asset-prefix`): every emitted script/CSS URL
 *    starts with that prefix and lives under `_text/static/`. Fetching those URLs from the prod server
 *    must succeed.
 ...
 *  - Unset (default): URLs live under `/_text/static/`. On-disk layout mirrors the URL so the
 *    static-file layer can serve hits and naturally return plain-text
 *    `404 + "Not Found"` on misses.
 *
 * Ported from Text.js: test/e2e/app-dir/asset-prefix/asset-prefix.test.ts
...
} from '../src/utils/asset-prefix.js'
import { resolveAppRouterAssetPath } from '../src/server/prod-server.js'
import { normalizeAssetPrefix } from '../src/config/text-config.js'
...
describe('normalizeAssetPrefix', () => {
  ...
  it('adds a leading slash to bare path prefixes', () => {
    // Text.js accepts both `cdn` and `/cdn`; normalize to the leading-slash form.
    expect(normalizeAssetPrefix('cdn')).toBe('/cdn')
  })
...
describe('resolveAssetsDir', () => {
  it('returns `_text/static` when no prefix is configured', () => {
    expect(resolveAssetsDir('')).toBe(ASSET_PREFIX_URL_DIR)
  })

  it('returns `<prefix>/_text/static` for path prefixes so disk and URL align', () => {
    expect(resolveAssetsDir('/custom-asset-prefix')).toBe('custom-asset-prefix/_text/static')
    expect(resolveAssetsDir('/cdn')).toBe(`cdn/${ASSET_PREFIX_URL_DIR}`)
  })

  it('returns `_text/static` for absolute-URL prefixes — the CDN owns the URL prefix', () => {
...
describe('resolveAssetUrlPrefix', () => {
  it('returns `/_text/static/` when no prefix is configured', () => {
    expect(resolveAssetUrlPrefix('')).toBe('/_text/static/')
  })

  it('concatenates the prefix with the static dir for path prefixes', () => {
    expect(resolveAssetUrlPrefix('/cdn')).toBe('/cdn/_text/static/')
    expect(resolveAssetUrlPrefix('/custom-asset-prefix')).toBe('/custom-asset-prefix/_text/static/')
  })

  it('preserves the full URL for absolute-URL prefixes', () => {
    expect(resolveAssetUrlPrefix('https://cdn.example.com')).toBe(
      'https://cdn.example.com/_text/static/',
    )
    expect(resolveAssetUrlPrefix('https://cdn.example.com/sub')).toBe(
      'https://cdn.example.com/sub/_text/static/',
    )
  })
})
...
describe('resolveAppRouterAssetPath', () => {
  it('accepts `/_text/static/<file>` when no prefix is configured', () => {
    expect(resolveAppRouterAssetPath('/_text/static/foo-abc.js', '', '')).toBe(
      '/_text/static/foo-abc.js',
    )
    expect(resolveAppRouterAssetPath('/about', '', '')).toBeNull()
  })

  it('does not accept legacy static asset requests', () => {
    expect(resolveAppRouterAssetPath('/_legacy/static/foo-abc.js', '', '')).toBeNull()
  })
  ...
  it('recognises `<prefix>/_text/static/<file>` for path-prefix configs', () => {
    expect(
      resolveAppRouterAssetPath(
        '/custom-asset-prefix/_text/static/foo-abc.js',
        '/custom-asset-prefix',
        '/custom-asset-prefix',
      ),
    ).toBe('/custom-asset-prefix/_text/static/foo-abc.js')
    expect(
      resolveAppRouterAssetPath(
        '/other-prefix/_text/static/foo-abc.js',
        '/custom-asset-prefix',
        '/custom-asset-prefix',
      ),
    ).toBeNull()
  ...
  it('strips the path component for absolute-URL prefixes with a path', () => {
    // Disk layout for absolute-URL prefixes is `_text/static/...` (no
    ...
    expect(
      resolveAppRouterAssetPath(
        '/sub/_text/static/foo-abc.js',
        '/sub',
        'https://cdn.example.com/sub',
      ),
    ).toBe('/_text/static/foo-abc.js')
  })

  it('accepts `/_text/static/<file>` for absolute-URL prefixes without a path', () => {
    expect(
      resolveAppRouterAssetPath('/_text/static/foo-abc.js', '', 'https://cdn.example.com'),
    ).toBe('/_text/static/foo-abc.js')
  })
...
describe('isTextStaticPath', () => {
  it('matches `/_text/static/*` at the root', () => {
    expect(isTextStaticPath('/_text/static/foo-abc.js', '', '')).toBe(true)
    expect(isTextStaticPath('/_text/static/chunks/main.js', '', '')).toBe(true)
    expect(isTextStaticPath('/_text/static/chunks/main.js', '', '')).toBe(true)
  })

  it('does not match unrelated paths', () => {
    expect(isTextStaticPath('/', '', '')).toBe(false)
    expect(isTextStaticPath('/about', '', '')).toBe(false)
    expect(isTextStaticPath('/_text/data/foo.json', '', '')).toBe(false)
    // `/_text/static` alone (no trailing slash) is not a static-asset path.
    expect(isTextStaticPath('/_text/static', '', '')).toBe(false)
  })

  it('strips basePath before matching', () => {
    expect(isTextStaticPath('/docs/_text/static/foo.js', '/docs', '')).toBe(true)
    expect(isTextStaticPath('/docs/about', '/docs', '')).toBe(false)
    // basePath set but request without basePath — still matches the root form.
    expect(isTextStaticPath('/_text/static/foo.js', '/docs', '')).toBe(true)
  })

  it('strips assetPathPrefix before matching', () => {
    expect(isTextStaticPath('/cdn/_text/static/foo.js', '', '/cdn')).toBe(true)
    expect(isTextStaticPath('/cdn/other', '', '/cdn')).toBe(false)
  })

  it('strips basePath then assetPathPrefix (independent prefixes)', () => {
    expect(isTextStaticPath('/docs/cdn/_text/static/foo.js', '/docs', '/cdn')).toBe(true)
  })

  it('does not partial-match prefixes', () => {
    // `/baseball` should NOT be stripped by `basePath: "/base"`.
    expect(isTextStaticPath('/baseball/_text/static/foo.js', '/base', '')).toBe(false)
  })
})
 ...
 * `text.config.ts` with extra config keys.
 *
 * The fixture is copied so tests can mutate text.config.ts independently
...
async function buildFixtureWithConfig(
  ...
  const tmpDir = mkdtempInTextPackage('text-asset-prefix-')
  ...
  // Symlink the workspace node_modules so the fixture can resolve Rue,
  // text, and @vitejs/plugin-rsc.
  linkWorkspaceNodeModules(path.join(fixtureRoot, 'node_modules'))
  linkLocalPackageDependencies(fixtureRoot)

  // Patch text.config.ts to add the extra keys. We splice them in right after
...
describe('assetPrefix end-to-end build', () => {
  ...
  it('path-prefix: emits assets under <prefix>/_text/static/ on disk and in HTML', async () => {
    ...
    const onDiskStatic = path.join(built.outDir, 'client', 'custom-asset-prefix', '_text', 'static')
    ...
    try {
      ...
      // Bootstrap is emitted as `<script type="module" src="…">` via Rue's
      // `bootstrapModules` option (see app-ssr-entry.ts). Mirrors the Text.js
      // fixture assertion:
      //   src?.includes('/custom-asset-prefix/_text/static') ||
      //   src?.includes('/custom-asset-prefix/_text/static/immutable')
      ...
      expect(bootstrapMatch![1]).toMatch(/^\/custom-asset-prefix\/_text\/static\//)
      ...
      for (const src of scriptSrcs) {
        expect(src.startsWith('/custom-asset-prefix/_text/static/')).toBe(true)
      }
  ...
  it('absolute URL: emits fully-qualified asset URLs and never includes /assets/', async () => {
    ...
    // Disk layout is just _text/static/ — the CDN owns the URL prefix.
    const onDiskStatic = path.join(built.outDir, 'client', '_text', 'static')
    expect(fs.existsSync(onDiskStatic), `expected on-disk layout under ${onDiskStatic}`).toBe(true)
    ...
    try {
      ...
      expect(bootstrapMatch![1]).toMatch(/^https:\/\/cdn\.example\.com\/_text\/static\//)
  ...
  it('basePath + assetPrefix: routes under basePath, assets under assetPrefix', async () => {
    ...
    const onDiskStatic = path.join(built.outDir, 'client', 'cdn-prefix', '_text', 'static')
    expect(fs.existsSync(onDiskStatic), `expected on-disk layout under ${onDiskStatic}`).toBe(true)
    ...
    try {
      ...
      expect(bootstrapMatch![1]).toMatch(/^\/cdn-prefix\/_text\/static\//)
  ...
  it('basePath alone: falls back to using basePath as assetPrefix', async () => {
    // Text.js parity — `basePath: "/app"` with no `assetPrefix` should serve
    // assets from `/app/_text/static/...` (NOT `/assets/...` and NOT
    // `/app/assets/...`). The fallback in resolveTextConfig() rewrites
    // `assetPrefix = basePath`, which feeds the rest of the pipeline.
    const built = await buildFixtureWithConfig(`basePath: "/app",`, register)

    // On-disk layout mirrors the asset URL: <basePath>/_text/static/...
    const onDiskStatic = path.join(built.outDir, 'client', 'app', '_text', 'static')
    expect(fs.existsSync(onDiskStatic), `expected on-disk layout under ${onDiskStatic}`).toBe(true)
    // Legacy `<basePath>/assets/` directory should NOT exist — the fallback
    // moves assets to the Text.js-canonical location.
    ...
    try {
      ...
      const bootstrapMatch = html.match(/<script[^>]+type="module"[^>]+src="([^"]+\.js)"/)
      expect(bootstrapMatch).not.toBeNull()
      // Asset URL lives under basePath at text's `_text/static/` path.
      expect(bootstrapMatch![1]).toMatch(/^\/app\/_text\/static\//)
  ...
  it('unset: emits URLs under /_text/static/', async () => {
    // Default behaviour — no assetPrefix set. URLs and on-disk layout both
    // use text's `_text/static/` path so requests for valid
    // chunks succeed and invalid ones produce a plain-text 404 from the
    // static-file layer (matching Text.js's router-server.ts).
    ...
    // removes Vite's historical default in favour of Text.js parity.
    ...
    try {
      ...
      const bootstrapMatch = html.match(/<script[^>]+type="module"[^>]+src="([^"]+\.js)"/)
      expect(bootstrapMatch).not.toBeNull()
      expect(bootstrapMatch![1]).toMatch(/^\/_text\/static\//)
      ...
      // Invalid `_text/static/*` paths return plain-text 404 (Text.js
      // parity — see packages/text/src/server/lib/router-server.ts).
      const invalidRes = await fetch(`${baseUrl}/_text/static/does-not-exist.js`)
  ...
  // Text.js parity fallback set `assetPrefix = basePath`, every asset 404'd.
  //
  ...
  it('Pages Router with basePath alone serves assets under <basePath>/_text/static/', async () => {
    const tmpDir = mkdtempInTextPackage('text-pages-asset-prefix-')
    register(() => fs.rmSync(tmpDir, { recursive: true, force: true }))

    // Spin up a minimal Pages Router fixture inline rather than copy
    // __tests__/fixtures/pages-basic — that fixture pulls in a large config
...
`,
    ...
    try {
      ...
      // `<basePath>/_text/static/`, which is text's static asset layout.
