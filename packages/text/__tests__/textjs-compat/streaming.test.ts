/**
 * Text.js Compatibility Tests: streaming / loading.tsx
 *
 ...
 * - fixtures/app-basic/app/textjs-compat/streaming-test/
 * - fixtures/app-basic/app/textjs-compat/streaming-test/nested/
 */
...
describe('Text.js compat: streaming', () => {
  ...
  beforeAll(async () => {
    ...
    await fetchHtml(baseUrl, '/textjs-compat/streaming-test')
  }, 60_000)
  ...
  // ── Streaming / Suspense ────────────────────────────────────
  // Text.js: streaming SSR with Suspense boundaries
  ...
  it('streaming page returns 200 with content', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/streaming-test')
  ...
  it('streamed content appears in final response', async () => {
    // Text.js: streaming SSR with Suspense boundaries
    // Source: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/app-rendering/rendering.test.ts
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/streaming-test')
  ...
  it('nested streaming page returns 200', async () => {
    // Text.js: streaming SSR with Suspense boundaries
    // Source: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/app-rendering/rendering.test.ts
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/streaming-test/nested')
    expect(res.status).toBe(200)
  ...
  it('nested streamed content resolves', async () => {
    // Text.js: streaming SSR with Suspense boundaries
    // Source: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/app-rendering/rendering.test.ts
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/streaming-test/nested')
    expect(html).toContain('Content A loaded')
    expect(html).toContain('Content B loaded')
  })

  // ── loading.tsx ─────────────────────────────────────────────
  // Text.js: loading.tsx acts as a route-level Suspense boundary
  // Source: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/app-rendering/rendering.test.ts
