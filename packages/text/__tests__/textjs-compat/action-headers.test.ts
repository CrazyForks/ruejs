/**
 * Text.js Compatibility Tests: headers() and cookies() in Server Actions
 *
 * Ported from Text.js behavior: headers() and cookies() must be accessible
 * from Server Actions, not just Server Components and Route Handlers.
 *
 * Related Text.js tests:
...
import { createRueBrowserActionProtocol } from '../../src/server/app-rsc-browser-action-protocol-core.js'
 ...
 *
 * Rue serializes server-action references as hidden inputs whose `name`
 * attribute encodes the action ID directly:
 *
 *   <input type="hidden" name="$RUE_ACTION_ID_/app/path/to/page.tsx#$$hoist_0_formGetHeader"/>
 *
 * The action ID (sent as the `x-rsc-action` header) is everything after
 * the `$RUE_ACTION_ID_` prefix.
 *
...
function extractActionId(html: string, exportHint: string): string | undefined {
  const re = /name="\$RUE_ACTION_ID_([^"]+)"/g
  let m
...
async function invokeAction(
  ...
  const actionProtocol = createRueBrowserActionProtocol()
  const res = await fetch(url, {
    ...
    body: await actionProtocol.encodeActionArgs(args),
  })
 ...
 */
async function extractReturnValue(text: string): Promise<unknown> {
  try {
    const decoded = await decodeRuePayloadReadableStream<{
      returnValue?: { data?: unknown }
    }>(new Response(text).body!)
    return decoded.returnValue?.data
  } catch {
    // Fall through to the legacy text parser for older compatibility payloads.
  }
...
describe('Text.js compat: headers() and cookies() in Server Actions', () => {
  ...
  it('action-headers page renders without error', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/action-headers')
    expect(res.status).toBe(200)
    expect(html).toContain('Action Headers Test')
  })

  it('action-headers page exposes form action IDs for both actions', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/action-headers')
  ...
  // Text.js behaviour: headers() MUST resolve inside a server action (phase="action").
  ...
  it('getHeaderFromAction returns the request header value', async () => {
    const actionId = '/app/textjs-compat/action-headers/actions.ts#getHeaderFromAction'
    ...
    //   0:{"root":"$@1","returnValue":{"ok":true,"data":"<value>"}}
    const value = await extractReturnValue(text)
    expect(value).toBe('returned-header-value')
  })

  it('getCookieFromAction returns the cookie value', async () => {
    const actionId = '/app/textjs-compat/action-headers/actions.ts#getCookieFromAction'
  ...
  it('headers() and cookies() both work in the same route handler invocation', async () => {
    const res = await fetch(`${baseUrl}/textjs-compat/api/headers-in-route`, {
      headers: {
