/**
 * Text.js Compatibility Tests: global-not-found (basic)
 *
 ...
 *
 * `app/global-not-found.tsx` is a Text.js 16 feature (originally behind
 ...
 *     layout — see `createNotFoundLoaderTree` in Text.js:
 ...
 *
 * Fixture: `__tests__/fixtures/global-not-found-basic/` — minimal app with a root
...
describe('Text.js compat: global-not-found (basic)', () => {
  ...
  // Ported from Text.js: test/e2e/app-dir/global-not-found/basic/global-not-found-basic.test.ts
  ...
  it('does not use the global-not-found document when notFound() is called from a page', async () => {
    ...
    //
    // NOTE: Text.js parity goes further — it renders the default `404 / This
    // page could not be found.` inside the root layout (so `<html lang="en">`
    // is present). Text currently returns a plain "Not Found" body for
