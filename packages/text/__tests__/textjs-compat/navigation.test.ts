/**
 * Text.js Compatibility Tests: navigation
 *
 ...
 *
 * NOTE: The vast majority of Text.js navigation tests are browser-based
 ...
 * - fixtures/app-basic/app/textjs-compat/nav-redirect-server/ (new)
 * - fixtures/app-basic/app/textjs-compat/nav-redirect-result/ (new)
 * - fixtures/app-basic/app/textjs-compat/nav-notfound-server/ (new)
 */
...
describe('Text.js compat: navigation', () => {
  ...
  // Text.js: 'should redirect in a server component'
  ...
  it('redirect() in server component produces 307', async () => {
    const res = await fetch(`${baseUrl}/textjs-compat/nav-redirect-server`, { redirect: 'manual' })
    expect(res.status).toBe(307)
    expect(res.headers.get('location')).toContain('/textjs-compat/nav-redirect-result')
  })

  it('redirect destination page renders correctly', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/nav-redirect-result')
    expect(html).toContain('Result Page')
  })

  // ── Server-side notFound ─────────────────────────────────────
  // Text.js: 'should trigger not-found in a server component'
  ...
  it('notFound() in server component produces 404', async () => {
    const res = await fetch(`${baseUrl}/textjs-compat/nav-notfound-server`)
  ...
  // Text.js: 'should contain default meta tags in error page'
  ...
  //
  // N/A: text.config.js redirects
  //   Tests config-based redirects, not supported in text the same way
  //
