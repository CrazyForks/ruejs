# Text.js Compatibility Test Tracking
...
**Local**: `tests/textjs-compat/app-rendering.test.ts`
**Fixtures**: `fixtures/app-basic/app/textjs-compat/`

| #   | Text.js Test                                             | Text Status | Notes                                                                                                                                                                      |
| --- | -------------------------------------------------------- | ----------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | should serve app/page.server.js at /                     | PASS        | Mapped to `/textjs-compat` sub-route                                                                                                                                       |
...
| 8   | mixed static and dynamic                                 | SKIP (N/A)  | Also skipped in Text.js source                                                                                                                                             |

**Result: 6/8 pass, 1 skip (text issue), 1 skip (N/A)**

### Findings

- **Rue `use()` hook warning**: All pages using `use(getData())` emit "Invalid hook call" warnings in stderr. The data renders correctly, but there's likely a duplicate Rue instance in the RSC environment. Not blocking but should be investigated.
...
**Local**: `tests/textjs-compat/not-found.test.ts`
**Fixtures**: `fixtures/app-basic/app/textjs-compat/not-found-*`
...
| 5   | Dynamic index page renders                          | PASS        | /textjs-compat/not-found-dynamic                                        |
...
| 16  | Build output: file traces, pages manifest           | N/A         | Text.js-specific .text/ build structure                                 |
| 17  | Edge runtime variant                                | N/A         | Text tests edge via separate Cloudflare projects                        |
...
**Local**: `tests/textjs-compat/global-error.test.ts`
**Fixtures**: `fixtures/app-basic/app/textjs-compat/global-error-{rsc,ssr}/`, `metadata-error-{with,without}-boundary/`

| #   | Text.js Test                                                          | Text Status | Notes                                                                                                                     |
| --- | --------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------- |
| 1   | error-server-test: server component throw caught by error.tsx         | PASS        | Verified in dev + production preview: text renders the local `error.tsx` boundary with HTTP 200.                          |
| 2   | error-nested-test: nested error caught by inner error.tsx             | PASS        | Verified in dev + production preview: text resolves to the nearest matching `error.tsx`, not the parent boundary.         |
| 3   | Server component throw without local error.tsx returns a response     | PASS        | Verified in dev + production preview: text renders `global-error.tsx` with HTTP 200 when no local boundary exists.        |
| 4   | Client component SSR throw without local error.tsx returns a response | PASS        | Verified in dev + production preview: text renders `global-error.tsx` with HTTP 200 for SSR-time client component throws. |
| 5   | generateMetadata() error caught by local error.tsx boundary           | PASS        | Verified in dev + production preview: text renders the co-located `error.tsx` boundary with HTTP 200.                     |
| 6   | generateMetadata() error without local boundary returns a response    | PASS        | Verified in dev + production preview: text escalates to `global-error.tsx` with HTTP 200 when no local boundary exists.   |
...
| 9   | Dev-only Redbox display verification                                  | N/A         | Tests Text.js-specific dev overlay format, not applicable                                                                 |
...
- **Tracked skips were stale**: Current text already matches Text.js for the HTTP-level cases in this chunk. The test file was green in dev, and production preview matches the same 200-status boundary behavior.
- **Server component and `generateMetadata()` errors use separate paths**: Server/render errors are rerouted through `renderErrorBoundaryPage()` from the RSC/SSR pipeline in `packages/text/src/entries/app-rsc-entry.ts`, while page-level `generateMetadata()` errors surface earlier from `buildPageElement()` and are routed to the same boundary renderer by a different catch path.
- **Remaining gaps are browser-only**: The unported cases are still the client-triggered `global-error` interactions and Text.js-specific dev redbox assertions, not server-side HTTP semantics.

---

## Chunk 4: dynamic (text/dynamic)

**Source**: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/dynamic/dynamic.test.ts
**Local**: `tests/textjs-compat/dynamic.test.ts`
**Fixtures**: `fixtures/app-basic/app/textjs-compat/dynamic/`

| #   | Text.js Test                                               | Text Status | Notes                                                           |
| --- | ---------------------------------------------------------- | ----------- | --------------------------------------------------------------- |
| 1   | SSR: lazy/Suspense loaded content                          | PASS        | "text-dynamic lazy" rendered in SSR HTML                        |
| 2   | SSR: dynamic() server component content                    | PASS        | "text-dynamic dynamic on server" rendered                       |
| 3   | SSR: dynamic() client component content                    | PASS        | "text-dynamic dynamic on client" rendered                       |
| 4   | SSR: dynamic() server-imported client content              | PASS        | "text-dynamic server import client" rendered                    |
| 5   | SSR: ssr:false content NOT in HTML                         | PASS        | "text-dynamic dynamic no ssr on client" absent from SSR         |
...
| 10  | should handle text/dynamic in hydration correctly          | N/A         | Requires Playwright — ssr:false content appears after hydration |
...
| 14  | should ignore text/dynamic in routes                       | N/A         | Route handlers covered in Chunk 5                               |
| 15  | should ignore text/dynamic in sitemap                      | N/A         | Sitemap generation not in scope                                 |
...
- **text/dynamic works well in App Router SSR**: All four dynamic import patterns (lazy/Suspense, server dynamic, client dynamic, server-importing-client) render correctly in SSR HTML.
- **ssr: false correctly excluded**: Content from `dynamic(() => import(...), { ssr: false })` is properly excluded from SSR HTML, matching Text.js behavior.
...
**Local**: `tests/textjs-compat/app-routes.test.ts`
**Fixtures**: `fixtures/app-basic/app/textjs-compat/api/*` (new), `fixtures/app-basic/app/api/*` (pre-existing)
...
| 12    | TextResponse.redirect()                            | PASS        | Returns 307 with Location header                                |
| 13    | TextResponse.json()                                | PASS        | Returns JSON with content-type header                           |
...
- **TextResponse helpers work**: `redirect()`, `json()` all produce correct responses.
...
**Local**: `tests/textjs-compat/metadata.test.ts`
**Fixtures**: `fixtures/app-basic/app/textjs-compat/metadata-*`
...
| 8     | Keywords meta tag                          | PASS        | Joins with ", " (space after comma) vs Text.js "," — both valid      |
...
| 28    | Hreflang alternate links                   | PASS        | Rue renders as `hrefLang` (camelCase)                                |
...
- **Minor formatting differences**:
  - Keywords joined with `", "` (space after comma) vs Text.js `","` — both valid HTML
  - Rue JSX renders `hrefLang` (camelCase) in HTML output instead of `hreflang` — browsers handle both
...
**Local**: `tests/textjs-compat/navigation.test.ts`
**Fixtures**: `fixtures/app-basic/app/textjs-compat/nav-*`
...
- **Navigation tests are overwhelmingly browser-based**: >80% of the Text.js navigation test suite requires Playwright for client-side interactions. The HTTP-level tests here provide a baseline confirming server-side navigation primitives work.
...
**Not ported — already covered by existing text tests.**

The Text.js test is ~25 cases, almost entirely browser-based (client-side nav, back/forward, URL bar, prefetch, loading states). The ~3 SSR-testable patterns (nested parallel slot matching, route group + parallel slots, 404 on direct slot access) are already covered by `tests/app-router.test.ts` (lines 141-288) with 13 existing tests for parallel routes and intercepting routes using pre-existing fixtures in `fixtures/app-basic/app/dashboard/@team/`, `@analytics/`, and `feed/@modal/`.

### Chunk 9: app (main kitchen sink)

**Source**: https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/app/index.test.ts

**Not ported — ~90% overlap with existing text tests.**

The Text.js test is a massive kitchen-sink with 50+ cases. The SSR-testable patterns (dynamic routes, catch-all, layouts, client component SSR, loading.tsx, search params, 404, metadata, RSC content-type) are already thoroughly covered by `tests/app-router.test.ts` and `tests/textjs-compat/*.test.ts` chunks 1-7. The remaining tests are browser-only (Link, HMR, client-side nav, rewrites, middleware).
...
The Text.js test is ~40+ cases focused on production build artifacts, ISR cache behavior, `revalidateTag`/`revalidatePath`, fetch caching configs, and prerender manifests. None of these apply to dev SSR testing. The few dev-testable patterns (`dynamicParams`, `generateStaticParams`, `force-dynamic/force-static`) are already covered by `tests/app-router.test.ts` (lines 711-850).
...
**Config**: `tests/e2e/app-router/textjs-compat/playwright.textjs-compat.config.ts`
**Run**: `node node_modules/@playwright/test/cli.js test -c tests/e2e/app-router/textjs-compat/playwright.textjs-compat.config.ts`
**Prereq**: Build text (`vp run text#build`) and start dev server (`npx vite --port 4174` from `fixtures/app-basic`)

### Chunk 4: dynamic (Playwright)

**Local**: `tests/e2e/app-router/textjs-compat/dynamic.spec.ts`

| #   | Test                                                      | Status | Notes                                                                   |
| --- | --------------------------------------------------------- | ------ | ----------------------------------------------------------------------- |
| 1   | ssr:false component appears after hydration               | PASS   | `#css-text-dynamic-no-ssr-client` visible after `__TEXT_RSC_ROOT__` set |
...
**Local**: `tests/e2e/app-router/textjs-compat/metadata.spec.ts`
...
**Local**: `tests/e2e/app-router/textjs-compat/navigation.spec.ts`
...
| 4   | client-side notFound() trigger renders not-found component | **SKIP** | Client-side `notFound()` from "use client" component crashes Rue tree instead of rendering not-found boundary. Body shows raw Vite RSC entry text. Fix: `packages/text/src/shims/navigation.ts` — need client-side NotFoundBoundary that catches TEXT_NOT_FOUND error. |
...
1. **RSC module caching across requests** — `Date.now()` cached in dev. Fix: `packages/text/src/entries/app-rsc-entry.ts`
...
4. ~~**Rue `use()` hook warning**~~ — **FIXED**. Not duplicate Rue — the pre-render check (`entries/app-rsc-entry.ts:1268`) calls `PageComponent()` directly outside Rue's render cycle, triggering "Invalid hook call" for components using `use()`. Fix: suppress the expected warning during the pre-render test.
5. ~~**Keywords separator formatting**~~ — **FIXED**. Changed `metadata.keywords.join(", ")` to `.join(",")` in `metadata.tsx:338` to match Text.js behavior.
6. ~~**Client-side notFound() crashes Rue tree**~~ — **FIXED**. Added `NotFoundBoundary` class component to `error-boundary.tsx` that catches `TEXT_NOT_FOUND`/`TEXT_HTTP_ERROR_FALLBACK;404` errors. Wrapped in `buildPageElement()` above `ErrorBoundary`, with pre-rendered not-found.tsx element as fallback. Playwright test now passes.
7. ~~**useParams() returns empty on client after hydration**~~ — **FIXED**. Root cause: `setClientParams()` was never called in the browser. Fix: server now sends `X-Text-Params` header in RSC responses (`entries/app-rsc-entry.ts:1295`), browser entry reads it during hydration and client-side navigation (`entries/app-rsc-entry.ts:1594-1597, 1612-1617`). All 3 Playwright useParams tests now pass.
8. ~~**Client-side error.tsx boundary doesn't activate during navigation**~~ — **FIXED**. Added `onCaughtError: function() {}` to `hydrateRoot()` call in `generateBrowserEntry()` to suppress Vite dev overlay for errors caught by Rue error boundaries. Combined with PR #51's `renderErrorBoundaryPage()` for SSR-side error rendering.

---

## Phase 2: Additional Test Chunks (COMPLETE)

Gap analysis against the full Text.js e2e/app-dir suite (365 test dirs) identified these
high-value areas where text **implements the feature** but had thin or no Text.js-compat
...
**Text.js sources**: `hooks`, `use-params`, `use-selected-layout-segment-s`, `params-hooks-compat`
**Local**: `tests/textjs-compat/hooks.test.ts`, `tests/e2e/app-router/textjs-compat/hooks.spec.ts`
...
Existing text tests already thoroughly cover forbidden/unauthorized: unit tests for
throw/digest, SSR integration tests for 403/401 rendering with custom boundaries, and
Playwright E2E status code checks. No additional Text.js-compat tests needed.

### Chunk 13: rsc-basic — Server/client component fundamentals ✅

**Text.js sources**: `rsc-basic`, `rsc-query-routing`, `rsc-redirect`
**Local**: `tests/textjs-compat/rsc-basic.test.ts`, `tests/e2e/app-router/textjs-compat/rsc-basic.spec.ts`
...
**Text.js sources**: `error-boundary-navigation`, `errors`
**Local**: `tests/e2e/app-router/textjs-compat/error-nav.spec.ts`
...
suppress Vite dev overlay for Rue-caught errors.

### Chunk 15: streaming / loading.tsx — Suspense boundaries during SSR ✅

**Text.js sources**: `app-rendering` (streaming subset), `searchparams-reuse-loading`,
`app-prefetch-false-loading`, `root-suspense-dynamic`
**Local**: `tests/textjs-compat/streaming.test.ts`, `tests/e2e/app-router/textjs-compat/streaming.spec.ts`
...
**Text.js sources**: `set-cookies`
**Local**: `tests/textjs-compat/set-cookies.test.ts`
...
**Text.js sources**: `app-css`, `tailwind-css`, `css-order`, `css-modules-scoping`
**Local**: `tests/textjs-compat/app-css.test.ts`
...
**Text.js sources**: `draft-mode`, `draft-mode-middleware`
**Local**: `tests/textjs-compat/draft-mode.test.ts`
**Result**: Vitest 4/4 pass
...
### Text Feature Audit Summary

Conducted a full audit of text's feature surface against 25 Text.js capabilities:

| Feature                               | Text           | Tested                    | Phase 2 Chunk | Phase 5        |
| ------------------------------------- | -------------- | ------------------------- | ------------- | -------------- |
...
| text/image                            | PARTIAL        | **Unit (55 tests)**       | —             | **P5-2, P5-3** |
| text/link (prefetch)                  | YES            | **Unit (24 tests) + e2e** | —             | **P5-1**       |
| text/script                           | YES            | **Unit (9 tests) + e2e**  | —             | **P5-10**      |
...
| text/head                             | YES            | **Unit (26 tests) + e2e** | —             | **P5-4**       |
| useSelectedLayoutSegment(s)           | YES            | Unit + integration        | 11            | —              |
| text/dynamic                          | YES            | **Unit (11 tests) + e2e** | —             | **P5-9**       |
| text/form                             | YES            | **Unit (6 tests)**        | —             | **P5-11**      |
...
Targeted the top 10 gaps from the coverage analysis — features text implements
but had no Text.js-compat test coverage for.

### Chunk 19: actions-navigation — Server action after client navigation ✅

**Text.js source**: `actions-navigation`
**Local**: `tests/e2e/app-router/textjs-compat/actions-nav.spec.ts`
...
**Text.js sources**: `actions-revalidate-remount`, `revalidatetag-rsc`
**Local**: `tests/textjs-compat/revalidate.test.ts`, `tests/e2e/app-router/textjs-compat/actions-revalidate.spec.ts`
...
**Text.js source**: `app-prefetch`
**Local**: `tests/textjs-compat/prefetch.test.ts`, `tests/e2e/app-router/textjs-compat/prefetch.spec.ts`
...
**Text.js source**: `metadata-suspense`
**Local**: `tests/textjs-compat/metadata-suspense.test.ts`
...
**1 skipped**: Duplicate `<title>` tags. Text emits the metadata twice — once in
...
**Text.js source**: `revalidate-dynamic`
**Local**: `tests/textjs-compat/revalidate.test.ts` (shared with Chunk 20)
...
**Text.js source**: `external-redirect`
**Local**: `tests/e2e/app-router/textjs-compat/external-redirect.spec.ts`
**Result**: Playwright 0/1 pass, 1 skip

**Skipped**: Text handles server action redirects via `x-action-redirect` headers
...
### Chunk 25: search-params-rue-key — Component stability across param changes ✅

**Text.js source**: `search-params-rue-key`
**Local**: `tests/e2e/app-router/textjs-compat/search-params-key.spec.ts`
...
**Text.js source**: `concurrent-navigations`
...
Text.js-specific and requires `createRouterAct` test infrastructure.
...
9. **Duplicate `<title>` tags with Suspense layout** — When a layout wraps children in `<Suspense>`, metadata `<title>` tag appears twice. Fix: `packages/text/src/entries/app-rsc-entry.ts` — hoist metadata rendering above Suspense boundaries.
10. **External redirect in server actions** — `redirect('https://example.com')` inside a server action does client-side RSC navigation instead of full page navigation. Fix: `packages/text/src/entries/app-rsc-entry.ts` browser entry — detect external origin and use `window.location.href`.

---

## Phase 4: OpenText Cloudflare Compatibility Tests

Ported from: https://github.com/opennextjs/opennextjs-cloudflare/tree/main/examples/e2e

### Background

[OpenText](https://github.com/opennextjs) deploys Text.js apps to non-Vercel platforms (Cloudflare
Workers, AWS Lambda, etc.). Their E2E test suites are **behavioral conformance tests** — they verify
that Text.js features work correctly across different deployment targets. These assertions (what the
user sees, what headers are returned, how caching behaves) are framework-agnostic and portable,
making them a good reference for text's own compatibility testing.

**Approach**: We port the _test patterns and assertions_ into our existing Playwright specs, running
against our Vite-based fixture apps. Each test links back to the OpenText source for reference.
...
| #   | OpenText Test                                                     | Text Status | Notes                                                    |
| --- | ----------------------------------------------------------------- | ----------- | -------------------------------------------------------- |
| 1   | ISR page returns HIT on prebuilt path (`dynamicParams=true`)      | PASS        | `x-text-cache: HIT` verified for `/products/1`           |
| 2   | ISR page returns MISS on non-prebuilt path (`dynamicParams=true`) | PASS        | New path gets `MISS` then `HIT` on text request          |
...
| 7   | OPTIONS returns 204 with Allow header            | PASS        | Auto-OPTIONS now sorts the Allow list to match Text.js and 405 responses no longer emit Allow           |
...
| 13  | Static GET route has `s-maxage` Cache-Control    | FIXME       | text does not read `revalidate` from route handler modules                                              |
| 14  | Revalidation timing in GET route handler         | FIXME       | Same: route handler cache headers not implemented                                                       |
| 15  | Route handler default export is ignored          | PASS        | Text.js parity: default export is ignored for dispatch (dev warning only); unmatched methods return 405 |
...
| 4   | `x-middleware-text` NOT in response headers       | PASS        | Internal header stripped                                |
| 5   | Request headers available in RSC                  | PASS        | Already tested                                          |
| 6   | `text.config.js` headers applied to response      | PASS        | Covered by ON-15 tests in config-redirect.spec.ts       |
| 7   | `x-powered-by` absent from responses              | PASS        | text never sends X-Powered-By; explicit assertion added |

### ON-7: text/after Deferred Work
...
| #   | OpenText Test                                       | Text Status | Notes                                                                                                                                    |
| --- | --------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `text.config.js` headers set on response            | PASS        | Covered by ON-15 tests in config-redirect.spec.ts                                                                                        |
| 2   | Middleware headers override config headers          | PASS        | Middleware headers always win over text.config.js headers for the same key (matches Text.js behavior); tested in config-redirect.spec.ts |
| 3   | `x-powered-by` absent when `poweredByHeader: false` | PASS        | text never sends X-Powered-By; explicit assertion added                                                                                  |
...
| ON-7. text/after            | 3      | 3      | 0     | 0       | 0     | `after.test.ts`                                 |
...
- **Fixme**: Tests written but marked `test.fixme()` due to text feature gaps
- **Pending**: Need additional fixture/config work beyond what was created
- **Skip**: Known text limitation (pre-existing)

### Known Feature Gaps (Fixme)

| Feature                | Test    | Issue                                                |
| ---------------------- | ------- | ---------------------------------------------------- |
| OPTIONS + Allow header | ON-3 #7 | text auto-OPTIONS does not set Allow header          |
...
- `tests/fixtures/app-basic/app/api/after-test/route.ts` — `text/after` deferred work route
...
- `tests/e2e/app-router/after.spec.ts` — `text/after` deferred work tests

**Test files (enhanced with OpenText compat tests):**
...
| 5   | Middleware rewrite with custom status code        | FIXME       | text drops status from `TextResponse.rewrite(url, { status })` |
...
**Fixtures**: `tests/fixtures/app-basic/text.config.ts` (new file)
...
| 5   | Redirect with has/missing cookie conditions             | FIXME       | text `matchRedirect()` does not support has/missing |
| 6   | Config rewrite serves content at original URL           | PASS        | `/config-rewrite` → `/`                             |
| 7   | Custom headers from text.config headers() on pages      | PASS        | `x-page-header` and `x-e2e-header` present          |
| 8   | Custom headers from text.config headers() on API routes | PASS        | `x-custom-header` present                           |
...
| #   | OpenText Test                                               | Text Status | Notes                                                                      |
| --- | ----------------------------------------------------------- | ----------- | -------------------------------------------------------------------------- |
| 1   | Catch-all API route captures multiple segments with hyphens | PASS        | `/api/catch-all/open-text/is/really/cool`                                  |
...
| 5   | Multi-value searchParams returned as arrays                 | FIXME       | text uses `URLSearchParams.forEach()` which overwrites duplicate keys      |
| 6   | Middleware forwards search params as request header         | FIXME       | text does not unpack `x-middleware-request-*` headers into request context |
...
| #   | OpenText Test                                           | Text Status    | Notes                                              |
| --- | ------------------------------------------------------- | -------------- | -------------------------------------------------- |
| 1   | Custom header on page routes from text.config headers() | PASS           | `x-page-header: about-page` verified               |
| 2   | Custom header on API routes from text.config headers()  | PASS           | `x-custom-header: text-app` verified               |
| 3   | Catch-all header pattern `/(.*)` applies to all routes  | PASS           | `x-e2e-header: text-e2e` on both page and API      |
| 4   | `poweredByHeader: false` suppresses X-Powered-By        | PASS (passive) | text never sends X-Powered-By regardless of config |
...
### Updated Summary (OpenText Compat)
...
| Feature                                        | Test         | Issue                                                                             |
| ---------------------------------------------- | ------------ | --------------------------------------------------------------------------------- |
| `TextResponse.rewrite()` status propagation    | ON-11 #5     | Status code from `TextResponse.rewrite(url, { status: 403 })` is silently dropped |
...
When `text.config.ts` defines custom headers via `headers()`, the dev server tried to set them
...
- `tests/fixtures/app-basic/text.config.ts` — Unified config for redirects, rewrites, custom headers
...
- `packages/text/src/entries/app-rsc-entry.ts` — Bug fix: skip config headers on redirect responses
- `tests/app-router.test.ts` — Removed dynamic text.config.mjs writing (uses permanent text.config.ts)

---

## Phase 5: Shim and Core Module Unit Tests

Unit tests for previously untested Text.js shims and core modules. These tests import
directly from source (`packages/text/src/shims/`, `packages/text/src/server/`,
`packages/text/src/routing/`) and test pure functions plus server markup helpers —
...
### P5-1: text/link — SSR rendering and pure functions ✅

**Local**: `tests/link.test.ts`
**Text.js mirrors**: `test/unit/link-rendering.test.ts`, `test/unit/link-warnings.test.tsx`
...
| 9     | Renders Rue element children                 | PASS   |       |
...
### P5-2: text/image — Component SSR and getImageProps ✅

**Local**: `tests/image-component.test.ts`
**Text.js mirrors**: `test/unit/text-image-new.test.ts`, `test/unit/text-image-get-img-props.test.ts`
...
### P5-4: text/head — SSR collection, tag filtering, escaping ✅

**Local**: `tests/head.test.ts`
**Text.js mirror**: `test/unit/text-head-rendering.test.ts`

| #     | Test                                                                                             | Status | Notes                   |
| ----- | ------------------------------------------------------------------------------------------------ | ------ | ----------------------- |
| 1-2   | Renders outside Text.js; returns null inline                                                     | PASS   |                         |
...
| 4-9 | `patternToTextFormat()`: `:id`→`[id]`, `:slug+`→`[...slug]`, `:slug*`→`[[...slug]]`, multiple segments, static passthrough, mixed | PASS   | Unique function not tested elsewhere |
...
| #     | Test                                                                                                                              | Status | Notes |
| ----- | --------------------------------------------------------------------------------------------------------------------------------- | ------ | ----- |
| 1-7   | Digest patterns: `TEXT_NOT_FOUND`, `TEXT_HTTP_ERROR_FALLBACK;{404,403,401}`, `TEXT_REDIRECT`, regular errors, non-special digests | PASS   |       |
...
### P5-9: text/dynamic — SSR rendering and ssr:false ✅

**Local**: `tests/dynamic.test.ts`
**Text.js mirror**: `test/unit/text-dynamic.test.tsx`
...
### P5-10: text/script — Strategy-based SSR rendering ✅
...
| --------------------------- | ------- | ------- | ----- | ----- |
| P5-1. text/link             | 24      | 24      | 0     | 0     |
| P5-2. text/image component  | 22      | 22      | 0     | 0     |
| P5-3. Image remote patterns | 33      | 33      | 0     | 0     |
| P5-4. text/head             | 26      | 26      | 0     | 0     |
| P5-5. Metadata routes       | 43      | 43      | 0     | 0     |
| P5-6. Route sorting         | 12      | 12      | 0     | 0     |
| P5-7. ISR cache             | 23      | 23      | 0     | 0     |
| P5-8. Error boundary        | 21      | 21      | 0     | 0     |
| P5-9. text/dynamic          | 11      | 11      | 0     | 0     |
| P5-10. text/script          | 9       | 9       | 0     | 0     |
| P5-11. text/form            | 6       | 6       | 0     | 0     |
| **Total**                   | **230** | **230** | **0** | **0** |
...
The 12 remaining tests in `route-sorting.test.ts` cover unique functionality: dynamic-before-catch-all ordering, deterministic sorting, static-over-dynamic preference, `patternToTextFormat()` conversion (6 tests), App Router route type discovery, and `apiRouter` sorting (2 tests).
...
| `tests/route-sorting.test.ts`   | 12    | Sorting precedence, patternToTextFormat, apiRouter     |
