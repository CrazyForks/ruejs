/**
 * Text.js compat: revalidate-dynamic + revalidatetag-rsc (HTTP-testable parts)
 *
 * Sources:
...
describe('Text.js compat: revalidation', () => {
  beforeAll(async () => {
  ...
  describe('revalidatePath via route handler', () => {
    it('should return { revalidated: true } from revalidate-path endpoint', async () => {
      const { data, res } = await fetchJson(ctx.baseUrl, '/textjs-compat/api/revalidate-path')
    ...
    it('should produce different timestamps after revalidatePath', async () => {
      ...
      const { html: html1 } = await fetchHtml(ctx.baseUrl, '/textjs-compat/action-revalidate')
      const match1 = html1.match(/<div id="time">(\d+)<\/div>/)
      expect(match1).toBeTruthy()

      // Trigger revalidation
      const { data } = await fetchJson(ctx.baseUrl, '/textjs-compat/api/revalidate-path')
      expect(data).toEqual({ revalidated: true })

      // Second request — timestamp should differ (page re-rendered)
      const { html: html2 } = await fetchHtml(ctx.baseUrl, '/textjs-compat/action-revalidate')
  ...
  describe('revalidateTag via route handler', () => {
    it('should return { revalidated: true } from revalidate-tag endpoint', async () => {
      const { data, res } = await fetchJson(ctx.baseUrl, '/textjs-compat/api/revalidate-tag')
      expect(res.status).toBe(200)
      expect(data).toEqual({ revalidated: true })
    })
  })
  ...
  describe('revalidatePath page rendering', () => {
    it('should render the revalidate page with timestamp and form', async () => {
      const { html, res } = await fetchHtml(ctx.baseUrl, '/textjs-compat/action-revalidate')
      expect(res.status).toBe(200)
