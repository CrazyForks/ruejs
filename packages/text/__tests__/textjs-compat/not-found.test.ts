/**
 * Text.js Compatibility Tests: not-found (basic)
 *
 ...
 * - fixtures/app-basic/app/textjs-compat/not-found-dynamic/ (new)
 * - fixtures/app-basic/app/textjs-compat/not-found-no-boundary/ (new)
 * - fixtures/app-basic/app/textjs-compat/not-found-error-boundary/ (new)
 *
 * NOTE: Some Text.js tests are browser-only (click button -> client-side notFound()).
...
describe('Text.js compat: not-found', () => {
  ...
  // ── Root not-found ──────────────────────────────────────────
  // Text.js: it('should return 404 status code for custom not-found page', ...)
  ...
  // Text.js: it('should use the not-found page for non-matching routes', ...)
  ...
  // ── Shell notFound() ───────────────────────────────────────
  // Text.js: it('should return 404 status if notFound() is called in shell', ...)
  ...
  // Text.js: it('should match dynamic route not-found boundary correctly', ...)
  ...
  it('dynamic index page renders normally', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-dynamic')
    expect(res.status).toBe(200)
    expect(html).toContain('dynamic')
  })

  it('dynamic [id] page renders for valid id', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-dynamic/123')
    expect(res.status).toBe(200)
    expect(html).toContain('dynamic [id]')
  })

  it('dynamic [id] notFound() uses scoped not-found boundary', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-dynamic/404')
    expect(res.status).toBe(404)
  ...
  // Text.js: it('should escalate notFound to parent layout if no not-found boundary present', ...)
  ...
  it('layout without not-found boundary renders its page normally', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-no-boundary')
    expect(res.status).toBe(200)
    expect(html).toContain('Dynamic with Layout')
  })

  it('dynamic [id] page renders for valid id (no-boundary)', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-no-boundary/123')
    expect(res.status).toBe(200)
  ...
  it('notFound() escalates to root not-found when no local boundary exists', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-no-boundary/404')
    expect(res.status).toBe(404)
    // Should render ROOT not-found.tsx since there's no local not-found boundary
    expect(html).toContain('404 - Page Not Found')
  })

  // ── Existing text tests: dashboard scoped not-found ──────
  // These exercise the pre-existing dashboard/not-found.tsx with dashboard/missing/page.tsx
  // (not from Text.js test suite, but validates the same pattern)
  ...
  // Text.js: it("should propagate notFound errors past a segment's error boundary", ...)
  ...
  it('notFound() in server component propagates past error boundary to not-found boundary', async () => {
    // /textjs-compat/not-found-error-boundary/nested/trigger-not-found
  ...
  // Text.js cascades metadata from parent layouts into not-found/error pages.

  it('not-found page should inherit metadata title from parent layout', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-not-found/missing')
    expect(res.status).toBe(404)
  ...
  it('not-found page should inherit metadata description from parent layout', async () => {
    const { html } = await fetchHtml(baseUrl, '/textjs-compat/metadata-not-found/missing')
  ...
  //
  // Fixture: textjs-compat/layout-params-notfound/[slug]/layout.tsx exports
  ...
  it('layout generateMetadata() in not-found fallback receives actual route params', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/layout-params-notfound/bad-slug')
    expect(res.status).toBe(404)
  ...
  it('layout generateMetadata() in not-found fallback uses actual slug value', async () => {
    const { res, html } = await fetchHtml(
      baseUrl,
      '/textjs-compat/layout-params-notfound/my-other-slug',
    )
    expect(res.status).toBe(404)
  ...
  // parent layout's NotFoundBoundary (per-layout boundary matching Text.js).

  it('valid slug renders page through layout', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-layout/hello')
    expect(res.status).toBe(200)
  ...
  it('notFound() from layout is caught by parent boundary', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-layout/invalid')
    expect(res.status).toBe(404)
  ...
  it('notFound() from layout returns 404 status', async () => {
    const res = await fetch(`${baseUrl}/textjs-compat/not-found-layout/does-not-exist`)
    expect(res.status).toBe(404)
  })
  ...
  // pages in Text.js). Without correct pre-render ordering, the page's
  ...
  it('layout+page notFound(): valid slug renders page', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-layout-page/hello')
    expect(res.status).toBe(200)
  ...
  it('layout+page notFound(): invalid slug caught by parent boundary (not 500)', async () => {
    const { res, html } = await fetchHtml(baseUrl, '/textjs-compat/not-found-layout-page/invalid')
  ...
  it('layout+page notFound(): RSC request returns 404 (not 500)', async () => {
    const res = await fetch(`${baseUrl}/textjs-compat/not-found-layout-page/invalid.rsc`, {
  ...
  it('RSC request for unmatched route returns 404 with valid RSC payload', async () => {
    ...
    expect(res.headers.get('content-type')).toContain('text/x-component')
    const body = await res.text()
    // RSC payload should contain the not-found content
    expect(body.length).toBeGreaterThan(0)
  ...
  it('RSC not-found response includes client component wrappers matching normal pages', async () => {
    // The RSC payload for not-found pages must include the same
    // component wrapper structure (ErrorBoundary, LayoutSegmentProvider) as
    // normal pages. Without these, client tree reconciliation during
  ...
  //   TO PORT: Add to __tests__/e2e/app-router/textjs-compat/not-found.spec.ts as Playwright test.
  //   FIXTURE: fixtures/app-basic/app/textjs-compat/not-found-error-boundary/page.tsx
  //
  ...
  //   FIXTURE: fixtures/app-basic/app/textjs-compat/not-found-error-boundary/nested/nested-2/page.tsx
  //
  ...
  //   Not worth porting — text's existing HMR tests cover this.
  ...
  //   WHY N/A: These test Text.js build output formats (.text/server/pages/404.html, nft.json).
  //   Text has a different build output structure. Not applicable.
  //
  // N/A: Edge runtime variant
  //   Source: index.test.ts#L131-L146
  //   WHY N/A: Tests re-running with `runtime = 'edge'` patched into layout.
  //   Text handles edge via Cloudflare Workers with separate test projects.
