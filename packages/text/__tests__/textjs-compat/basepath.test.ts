/**
 * Text.js Compatibility Tests: basePath plumbing
 *
 * Covers the surfaces where Text.js threads the configured `basePath` into
 ...
 *
 * Ported from Text.js: test/e2e/app-dir/app-basepath/index.test.ts
 * https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/app-basepath/index.test.ts
 */
import { describe, expect, it } from 'vite-plus/test'
import { applyFileBasedMetadata } from '../../src/server/file-based-metadata.js'
...
describe('basePath: server-action redirect', () => {
  ...
  it('applyActionRedirectBasePath prefixes internal absolute paths', async () => {
    const { applyActionRedirectBasePath } =
      await import('../../src/server/app-server-action-execution.js')
...
// Ported from Text.js: test/e2e/basepath/redirect-and-rewrite.test.ts
...
//     the basePath (per Text.js they evaluate against the raw pathname).
//
// Matches `.textjs-ref/packages/text/src/lib/load-custom-routes.ts:530-580`.
describe('basePath: rewrite/redirect/header gating', () => {
  ...
  it('basePath: false rewrite rule does NOT match when request is under basePath', async () => {
    // Mirrors Text.js: rewriting `/docs/rewrite-no-basePath` falls through to
  ...
  it('rules behave like default when basePath is empty (back-compat)', async () => {
    ...
    expect(matchRewrite('/x', rewrites, emptyCtx(), { basePath: '', hadBasePath: true })).toBe('/y')
  })
  ...
  it('matchers without explicit basePathState default to permissive (back-compat)', async () => {
    ...
    const { matchRewrite, matchRedirect, matchHeaders } =
      await import('../../src/config/config-matchers.js')
    const rewrites = [{ source: '/x', destination: '/y' }]
