/**
 * Text.js compat: app-prefetch (HTTP-testable parts)
 *
...
describe('Text.js compat: prefetch', () => {
  beforeAll(async () => {
  ...
  it('should serve RSC payload at .rsc endpoint', async () => {
    const res = await fetch(`${ctx.baseUrl}/textjs-compat/prefetch-test/target.rsc`, {
  ...
  it('should render prefetch page with links', async () => {
    const { html } = await fetchHtml(ctx.baseUrl, '/textjs-compat/prefetch-test')
  ...
  it('should navigate to target page (prefetch=true)', async () => {
    const { html } = await fetchHtml(ctx.baseUrl, '/textjs-compat/prefetch-test/target')
    expect(html).toContain('Prefetch Target Page')
  })

  it('should navigate to target page (prefetch=false)', async () => {
    const { html } = await fetchHtml(ctx.baseUrl, '/textjs-compat/prefetch-test/no-prefetch')
