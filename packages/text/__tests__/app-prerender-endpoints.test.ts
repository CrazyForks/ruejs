...
import { handleAppPrerenderEndpoint } from '../src/server/app-prerender-endpoints.js'
import { createAppPrerenderStaticParamsResolver } from '../src/server/app-prerender-static-params.js'
import { getRootParam } from '../src/shims/root-params.js'
...
describe('App prerender endpoint helpers', () => {
  ...
  it('returns 404 for prerender endpoints outside prerender mode', async () => {
    const response = await handleAppPrerenderEndpoint(
      new Request('http://localhost/__text/prerender/static-params?pattern=/blog/:slug'),
      {
  ...
  it('calls generateStaticParams with object parent params and serializes the result', async () => {
    ...
    const response = await handleAppPrerenderEndpoint(
      new Request(
        'http://localhost/__text/prerender/static-params?pattern=/blog/:slug&parentParams=%7B%22category%22%3A%22docs%22%7D',
      ),
      {
        isPrerenderEnabled: () => true,
        pathname: '/__text/prerender/static-params',
  ...
  it('seeds root params while calling generateStaticParams', async () => {
    ...
    const response = await handleAppPrerenderEndpoint(
      new Request(
        'http://localhost/__text/prerender/static-params?pattern=/:locale/blog/:slug&parentParams=%7B%22locale%22%3A%22en%22%2C%22slug%22%3A%22hello%22%7D',
      ),
      {
        isPrerenderEnabled: () => true,
        pathname: '/__text/prerender/static-params',
  ...
  it('passes empty parent params when static param input is not an object', async () => {
    ...
    const response = await handleAppPrerenderEndpoint(
      new Request(
        'http://localhost/__text/prerender/static-params?pattern=/blog/:slug&parentParams=5',
      ),
      {
        isPrerenderEnabled: () => true,
        pathname: '/__text/prerender/static-params',
  ...
  it('loads Pages Router routes lazily and calls getStaticPaths', async () => {
    ...
    const response = await handleAppPrerenderEndpoint(
      new Request(
        'http://localhost/__text/prerender/pages-static-paths?pattern=/posts/:id&locales=%5B%22en%22%5D&defaultLocale=en',
      ),
      {
        isPrerenderEnabled: () => true,
  ...
  it('returns JSON null when the requested prerender function is absent', async () => {
    const staticParamsResponse = await handleAppPrerenderEndpoint(
      new Request('http://localhost/__text/prerender/static-params?pattern=/missing'),
      {
        isPrerenderEnabled: () => true,
        pathname: '/__text/prerender/static-params',
        staticParamsMap: {},
    ...
    const pagesResponse = await handleAppPrerenderEndpoint(
      new Request('http://localhost/__text/prerender/pages-static-paths?pattern=/missing'),
      {
        isPrerenderEnabled: () => true,
  ...
  it('returns explicit endpoint errors for missing query fields and thrown user functions', async () => {
    const missingPatternResponse = await handleAppPrerenderEndpoint(
      new Request('http://localhost/__text/prerender/static-params'),
      {
        isPrerenderEnabled: () => true,
        pathname: '/__text/prerender/static-params',
        staticParamsMap: {},
