...
import { generateRouteTypes } from '../src/typegen.js'
...
async function withTempProject<T>(run: (root: string) => Promise<T>): Promise<T> {
  const root = await mkdtemp(path.join(os.tmpdir(), 'text-typegen-'))
  try {
...
describe('generateRouteTypes', () => {
  it('generates Text-compatible global route helper types from the App Router tree', async () => {
    await withTempProject(async root => {
      ...
      expect(outputPath).toBe(path.join(root, '.text/types/routes.d.ts'))
      expect(generated).toContain('declare namespace TextRouteTypes')
      ...
      expect(generated).toContain('type LayoutProps<Route extends TextRouteTypes.LayoutRoute>')
      expect(generated).toContain(
        'type RouteContext<Route extends TextRouteTypes.RouteHandlerRoute = TextRouteTypes.RouteHandlerRoute>',
      )
    })
  })
  ...
  it('writes a Text-compatible text-env.d.ts stub when one is missing', async () => {
    await withTempProject(async root => {
      await writeProjectFile(root, 'app/layout.tsx', EMPTY_LAYOUT)
      await writeProjectFile(root, 'app/page.tsx', EMPTY_PAGE)

      await generateRouteTypes({ root })
      const generated = await readFile(path.join(root, 'text-env.d.ts'), 'utf-8')

      expect(generated).toContain('/// <reference types="text" />')
      expect(generated).toContain('/// <reference types="text/image-types/global" />')
      expect(generated).toContain('import "./.text/types/routes.d.ts";')
      expect(generated).not.toContain('/// <reference path="./.text/types/routes.d.ts" />')
    })
  })

  it('preserves an existing text-env.d.ts', async () => {
    await withTempProject(async root => {
      await writeProjectFile(root, 'app/layout.tsx', EMPTY_LAYOUT)
      await writeProjectFile(root, 'app/page.tsx', EMPTY_PAGE)
      const customContent = '/// <reference types="custom" />\n'
      await writeProjectFile(root, 'text-env.d.ts', customContent)

      await generateRouteTypes({ root })
      const preserved = await readFile(path.join(root, 'text-env.d.ts'), 'utf-8')
  ...
  it('updates generated route helper types when App Router files are added in dev', async () => {
    await withTempProject(async root => {
      ...
      try {
        // `appDir` in the text plugin options names the project root, not
        // the App Router directory; the plugin auto-detects `app/` (or
        // `src/app/`) under it. Pass the project root explicitly here so
        // the dev server uses the same root path for both Vite and text.
  ...
  it('does not block dev server startup when initial route type generation fails', async () => {
    await withTempProject(async root => {
      await writeProjectFile(root, 'app/layout.tsx', EMPTY_LAYOUT)
      await writeProjectFile(root, 'app/page.tsx', EMPTY_PAGE)
      await writeProjectFile(root, '.text', 'not a directory\n')
