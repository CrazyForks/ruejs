...
describe('Pages i18n domain helpers', () => {
  let addLocalePrefix: typeof import('../src/utils/domain-locale.js').addLocalePrefix
  let detectDomainLocale: typeof import('../src/server/pages-i18n.js').detectDomainLocale
  let getLocaleRedirect: typeof import('../src/server/pages-i18n.js').getLocaleRedirect
  let resolvePagesI18nRequest: typeof import('../src/server/pages-i18n.js').resolvePagesI18nRequest

  beforeAll(async () => {
    const mod = await import('../src/server/pages-i18n.js')
    ;({ addLocalePrefix } = await import('../src/utils/domain-locale.js'))
  ...
  it('does not let TEXT_LOCALE override the current domain default locale', () => {
    expect(
      getLocaleRedirect({
        headers: { cookie: 'TEXT_LOCALE=fr' },
        textConfig: { i18n, basePath: '', trailingSlash: false },
  ...
  it('does not redirect same-domain locale aliases when the domain default already matches', () => {
    expect(
      getLocaleRedirect({
        headers: { cookie: 'TEXT_LOCALE=nl-BE' },
        textConfig: { i18n, basePath: '', trailingSlash: false },
        pathLocale: undefined,
  ...
  it('uses Accept-Language rather than TEXT_LOCALE to choose the preferred domain', () => {
    expect(
      getLocaleRedirect({
        headers: {
  ...
  it('preserves the search string on root locale redirects', () => {
    ...
    ).toBe('http://example.fr/?utm=campaign&text=%2Fcheckout')
  })
})
...
//
// Reference Text.js implementation:
// https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/i18n/normalize-locale-path.ts
// https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/get-text-pathname-info.ts
describe('stripI18nLocaleForApiRoute (issue #1336 item 3)', () => {
  ...
  let stripI18nLocaleForApiRoute: typeof import('../src/server/pages-i18n.js').stripI18nLocaleForApiRoute

  beforeAll(async () => {
    ;({ stripI18nLocaleForApiRoute } = await import('../src/server/pages-i18n.js'))
  })
