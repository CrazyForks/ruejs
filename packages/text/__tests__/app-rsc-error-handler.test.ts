...
import { createAppRscOnErrorHandler } from '../src/server/app-rsc-error-handler.js'
import { errorDigest } from '../src/server/app-rsc-errors.js'
...
describe('createAppRscOnErrorHandler', () => {
  it('returns a function that short-circuits on digest errors (TEXT_REDIRECT, TEXT_NOT_FOUND, etc.)', () => {
    const reportRequestError = vi.fn()
    const onError = createAppRscOnErrorHandler(reportRequestError, makeReq(), '/feed', '/feed')

    expect(onError({ digest: 'TEXT_NOT_FOUND' })).toBe('TEXT_NOT_FOUND')
    expect(onError({ digest: 'TEXT_REDIRECT;push;/login;307' })).toBe(
      'TEXT_REDIRECT;push;/login;307',
    )
