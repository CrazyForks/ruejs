/**
 * Tests for scripts/classify-textjs-suites.mjs.
 *
 * The classifier reads from a Text.js checkout on disk, so each test creates
 * a tiny fake Text.js checkout under a tmpdir, populates it with a fixture
...
import { classifySuite } from '../../../scripts/classify-textjs-suites.mjs'
...
describe('classifySuite', () => {
  ...
  beforeEach(async () => {
    root = await fsp.mkdtemp(path.join(os.tmpdir(), 'classify-textjs-'))
  })
  ...
  it("falls back to 'app' for inline-fixture suites under test/e2e/app-dir/", async () => {
    // The .test.ts builds its fixture via textTestSetup({ files: { ... } })
  ...
  it('honours the curated APP_ROUTER_NON_APP_DIR_SUITES list', async () => {
    ...
    const suite = 'test/e2e/text-form/default/text-form-prefetch.test.ts'
  ...
  it("treats a fixture-wrapper 'app/' (contains text.config.js) as a project root, not App Router", async () => {
    // Mirrors test/e2e/og-api/ in Text.js — the outer directory named
    // `app` holds the Text.js project (text.config.js + pages/) and the
    ...
    await writeFile(path.join(root, suite))
    // Outer `app/` is the wrapper — text.config.js identifies it
    await writeFile(path.join(root, 'test/e2e/wrapper-fixture/app/text.config.js'))
  ...
  it("treats a fixture-wrapper 'app/' containing only pages/ as Pages Router", async () => {
    // Mirrors test/e2e/browserslist/ in Text.js — outer `app/` is the
    // wrapper, inner `pages/` is the real Pages Router fixture.
    const suite = 'test/e2e/wrapper-pages/index.test.ts'
    await writeFile(path.join(root, suite))
    await writeFile(path.join(root, 'test/e2e/wrapper-pages/app/text.config.js'))
  ...
  it('does not mistake an App Router app/ that contains a noop middleware.js for a wrapper', async () => {
    ...
    // contains a noop middleware.js file (used to assert that Text.js
  ...
  it('does not walk into node_modules / .text when scanning fixtures', async () => {
    ...
    await writeFile(path.join(root, 'test/e2e/with-noise/.text/server/app/page.js'))
