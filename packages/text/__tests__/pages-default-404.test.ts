 ...
 * `pages/_error.tsx` is defined) must include the canonical Text.js body
 * `"This page could not be found."` (with trailing period). Pre-fix text
...
} from '../src/server/pages-default-404.js'

describe('buildDefaultPagesNotFoundResponse', () => {
  it('returns a 404 status with the canonical Text.js body', async () => {
    const response = buildDefaultPagesNotFoundResponse()
    expect(response.status).toBe(404)
    const body = await response.text()
    // The Text.js deploy suite asserts on this substring (with the trailing
    ...
    // Old text placeholder body must NOT leak through.
    expect(body).not.toContain('404 - Page not found')
  })

  it('uses Text.js-compatible content-type', () => {
    const response = buildDefaultPagesNotFoundResponse()
