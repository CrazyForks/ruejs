...
import { buildRouteTrie, trieMatch } from '../src/routing/route-trie.js'
...
describe('buildRouteTrie + trieMatch', () => {
  ...
  describe('optional catch-all routes', () => {
    it('matches optional catch-all with zero segments without materializing the param', () => {
      // Text.js route-regex makes the optional catch-all capture group optional, and
      // route-matcher only writes params for defined capture groups:
      // https://github.com/vercel/next.js/blob/canary/packages/text/src/shared/lib/router/utils/route-regex.ts
  ...
  describe('param decoding', () => {
    ...
    it('preserves malformed percent escapes without throwing', () => {
      // Text.js throws DecodeError (400) on malformed, but text's
