...
describe('hasMdxFiles caching', () => {
  let hasMdxFiles: (typeof import('../src/utils/mdx-scan.js'))['hasMdxFiles']
  let mdxScanCache: (typeof import('../src/utils/mdx-scan.js'))['mdxScanCache']

  beforeAll(async () => {
    const mod = await import('../src/utils/mdx-scan.js')
  ...
  it('returns true when .mdx files exist in appDir', async () => {
    ...
    const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-mdx-'))
  ...
  it('does not rescan the filesystem on second call with same root', async () => {
    const fsp = await import('node:fs/promises')
    const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-mdx-cache-'))
    const appDir = path.join(tmpDir, 'app')
    await fsp.mkdir(appDir, { recursive: true })
    await fsp.writeFile(path.join(appDir, 'page.mdx'), '# Hello')
  ...
  it('caches false results too (no redundant scans for missing mdx)', async () => {
    const fsp = await import('node:fs/promises')
    const tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-mdx-nomdx-'))
    const appDir = path.join(tmpDir, 'app')
    await fsp.mkdir(appDir, { recursive: true })
    await fsp.writeFile(path.join(appDir, 'page.tsx'), 'export default () => null')
  ...
  it('caches independently per root directory', async () => {
    const fsp = await import('node:fs/promises')
    const tmpDir1 = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-mdx-root1-'))
    const tmpDir2 = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-mdx-root2-'))
...
describe('resolvePostcssStringPlugins caching', () => {
  let resolvePostcssStringPlugins: (typeof import('../src/plugins/postcss.js'))['resolvePostcssStringPlugins']
  let postcssCache: (typeof import('../src/plugins/postcss.js'))['postcssCache']

  beforeAll(async () => {
    const mod = await import('../src/plugins/postcss.js')
  ...
  async function createTmpProject(configFileName: string, configContent: string): Promise<string> {
    const fsp = await import('node:fs/promises')
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-postcss-cache-'))
...
module.exports.postcss = true;`,
  ...
  it('caches undefined results (no config file)', async () => {
    const fsp = await import('node:fs/promises')
    const dir = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-postcss-noconf-'))
  ...
  it('caches independently per project root', async () => {
    ...
    const fsp = await import('node:fs/promises')
    const dir2 = await fsp.mkdtemp(path.join(os.tmpdir(), 'text-postcss-noconf2-'))
