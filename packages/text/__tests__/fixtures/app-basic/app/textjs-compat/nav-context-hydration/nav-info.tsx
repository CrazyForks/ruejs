 ...
 * assert the SSR-rendered values match the __TEXT_RSC_NAV__ payload that
 * the browser entry restores before hydration.
 *
 * The fix under test: __TEXT_RSC_NAV__ is embedded in <head> as a script
 * tag so that useSyncExternalStore's getServerSnapshot returns the same
 * pathname/searchParams that were rendered on the server, preventing hydration
 * mismatch error #418.
 *
