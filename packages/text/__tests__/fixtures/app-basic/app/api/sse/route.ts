 ...
 *
 * Ported from: https://github.com/opennextjs/opennextjs-cloudflare/blob/main/examples/e2e/app-router/open-text/app/sse/
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      for (let i = 1; i <= 3; i++) {
        ...
        controller.enqueue(encoder.encode(`data: ${JSON.stringify({ message: `hello:${i}` })}\n\n`))
      }
