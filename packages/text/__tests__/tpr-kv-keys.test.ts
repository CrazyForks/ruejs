...
import { buildTprKVPairs } from '../src/cloudflare/tpr.js'
import { isrCacheKey } from '../src/server/isr-cache.js'
...
describe('TPR KV key format', () => {
  ...
  it('uses revalidate from x-text-revalidate header when present', () => {
    const pairs = buildTprKVPairs(entry('/products', { 'x-text-revalidate': '30' }), buildId, 60)
  ...
  it('uses flat 30-day TTL even for very short revalidate windows', () => {
    const pairs = buildTprKVPairs(entry('/fast', { 'x-text-revalidate': '1' }), buildId, 60)
  ...
  it('uses 24-hour fallback TTL when revalidation is disabled', () => {
    const pairs = buildTprKVPairs(entry('/archive', { 'x-text-revalidate': '0' }), buildId, 60)
