...
async function createImageFixture(router: 'app' | 'pages'): Promise<string> {
  ...
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), `text-${router}-image-parity-`))
...
function runLocalImageUrlParitySuite(router: 'app' | 'pages'): void {
  describe(`${router === 'app' ? 'App' : 'Pages'} Router text/image local URL parity`, () => {
    ...
    // Ported from Text.js: test/integration/text-image-new/unicode/test/index.test.ts
    // https://github.com/vercel/next.js/blob/canary/test/integration/text-image-new/unicode/test/index.test.ts
    it('serves internal unicode image URLs through the optimizer route', async () => {
      ...
      expect(imageUrl.pathname).toBe('/_text/image')
    ...
    // Both /_text/image and /_text/image are accepted so apps wired to
    // either prefix get images served through the same optimizer pipeline.
    it('routes /_text/image requests through the optimizer', async () => {
      const textUrl = new URL('/_text/image', baseUrl)
      textUrl.searchParams.set('url', '/hello world.png')
      textUrl.searchParams.set('w', '64')
      textUrl.searchParams.set('q', '75')
