...
} from '../src/server/app-rsc-request-normalization.js'
import { TEXT_RSC_RENDER_MODE_HEADER } from '../src/server/app-rsc-cache-busting.js'
import {
...
} from '../src/server/client-reuse-manifest.js'
import { createArtifactCompatibilityEnvelope } from '../src/server/artifact-compatibility.js'
import {
  TEXT_CLIENT_REUSE_MANIFEST_HEADER,
...
describe('normalizeRscRequest — basePath', () => {
  ...
  it('bypasses basePath check for /__text/ internal prerender endpoints', () => {
    // Prerender endpoints must be reachable even with a basePath configured.
    const result = normalizeRscRequest(req('/__text/prerender/status'), '/app')
    expect(result).not.toBeInstanceOf(Response)
  })
...
describe('normalizeRscRequest — RSC detection and cleanPathname', () => {
  ...
  it('accepts dev legacy query RSC requests from Vite HMR', () => {
    const result = withNodeEnv('development', () =>
      normalized(
        normalizeRscRequest(req('/about?_rsc=hmr', { accept: 'text/x-component', rsc: '1' }), ''),
      ),
    )
    expect(result.isRscRequest).toBe(true)
    expect(result.cleanPathname).toBe('/about')
  ...
  it('does not accept legacy query RSC requests in production', () => {
...
describe('normalizeRscRequest — interception context sanitization', () => {
  ...
  it('returns null for empty interception context header', () => {
    ...
    const result = normalized(
      normalizeRscRequest(req('/page', { 'X-Text-Interception-Context': '' }), ''),
    )
    expect(result.interceptionContextHeader).toBeNull()
  })
  ...
  it('preserves legitimate interception context value', () => {
    const result = normalized(
      normalizeRscRequest(req('/page', { 'X-Text-Interception-Context': '(.)/slot/page' }), ''),
    )
  ...
  it('preserves interception context value with nested separators', () => {
    const result = normalized(
      normalizeRscRequest(
        req('/page', {
          'X-Text-Interception-Context': '(..)/(.)slot/nested',
        }),
        '',
      ),
    )
  ...
  it('strips null bytes from interception context header to prevent header injection', () => {
    ...
    const request = {
      ...
      headers: {
        get(name: string) {
          if (name.toLowerCase() === TEXT_INTERCEPTION_CONTEXT_HEADER.toLowerCase()) {
            return 'foo\0bar'
          }
          return null
        },
...
describe('normalizeRscRequest — mounted slots normalization', () => {
  it('sorts slot ids so different client orderings hit the same RSC cache entry', () => {
    ...
    const result = normalized(
      normalizeRscRequest(req('/page', { 'x-text-mounted-slots': 'slot:b slot:a' }), ''),
    )
  ...
  it('deduplicates slot ids', () => {
    const result = normalized(
      normalizeRscRequest(req('/page', { 'x-text-mounted-slots': 'slot:a slot:b slot:a' }), ''),
    )
    expect(result.mountedSlotsHeader).toBe('slot:a slot:b')
  })
  ...
  it('returns null for blank mounted-slots header', () => {
    const result = normalized(
      normalizeRscRequest(req('/page', { 'x-text-mounted-slots': '   \t  ' }), ''),
    )
    expect(result.mountedSlotsHeader).toBeNull()
  })
  ...
  it('normalizes the semantic render mode marker', () => {
    const refresh = normalized(
      normalizeRscRequest(
        req('/page.rsc', {
          [TEXT_RSC_RENDER_MODE_HEADER]: APP_RSC_RENDER_MODE_REFRESH_PRESERVE_UI,
        }),
        '',
      ),
    )
    const normal = normalized(
      normalizeRscRequest(req('/page.rsc', { [TEXT_RSC_RENDER_MODE_HEADER]: 'true' }), ''),
    )
    const prefetchShell = normalized(
      normalizeRscRequest(
        req('/page.rsc', {
          [TEXT_RSC_RENDER_MODE_HEADER]: APP_RSC_RENDER_MODE_PREFETCH_LOADING_SHELL,
        }),
        '',
      ),
    )
    const html = normalized(
      normalizeRscRequest(
        req('/page', { [TEXT_RSC_RENDER_MODE_HEADER]: APP_RSC_RENDER_MODE_REFRESH_PRESERVE_UI }),
        '',
      ),
    )
...
describe('normalizeRscRequest — ClientReuseManifest boundary', () => {
  it('parses ClientReuseManifest only for canonical RSC payload requests', () => {
    ...
    const rsc = normalized(
      normalizeRscRequest(req('/page.rsc', { [TEXT_CLIENT_REUSE_MANIFEST_HEADER]: header }), ''),
    )
    const html = normalized(
      normalizeRscRequest(req('/page', { [TEXT_CLIENT_REUSE_MANIFEST_HEADER]: header }), ''),
    )
  ...
  it('keeps rejected ClientReuseManifest hints as disabled metadata instead of failing routing', () => {
    const result = normalized(
      normalizeRscRequest(
        req('/page.rsc', {
          [TEXT_CLIENT_REUSE_MANIFEST_HEADER]: JSON.stringify({
            entries: [],
...
describe('normalizeRscRequest — compound scenarios', () => {
  ...
  it('basePath + /__text/ bypass: /__text/ with basePath returns valid result', () => {
    const result = normalized(
      normalizeRscRequest(req('/__text/prerender/pages-static-paths'), '/app'),
    )
    expect(result.pathname).toBe('/__text/prerender/pages-static-paths')
  })
})
...
describe('normalizeMountedSlotsHeader', () => {
  ...
  it('deduplicates and sorts whitespace-separated slot ids', () => {
    expect(normalizeMountedSlotsHeader(' sidebar  modal sidebar\tcart ')).toBe('cart modal sidebar')
  })
