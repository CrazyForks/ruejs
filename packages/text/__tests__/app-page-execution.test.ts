...
} from '../src/server/app-page-execution.js'
import { readStreamAsText } from '../src/utils/text-stream.js'
...
describe('app page execution helpers', () => {
  it('parses redirect and access-fallback digests', () => {
    expect(
      resolveAppPageSpecialError({
        digest: 'TEXT_REDIRECT;replace;%2Fredirected;308',
      }),
    ).toEqual({
      kind: 'redirect',
      location: '/redirected',
    ...
    expect(
      resolveAppPageSpecialError({
        digest: 'TEXT_HTTP_ERROR_FALLBACK;403',
      }),
    ).toEqual({
  ...
  it('prefixes redirect Location with basePath for app-internal paths', async () => {
    ...
    expect(alreadyPrefixedRootWithHash.headers.get('location')).toBe('https://example.com/blog#top')
  ...
  it('appends pending cookies (cookies().set during render) to redirect responses', async () => {
    // Mirrors Text.js's `appendMutableCookies(headers, requestStore.mutableCookies)`
    ...
    // Matches Text.js, which only calls appendMutableCookies in the redirect
    // branch.
  ...
  it('encodes redirect digest in payload + 200 when buildRscRedirectPayloadStream is provided (RSC request)', async () => {
    // When a server component throws redirect() during RSC rendering, the
    // redirect digest is serialized into the payload stream and the response
    // stays 200. The status line never carries the redirect.
    //
    // See: https://github.com/cloudflare/vinext/issues/1347
    const buildRscRedirectPayloadStream = vi.fn(
    ...
    expect(buildRscRedirectPayloadStream).toHaveBeenCalledTimes(1)
    expect(buildRscRedirectPayloadStream).toHaveBeenCalledWith({
      digest: 'TEXT_REDIRECT;replace;/redirected;307;',
    })
    await expect(response.text()).resolves.toBe('E:TEXT_REDIRECT;replace;/redirected;307;')
  })

  it('always emits 200 + payload for metadata-originated redirects (even on document SSR)', async () => {
    // generateMetadata() redirects are streamed inside the App payload
    // because metadata resolution is suspended in Text.js. The HTTP status
    // stays 200 for both RSC and full document requests. Mirrors Text.js
    ...
    const buildRscRedirectPayloadStream = vi.fn(
      () =>
        new ReadableStream<Uint8Array>({
          start(controller) {
            controller.enqueue(new TextEncoder().encode('redirect-payload'))
            controller.close()
          },
        }),
    )
  ...
  it('preserves the 307/308 status code in the encoded digest (permanentRedirect)', async () => {
    ...
    const buildRscRedirectPayloadStream = (opts: { digest: string }) => {
      captured.push(opts)
    ...
    expect(captured).toEqual([{ digest: 'TEXT_REDIRECT;replace;/permanent-target;308;' }])
  })

  it('falls back to 307 when buildRscRedirectPayloadStream is absent (backward compat)', async () => {
    // Callers that don't yet plumb the payload-stream builder keep the legacy
  ...
  it('produces fused ssrStream + sideStream when capturing (#981)', async () => {
    const capture = teeAppPageRscStreamForCapture(createStream(['payload-', 'chunk']), true)
    ...
    expect(ssrText).toBe('payload-chunk')
    expect(sideText).toBe('payload-chunk')
  })
  ...
  it('emits the `x-edge-runtime: 1` marker on RSC redirect payload responses for edge-runtime routes', async () => {
    const response = await buildAppPageSpecialErrorResponse({
      buildRscRedirectPayloadStream: ({ digest }) =>
        new ReadableStream<Uint8Array>({
          start(controller) {
  ...
  it('omits the `x-edge-runtime` marker on RSC redirect payload responses for nodejs-runtime routes', async () => {
    const response = await buildAppPageSpecialErrorResponse({
      buildRscRedirectPayloadStream: ({ digest }) =>
        new ReadableStream<Uint8Array>({
          start(controller) {
