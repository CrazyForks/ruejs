/**
 * Deploy-mode tests for `text/after` (`unstable_after`).
 *
 ...
 * https://github.com/vercel/next.js/blob/canary/test/e2e/app-dir/text-after-app-deploy/index.test.ts
 *
 * Issue: https://github.com/cloudflare/vinext/issues/1365
 */
import { describe, expect, it } from 'vite-plus/test'
import { generateAppRouterWorkerEntry, generatePagesRouterWorkerEntry } from '../src/deploy.js'
...
describe('after() in deploy mode — ctx.waitUntil wiring', () => {
  ...
  it('after() preserves ctx across async hops simulating the App Router request lifecycle', async () => {
    ...
    const { runWithExecutionContext, getRequestExecutionContext } =
      await import('../src/shims/request-context.js')
  ...
  it('App Router worker entry wraps the RSC handler with runWithExecutionContext when ctx is provided', async () => {
    // Verify the default text/server/app-router-entry hooks ctx.waitUntil
    ...
    const src = await readFile(
      resolve(import.meta.dirname, '../src/server/app-router-entry.ts'),
      'utf8',
    )
...
describe('after() in deploy mode — App Router worker entry', () => {
  it('delegates to handler.fetch with ctx so text/server/app-router-entry can wrap with runWithExecutionContext', () => {
...
describe('after() in deploy mode — Pages Router API handler', () => {
  it('handlePagesApiRoute wraps the user handler in runWithExecutionContext when ctx is provided', async () => {
    const { handlePagesApiRoute } = await import('../src/server/pages-api-route.js')
    const { getRequestExecutionContext } = await import('../src/shims/request-context.js')
