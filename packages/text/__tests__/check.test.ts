...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'text-check-'))
})
...
describe('scanImports', () => {
  it('detects supported text/* imports', () => {
    writeFile(
      'app/page.tsx',
      `
    ...
    expect(items.find(i => i.name === 'text/link')?.status).toBe('supported')
    expect(items.find(i => i.name === 'text/image')?.status).toBe('supported')
  })

  it('detects partial imports', () => {
    writeFile('app/page.tsx', `import { GoogleFont } from "text/font/google";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/font/google')
    expect(items[0].status).toBe('partial')
  })

  it('reports accurate text/font/local detail', () => {
    writeFile('app/page.tsx', `import localFont from "text/font/local";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/font/local')
  ...
  it('detects unsupported imports', () => {
    writeFile('pages/amp.tsx', `import { useAmp } from "text/amp";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/amp')
  ...
  it('tracks which files use each import', () => {
    writeFile('app/page.tsx', `import Link from "text/link";`)
    writeFile('app/about/page.tsx', `import Link from "text/link";`)

    const items = scanImports(tmpDir)
    const linkItem = items.find(i => i.name === 'text/link')
  ...
  it('detects require() calls too', () => {
    writeFile('lib/util.js', `const router = require("text/router");`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/router')
    expect(items[0].status).toBe('supported')
  })

  it('returns empty for projects with no text imports', () => {
    writeFile('src/index.ts', `import { createElement } from "@rue-js/rue";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(0)
  })

  it('marks unrecognized text/* imports as unsupported', () => {
    writeFile('app/page.tsx', `import foo from "text/nonexistent";`)
  ...
  it('recognizes text/compat/router as supported', () => {
    writeFile('components/shared-nav.tsx', `import { useRouter } from "text/compat/router";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/compat/router')
    expect(items[0].status).toBe('supported')
  })

  it('recognizes text/form as supported', () => {
    writeFile('app/page.tsx', `import Form from "text/form";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/form')
    expect(items[0].status).toBe('supported')
  })

  it('recognizes text/web-vitals as supported', () => {
    writeFile('pages/_app.tsx', `import { reportWebVitals } from "text/web-vitals";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/web-vitals')
    expect(items[0].status).toBe('supported')
  })

  it('recognizes text/constants as supported', () => {
    writeFile('lib/phases.ts', `import { PHASE_DEVELOPMENT_SERVER } from "text/constants";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/constants')
    expect(items[0].status).toBe('supported')
  })

  it("recognizes `import { Metadata } from 'text'` as supported", () => {
    writeFile(
      'app/layout.tsx',
      `import { Metadata } from "text";\nexport const metadata: Metadata = { title: "App" };`,
    )
  ...
  it('skips `import type` statements entirely', () => {
    ...
    const items = scanImports(tmpDir)
    // Should only find text/link, not text (since import type is skipped)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/link')
  })

  it('skips `import type` for text/* paths too', () => {
    writeFile('app/page.tsx', `import type { TextRequest } from "text/server";`)
  ...
  it('sorts unsupported first, then partial, then supported', () => {
    writeFile(
      ...
      `
      import Link from "text/link";
      import { GoogleFont } from "text/font/google";
      import { useAmp } from "text/amp";
    `,
    )
  ...
  it('ignores node_modules and .text directories', () => {
    writeFile('node_modules/foo/index.ts', `import Link from "text/link";`)
    writeFile('.text/server.js', `import Link from "text/link";`)
    writeFile('app/page.tsx', `import Image from "text/image";`)

    const items = scanImports(tmpDir)
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text/image')
  })
  ...
  it('recognizes text/dist/shared/lib/router-context.shared-runtime as supported', () => {
    writeFile(
      'lib/router.tsx',
      `import { RouterContext } from "text/dist/shared/lib/router-context.shared-runtime";`,
    )

    const items = scanImports(tmpDir)
    const item = items.find(i => i.name === 'text/dist/shared/lib/router-context.shared-runtime')
    expect(item?.status).toBe('supported')
  })

  it('recognizes all other shimmed text/dist/* paths as supported', () => {
    const distImports = [
      'text/dist/shared/lib/app-router-context.shared-runtime',
      'text/dist/shared/lib/app-router-context',
      'text/dist/shared/lib/utils',
      'text/dist/server/api-utils',
      'text/dist/server/web/spec-extension/cookies',
      'text/dist/compiled/@edge-runtime/cookies',
      'text/dist/server/app-render/work-unit-async-storage.external',
      'text/dist/client/components/work-unit-async-storage.external',
      'text/dist/client/components/request-async-storage.external',
      'text/dist/client/components/request-async-storage',
      'text/dist/client/components/navigation',
      'text/dist/server/config-shared',
    ]
...
describe('analyzeConfig', () => {
  it("reports 'no config file' when none exists", () => {
    ...
    expect(items).toHaveLength(1)
    expect(items[0].name).toBe('text.config')
    expect(items[0].status).toBe('supported')
  })
  ...
  it('detects supported config options', () => {
    ...
    // rueStrictMode is reported as `partial` until text maps the option to
    // Rue hydration behavior — currently the config is read but not enforced.
    // See `packages/text/src/check.ts` for the rationale.
    expect(items.find(i => i.name === 'rueStrictMode')?.status).toBe('partial')
  })
  ...
  // Mirrors Text.js: test/e2e/app-dir/app-shells
  ...
  it.each([
    ...
    writeFile('text.config.mjs', `export default { ${parent}: { ${body} } };`)
    const items = analyzeConfig(tmpDir)
    expect(items.find(i => i.name === name)).toBeDefined()
  })

  it('reads text.config.ts files', () => {
    writeFile('text.config.ts', `const config = { basePath: "/app" }; export default config;`)
...
describe('checkLibraries', () => {
  ...
  it('returns empty when no known libraries are used', () => {
    writeFile(
      'package.json',
      JSON.stringify({
        dependencies: { rue: '^19.0.0', 'some-lib': '^1.0.0' },
      }),
    )
  ...
  it('detects supported libraries', () => {
    writeFile(
      'package.json',
      JSON.stringify({
        dependencies: { 'text-themes': '^0.3.0', tailwindcss: '^3.0.0', zod: '^3.0.0' },
      }),
    )
  ...
  it('detects unsupported libraries', () => {
    writeFile(
      'package.json',
      JSON.stringify({
        dependencies: { '@auth/textjs': '^5.0.0', 'text-auth': '^4.0.0' },
      }),
    )
  ...
  it('detects @clerk/textjs as partial', () => {
    writeFile(
      'package.json',
      JSON.stringify({
        dependencies: { '@clerk/textjs': '^7.0.0' },
      }),
    )
...
describe('checkConventions', () => {
  ...
  it('detects proxy.ts (Text.js 16)', () => {
    ...
    expect(proxy?.name).toContain('Text.js 16')
  })
  ...
  it('flags missing type:module in package.json', () => {
    writeFile('app/page.tsx', `export default function Home() { return <div/>; }`)
    writeFile('package.json', JSON.stringify({ dependencies: { text: 'workspace:*' } }))
  ...
  it('does not flag type:module when present', () => {
    ...
    writeFile(
      'package.json',
      JSON.stringify({ type: 'module', dependencies: { text: 'workspace:*' } }),
    )
  ...
  it('flags transition canary imports as unsupported', () => {
    const runtimePackage = ['re', 'act'].join('')
    writeFile(
      'app/page.tsx',
      `import { ViewTransition, addTransitionType } from "${runtimePackage}";\nexport default function Home() { addTransitionType("navigate"); return <ViewTransition><div/></ViewTransition>; }`,
    )

    const items = checkConventions(tmpDir)
    const vt = items.find(i => i.name.includes('transition canary'))
    expect(vt).toBeDefined()
    expect(vt?.status).toBe('unsupported')
    expect(vt?.detail).toContain('not rewritten by text')
...
describe('runCheck', () => {
  ...
  it('calculates score correctly — 100% for all supported', () => {
    ...
    const result = runCheck(tmpDir)
    // All items should be supported: text/link, no config file, tailwindcss, App Router, 1 page, 1 layout
  ...
  it('calculates score correctly — partial items count 50%', () => {
    // 1 supported import (text/link) + 1 partial import (text/font/google) + no-config (supported) + 2 conventions (App Router + 1 page)
    writeFile(
      'app/page.tsx',
      `
      import Link from "text/link";
  ...
  it('calculates score correctly — unsupported items drag score down', () => {
    ...
    writeFile('text.config.mjs', `export default { webpack: (config) => config };`)
    writeFile(
      'package.json',
      JSON.stringify({ type: 'module', dependencies: { 'text-auth': '^4.0.0' } }),
    )
  ...
  it('calculates score correctly for src/app project', () => {
    writeFile('src/app/page.tsx', `import Link from "text/link";`)
    writeFile(
      'src/app/layout.tsx',
      `export default function Layout({ children }) { return <html><body>{children}</body></html>; }`,
    )
...
describe('formatReport', () => {
  it('produces a string with section headers', () => {
    ...
    expect(report).toContain('text compatibility report')
  ...
  it('shows issues section when there are unsupported items', () => {
    writeFile('app/page.tsx', `import { useAmp } from "text/amp";`)
    ...
    expect(report).toContain('text/amp')
  })
  ...
  it('shows partial support section when there are partial items', () => {
    ...
    expect(report).toContain('text/font/google')
  })
  ...
  it('includes actionable text steps', () => {
    writeFile('app/page.tsx', `import Link from "text/link";`)
    writeFile('package.json', JSON.stringify({ type: 'module', dependencies: {} }))
    ...
    expect(report).not.toContain('rue-rsc-payload')
...
describe('integration: pages-basic fixture', () => {
  ...
  it('detects config options from text.config.mjs', () => {
