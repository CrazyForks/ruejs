...
} from '../src/server/request-pipeline.js'
import { TEXT_PRERENDER_ROUTE_PARAMS_HEADER } from '../src/server/headers.js'
import { buildRequestHeadersFromMiddlewareResponse } from '../src/server/middleware-request-headers.js'
...
describe('stripBasePath', () => {
  ...
  it('strips when the text character is a path separator', () => {
...
describe('resolvePublicFileRoute', () => {
  it('signals GET public files and preserves middleware headers/status', () => {
    ...
    expect(response!.headers.get('x-text-static-file')).toBe('%2Flogo.svg')
  ...
  it('creates standalone static file signals from normal modules', () => {
    ...
    expect(response.headers.get('x-text-static-file')).toBe('%2Frobots.txt')
...
describe('validateServerActionPayload', () => {
  it('allows plain JSON action bodies with no RSC container references', async () => {
...
describe('processMiddlewareHeaders', () => {
  it('strips x-middleware-text header', () => {
    const headers = new Headers({
      'x-middleware-text': '1',
      'content-type': 'text/html',
    })
    processMiddlewareHeaders(headers)
    expect(headers.has('x-middleware-text')).toBe(false)
...
//
// Text.js strips these via filterInternalHeaders() at the router-server
// entry point before any handler sees the request:
// https://github.com/vercel/next.js/blob/canary/packages/text/src/server/lib/router-server.ts

describe('INTERNAL_HEADERS', () => {
  it("matches Text.js's exact header list", () => {
    // Keep in sync with Text.js:
    // https://github.com/vercel/next.js/blob/canary/packages/text/src/server/lib/server-ipc/utils.ts
    const expected = [
...
describe('filterInternalHeaders', () => {
  ...
  it('strips text-only internal headers without extending Text.js INTERNAL_HEADERS', () => {
    const headers = new Headers({
      [TEXT_PRERENDER_ROUTE_PARAMS_HEADER]: 'forged',
      'user-agent': 'test',
    })

    const result = filterInternalHeaders(headers)

    expect(INTERNAL_HEADERS).not.toContain(TEXT_PRERENDER_ROUTE_PARAMS_HEADER)
    expect(TEXT_INTERNAL_HEADERS).toEqual([TEXT_PRERENDER_ROUTE_PARAMS_HEADER])
    expect(result.has(TEXT_PRERENDER_ROUTE_PARAMS_HEADER)).toBe(false)
    expect(result.get('user-agent')).toBe('test')
  })
  ...
  it('strips headers case-insensitively', () => {
    ...
    expect(result.has('x-textjs-data')).toBe(false)
  ...
  it('strips x-middleware-rewrite forged as a request header', () => {
    ...
    expect(result.has('x-middleware-text')).toBe(false)
