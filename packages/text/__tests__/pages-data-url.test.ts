...
import {
  ...
  buildPagesDataHref,
  matchPagesPattern,
} from '../src/shims/internal/pages-data-url.js'
import { parseTextDataPathname } from '../src/server/pages-data-route.js'
...
describe('pages-data-url (client-side)', () => {
  ...
  describe('buildPagesDataPath', () => {
    it('encodes the root pathname as /index.json', () => {
      expect(buildPagesDataPath(BUILD_ID, '/')).toBe(`/_text/data/${BUILD_ID}/index.json`)
    })

    it('encodes a flat path', () => {
      expect(buildPagesDataPath(BUILD_ID, '/about')).toBe(`/_text/data/${BUILD_ID}/about.json`)
    })

    it('encodes a nested path', () => {
      expect(buildPagesDataPath(BUILD_ID, '/blog/foo')).toBe(
        `/_text/data/${BUILD_ID}/blog/foo.json`,
      )
    })

    it('disambiguates an explicit /index page from the root', () => {
      // Text.js denormalisation: `pages/index/index.tsx` would resolve to
      ...
      expect(buildPagesDataPath(BUILD_ID, '/index')).toBe(
        `/_text/data/${BUILD_ID}/index/index.json`,
      )
    })

    it('disambiguates an /index/foo page from /foo', () => {
      // Matches Text.js' getAssetPathFromRoute: any path starting with
      ...
      expect(buildPagesDataPath(BUILD_ID, '/index/foo')).toBe(
        `/_text/data/${BUILD_ID}/index/index/foo.json`,
      )
    })
    ...
    it('preserves locale prefix (callers are responsible for adding it)', () => {
      expect(buildPagesDataPath(BUILD_ID, '/en/about')).toBe(
        `/_text/data/${BUILD_ID}/en/about.json`,
      )
    })
  })
  ...
  describe('round-trip with parseTextDataPathname', () => {
    ...
    for (const path of cases) {
      it(`parses ${path} back to itself`, () => {
        ...
        const parsed = parseTextDataPathname(built, BUILD_ID)
    ...
    it('round-trips /index by encoding as /index/index.json', () => {
      ...
      expect(built).toBe(`/_text/data/${BUILD_ID}/index/index.json`)
    ...
    it('round-trips /index/foo through both helpers', () => {
      ...
      expect(built).toBe(`/_text/data/${BUILD_ID}/index/index/foo.json`)
      const parsed = parseTextDataPathname(built, BUILD_ID)
