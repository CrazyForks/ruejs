...
} from '../src/server/app-page-cache.js'
...
describe('app page cache helpers', () => {
  ...
  it('builds cached HTML and RSC responses', async () => {
    ...
    expect(htmlResponse?.headers.get('x-text-cache')).toBe('HIT')
    ...
    expect(htmlResponse?.headers.get('x-textjs-cache')).toBe('HIT')
    await expect(htmlResponse?.text()).resolves.toBe('<!DOCTYPE html><h1>cached</h1>')

    const rscResponse = withEnvVar('__TEXT_RSC_COMPATIBILITY_ID', 'compat-a', () =>
    ...
    expect(rscResponse?.headers.get(TEXT_RSC_COMPATIBILITY_ID_HEADER)).toBe('compat-a')
    expect(rscResponse?.headers.get('x-textjs-cache')).toBe('STALE')
  ...
  it('merges middleware response headers into cached HTML responses', async () => {
    ...
    expect(response?.headers.get('Vary')).toBe(`${TEXT_RSC_VARY_HEADER}, Accept-Encoding`)
  ...
  it('merges middleware response headers into cached RSC responses', async () => {
    ...
    expect(response?.headers.get('Vary')).toBe(`${TEXT_RSC_VARY_HEADER}, Origin`)
  ...
  it('returns cached HIT responses and clears request state', async () => {
    ...
    await expect(response?.text()).resolves.toBe('<!DOCTYPE html><h1>cached</h1>')
  ...
  it('keys RSC cache reads by mounted-slot header and echoes the variant header', async () => {
    ...
    expect(response?.headers.get('x-text-mounted-slots')).toBe('slot:auth:/')
  })
