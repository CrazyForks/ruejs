...
import { appendSearchParamsToUrl, mergeRewriteQuery, parseQueryString } from '../src/utils/query.js'
...
describe('mergeRewriteQuery', () => {
  ...
  it('supports absolute rewrite URLs (preserves origin)', () => {
    const result = mergeRewriteQuery('http://localhost/path?foo=1', 'http://localhost/target?bar=2')
...
describe('appendSearchParamsToUrl', () => {
  it('adds query params to a path with no existing query', () => {
    const url = appendSearchParamsToUrl('/search', [['q', 'text']])
    expect(url).toBe('/search?q=text')
  })

  it('preserves existing query params when appending new ones', () => {
    const url = appendSearchParamsToUrl('/search?lang=en', [['q', 'text']])
    expect(url).toBe('/search?lang=en&q=text')
  })
  ...
  it('preserves hash fragments after appending query params', () => {
    const url = appendSearchParamsToUrl('/search?lang=en#results', [['q', 'text']])
    expect(url).toBe('/search?lang=en&q=text#results')
  })

  it('preserves hashes when the base URL has no existing query string', () => {
    const url = appendSearchParamsToUrl('/search#results', [['q', 'text']])
    expect(url).toBe('/search?q=text#results')
  })
  ...
  it('preserves absolute URLs', () => {
    ...
    expect(url).toBe('https://example.com/search?lang=en&q=text#results')
  })
  ...
  it('supports query-only relative URLs', () => {
    const url = appendSearchParamsToUrl('?lang=en', [['q', 'text']])
    expect(url).toBe('?lang=en&q=text')
  })
})
