...
import { emitStandaloneOutput } from '../src/build/standalone.js'

let tmpDir: string

function createTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'text-standalone-test-'))
}
...
describe('emitStandaloneOutput', () => {
  it('copies packages listed in text-externals.json and their transitive deps', () => {
    ...
    writeFile(appRoot, 'dist/client/_text/static/main.js', "console.log('client');\n")
    writeFile(appRoot, 'dist/server/entry.js', 'console.log("server");\n')
    // The externals manifest is written by text:server-externals-manifest at build time.
    // It contains only the packages the server bundle actually imports at runtime.
    writeFile(
      appRoot,
      'dist/server/text-externals.json',
      JSON.stringify(['rue', 'server-runtime-lib']),
    )
    writeFile(appRoot, 'public/robots.txt', 'User-agent: *\n')

    writePackage(appRoot, 'dep-a', { 'dep-b': '1.0.0' })
    writePackage(appRoot, 'dep-b')
    writePackage(appRoot, 'rue')
    writePackage(appRoot, 'server-runtime-lib', { 'server-runtime-child': '1.0.0' })
    writePackage(appRoot, 'server-runtime-child')

    const fakeTextRoot = path.join(tmpDir, 'fake-text')
    writeFile(
      fakeTextRoot,
      'package.json',
      JSON.stringify(
    ...
    const result = emitStandaloneOutput({
      ...
      textPackageRoot: fakeTextRoot,
    })

    // Packages from the externals manifest are copied.
    expect(result.copiedPackages).toContain('rue')
    expect(result.copiedPackages).toContain('server-runtime-lib')
    expect(result.copiedPackages).toContain('server-runtime-child')
    expect(result.copiedPackages).toContain('text')
    ...
    expect(
      fs.existsSync(path.join(appRoot, 'dist/standalone/dist/client/_text/static/main.js')),
    ).toBe(true)
    ...
    expect(fs.existsSync(path.join(appRoot, 'dist/standalone/node_modules/rue/package.json'))).toBe(
      true,
    )
    expect(
      fs.existsSync(
        path.join(appRoot, 'dist/standalone/node_modules/server-runtime-lib/package.json'),
      ),
    ).toBe(true)
    expect(
      fs.existsSync(
        path.join(appRoot, 'dist/standalone/node_modules/server-runtime-child/package.json'),
      ),
    ).toBe(true)
    expect(
    ...
    expect(
      fs.existsSync(
        path.join(appRoot, 'dist/standalone/node_modules/text/dist/server/prod-server.js'),
      ),
    ).toBe(true)
  })
  ...
  it('copies transitive dependencies of manifest packages', () => {
    ...
    writeFile(appRoot, 'dist/server/text-externals.json', JSON.stringify(['dep-a']))
    ...
    writeFile(
      fakeTextRoot,
      'package.json',
      JSON.stringify({ name: 'text', type: 'module' }, null, 2),
    )
    writeFile(
      fakeTextRoot,
      'dist/server/prod-server.js',
  ...
  it('falls back gracefully when text-externals.json is missing (no manifest)', () => {
    const appRoot = path.join(tmpDir, 'app')
    fs.mkdirSync(appRoot, { recursive: true })
    ...
    writeFile(appRoot, 'dist/client/_text/static/main.js', "console.log('client');\n")
    writeFile(appRoot, 'dist/server/entry.js', 'console.log("server");\n')
    // No text-externals.json written.
    ...
    // Only text itself should be present (always embedded).
    expect(result.copiedPackages).toEqual(['text'])
    expect(fs.existsSync(path.join(appRoot, 'dist/standalone/server.js'))).toBe(true)
  })
  ...
  it('falls back when package.json is hidden by exports map', () => {
    ...
    writeFile(appRoot, 'dist/server/text-externals.json', JSON.stringify(['dep-hidden']))
  ...
  it('throws when a required runtime dependency cannot be resolved', () => {
    ...
    writeFile(appRoot, 'dist/server/text-externals.json', JSON.stringify(['missing-required']))
  ...
  it("copies text's own runtime dependencies into standalone node_modules", () => {
    const appRoot = path.join(tmpDir, 'app')
    fs.mkdirSync(appRoot, { recursive: true })
    ...
    writeFile(appRoot, 'dist/server/text-externals.json', JSON.stringify([]))

    // Set up a fake text package that has its own runtime dependency (rsc-html-stream),
    // which the app does NOT depend on but text's prod-server needs at runtime.
    const fakeTextRoot = path.join(tmpDir, 'fake-text')
    writeFile(
      fakeTextRoot,
      'package.json',
    ...
    // Install rsc-html-stream in the fake text's node_modules so it can be resolved.
    writePackage(fakeTextRoot, 'rsc-html-stream')
    ...
    // text's own runtime dep must be present even though the app doesn't list it.
  ...
  it('copies text runtime dependencies that only expose subpath exports', () => {
    const appRoot = path.join(tmpDir, 'app')
    fs.mkdirSync(appRoot, { recursive: true })
    ...
    writePackage(fakeTextRoot, 'rsc-html-stream', {}, { exports: { './server': './server.js' } })
    writeFile(fakeTextRoot, 'node_modules/rsc-html-stream/server.js', 'export {};\n')
  ...
  it('copies packages referenced through symlinked node_modules entries', () => {
    ...
    writeFile(appRoot, 'dist/server/text-externals.json', JSON.stringify(['dep-link']))
