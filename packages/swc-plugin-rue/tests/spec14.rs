...
"##;
    ...
    assert!(normalized.contains(&normalize(
        "const __slot = (item.commit.author.name); renderAnchor(__slot, _el20, _list5);"
    )));
    assert!(normalized.contains(&normalize(
        "const __slot = (item.commit.author.name); renderAnchor(__slot, _el21, _list6);"
    )));
    ...
    assert!(
        normalized.contains(&normalize("_$settextContent(_el16, truncate(item.commit.message));"))
    );
    assert!(
        normalized
            .contains(&normalize("_$settextContent(_el24, formatDate(item.commit.author.date));"))
    );
}
