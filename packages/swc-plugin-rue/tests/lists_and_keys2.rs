        ...
        watchEffect(()=>{
            ...
            const _map1_newElements = _$compiledKeyedList({
                ...
                renderItem: (todo, parent, start, end, _idx)=>{
                    ...
                    const _el6 = _$createElement("p", _root);
                    _$appendChild(_root, _el6);
                    const _list3 = _$createComment("rue:slot:anchor");
                    _$appendChild(_el6, _list3);
                    watchEffect(()=>{
                      const __slot = (todo.id);
                      untrack(()=>renderAnchor(__slot, _el6, _list3));
                    });
                    const _el7 = _$createElement("div", _root);
                    _$appendChild(_root, _el7);
                    watchEffect(()=>{
                      _$setClassName(_el7, `flex items-center justify-between rounded-lg border p-3 mb-2 ${todo.completed ? 'bg-gray-50' : 'bg-white'}`);
                    });
                    const _el8 = _$createElement("span", _el7);
                    _$appendChild(_el7, _el8);
                    _$addEventListener(_el8, "click", (()=>toggleTodo(todo.id)));
                    watchEffect(()=>{
                      _$setClassName(_el8, `cursor-pointer ${todo.completed ? 'line-through text-gray-500' : 'text-gray-800'}`);
                    });
                    const _list4 = _$createComment("rue:slot:anchor");
                    _$appendChild(_el8, _list4);
                    watchEffect(()=>{
                      const __slot = (todo.text);
                      untrack(()=>renderAnchor(__slot, _el8, _list4));
                    });
                    const _el9 = _$createElement("button", _el7);
...
"##;
    ...
    assert!(normalized.contains("_$compiledText(_el11, ()=>_$rowItem1.get().text)"), "{normalized}");
