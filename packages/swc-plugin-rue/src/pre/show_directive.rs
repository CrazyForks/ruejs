...
mod tests {
    ...
    fn parse_ts_expr(src: &str) -> Expr {
        ...
        let fm = cm.new_source_file(
            FileName::Custom("show-directive-test.ts".into()).into(),
            src.to_string(),
        );
    ...
    fn folds_static_show_styles_for_strings_objects_and_primitives() {
        let object_fold = fold_vapor_show_style(parse_ts_expr("({ color: 'tomato' })"), false)
            .expect("object fold");
        ...
        let known_string_fold = fold_vapor_show_style(parse_ts_expr("String(theme)"), false)
            .expect("known string fold");
        assert!(
