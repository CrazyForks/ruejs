...
/** Vapor 后端与默认后端共享同一五阶段能力探测；缺任一阶段即明确 fallback。 */
export const getOwnedMountProtocol = (): OwnedMountProtocol | undefined => {
  const runtime = getRueRuntime() as Partial<OwnedMountProtocol>
  if (
...
const createIslandDescriptorMountHandle = (descriptor: RueIslandDescriptor): RenderableOutput =>
  createRepeatableVaporHandle(parentContext => {
    ...
    const content =
      hydrate === 'only'
        ? descriptor.fallback
        : createComponent(descriptor.component, descriptor.props)
...
export const renderBetween = (
  ...
  return componentName === 'KeepAlive'
    ? withKeepAlivePropsRegistrationTarget(mountHandleValue, () =>
        getRueRuntime().renderBetween(mountHandleValue, targetParent, start, end),
      )
    : isAsyncExternalComponent
...
/** 仅释放 Vapor 入口本地记录的同步不透明 direct owner。 */
