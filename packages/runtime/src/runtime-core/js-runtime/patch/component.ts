...
const mountedFocusRoot = <HostNode>(mounted: Mounted<HostNode> | undefined): unknown => {
  const first = mounted?.fragmentNodes?.[0]
  if (first && (typeof first === 'object' || typeof first === 'function')) {
    return Reflect.get(first, 'parentNode') ?? first
  }
  return mounted?.host
}
...
const createComponentRenderEffect = <HostNode>(
  ...
  if (typeof watchEffect !== 'function') {
    trackedRender()
    return { dispose() {}, rerender: trackedRender, revision: () => revision }
  }

  const handle: unknown = Reflect.apply(watchEffect, reactive, [trackedRender])

  let disposed = false
  return {
    rerender: trackedRender,
    revision: () => revision,
    dispose() {
      if (disposed) return
      disposed = true
  ...
  try {
    const renderSubtree = () =>
      runComponentRenderEntry(state, () => {
        ...
        const focusSnapshot = initialRender
          ? undefined
          : captureFocusSnapshot(mountedFocusRoot(subtree))
        ...
        if (!initialRender) restoreFocusSnapshot(focusSnapshot, mountedFocusRoot(subtree))
        initialRender = false
        if (record) {
...
export const patchComponent = <HostNode>(
  ...
  if (input.type.updateMode === 'fine-grained' && !mounted.renderEffect) {
    const focusSnapshot = captureFocusSnapshot(mountedFocusRoot(mounted))
    runComponentRenderEntry(state, () => {
    ...
    restoreFocusSnapshot(focusSnapshot, mountedFocusRoot(mounted))
    mounted.type = input.type.component
    mounted.updateMode = input.type.updateMode
    mounted.key = input.key
    return mounted
  ...
  if (mounted.renderEffect) {
    const revisionBeforeUpdate = mounted.renderEffect.revision()
    state.components.update(mounted.instance, input)
    // Fine-grained components normally invalidate through their props proxy. Explicitly marked
    // factories may return an opaque Vapor setup that first reads props while mounting outside
    // the component render effect, so their parent-driven prop patches need an explicit pass.
    const renderReactiveFactory =
      (
        input.type.component as typeof input.type.component & {
          __rue_component_render_reactive_factory__?: boolean
    ...
    if (
      (input.type.updateMode === 'rerender' || renderReactiveFactory) &&
      mounted.renderEffect.revision() === revisionBeforeUpdate
    ) {
  ...
  restoreFocusSnapshot(focusSnapshot, mountedFocusRoot(subtree))
