...
const compatibilityEntryError = (): never => {
  throw new TypeError('Rue runtime: Element and Fragment inputs require the full runtime entry')
}
...
const isMountableVaporHost = (value: unknown): boolean => {
  ...
  return [1, 3, 8, 11].includes(nodeType as number) || typeof Reflect.get(value, 'tag') === 'string'
}
...
const mountVapor = <HostNode>(
  ...
  const existingParentChildren = new Set(
    isObjectLike(parentContext) && Reflect.get(parentContext, 'childNodes') != null
      ? Array.from(Reflect.get(parentContext, 'childNodes') as ArrayLike<HostNode>)
      : [],
  )
  const result = runVaporSetup(state, input, parentContext)
  const effectScopeId = result?.scopeId ?? input.mountEffectScopeId
  let vaporHost = result?.host ?? input.elHint
  const cleanupBucket = input.mountCleanupBucket
  const insertedParentChildren =
    isObjectLike(parentContext) && Reflect.get(parentContext, 'childNodes') != null
      ? Array.from(Reflect.get(parentContext, 'childNodes') as ArrayLike<HostNode>).filter(
          child => !existingParentChildren.has(child),
        )
      : []

  let compiledDepth = 0
  let compiledStaging: HostNode | undefined
  while (
    compiledDepth < 16 &&
    isObjectLike(vaporHost) &&
    typeof Reflect.get(vaporHost, '__rue_compiled_mount') === 'function'
  ) {
    const compiledHandle = vaporHost as unknown as {
      __rue_compiled_mount(parent: HostNode): HostNode | null | undefined
      dispose?: () => void
    }
    compiledStaging ??= host.createDocumentFragment()
    const compiledResult = compiledHandle.__rue_compiled_mount(compiledStaging) ?? undefined
    vaporHost = compiledResult
    if (typeof compiledHandle.dispose === 'function') {
      ;(cleanupBucket ?? (input.mountCleanupBucket = [])).push(() => compiledHandle.dispose?.())
    }
    compiledDepth += 1
  }
  if (compiledStaging && host.collectFragmentChildren(compiledStaging).length > 0) {
    vaporHost = compiledStaging
  } else if (insertedParentChildren.length > 0) {
    const staging = host.createDocumentFragment()
    for (const child of insertedParentChildren) host.appendChild(staging, child)
    vaporHost = staging
  }

  if (!isMountableVaporHost(vaporHost)) {
    disposeVaporResources(state, cleanupBucket, effectScopeId)
    const shape = isObjectLike(vaporHost)
      ? `${Reflect.ownKeys(vaporHost).map(String).join(',')}:${Reflect.get(vaporHost, 'constructor')?.name ?? ''}`
      : typeof vaporHost
    throw new TypeError(
      `Unsupported object returns are no longer accepted for vapor setup (${shape})`,
    )
  }
