...
export const renderComponent = <HostNode>(
  ...
  state.components.render<Mounted<HostNode> | undefined>(instance, input, props => {
    ...
    const normalized = normalizeComponentResult(state, value)
    const mount = () => mountSubtree(normalized)
    // The component render effect owns reads made while evaluating the component itself. Child
    // mounting establishes separate owners/effects and must not subscribe the parent render;
    // otherwise a child-local signal can rebuild the entire ancestor subtree.
    const untrack = state.kernel.reactive.untrack
    const subtree =
      typeof untrack === 'function'
        ? (untrack.call(state.kernel.reactive, mount) as Mounted<HostNode> | undefined)
        : mount()
