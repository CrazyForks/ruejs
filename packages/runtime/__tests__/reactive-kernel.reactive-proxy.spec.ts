...
import { createValueHooks } from '../src/runtime-core/js-reactive/hooks/values'
...
} from '../src/runtime-core/reactive-kernel/reactive'
...
describe('runtime TypeScript Signal paths', () => {
...
describe('runtime TypeScript Reactive Proxy', () => {
  ...
  it('reads collection, TypedArray, and DOM host properties with their required receivers', () => {
    const { kernel } = createTypeScriptKernel()
    ...
    expect(() => state.values.size).not.toThrow()
    expect(state.values.size).toBe(2)
    expect(() => state.bytes.byteLength).not.toThrow()
    expect(state.bytes.byteLength).toBe(3)
    expect(() => state.node.dataset.label).not.toThrow()
...
describe('runtime TypeScript Ref family and value Hooks', () => {
