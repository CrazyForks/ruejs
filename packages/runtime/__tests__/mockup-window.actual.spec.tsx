...
describe('MockupWindow actual page', () => {
  it('renders window mockup demos and restores preview after tab toggling', async () => {
    ...
    const backgroundDemo = findDemo(container, '# window mockup with background color') as HTMLElement | null
    ...
    await waitForContent(() => {
      ...
      expect(backgroundDemo?.querySelector('[data-testid="mockup-window-background"]')).not.toBeNull()
    })

    await click(findTabButton(borderDemo!, 'JSX代码'))
    expect(findDemo(container, '# window mockup with border')?.querySelector('.mockup-window')).toBeNull()
    await click(findTabButton(findDemo(container, '# window mockup with border')!, '预览'))

    await waitForContent(() => {
      expect(findDemo(container, '# window mockup with border')?.querySelector('.mockup-window')).not.toBeNull()
    })
  })
})