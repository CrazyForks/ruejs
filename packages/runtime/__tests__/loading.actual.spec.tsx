...
describe('Loading actual page', () => {
  it('renders loading demos and restores the spinner preview after toggling code', async () => {
    ...
    await waitForContent(() => {
      expect(basicDemo?.querySelectorAll('[data-testid="loading-basic-demo"] .loading').length).toBe(3)
      expect(nestedDemo?.querySelector('[data-rue-loading-section="true"]')?.textContent).toContain('正在拉取洞察')
      expect(percentDemo?.querySelectorAll('[data-rue-loading-percent="true"]').length).toBe(4)
      expect(spinnerDemo?.querySelectorAll('[data-testid="loading-spinner-demo"] .loading-spinner').length).toBe(5)
      expect(colorsDemo?.querySelectorAll('[data-testid="loading-colors-demo"] .loading').length).toBe(8)
    })

    await click(findTabButton(spinnerDemo!, 'JSX代码'))
    expect(findDemo(container, '# Loading spinner')?.querySelectorAll('.loading-spinner').length).toBe(0)
    await click(findTabButton(findDemo(container, '# Loading spinner')!, '预览'))

    await waitForContent(() => {
      expect(findDemo(container, '# Loading spinner')?.querySelectorAll('.loading-spinner').length).toBe(5)
    })
  })
})
